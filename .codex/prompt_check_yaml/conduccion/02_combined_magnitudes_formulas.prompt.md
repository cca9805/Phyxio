...
Generate two coherent YAML files: `formulas.yaml` and `magnitudes.yaml`.
Return them with exact file markers so they can be split safely:
=== BEGIN FILE: formulas.yaml ===
<valid formulas.yaml>
=== END FILE: formulas.yaml ===
=== BEGIN FILE: magnitudes.yaml ===
<valid magnitudes.yaml>
=== END FILE: magnitudes.yaml ===

Do not add text outside those markers.

Canonical map context. Use these values exactly where meta.yaml requires canonical fields:
id: conduccion
nombre: {'es': 'Conduccion', 'en': 'Conduction'}
area: fisica-clasica
...
orden: 257100
niveles: {'es': ['ESO', 'BACHILLERATO', 'UNIVERSIDAD'], 'en': ['Secondary', 'Upper Secondary', 'University']}
icon: 💪
graficos: ['Coord']
...
- Every formula requires: id, title.es/en, equation, latex, rearrangements, category, relation_type, physical_meaning.es/en, constraints, validity.es/en, dimension_check, calculable, motivo_no_calculable, used_in, interpretation_tags, result_semantics.target, result_semantics.kind, result_semantics.sign_meaning.es/en, result_semantics.absolute_value_meaning.es/en, domain_checks, coherence_checks, graph_implications, pedagogical_triggers.
...
- Every magnitude requires: id, symbol, nombre.es/en, descripcion.es/en, unidad_si, dimension, is_vector, components, category, physical_role, used_in, common_mistake, typical_range, sign_behavior.has_sign, sign_behavior.meaning.es/en, zero_behavior.allowed, zero_behavior.meaning.es/en, value_nature.kind, value_nature.nonnegative_only, value_nature.expected_interval, interpretation_role.primary_for, interpretation_role.secondary_for, graph_binding.channels, pedagogical_notes.es/en.
