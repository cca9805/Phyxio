...
def phyxio_perfect_indenter_v12(file_path):
    ...
    # Absolute root keys for ALL files
    root_keys_global = ["version", "id", "area", "bloque", "subbloque", "parent_id", "ruta_relativa", "orden", "type", "niveles", "icon", "description_en", "pregunta_fisica_central", "representacion_dominante", "magnitud_dominante", "tags", "prerequisitos", "graficos", "dificultad", "tiempo_estimado_min", "interpretacion", "magnitudes", "formulas", "topic", "variables", "ui", "config", "steps"]
    
    if file_name == "meta.yaml":
        root_keys_global += ["nombre", "descripcion", "physical_role"]
    elif file_name == "interpretacion.yaml":
        root_keys_global += ["nombre", "descripcion"]
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v12(path):
                    count += 1
                    print(f"Perfect Indented V12: {path}")
