...
def phyxio_perfect_indenter_v15(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            if key in h:
                ...
                modified = True
                if key not in ["es", "en", "id", "title"]: # Don't update parent for these leaves
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v15(path):
                    count += 1
                    print(f"Perfect Indented V15: {path}")
