...
def phyxio_perfect_indenter_v19(file_path):
    ...
    # Root keys (column 0) - EXCLUDING 'id' which is context dependent
    root_keys_base = ["version", "leaf_id", "area", "bloque", "subbloque", "parent_id", "ruta_relativa", "orden", "type", "niveles", "icon", "description_en", "pregunta_fisica_central", "representacion_dominante", "magnitud_dominante", "tags", "prerequisitos", "graficos", "dificultad", "tiempo_estimado_min", "interpretacion", "magnitudes", "formulas", "topic", "variables", "config", "steps"]
    ...
    h = {
        "id": -3, "leaf_id": 2, "symbol": 4, "nombre": 4, "title": -3, "equation": 4, "latex": 4, 
        ...
        "physical_role": 4, "default_formula": 2, "groups": 2, "items": 6
    }
    ...
    for i in range(len(lines)):
        ...
        if m_list_item:
            ...
            elif parent_key == "items": target = 8 
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v19(path):
                    count += 1
                    print(f"Perfect Indented V19: {path}")
