...
def phyxio_perfect_indenter_v29(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_list_item:
            ...
            if val.startswith("title:"):
                ...
                # Push - title to stack so its children get indented
                while len(stack) > 1 and target <= stack[-1][0]:
                    stack.pop()
                stack.append((target, "item"))
            else:
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v29(path):
                    count += 1
                    print(f"Perfect Indented V29: {path}")
