...
def phyxio_perfect_indenter_v31(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            # Update stack: if this is a block parent (no rest OR rest is an anchor/alias), push it
            # Phyxio v5 uses & and * for anchors/aliases
        ...
        if m_id:
            ...
            p_idx = len(stack) - 1
            while p_idx >= 0 and stack[p_idx][1] not in ["magnitudes", "formulas", "variables", "steps", "groups", "items", "priority_order", "sections", "channels"]:
                p_idx -= 1
            target = stack[p_idx][0] + 2
        ...
        if m_list_item:
            ...
            # Find parent that allows list items
            p_idx = len(stack) - 1
            while p_idx >= 0 and stack[p_idx][1] in ["es", "en"]: p_idx -= 1
            target = stack[p_idx][0] + 2
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v31(path):
                    count += 1
                    print(f"Perfect Indented V31: {path}")
