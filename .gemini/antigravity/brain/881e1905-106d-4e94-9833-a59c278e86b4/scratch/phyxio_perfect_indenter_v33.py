...
def phyxio_perfect_indenter_v33(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            # Update stack ONLY if this is a block parent
            # Leaf check: if it has a value and that value is NOT an anchor/alias, it's a leaf
            is_leaf = False
            if rest:
        ...
        if m_id:
            ...
            p_idx = len(stack) - 1
            while p_idx >= 0 and stack[p_idx][1] not in ["magnitudes", "formulas", "variables", "steps", "groups", "items", "priority_order", "sections", "channels", "output_contract"]:
                p_idx -= 1
            target = stack[p_idx][0] + 2
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v33(path):
                    count += 1
                    print(f"Perfect Indented V33: {path}")
