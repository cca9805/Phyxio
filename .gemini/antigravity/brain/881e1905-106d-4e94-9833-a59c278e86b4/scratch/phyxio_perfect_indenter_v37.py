...
def phyxio_perfect_indenter_v37(file_path):
    ...
    # stack: (original_indent, target_indent, key)
    stack = [(-1, -1, "root")]
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            # Pop stack based on ORIGINAL indentation
        ...
        if m_id:
            ...
            p_idx = len(stack) - 1
            while p_idx >= 0 and stack[p_idx][2] not in ["magnitudes", "formulas", "variables", "steps", "groups", "items", "priority_order", "sections", "channels", "output_contract"]:
                p_idx -= 1
            target = stack[p_idx][1] + 2
        ...
        if m_list_item:
            ...
            while len(stack) > 1 and orig_indent < stack[-1][0]: # Note: < for list items if they are at the same level
                stack.pop()
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v37(path):
                    count += 1
                    print(f"Perfect Indented V37: {path}")
