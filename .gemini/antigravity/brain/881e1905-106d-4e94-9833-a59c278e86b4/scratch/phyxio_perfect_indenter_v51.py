...
def phyxio_perfect_indenter_v51(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_id:
            ...
            # Special case for Phyxio: - id: v1
            # We must ensure it stays on one line and has correct indent
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v51(path):
                    count += 1
                    print(f"Perfect Indented V51: {path}")
