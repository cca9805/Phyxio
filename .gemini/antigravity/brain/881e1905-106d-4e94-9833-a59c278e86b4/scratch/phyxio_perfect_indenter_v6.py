...
def phyxio_perfect_indenter_v6(file_path):
    ...
    # We use a stack of (indent, key) to track hierarchy
    stack = [( -1, "root")]
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            if key in h:
                # Find current parent key that isn't a bilingual leaf
                parent_key = "root"
                ...
                if target == -1: # es/en
                    ...
                    elif parent_key in ["tab_label", "title", "nombre", "descripcion", "physical_meaning", "validity", "pedagogical_triggers", "common_mistake", "typical_range"]:
                        if file_name == "meta.yaml" and parent_key in ["nombre", "descripcion", "physical_role"]: target = 2
                        else: target = 6
                    else: target = 6
                
                new_lines.append(f"{' '*target}{key}: {rest}\n")
                modified = True
                # If it's es or en, don't change the parent in the stack for siblings
                if key not in ["es", "en"]:
        ...
        if m_list_item:
            ...
            # ... similar logic to V5 ...
            target = 6
            # Use stack to find parent
            parent_key = stack[-1][1] if stack else "root"
            if file_name == "meta.yaml" and parent_key in ["niveles", "tags", "prerequisitos", "graficos"]: target = 4
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v6(path):
                    count += 1
                    print(f"Perfect Indented V6: {path}")
