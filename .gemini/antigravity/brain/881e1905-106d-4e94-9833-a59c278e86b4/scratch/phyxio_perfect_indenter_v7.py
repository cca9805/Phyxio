...
def phyxio_perfect_indenter_v7(file_path):
    ...
    for i in range(len(lines)):
        ...
        if m_list_item:
            ...
            # Additional check: what was the last es/en parent?
            last_bi = ""
            for s_indent, s_key in reversed(stack):
                if s_key in ["es", "en"]:
                    last_bi = s_key
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v7(path):
                    count += 1
                    print(f"Perfect Indented V7: {path}")
