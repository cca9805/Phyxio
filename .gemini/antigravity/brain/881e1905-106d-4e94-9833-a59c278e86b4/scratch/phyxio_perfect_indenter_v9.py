...
def phyxio_perfect_indenter_v9(file_path):
    ...
    # Absolute root keys (always column 0)
    root_keys_global = ["version", "area", "bloque", "subbloque", "parent_id", "ruta_relativa", "orden", "type", "niveles", "icon", "description_en", "pregunta_fisica_central", "representacion_dominante", "magnitud_dominante", "tags", "prerequisitos", "graficos", "dificultad", "tiempo_estimado_min", "interpretacion", "magnitudes", "formulas", "topic", "variables"]
    
    if file_name == "meta.yaml":
        root_keys_global += ["id", "nombre", "descripcion", "physical_role"]
    ...
    for i in range(len(lines)):
        ...
        if m_key:
            ...
            if key in root_keys_global:
                # If we are in meta.yaml, and we see 'enabled' at column 0, it shouldn't be root!
                # But 'enabled' is not in root_keys_global anyway.
        ...
        if m_id:
            ...
            for si, sk in reversed(stack):
                if sk in ["magnitudes", "formulas", "variables"]:
                    p_key = sk
                    break
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith((".yaml")):
                ...
                if phyxio_perfect_indenter_v9(path):
                    count += 1
                    print(f"Perfect Indented V9: {path}")
