# _core.py — Validador Phyxio completo. NO editar directamente.
# Este archivo es generado por tools/refactor_validator.py
...
def find_markdown_math_render_issues(text: str, formula_ids: Set[str] | None = None, max_hits: int = 12) -> List[str]:
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        # Enfoque robusto: dividir por backticks y verificar solo segmentos internos
        # segmentos[0]=fuera, [1]=dentro, [2]=fuera, [3]=dentro...
        segments = line.split('`')
        for i, segment in enumerate(segments):
            if i % 2 == 1 and '[[' in segment and ']]' in segment:  # Índices impares = dentro de backticks
...
# Placeholders y valores inválidos que deben rechazarse
PLACEHOLDER_PATTERNS = [
    r"\bcontenido\s+pendiente\b",
    r"\bcontenido\s+por\s+completar\b",
    r"\bpor\s+redactar\b",
...
def is_placeholder_or_empty(value: Any) -> bool:
    """Detecta si un valor es placeholder, vacío o tiene valor por defecto inválido.
    
    Retorna True si el valor debe rechazarse (es placeholder/vacío).
    Retorna False si el valor parece válido.
    """
    if value is None:
        return True
    if isinstance(value, str):
        ...
        # Patrones que deben coincidir exactamente (no substring)
        EXACT_MATCH_PATTERNS = [
            "target_id", "leaf_id", "formula_id", "magnitude_id",
            "placeholder", "TODO", "FIXME", "XXX", 
            "pending", "pending...", "por_definir", "por_definir...", 
            "definir", "completar", "completar...",
            "rellenar", "falta", "missing", "null", "undefined", "none",
            "0", "[]", "{}", "...", "…."
        ]
        ...
        # Detectar patrones tipo "id: <target_id>" o similares (contienen < >)
        if "<" in stripped and ">" in stripped:
            return True
            
        # Solo espacios o vacío ya fue manejado arriba
        if stripped in ["0", "[]", "{}", "null", "None", "undefined"]:
            return True
            
    if isinstance(value, (list, dict)):
        return len(value) == 0
    if isinstance(value, bool):
        return False  # True/False son valores válidos
    if isinstance(value, (int, float)):
        return value == 0  # 0 numérico es sospechoso pero no siempre inválido
    return False

def is_valid_id(value: str) -> bool:
    """Valida que un ID sea válido (no placeholder, no vacío, formato correcto)."""
    if not value or not isinstance(value, str):
        return False
    if is_placeholder_or_empty(value):
        return False
    # IDs válidos: lowercase, sin espacios, con contenido sustancial
    stripped = value.strip()
    if len(stripped) < 2:  # IDs de 1 carácter son sospechosos
        return False
...
def validate_meta(path: Path) -> List[CheckResult]:
    ...
    # Descripciones no vacias y <= 20 palabras (estructura i18n)
    descripcion = data.get("descripcion", {})
    descripcion_es = descripcion.get("es", "") if isinstance(descripcion, dict) else data.get("descripcion", "")
    descripcion_en = descripcion.get("en", "") if isinstance(descripcion, dict) else data.get("description_en", "")
    
    es_valid = descripcion_es and 1 <= len(descripcion_es.split()) <= 20
    en_valid = descripcion_en and 1 <= len(descripcion_en.split()) <= 20
    
    res.append(CheckResult(
        es_valid,
        "descripcion <= 20 palabras",
        f"'{descripcion_es}' ({len(descripcion_es.split())} palabras)" if descripcion_es else "vacío",
    ))
    res.append(CheckResult(
        en_valid,
        "description_en <= 20 palabras",
        f"'{descripcion_en}' ({len(descripcion_en.split())} palabras)" if descripcion_en else "vacío",
    ))
...
def validate_magnitudes(path: Path) -> Tuple[List[CheckResult], Set[str], Set[str]]:
    ...
    for idx, item in enumerate(items or []):
        ...
        # Validar que ID no sea placeholder
        mag_id = item.get("id", "")
        if mag_id:
            if not is_valid_id(mag_id):
                mag_placeholders.append({"idx": idx, "field": "id", "value": mag_id})
            else:
                mag_ids.add(mag_id)
        ...
        # Validar campos bilingües no vacíos y sin placeholders
        ...
        if not (isinstance(nombre, dict) and "es" in nombre and "en" in nombre):
            ...
            if is_placeholder_or_empty(nombre.get("es", "")):
                mag_placeholders.append({"idx": idx, "field": "nombre.es", "value": nombre.get("es")})
            if is_placeholder_or_empty(nombre.get("en", "")):
                mag_placeholders.append({"idx": idx, "field": "nombre.en", "value": nombre.get("en")})
        
        if not (isinstance(descripcion, dict) and "es" in descripcion and "en" in descripcion):
            problems.append((idx, "descripcion_es_en"))
        else:
            if is_placeholder_or_empty(descripcion.get("es", "")):
                mag_placeholders.append({"idx": idx, "field": "descripcion.es", "value": descripcion.get("es")})
            if is_placeholder_or_empty(descripcion.get("en", "")):
                mag_placeholders.append({"idx": idx, "field": "descripcion.en", "value": descripcion.get("en")})
        
        # Validar symbol no placeholder
        if is_placeholder_or_empty(symbol):
            mag_placeholders.append({"idx": idx, "field": "symbol", "value": symbol})
        
        # Validar campos de texto simple no vacíos
        for text_field in ["unidad_si", "dimension", "category", "physical_role"]:
            if text_field in item and is_placeholder_or_empty(item[text_field]):
                mag_placeholders.append({"idx": idx, "field": text_field, "value": item[text_field]})
    
    res.append(CheckResult(not problems, "Cada magnitud tiene todos los campos v5", problems))
    res.append(CheckResult(
        not mag_placeholders,
        "magnitudes.yaml: campos sin placeholders/vacíos",
        mag_placeholders if mag_placeholders else None
    ))
...
def validate_formulas(path: Path) -> Tuple[List[CheckResult], Set[str], Set[str]]:
    ...
    for idx, item in enumerate(items or []):
        ...
        if formula_id:
            if not is_valid_id(formula_id):
                formula_placeholders.append({"idx": idx, "field": "id", "value": formula_id})
            else:
        ...
        for key in ["title","physical_meaning","validity"]:
            ...
            if not (isinstance(val, dict) and "es" in val and "en" in val):
                ...
                if is_placeholder_or_empty(val.get("es", "")):
                    formula_placeholders.append({"idx": idx, "field": f"{key}.es", "value": val.get("es")})
                if is_placeholder_or_empty(val.get("en", "")):
                    formula_placeholders.append({"idx": idx, "field": f"{key}.en", "value": val.get("en")})
        
        # Validar campos de texto críticos no vacíos
        for text_field in ["equation", "latex", "category", "relation_type"]:
            if text_field in item and is_placeholder_or_empty(item[text_field]):
                formula_placeholders.append({"idx": idx, "field": text_field, "value": item[text_field]})
        rearrangements = item.get("rearrangements", [])
    ...
    res.append(CheckResult(
        not formula_placeholders,
        "formulas.yaml: campos sin placeholders/vacíos",
        formula_placeholders if formula_placeholders else None
    ))
    return res, symbols, formula_ids

def validate_graficos(path: Path, meta_data: dict) -> List[CheckResult]:
    """Valida graficos.yaml según plantilla v5.
    Verifica coherencia con meta.yaml (campo graficos) y estructura de secciones.
    """
    from .utils import load_yaml  # Import local
    
    results: List[CheckResult] = []
    
    # Obtener tipos de gráficos declarados en meta.yaml
    graficos_meta = meta_data.get("graficos") or []
    
    try:
        data = load_yaml(path)
    except Exception as e:
        return [CheckResult(False, f"{path.name} YAML válido", str(e))]
    
    if data is None:
        return [CheckResult(False, f"{path.name} contenido YAML válido", "Archivo vacío o YAML inválido")]
    
    # Validar campos raíz
    version = data.get("version", "")
    leaf_id = data.get("leaf", "")
    meta_leaf_id = meta_data.get("id", "")
    
    results.append(CheckResult(
        version == "1.0",
        f"{path.name} version es '1.0'",
        None if version == "1.0" else f"version='{version}' (esperado: '1.0')"
    ))
    
    results.append(CheckResult(
        leaf_id == meta_leaf_id,
        f"{path.name} leaf coincide con meta.yaml",
        None if leaf_id == meta_leaf_id else f"leaf='{leaf_id}' vs meta.id='{meta_leaf_id}'"
    ))
    
    # Mapeo de tipos meta.yaml → secciones graficos.yaml
    type_to_section = {"Coord": "coord", "Dcl": "dcl", "Svg": "svg"}
    required_sections = [type_to_section[t] for t in graficos_meta if t in type_to_section]
    
    # Validar que existan las secciones requeridas según meta.yaml
    for section in required_sections:
        exists = section in data
        results.append(CheckResult(
            exists,
            f"{path.name} sección '{section}' existe (meta.yaml declara {section.capitalize()})",
            None if exists else f"Falta sección '{section}' pero meta.yaml declara tipo '{section.capitalize()}'"
        ))
        
        if exists and section in data:
            section_data = data[section]
            # Validar campos bilingües obligatorios
            for field in ["title"]:
                if field in section_data:
                    has_es = "es" in section_data[field] and section_data[field]["es"]
                    has_en = "en" in section_data[field] and section_data[field]["en"]
                    results.append(CheckResult(
                        has_es and has_en,
                        f"{path.name} [{section}].{field} bilingüe completo",
                        None if (has_es and has_en) else f"[{section}].{field} incompleto: es={has_es}, en={has_en}"
                    ))
            
            # Validar campos específicos por tipo
            if section == "coord":
                for axis in ["xAxis", "yAxis"]:
                    if axis in section_data:
                        has_es = "es" in section_data[axis] and section_data[axis]["es"]
                        has_en = "en" in section_data[axis] and section_data[axis]["en"]
                        results.append(CheckResult(
                            has_es and has_en,
                            f"{path.name} [{section}].{axis} bilingüe completo",
                            None if (has_es and has_en) else f"[{section}].{axis} incompleto"
                        ))
                
                # Validar agentHints
                if "agentHints" in section_data:
                    hints = section_data["agentHints"]
                    has_hints_es = "es" in hints and isinstance(hints["es"], list) and len(hints["es"]) >= 1
                    has_hints_en = "en" in hints and isinstance(hints["en"], list) and len(hints["en"]) >= 1
                    results.append(CheckResult(
                        has_hints_es and has_hints_en,
                        f"{path.name} [{section}].agentHints tiene al menos 1 pista por idioma",
                        None if (has_hints_es and has_hints_en) else f"[{section}].agentHints incompleto"
                    ))
            
            elif section in ["dcl", "svg"]:
                # Validar sceneId
                has_scene_id = "sceneId" in section_data and section_data["sceneId"]
                results.append(CheckResult(
                    has_scene_id,
                    f"{path.name} [{section}].sceneId existe",
                    None if has_scene_id else f"[{section}].sceneId vacío o ausente"
                ))
                
                # Validar objects
                if "objects" in section_data:
                    objs = section_data["objects"]
                    has_objs_es = "es" in objs and isinstance(objs["es"], list) and len(objs["es"]) >= 1
                    has_objs_en = "en" in objs and isinstance(objs["en"], list) and len(objs["en"]) >= 1
                    results.append(CheckResult(
                        has_objs_es and has_objs_en,
                        f"{path.name} [{section}].objects tiene al menos 1 objeto por idioma",
                        None if (has_objs_es and has_objs_en) else f"[{section}].objects incompleto"
                    ))
    ...
    # Validar que NO existan secciones que no están declaradas en meta.yaml
    for section in ["coord", "dcl", "svg"]:
        if section in data and section not in required_sections:
            results.append(CheckResult(
                False,
                f"{path.name} sección '{section}' coincide con meta.yaml",
                f"Sección '{section}' existe en graficos.yaml pero meta.yaml no declara tipo '{section.capitalize()}'"
            ))
    
    # Si no hay gráficos en meta.yaml, graficos.yaml debe tener solo version y leaf
    if not graficos_meta:
        extra_sections = [k for k in data.keys() if k not in ["version", "leaf"]]
        results.append(CheckResult(
            not extra_sections,
            f"{path.name} sin secciones de gráficos (meta.yaml no declara gráficos)",
            None if not extra_sections else f"Secciones sobrantes: {extra_sections}"
        ))
...
def validate_interpretacion(path: Path, mag_ids: Set[str], formula_ids: Set[str]) -> List[CheckResult]:
    ...
    def _rule_trigger_ok(rule: Dict[str, Any]) -> bool:
        ...
        when_ok = isinstance(when, str) and bool(when.strip()) and not is_placeholder_or_empty(when)
    ...
    res: List[CheckResult] = []
    # Campos obligatorios raíz según plantilla v5
    required_root_fields = [
        "version", "id", "leaf_id", "nombre", "scope", 
        "output_contract", "result_blocks", "ui", "mini_graph", 
        "output_policy", "dependencies", "targets"
    ]
    missing_root = [k for k in required_root_fields if k not in data]
    res.append(CheckResult(not missing_root, "interpretacion.yaml: campos raíz obligatorios", missing_root))
    
    # Validar versión exacta
    version = data.get("version", "")
    res.append(CheckResult(
        version == "5.0.0",
        "interpretacion.yaml: version es '5.0.0'",
        None if version == "5.0.0" else f"version='{version}' (esperado: '5.0.0')"
    ))
    ...
    # Validar que nombre no tenga placeholders
    if isinstance(nombre, dict):
        nombre_es = nombre.get("es", "")
        nombre_en = nombre.get("en", "")
        res.append(CheckResult(
            not is_placeholder_or_empty(nombre_es),
            "interpretacion.yaml: nombre.es sin placeholders",
            None if not is_placeholder_or_empty(nombre_es) else f"Valor inválido: '{nombre_es}'"
        ))
        res.append(CheckResult(
            not is_placeholder_or_empty(nombre_en),
            "interpretacion.yaml: nombre.en sin placeholders",
            None if not is_placeholder_or_empty(nombre_en) else f"Valor inválido: '{nombre_en}'"
        ))
    ...
    # Validar que dependencies no tenga valores placeholder
    if isinstance(deps, dict):
        placeholder_deps = []
        for key, value in deps.items():
            if is_placeholder_or_empty(value):
                placeholder_deps.append(f"{key}={value}")
        res.append(CheckResult(
            not placeholder_deps,
            "interpretacion.yaml: dependencies sin valores placeholder/vacíos",
            placeholder_deps if placeholder_deps else None
        ))
    
    # Validar campos obligatorios de dependencies
    required_dep_fields = ["formulas", "magnitudes", "requires_formulas", "requires_magnitudes", "supports_graph_binding"]
    missing_dep = [k for k in required_dep_fields if k not in deps]
    res.append(CheckResult(
        not missing_dep,
        "interpretacion.yaml: dependencies campos obligatorios",
        missing_dep if missing_dep else None
    ))
    res.append(
        CheckResult(
    ...
    if rb_ok:
        ...
        for sec in required_sections:
            ...
            if not isinstance(block, dict):
                missing_enabled.append(sec)
                missing_order.append(sec)
                continue
            # Validar title bilingüe
        ...
        res.append(
            CheckResult(
                not missing_enabled,
                "interpretacion.yaml: result_blocks.<seccion>.enabled obligatorio",
                missing_enabled,
            )
        )
        res.append(
            CheckResult(
                not missing_order,
                "interpretacion.yaml: result_blocks.<seccion>.order obligatorio",
                missing_order,
            )
        )
    else:
    ...
    # ---------------------------------------------
    # VALIDACIONES UI, MINI_GRAPH, OUTPUT_POLICY
    # ---------------------------------------------
    
    # Validar ui
    ui = data.get("ui", {}) or {}
    required_ui_fields = ["enabled", "display_modes", "labels", "priority_order", "inline_limits"]
    missing_ui = [k for k in required_ui_fields if k not in ui]
    res.append(CheckResult(
        not missing_ui,
        "interpretacion.yaml: ui campos obligatorios",
        missing_ui if missing_ui else None
    ))
    
    # Validar mini_graph
    mini_graph = data.get("mini_graph", {}) or {}
    required_mg_fields = ["enabled", "preferred_type"]
    missing_mg = [k for k in required_mg_fields if k not in mini_graph]
    res.append(CheckResult(
        not missing_mg,
        "interpretacion.yaml: mini_graph campos obligatorios",
        missing_mg if missing_mg else None
    ))
    
    # Validar output_policy
    output_policy = data.get("output_policy", {}) or {}
    required_op_fields = ["show_summary_first", "max_inline_messages", "show_warnings", "show_likely_errors"]
    missing_op = [k for k in required_op_fields if k not in output_policy]
    res.append(CheckResult(
        not missing_op,
        "interpretacion.yaml: output_policy campos obligatorios",
        missing_op if missing_op else None
    ))
    
    # R5: mini_graph.enabled ↔ graph_reading_policy.enabled (coherencia obligatoria)
    graph_reading_policy = data.get("graph_reading_policy", {}) or {}
    mg_enabled = mini_graph.get("enabled", False)
    grp_enabled = graph_reading_policy.get("enabled", False) if isinstance(graph_reading_policy, dict) else False
    
    res.append(CheckResult(
        mg_enabled == grp_enabled,
        "interpretacion.yaml: mini_graph.enabled ↔ graph_reading_policy.enabled",
        None if mg_enabled == grp_enabled else f"mini_graph.enabled={mg_enabled} pero graph_reading_policy.enabled={grp_enabled} (deben coincidir)"
    ))
    
    # Validar graph_reading_policy cuando está enabled
    if mg_enabled and isinstance(graph_reading_policy, dict):
        required_grp_fields = ["enabled", "primary_variable", "reading_sequence", "key_points"]
        missing_grp = [k for k in required_grp_fields if k not in graph_reading_policy]
        res.append(CheckResult(
            not missing_grp,
            "interpretacion.yaml: graph_reading_policy campos obligatorios (cuando enabled)",
            missing_grp if missing_grp else None
        ))
    
    targets = data.get("targets", {})
    res.append(CheckResult(isinstance(targets, dict) and len(targets) > 0, "interpretacion.yaml: targets no vacio", None))
    
    # Validar 9 bloques obligatorios por target (incluyendo physical_intuition y limiting_cases)
    ...
    if isinstance(targets, dict) and targets:
        ...
        targets_placeholder_ids = []
        targets_placeholder_rules = []
        targets_bad_summary_condition = []
        
        for target_id, target_block in targets.items():
            # Validar que target_id no sea placeholder
            if not is_valid_id(target_id):
                targets_placeholder_ids.append(target_id)
                continue
            ...
            for key in required_target_lists:
                ...
                for ridx, rule in enumerate(rules):
                    ...
                    # Validar que campos de regla no sean placeholders
                    if is_placeholder_or_empty(rule_id):
                        targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "id", "valor": rule_id})
                    if not has_condition and is_placeholder_or_empty(rule_when):
                        targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "when", "valor": rule_when})
                    if has_condition and not _valid_condition_block(rule.get("condition")):
                        targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "condition", "valor": rule.get("condition")})
                    if is_placeholder_or_empty(rule_status):
                        targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "status", "valor": rule_status})
                    ...
                    # Validar que textos no sean placeholders
                    if text_ok:
                        if is_placeholder_or_empty(text.get("es", "")):
                            targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "text.es", "valor": text.get("es")})
                        if is_placeholder_or_empty(text.get("en", "")):
                            targets_placeholder_rules.append({"target": target_id, "field": key, "index": ridx, "campo": "text.en", "valor": text.get("en")})
                    
                    rule_ok = (
                        isinstance(rule_id, str)
                        and bool(rule_id.strip())
                        and _rule_trigger_ok(rule)
                        and isinstance(rule_status, str)
                        and bool(rule_status.strip())
                        and text_ok
                    )
                    if not rule_ok:
                        targets_bad_rule_schema.append(
        ...
        # Añadir resultados de validaciones de placeholders
        res.append(CheckResult(
            not targets_placeholder_ids,
            "interpretacion.yaml: targets IDs válidos (no placeholders)",
            targets_placeholder_ids if targets_placeholder_ids else None
        ))
        res.append(CheckResult(
            not targets_placeholder_rules,
            "interpretacion.yaml: reglas sin placeholders/vacíos",
            targets_placeholder_rules if targets_placeholder_rules else None
        ))
        res.append(CheckResult(
            not targets_bad_summary_condition,
            "interpretacion.yaml: summary_rules usa when simple o condition estructurado",
            targets_bad_summary_condition if targets_bad_summary_condition else None
        ))
        res.append(
            CheckResult(
...
def validate_exact_files(leaf_dir: Path) -> List[CheckResult]:
    ...
    if meta_path.exists():
        try:
            meta_data = load_yaml(meta_path)
            if isinstance(meta_data, dict) and isinstance(meta_data.get("graficos"), list):
                graph_types = [g for g in meta_data.get("graficos", []) if isinstance(g, str) and g.strip()]
        except Exception:
            graph_types = []
    if graph_types:
        expected.append("graficos.yaml")
    elif "graficos.yaml" in current:
        expected.append("graficos.yaml")
...
def route_graph_namespace(route: str) -> str:
    ...
    # Namespaces directos: carpetas que tienen su propio index.js en graphs/
    _DIRECT_NAMESPACES = {
        "electromagnetismo",
        "fluidos",
        "gravitacion",
    ...
    parts = [p for p in str(route or "").split("/") if p]
    # Primero: buscar cualquier segmento que sea un namespace directo
    for part in parts:
        if part in _DIRECT_NAMESPACES:
            return part
    # Segundo: lógica legacy para mecanica → subbloque
    try:
...
def resolve_graph_registry(leaf_dir: Path, meta_data: dict) -> Tuple[Dict[str, Any] | None, List[CheckResult]]:
    ...
    # Primero intentar con rawGraphs ya que es el formato más común en dinamica
    graphs_body = ""
    
    # Intentar formato: const rawGraphs = {...}
    raw_match = re.search(r'\bconst\s+rawGraphs\s*=\s*\{', active_idx)
    if raw_match:
        # Extraer el body usando balanceo de llaves
    ...
    if not graphs_body:
        ...
        if not export_match:
            return None, [CheckResult(False, "index.js no contiene 'export const graphs = {...}' ni 'const rawGraphs = {...}'", str(index_path))]
    ...
    if not graphs_body:
        return None, [CheckResult(False, "No se pudo extraer el cuerpo de graphs del index.js", str(index_path))]
    ...
    def _parse_graphs_entries(body: str) -> Dict[str, Dict[str, str]]:
        ...
        Solo itera al nivel 0 del body para evitar capturar ids internos."""
    ...
    # graphs_body ya fue extraído arriba (línea ~3056 o ~3093)
...
def validate_cognitive_load(
    ...
    # Se cuentan SOLO referencias explícitas {{f:id}} en el texto, no por
    # coincidencia de target de magnitud (eso generaría falsos positivos cuando
    # el esencial presenta magnitudes en prosa sin usar fórmulas).
    explicit_formula_ids_in_esencial = set(re.findall(r"\{\{f:([^}]+)\}\}", esencial))
    core_formula_ids = {f.get("id", "") for f in core_formulas}
    essential_formula_hits = len(explicit_formula_ids_in_esencial & core_formula_ids)
    checks.append(CheckResult(
...
def validate_modelos_reinforced(leaf_dir: Path) -> List[CheckResult]:
    ...
    has_transition = bool(re.search(
        r'cuando\s+conviene|transicion|pasar\s+al|cambiar\s+a'
        r'|when\s+to\s+switch|transition\s+to|switch\s+to|when\s+to\s+use',
        text, re.IGNORECASE
    ))
    results.append(CheckResult(
        has_alt,
...
# CHECK 13B — Formato expandido obligatorio del registro en index.js

def validate_graph_index_format(leaf_dir: Path, meta_data: dict) -> List[CheckResult]:
    """Valida que cada entrada del leaf en index.js use el nuevo formato expandido obligatorio.
    Cada tipo de gráfico (Coord, Dcl, Svg) tiene campos específicos requeridos y opcionales."""
    graficos = meta_data.get("graficos") or []
    if not graficos:
        return [CheckResult(True, "graph_index_format: sin gráficos declarados")]
    ...
    # Campos comunes requeridos para todos los tipos de gráfico
    COMMON_REQUIRED = ["id", "graphId", "title", "relation", "readingFocus", "component"]
    ...
    FIELDS_BY_TYPE = {
        "Coord": {
            ...
            "optional": [
                ...
                "physicalReading", "slopeMeaning", "interceptMeaning", "curvatureMeaning",
                "signMeaning", "areaMeaning", "domainMeaning",
                "modelConnection", "formulaConnection",
                "agentHints", "commonMistakes",
                "recommendedTab", "type", "graphType"
            ]
        },
        "Dcl": {
            "required": [],
            "optional": [
                "forces", "object", "freeBodyCondition", "equilibriumCondition",
                "netForce", "accelerationDirection", "referenceFrameChoice",
                "forceTypes", "signConvention",
                "agentHints", "commonMistakes",
                "recommendedTab", "graphType"
            ]
        },
        "Svg": {
            "required": [],
            "optional": [
                "sceneId", "sceneLabel", "objects", "mainObject", "referenceFrame",
                "geometry", "interactions", "modelElements", "visualCues", "scaleMeaning",
                "modelConnection", "formulaConnection",
                "physicalReading",
                "agentHints", "commonMistakes",
                "recommendedTab", "type", "graphType"
    ...
    raw_match = re.search(r'\bconst\s+rawGraphs\s*=\s*\{', active_idx)
    if raw_match:
        def _extract_bal(text: str, start: int) -> str:
            depth = 0
            i = start
            while i < len(text):
                if text[i] == '{': depth += 1
            ...
            return text[start + 1:]
        start_brace = raw_match.end() - 1
        graphs_body = _extract_bal(active_idx, start_brace)
    
    # Si no encontró rawGraphs, intentar formato directo
    if not graphs_body:
        export_match = re.search(r'export const graphs\s*=\s*\{', active_idx)
        if not export_match:
            return [CheckResult(False, "graph_index_format: no se pudo leer export const graphs ni const rawGraphs")]
    ...
    if not graphs_body:
        return [CheckResult(False, "graph_index_format: no se pudo extraer el cuerpo de graphs")]

    entry_pattern = re.compile(r'"' + re.escape(leaf_id) + r'"\s*:\s*\{')
    entry_m = entry_pattern.search(graphs_body)
    if not entry_m:
        return [CheckResult(False, f"graph_index_format: leaf '{leaf_id}' no encontrado en graphs body")]
    ...
    outer_brace = entry_m.end() - 1
    outer_body = _extract_bal(graphs_body, outer_brace)
    outer_body_start = outer_brace + 1

    for gtype in graficos:
        gtype_m = re.search(r'\b' + gtype + r'\s*:\s*(\{|(\w+))', outer_body)
        if not gtype_m:
            results.append(CheckResult(False,
                f"graph_index_format: '{gtype}' no encontrado en entry '{leaf_id}'"))
            continue

        if gtype_m.group(2):
            results.append(CheckResult(False,
                f"graph_index_format: graficos[{gtype}] usa formato abreviado legacy",
                {
                    "leaf_id": leaf_id,
                    "gtype": gtype,
                    "required_format": f'"{leaf_id}": {{ {gtype}: {{ id, graphId, title, ...campos específicos..., component }} }}',
                    "fix": "Expandir la entrada con todos los campos obligatorios",
                }))
            continue

        inner_brace_rel = gtype_m.end() - 1
        inner_brace_abs = outer_body_start + inner_brace_rel
        inner_body = _extract_bal(graphs_body, inner_brace_abs)

        # Obtener campos requeridos específicos para este tipo de gráfico
        type_fields = FIELDS_BY_TYPE.get(gtype, {"required": [], "optional": []})
        required_fields = COMMON_REQUIRED + type_fields["required"]

        missing = [f for f in required_fields if not re.search(r'\b' + f + r'\s*:', inner_body)]
        if missing:
            results.append(CheckResult(False,
                f"graph_index_format: graficos[{gtype}] faltan campos obligatorios",
                {"leaf_id": leaf_id, "gtype": gtype, "missing_fields": missing}))
        else:
            results.append(CheckResult(True,
                f"graph_index_format: graficos[{gtype}] tiene todos los campos obligatorios"))

    return results if results else [CheckResult(True, "graph_index_format: formato correcto")]
...
def validate_leaf(leaf_dir: Path) -> Dict[str, Any]:
    ...
    graficos_path = leaf_dir / "graficos.yaml"
    if graficos_path.exists():
        all_results["graficos"] = validate_graficos(graficos_path, meta_data)
    else:
        graph_types = meta_data.get("graficos") if isinstance(meta_data, dict) else []
        if isinstance(graph_types, list) and not graph_types:
            all_results["graficos"] = [CheckResult(True, "meta.yaml no declara gráficos — sin comprobación")]
        else:
            all_results["graficos"] = [CheckResult(False, "graficos.yaml no existe")]
    ...
    # CHECK 13B — Formato expandido obligatorio del registro en index.js
    if meta_data:
        all_results["graph_index_format"] = validate_graph_index_format(leaf_dir, meta_data)
