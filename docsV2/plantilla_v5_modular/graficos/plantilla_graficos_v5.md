...
- **Contrato semántico enriquecido en `graficos.yaml`**: el bloque `coord`
  debe incluir `id`, `graphId`, `type`, `graphType`, `xMagnitude`,
  `yMagnitude`, `relation`, `physicalReading`, `slopeMeaning`,
  `interceptMeaning`, `curvatureMeaning`, `signMeaning`, `areaMeaning`,
  `domainMeaning`, `modelConnection`, `formulaConnection`, `readingFocus`,
  `agentHints`, `commonMistakes`, `curveProfile`, `dataRange` y
  `recommendedTab`.

Si el Coord no declara estos tres elementos, no se puede verificar que
el gráfico muestra lo que dice mostrar → **rechazo**.

Si el Coord usa el formato legacy reducido (`title`, `xAxis`, `yAxis`,
`agentHints`) sin el contrato semántico enriquecido, el gráfico es
incompleto → **rechazo**.

`formulaConnection` debe ser string LaTeX. No se admite JSX ni
`<MathInline />` dentro de `graficos.yaml`.

## Svg

Todo gráfico Svg debe declarar:

- **Contrato semántico enriquecido en `graficos.yaml`**: el bloque `svg`
  debe incluir `id`, `graphId`, `type`, `graphType`, `sceneLabel`,
  `objects`, `mainObject`, `referenceFrame`, `geometry`, `interactions`,
  `modelElements`, `visualCues`, `scaleMeaning`, `relation`,
  `physicalReading`, `modelConnection`, `formulaConnection`,
  `readingFocus`, `agentHints`, `commonMistakes`, `animationTriggers`,
  `interactiveElements` y `recommendedTab`.
...
Si el SVG usa el formato reducido (`title`, `sceneId`, `objects`) sin
lectura física, modelo, fórmula, errores frecuentes y foco de lectura, se
considera ilustrativo → **rechazo**.

## DCL

Todo gráfico DCL debe declarar:

- **Contrato semántico enriquecido en `graficos.yaml`**: el bloque `dcl`
  debe incluir `id`, `graphId`, `type`, `graphType`, `sceneLabel`,
  `objects`, `mainObject`, `referenceFrame`, `geometry`, `interactions`,
  `modelElements`, `visualCues`, `scaleMeaning`, `relation`,
  `physicalReading`, `modelConnection`, `formulaConnection`,
  `readingFocus`, `agentHints`, `commonMistakes`, `forceVectors`,
  `equilibriumState` y `recommendedTab`.
...
Si una fuerza del DCL no declara `magnitudeRef`, `formulaRefs` y
`signConvention`, el diagrama no es trazable → **rechazo**.
...
# 8B. Formato de registro en index.js (OBLIGATORIO — Formato Expandido)

Todo gráfico Coord registrado en `export const graphs` del `index.js` del
namespace debe usar el **formato expandido** con metadatos de contexto. El
formato abreviado legacy `{ Coord: ComponentName }` está **prohibido** y el
validador lo rechaza con FAIL.

## Formato obligatorio

```js
import MiLeafGraphsCoord from "./Coord/MiLeafGraphsCoord.jsx";

export const graphs = {
  "mi-leaf": {
    Coord: {
      id: "mi-leaf-Coord",
      graphId: "mi-leaf-Coord",
      title: "Gráfica Coord · Mi Leaf",
      xAxis: "magnitud eje X con unidades (ej: volumen V (m³))",
      yAxis: "magnitud eje Y con unidades (ej: presión p (Pa))",
      relation: "Frase que describe la relación física que muestra el gráfico.",
      readingFocus: "Instrucción de lectura: qué debe observar el alumno antes de usar fórmulas.",
      component: MiLeafGraphsCoord,
    },
  },
};
```
...
- `id` — `"<leaf_id>-<graph_type>"` (ej: `"isotermico-Coord"`)
- `graphId` — igual que `id`
- `title` — `"Gráfica <Type> · <Nombre legible del leaf>"`
- `xAxis` — descripción del eje X con magnitud física real y unidades del leaf
- `yAxis` — descripción del eje Y con magnitud física real y unidades del leaf
- `relation` — una frase que explica qué relación física muestra el gráfico
- `readingFocus` — instrucción didáctica de qué leer antes de aplicar fórmulas
- `component` — referencia directa al componente importado

## Reglas

- `xAxis` e `yAxis` deben ser específicos del leaf, no genéricos ("magnitud independiente").
- `relation` debe mencionar la ecuación o la ley física gobernante cuando sea directa.
- `readingFocus` debe mencionar al menos un elemento visual concreto del gráfico (pendiente, área, hipérbola, etc.).
- El formato abreviado `{ Coord: MiComponente }` genera `FAIL: graph_index_format: graficos[Coord] usa formato abreviado legacy`.
- La ausencia de cualquier campo genera `FAIL: graph_index_format: graficos[Coord] faltan campos obligatorios`.
