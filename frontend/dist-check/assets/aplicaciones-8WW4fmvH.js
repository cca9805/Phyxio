const e=`## 1. Diseño y dimensionado de conductores de potencia
En la ingeniería eléctrica, una de las aplicaciones más críticas del modelo de conducción lineal es el dimensionado de cables para la distribución de energía. Un cable mal dimensionado no solo provoca una caída de tensión excesiva en el extremo de la carga, sino que puede sobrecalentarse debido al efecto Joule, comprometiendo el aislamiento y provocando incendios. Los ingenieros deben equilibrar el coste del material (cobre o aluminio) con la eficiencia energética del sistema.

Al aumentar la [[L]] de la línea, la [[R]] total crece proporcionalmente, lo que obliga a aumentar la sección transversal [[A]] para mantener las pérdidas bajo control. Este análisis es fundamental en el diseño de infraestructuras que van desde una vivienda particular hasta redes de transporte interurbanas. Además, el uso de materiales con baja [[rho]] es imperativo para maximizar la capacidad de transporte sin aumentar excesivamente el peso de la instalación.

Variable dominante: [[A]] (sección del conductor).
...
En el diseño de circuitos electrónicos, las resistencias se utilizan para establecer puntos de operación precisos (polarización) y para proteger componentes sensibles. Por ejemplo, un diodo LED requiere una resistencia en serie para limitar la [[I]] a un valor seguro; de lo contrario, la baja resistencia interna del diodo bajo polarización directa provocaría una corriente tan alta que destruiría la unión semiconductora en milisegundos.

Aplicando la ley fundamental, el diseñador puede calcular exactamente cuánta [[V]] debe "caer" en la resistencia para que el resto del circuito opere en condiciones óptimas. Esta aplicación es ubicua: desde el cargador de un teléfono móvil hasta los sistemas de control de un satélite, las resistencias actúan como los "reguladores de tráfico" del flujo de electrones.

Variable dominante: [[R]] (valor nominal del componente).
...
El diseño implica calcular la [[L]] y [[A]] del hilo calefactor para que, a la [[V]] de red estándar, se obtenga la potencia deseada. La relación entre magnitudes permite predecir el consumo de [[I]] y asegurar que la instalación eléctrica soporte la carga sin disparar las protecciones. Es una transformación de energía que, aunque degradada desde el punto de vista de la entropía, es extremadamente útil para procesos industriales y confort térmico.

Variable dominante: [[rho]] y [[L]] (geometría del elemento calefactor).
...
Límite de validez: Requiere muestras con geometría bien definida ([[L]] y [[A]] uniformes) para que la extracción de la resistividad sea precisa.
