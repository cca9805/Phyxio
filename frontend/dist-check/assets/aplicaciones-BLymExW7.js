const e=`## 1. Condensadores de película en electrónica de consumo
...
Límite de validez: el modelo LHI es válido mientras la frecuencia de operación esté por debajo de la frecuencia de relajación del polímero y el campo [[E_d]] no supere el 20 % de la rigidez dieléctrica del material.

## 2. Aislamiento de cables de alta tensión

Los cables de transmisión eléctrica a alta tensión (100–500 kV) están rodeados de una capa de polietileno entrecruzado (XLPE) con [[epsilon_r]] ≈ 2.3. El objetivo primario no es maximizar la capacitancia, sino aislar eléctricamente el conductor y controlar la distribución del campo [[E_d]] entre el conductor y la pantalla exterior.

Un [[epsilon_r]] bajo mantiene [[E_d]] relativamente uniforme radialmente y reduce las pérdidas dieléctricas en corriente alterna. El dieléctrico actúa aquí como escudo: reduce el campo en la región más próxima al conductor, que es donde el campo es más intenso en geometría cilíndrica. Si [[epsilon_r]] fuera más alto, la capacitancia del cable aumentaría y con ella la corriente reactiva de carga, que puede superar la capacidad térmica del cable en líneas largas.

Variable dominante: [[epsilon_r]] del aislante, que determina la capacitancia por unidad de longitud del cable y el perfil del campo [[E_d]] radial.
Límite de validez: el modelo es válido mientras [[E_d]] en el punto de mayor campo (superficie del conductor) sea inferior a la rigidez dieléctrica del XLPE, aproximadamente 20 MV/m.

## 3. Sensores capacitivos de humedad y composición

Los sensores capacitivos de humedad utilizan un material higroscópico como dieléctrico. Al absorber agua del ambiente, [[epsilon_r]] del compuesto aumenta porque el agua líquida tiene [[epsilon_r]] ≈ 80. El cambio de [[C_d]] es directamente proporcional al cambio de [[epsilon_r]] del composite, y medir [[C_d]] permite inferir el contenido de humedad.

En aplicaciones de control industrial, el mismo principio se usa para detectar el nivel de un líquido en un tanque: el condensador parcialmente sumergido tiene un [[epsilon_r]] efectivo intermedio entre el del líquido y el del aire, y [[C_d]] varía linealmente con el nivel. La calibración requiere conocer [[C_0]] con el tanque vacío y [[C_d]] con el tanque lleno para determinar el rango dinámico del sensor.

Variable dominante: la variación de [[epsilon_r]] del dieléctrico activo (composite higroscópico o mezcla líquido-aire), que modifica [[C_d]] de forma proporcional y medible.
...
El principio de funcionamiento se basa en que un material de [[epsilon_r]] alto concentra las líneas de campo en su interior, redistribuyendo el campo externo. Materiales con [[epsilon_r]] entre 50 y 300 (soluciones de calcio-sulfato o cerámicas especiales) se modelan como "lentes dieléctricas" que homogeneizan la distribución del campo [[E_d]] en el volumen de imagen. Este efecto no puede modelarse con el condensador plano ideal: requiere electrostática en geometría tridimensional, pero el parámetro clave que gobierna la redistribución sigue siendo [[epsilon_r]] del material seleccionado.
