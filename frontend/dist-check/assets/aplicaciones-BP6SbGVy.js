const e=`# Aplicaciones de la formulación diferencial de Maxwell
...
En los procesadores modernos que operan a gigahercios, el tiempo que tarda el campo en viajar entre transistores no es despreciable. La ley de Ampère-Maxwell, incluyendo la corriente de desplazamiento [[dE_dt]], es crítica para evitar interferencias electromagnéticas (EMI) que podrían corromper los datos. Los ingenieros de hardware utilizan herramientas de simulación basadas en Maxwell diferencial para mapear el "ruido" local y diseñar blindajes efectivos. A estas frecuencias, cada pista de cobre del chip se comporta como una antena, y solo la descripción local puede predecir cómo se acoplan los campos entre componentes vecinos.
- **Variable dominante:** Variación temporal del campo eléctrico [[dE_dt]].
...
Los sistemas de radar de los aviones y coches autónomos emiten pulsos que rebotan en objetos. El análisis diferencial de la divergencia [[divE]] y el rotacional permite reconstruir la forma y velocidad del objeto detectado a partir de la perturbación local que este causa en el frente de onda. En los radares de apertura sintética (SAR), la capacidad de procesar la fase local de la onda reflejada permite obtener imágenes de la superficie terrestre con resolución de centímetros, incluso a través de nubes o en total oscuridad. La forma diferencial es la clave para estos algoritmos de reconstrucción de imagen.
- **Variable dominante:** Divergencia [[divE]] y fase local de la onda.
...
En los reactores de fusión (como el ITER), el combustible es un gas cargado (plasma). La ley de Gauss [[divE]] y la densidad de carga [[rho]] local son las que dictan cómo deben configurarse los campos magnéticos para mantener el plasma confinado sin tocar las paredes del reactor. El control del plasma requiere ajustes en microsegundos basados en la lectura local de las fluctuaciones del campo. Sin la formulación diferencial, sería imposible modelar las inestabilidades magnetohidrodinámicas que amenazan con apagar la reacción de fusión.
- **Variable dominante:** Densidad de carga [[rho]] y divergencia [[divE]].
...
- **Variable dominante:** Rotacional magnético [[curlB]] y densidad de corriente [[J]].
...
- **Variable dominante:** Conductividad local y rotacional [[curlE]].
