...
Una lente funciona porque la luz cambia de velocidad al entrar en el vidrio y vuelve a cambiar al salir. Ese cambio de [[v_m]] se traduce en un cambio de direccion de los frentes de onda y permite concentrar o dispersar rayos. El indice [[n]] decide cuanta desviacion se obtiene para una geometria dada.
...
Variable dominante: [[n]] controla la reduccion de velocidad y la desviacion optica que permite formar imagenes.
...
En una fibra, el nucleo y el revestimiento tienen indices distintos para mantener la luz guiada. La onda se propaga con una velocidad de fase menor que [[c]] y con una longitud de onda interna [[lambda_m]] menor que la de vacio. Esa escala interna afecta la fase acumulada en largas distancias.

La aplicacion real exige separar estimacion simple y diseño completo. El indice efectivo da el primer retardo, pero los pulsos de datos tambien dependen de dispersion, modos, curvaturas y perdidas. Aun asi, el control de [[n]] es la base material del guiado.
...
Variable dominante: [[v_m]] gobierna el retardo de fase, mientras [[n]] controla el guiado mediante contraste entre materiales.
...
La lectura por propagacion evita decir que el prisma crea colores. El prisma separa componentes que ya estaban presentes, usando diferencias de [[n]]. La longitud de onda dentro del material tambien cambia para cada componente, lo que afecta fase y camino optico.
...
Variable dominante: [[n]] dependiente de frecuencia determina la separacion angular de las componentes.
...
La fisica del instrumento es la misma del leaf: el medio modifica [[v_m]] y [[lambda_m]] mediante [[n]]. Pero la lectura practica exige calibracion, temperatura controlada y longitud de onda conocida. Sin esas condiciones, el indice medido puede no representar la muestra de forma fiable.
...
Variable dominante: [[n]] es la magnitud medida y se interpreta como huella optica del medio.
