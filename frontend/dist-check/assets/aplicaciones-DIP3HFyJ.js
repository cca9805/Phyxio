...
\r
Variable dominante: [[v_frame]] aportada por la corriente del rio.\r
\r
Limite de validez: si la corriente cambia mucho con la posicion o con el tiempo, el modelo de velocidad relativa constante deja de ser suficiente.\r
\r
## 3. Robot sobre cinta transportadora\r
\r
En una linea de empaquetado, un robot debe colocar etiquetas sobre cajas que avanzan en una cinta. Para cerrar bien el gesto del brazo importa la velocidad de la caja respecto al robot, no solo su velocidad respecto al suelo de la nave. En esta aplicacion [[v_rel]] permite sincronizar la accion fina, mientras [[v_abs]] sigue siendo necesaria para coordinar toda la linea de produccion.\r
\r
La riqueza de este escenario es que conviven dos escalas operativas. El ingeniero de planta piensa en productividad global, para lo que mira [[v_abs]] ; el controlador local del robot piensa en error de posicion en el instante de contacto, para lo que necesita [[v_rel]] . Si ambas capas se mezclan, el brazo puede estar perfectamente programado en papel y aun asi fallar de forma sistematica en el gesto final.\r
\r
Variable dominante: [[v_abs]] de la caja respecto al suelo cuando se sincronizan varias estaciones.\r
\r
Limite de validez: si se mezclan la velocidad local caja-robot y la velocidad global caja-suelo, el tiempo de contacto sale mal aunque las unidades sean correctas.\r
\r
## 4. Analisis de video deportivo con camara movil\r
\r
Una camara montada en una motocicleta sigue a un ciclista en carrera. Para el realizador, el ciclista puede parecer casi quieto en pantalla aunque se mueva rapido respecto a la carretera. La velocidad relativa explica por que la imagen estable no implica reposo real: la camara comparte casi la misma velocidad del ciclista y hace pequena la [[v_rel]] entre ambos.\r
\r
Este ejemplo muestra muy bien la diferencia entre percepcion visual y descripcion fisica. El espectador puede inferir, por pura intuicion de imagen, que el ciclista avanza lento, cuando en realidad solo ocurre que la camara comparte su movimiento. En analisis deportivo esto importa para interpretar adelantamientos, cambios de ritmo y cierres de sprint sin dejarse enganar por la referencia movil de la grabacion.\r
\r
Variable dominante: [[v_rel]] entre ciclista y camara.\r
\r
Limite de validez: si la camara gira o acelera de forma brusca, la lectura simple de velocidad relativa ya no captura toda la geometria de la escena.\r
\r
## 5. Informacion al pasajero en un tren\r
\r
Un panel dentro de un tren quiere mostrar a la vez la velocidad del convoy respecto a la via y la velocidad de una persona caminando por el pasillo respecto al propio tren. El sistema debe evitar mezclar ambas cifras como si describieran lo mismo. Aqui la utilidad del leaf no es calcular mas rapido, sino presentar [[v_frame]] y [[v_rel]] con significado claro para que el pasajero no interprete mal la informacion.\r
\r
El valor de esta aplicacion esta en la comunicacion fisica correcta. Un pasajero puede ver en la pantalla \\(250\\;\\text{km/h}\\) para el tren y, al mismo tiempo, caminar a \\(1\\;\\text{m/s}\\) dentro del vagon. Ninguno de los dos datos invalida al otro. El leaf sirve para construir interfaces o materiales didacticos que no mezclen observadores y, por tanto, no conviertan una situacion simple en una aparente contradiccion.\r
\r
Variable dominante: la separacion conceptual entre [[v_frame]] del tren y [[v_rel]] de la persona dentro del vagon.\r
\r
Limite de validez: si la interfaz no explicita quien observa cada velocidad, el usuario puede leer dos numeros correctos como si fueran incompatibles.\r
`;export{a as default};
