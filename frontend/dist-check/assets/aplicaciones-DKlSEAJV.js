const e=`# Aplicaciones de Teorema de transporte de Reynolds
...
Variable dominante: [[B]] fija la escala de referencia y permite ordenar los datos antes de elegir metodo. Si su valor queda fuera del intervalo previsto, el problema debe replantearse desde las hipotesis.
...
En turbomaquinas, el objetivo es diagnosticar una serie de medidas. Se observan tendencias, signos y curvaturas, y despues se comprueba si la relacion del leaf puede explicarlas sin añadir mecanismos externos. El valor didactico esta en comparar datos con criterio fisico, no en ajustar parametros a ciegas.

Variable dominante: [[B]] actua como indicador de tendencia acumulada. Un cambio monotono, una inversion de signo o una saturacion dan informacion distinta sobre el fenomeno.
...
En boquillas, el leaf ayuda a escoger entre una descripcion reducida y otra mas completa. Se comienza con el modelo ideal para localizar la causa principal, y solo despues se decide si una extension aporta informacion nueva. Asi se evita confundir complejidad con precision.

Variable dominante: [[B]] separa el mecanismo esencial de los efectos accesorios. Si domina claramente, la descripcion reducida conserva valor explicativo; si compite con terminos omitidos, la reduccion pierde fuerza.
...
En depositos abiertos, la aplicacion se centra en margen de diseño. Una frontera, una geometria o una condicion inicial se modifica lentamente para ver cuanto tarda el sistema en abandonar el regimen seguro. El resultado se interpreta como margen operativo, no como cifra aislada.

Variable dominante: [[B]] mide la distancia conceptual hasta el cambio de comportamiento. Su sensibilidad frente a pequeñas variaciones indica si el diseño es robusto o delicado.
...
En balances de momento, el uso principal es evaluar comprension. El alumno debe justificar sistema, unidades, signo, escala y caso limite antes de presentar la conclusion. Esta aplicacion convierte el leaf en una guia de razonamiento y no solo en una coleccion de formulas.

Variable dominante: [[B]] funciona como pregunta de control. Si el alumno no puede explicar que representa, tampoco puede defender el resultado aunque el algebra este bien escrita.
