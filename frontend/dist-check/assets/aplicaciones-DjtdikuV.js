const e=`# Aplicaciones de Ecuaciones de Euler del solido rigido
...
Variable dominante: [[tau1]] fija la escala de referencia y permite ordenar los datos antes de elegir metodo. Si su valor queda fuera del intervalo previsto, el problema debe replantearse desde las hipotesis.
...
En giroscopos, el objetivo es diagnosticar una serie de medidas. Se observan tendencias, signos y curvaturas, y despues se comprueba si la relacion del leaf puede explicarlas sin añadir mecanismos externos. El valor didactico esta en comparar datos con criterio fisico, no en ajustar parametros a ciegas.

Variable dominante: [[tau1]] actua como indicador de tendencia acumulada. Un cambio monotono, una inversion de signo o una saturacion dan informacion distinta sobre el fenomeno.
...
En trompos asimetricos, el leaf ayuda a escoger entre una descripcion reducida y otra mas completa. Se comienza con el modelo ideal para localizar la causa principal, y solo despues se decide si una extension aporta informacion nueva. Asi se evita confundir complejidad con precision.

Variable dominante: [[tau1]] separa el mecanismo esencial de los efectos accesorios. Si domina claramente, la descripcion reducida conserva valor explicativo; si compite con terminos omitidos, la reduccion pierde fuerza.
...
En control de actitud, la aplicacion se centra en margen de diseño. Una frontera, una geometria o una condicion inicial se modifica lentamente para ver cuanto tarda el sistema en abandonar el regimen seguro. El resultado se interpreta como margen operativo, no como cifra aislada.

Variable dominante: [[tau1]] mide la distancia conceptual hasta el cambio de comportamiento. Su sensibilidad frente a pequeñas variaciones indica si el diseño es robusto o delicado.
...
En maquinas rotativas, el uso principal es evaluar comprension. El alumno debe justificar sistema, unidades, signo, escala y caso limite antes de presentar la conclusion. Esta aplicacion convierte el leaf en una guia de razonamiento y no solo en una coleccion de formulas.

Variable dominante: [[tau1]] funciona como pregunta de control. Si el alumno no puede explicar que representa, tampoco puede defender el resultado aunque el algebra este bien escrita.
