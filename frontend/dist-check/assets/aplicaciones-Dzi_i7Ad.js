const e=`## 1. Apantallamiento electromagnético en jaulas de Faraday

La jaula de Faraday es uno de los ejemplos más directos de la ley de Gauss aplicada a conductores. En estado de equilibrio electrostático, la superficie gaussiana elegida dentro del material conductor encierra carga encerrada nula y, por tanto, el flujo [[Phi_E]] a través de ella es cero. Esto implica que el campo [[E_G]] en el interior del conductor es exactamente cero, lo que a su vez significa que ningún campo exterior penetra en la región interior de una caja conductora cerrada.
...
El condensador de placas paralelas es el dispositivo electrostático más común en electrónica. La ley de Gauss aplicada a una superficie gaussiana tipo caja que atraviesa una de las placas da directamente el campo [[E_G]] entre las placas en función de la densidad superficial de carga libre. La superficie activa de la caja gaussiana es el área [[A_G]] de la placa encerrada.

El campo entre las placas es uniforme (con [[epsilon_0]] para el vacío o la permitividad del dieléctrico para otros medios), lo que permite calcular la capacitancia de forma exacta. En el diseño de **circuitos integrados**, las capas conductoras separadas por óxidos ultrafinos forman condensadores nanoscópicos cuya capacitancia se estima directamente con la fórmula derivada de la ley de Gauss. La miniaturización implica que [[A_G]] es microscópica y [[epsilon_0]] se reemplaza por la permitividad del dieléctrico del óxido.
...
Los cables de alta tensión en líneas de transmisión eléctrica producen campos eléctricos que deben mantenerse por debajo de los umbrales de ruptura dieléctrica del aire y de los niveles máximos de exposición humana. La ley de Gauss con simetría cilíndrica permite calcular [[E_G]] en función de la distancia al cable, la carga por unidad de longitud [[Q_enc]]/L y [[epsilon_0]].
...
En los reactores de fusión por confinamiento magnético, el plasma contiene cargas eléctricas separadas (iones y electrones) cuya distribución espacial determina el campo eléctrico interno. La ley de Gauss permite relacionar la carga espacial neta ([[Q_enc]] de iones menos [[Q_enc]] de electrones) con el campo eléctrico dentro del plasma.

En el plasma cuasineutral de un tokamak, [[Q_enc]] neta es casi cero y el campo eléctrico interno es muy pequeño. Las desviaciones de la neutralidad (inestabilidades de carga espacial) generan campos eléctricos que la ley de Gauss permite estimar cuantitativamente. Estos campos son la señal diagnóstica de inestabilidades que pueden comprometer el confinamiento. El control de [[Phi_E]] y [[E_G]] internos es una de las variables de monitorización continua en los sistemas de control del reactor.
