const a=`## 1. Diseno de orbita baja para observacion terrestre\r
En observacion terrestre, la altura elegida determina frecuencia de revisita, calidad espacial y costo operacional. El equipo parte de un objetivo fisico concreto: observar la misma zona con alta frecuencia sin saturar combustible de mantenimiento. Para eso compara radios orbitales cercanos y usa velocidad circular como restriccion dinamica principal.\r
\r
La hipotesis dominante es trabajar con modelo central ideal en fase preliminar. Esta simplificacion permite detectar rapidamente alternativas inviables antes de simulacion detallada. La interpretacion fisica clave es que al aumentar [[r]] disminuye [[v_orb]], pero aumenta [[T]], por lo que una mejora en cobertura continua puede venir con peor ritmo de revisita.\r
\r
...
\r
## 3. Sincronizacion de satelites en constelaciones\r
En constelaciones, pequenas diferencias de velocidad generan derivas de fase que se acumulan con el tiempo. La aplicacion practica consiste en ajustar insercion y correcciones para mantener separacion angular prevista entre plataformas. Aqui, la magnitud operacional critica no es solo velocidad instantanea, sino el periodo asociado.\r
\r
La hipotesis dominante es que todos los satelites comparten la misma fuente central y un regimen cercano a circular en el horizonte de control. La interpretacion fisica principal es que variaciones pequenas en [[T]] alteran significativamente ventanas de cobertura y enlaces con estaciones.\r
\r
...
\r
## 4. Analisis rapido de viabilidad para mision lunar o marciana\r
Al cambiar de cuerpo central, el esquema fisico se mantiene, pero cambian escalas por [[M]] y [[mu]]. Esta aplicacion permite trasladar intuicion sin copiar numeros de Tierra. En evaluacion inicial se estiman velocidades y periodos para alturas candidatas y se verifica plausibilidad de operaciones.\r
\r
La simplificacion dominante es usar campo central ideal para primera criba de escenarios. La lectura fisica importante es que una misma altura absoluta puede implicar regimenes orbitales muy diferentes en cuerpos con distinta masa. Esto cambia tiempos de orbita, ventanas de comunicacion y coste energetico.\r
\r
Variable dominante: [[mu]]\r
Límite de validez: Se requiere modelo extendido cuando objetivos cientificos exigen precision fina de trayectoria o hay perturbaciones locales relevantes.\r
\r
## 5. Diagnostico de telemetria orbital inconsistente\r
Cuando la telemetria no coincide con prediccion, la velocidad orbital sirve como prueba diagnostica inicial. Se comparan valores esperados y medidos para detectar si el problema es de datos, de conversion de unidades o de modelo fisico insuficiente. Esta aplicacion reduce tiempo de respuesta en operaciones.\r
\r
La hipotesis dominante es usar el modelo ideal como linea base de coherencia. La interpretacion fisica se centra en causalidad: si [[v_orb]] y [[T]] no respetan tendencia esperada con [[r]], hay inconsistencia que debe rastrearse antes de ejecutar correcciones costosas.\r
\r
Variable dominante: [[E]]\r
Límite de validez: El diagnostico base no reemplaza una propagacion completa cuando hay maniobras activas o perturbaciones no modeladas de alta magnitud.\r
\r
## 6. Divulgacion cientifica y alfabetizacion espacial\r
...
\r
La simplificacion dominante es usar casos de referencia con escalas conocidas para construir intuicion de orden de magnitud. La interpretacion fisica final conecta dinamica, energia y geometria en lenguaje accesible.\r
\r
Variable dominante: [[v_escape]]\r
