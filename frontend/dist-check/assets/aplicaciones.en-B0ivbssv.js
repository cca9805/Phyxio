...
Dominant variable: the wavelength [[lambda]], which determines the physical dimensions of the antenna and governs the design of the entire radiating system.
Validity limit: the plane-wave model and vacuum dispersion relation are valid when the antenna operates in free space or dry air. In environments with nearby conducting objects, reflections and diffraction modify the [[E]] and [[B]] distribution around the antenna.

## 2. Optical fibres and high-speed communications

Silica optical fibres confine and guide EM waves in the infrared band, primarily at [[lambda]] ≈ 1550 nm ([[omega]] ≈ 1.22×10¹⁵ rad/s), where material attenuation is minimal. Inside the fibre, the phase velocity is not [[c]] but [[c]] divided by the refractive index n ≈ 1.46, introducing a propagation delay of about 4.88 ns/m relative to vacuum. The more limiting problem, however, is chromatic dispersion: different spectral components of the pulse (slightly different [[omega]]) travel at different group velocities because n depends on [[omega]]. In a conventional fibre, chromatic dispersion is about 17 ps/(nm·km) at 1550 nm, broadening picosecond pulses into microseconds after thousands of kilometres. The engineering solution is to use dispersion-shifted fibres or dispersion compensators to maintain pulse integrity.

Dominant variable: the group velocity d[[omega]]/d[[k]], which controls pulse broadening and limits the data transmission rate.
...
Radar systems emit microwave pulses with [[omega]] of order 10⁹–10¹⁰ rad/s and measure the time it takes the pulse to travel to the target and back. The wave equation guarantees that the pulse travels at [[c]] in air (with a correction below 0.03%), so the round-trip time divided by two and multiplied by [[c]] gives the distance with high precision. A C-band weather radar ([[lambda]] ≈ 5 cm) detects raindrops of diameter comparable to [[lambda]] with maximum back-scattering efficiency according to Mie theory. An airborne synthetic-aperture radar (SAR) achieves decimetre spatial resolutions by coherently processing the phase of echoes received at different positions, which is only possible because the wave equation predicts phase precisely.

Dominant variable: the propagation speed [[c]], which converts pulse flight time into distance with centimetre-or-better precision.
Validity limit: the assumption of constant [[c]] in air breaks down when the troposphere has strong temperature and humidity gradients (atmospheric ducts), which can redirect waves and produce spurious echoes at incorrect apparent distances.
...
Since 1983, the SI metre has been defined exactly in terms of [[c]]: one metre is the distance light travels in vacuum in 1/299 792 458 of a second. This paradigm shift inverts the relationship between measurement and definition: previously [[c]] was measured and the metre was defined by a reference artefact; now [[c]] is an exact number with no uncertainty, and any distance measurement is obtained by measuring light transit times. High-precision laser interferometers (such as those in the LIGO gravitational-wave experiment) exploit the EM wave equation directly to convert optical phase differences into displacements with a resolution of 10⁻¹⁸ m, 17 orders of magnitude below the metre. The constant [[c]] also fixes [[epsilon0]] since the 2019 SI redefinition, linking the vacuum permeability [[mu0]] to the fine-structure constant and the elementary charge.

Dominant variable: [[c]] as the universal constant connecting time and space, and fixing the scale of all electromagnetic measurements in the SI.
Validity limit: the metrological definition of [[c]] is exact in vacuum. In material media or in regions with appreciable spacetime curvature (near massive objects), the effective propagation speed of light differs from [[c]] and instruments must be corrected accordingly.
`;export{e as default};
