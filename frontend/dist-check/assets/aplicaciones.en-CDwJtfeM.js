const e=`## 1. Electrostatic particle accelerator (Van de Graaff)
...
The relationship between stored energy and capacitor potential depends on its capacitance, but the energy principle is invariant: all the work done by the power supply charging the capacitor is stored as [[U_e]] and fully released into the load (the patient) during discharge. Interpreting [[U_e]] allows estimating whether the stored energy is sufficient to produce the therapeutic current for the required time, without needing to know the exact shape of the discharge waveform.
...
The principle that predicts how much the velocities of two isotopes differ is directly energy conservation: the potential energy [[U_e]] released by the field is the same for both, but the resulting kinetic energy is distributed differently according to each ion's mass. The relative velocity difference between two very similar isotopes (such as uranium-235 and uranium-238) is small but sufficient for designing cascade separation systems.
...
The result is expressed through the Madelung constant, which encapsulates the specific geometry of the crystal lattice. For the NaCl crystal, the cohesive energy is approximately −787 kJ per mole. This figure can be order-of-magnitude estimated using the [[U_e]] formula for a single adjacent ion pair and multiplying by the number of pairs in the mole, yielding results of the same order. The cohesive energy explains why ionic crystals have high melting points and are mechanically hard: considerable energy is needed to break the network of Coulomb interactions holding the structure together.
