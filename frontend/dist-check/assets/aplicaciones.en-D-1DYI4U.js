const e=`## 1. Power transformer cores

In a transformer, the magnetic core guides flux between coils with a relatively small magnetizing current. The reason is not that the coil changes nature, but that the material offers high [[mu_r]] over the operating segment and converts moderate [[H]] into useful [[B]] for inducing voltage in the secondary winding.
...
This application shows that "magnetic material" does not always mean "largest possible field". In a switched-mode supply, the goal is to store and transfer energy in a controlled way, keep the core cool, and prevent [[B]] from entering a region where the electronic controller loses margin.
...
A high-permeability magnetic shield does not block the field like an opaque wall. It redirects field lines through a lower-reluctance path, so the protected region is left with smaller [[B]]. The key is that the material concentrates the field inside itself without saturating.
...
Some sensors use ferromagnetic pieces to concentrate the external field in a sensitive region. The material increases local [[B]] so that a Hall, magnetoresistive, or inductive probe works with a better signal. The correct reading separates the environmental field from geometric and material amplification.
