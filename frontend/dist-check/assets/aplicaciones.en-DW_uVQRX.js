const e=`# Applications of Maxwell's Differential Formulation
...
In mobile phone antennas (5G/4G), Maxwell's equations in differential form allow calculating how current densities [[J]] in conductors generate waves that propagate through the air. Using the curl [[curlB]] is essential to understand how the signal detaches from the antenna and travels as a self-sustaining entity. Without the local description, it would be impossible to design antennas with beamforming that directs energy exactly toward the user.
- **Dominant variable:** Current density [[J]] and curl [[curlB]].
...
Radar systems in aircraft and autonomous cars emit pulses that bounce off objects. Differential analysis of divergence [[divE]] and curl allows for reconstructing the shape and speed of the detected object from the local disturbance it causes in the electromagnetic wavefront. In synthetic aperture radars (SAR), the ability to process the local phase of the reflected wave allows obtaining Earth surface images with centimeter resolution, even through clouds or in total darkness. The differential form is the key to these image reconstruction algorithms.
- **Dominant variable:** Divergence [[divE]] and local wave phase.
...
In fusion reactors (such as ITER), the fuel is a charged gas (plasma). Gauss's law [[divE]] and local charge density [[rho]] are what dictate how magnetic fields must be configured to keep the plasma confined without touching the reactor walls. Plasma control requires adjustments in microseconds based on the local reading of field fluctuations. Without the differential formulation, it would be impossible to model the magnetohydrodynamic instabilities that threaten to shut down the fusion reaction.
- **Dominant variable:** Charge density [[rho]] and divergence [[divE]].
...
- **Dominant variable:** Magnetic curl [[curlB]] and current density [[J]].
...
- **Dominant variable:** Local conductivity and curl [[curlE]].
