const e=`# Applications of transformers
...
In planning, the same transformer is checked against daily load curves rather than against a single instant. A unit that is acceptable at night may approach its thermal limit during evening demand peaks. This is why [[S]], [[I_2]], and losses [[P_p]] are interpreted together when deciding whether reinforcement is needed.

Dominant variable: apparent power [[S]] limits the maximum transformer capacity and decides how many homes can be supplied without thermal overload.
...
Dominant variable: secondary voltage [[V_2]] must remain within strict limits even when the primary grid fluctuates.
...
Dominant variable: secondary current [[I_2]] determines arc intensity, penetration, and the thermal regime of the weld.
...
Dominant variable: the transformation ratio [[N_1]]/[[N_2]] must be exact and stable throughout the operating range.
...
The optimal turns ratio is the square root of the impedance ratio. In the example, the ratio between [[N_1]] and [[N_2]] should be close to the square root of 3000 divided by 8, around 19.4. The hypothesis is that the transformer has a flat frequency response in the audio band, between 20 Hz and 20 kHz.
