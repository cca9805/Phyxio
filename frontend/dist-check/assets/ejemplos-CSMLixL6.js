...
## Situacion fisica\r
\r
Considera un punto a \`6 m\` del origen con angulo \`40 grados\` en un problema fisico donde se necesita usar describir posicion mediante radio y angulo cuando la simetria circular simplifica el problema. El objetivo no es pulsar una funcion trigonometrica al azar, sino traducir la situacion a una geometria clara. Primero se decide que representa el angulo, desde que eje se mide y que magnitud debe calcularse.\r
\r
...
\r
La traduccion usa la relacion \`x = r cos(theta), y = r sin(theta), r = sqrt(x^2+y^2)\`. El dato conocido se coloca en el papel geometrico adecuado: modulo, radio, hipotenusa, cateto, arco, componente o coordenada. Si el angulo se mide desde otro eje, la funcion usada puede cambiar.\r
\r
...
## Resultado dado\r
\r
Supongamos que aparece una expresion como \`x = r cos(theta), y = r sin(theta), r = sqrt(x^2+y^2)\` ya escrita en una solucion. La pregunta inversa es: que geometria tuvo que existir para que esa expresion fuera razonable.\r
\r
