...
Se estudia un sistema donde el routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional. El enunciado da datos coherentes para [[Routh]], [[Lagr]], [[pa]], [[qa]], [[qdot_a]], [[qi]] y pide decidir si el regimen ideal puede explicar el comportamiento observado. No se busca una sustitucion rapida: se debe construir una lectura fisica completa.

## Datos

Las magnitudes se consideran medidas en el mismo sistema de referencia y con unidades SI: [[Routh]], [[Lagr]], [[pa]], [[qa]], [[qdot_a]], [[qi]], [[E]]. La magnitud dominante es [[Routh]]. Los valores numericos se dejan en forma parametrica hasta el final para que la solucion muestre primero la estructura.
...
El modelo fisico se basa en que El Routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional. Su interes es operativo: conserva constantes y elimina variables que no aportan dinamica independiente. Por tanto, [[Routh]] organiza la lectura y las relaciones nucleares del leaf determinan que cantidades pueden compararse.

## Justificación del modelo

El modelo vale porque las correcciones omitidas se mantienen por debajo de la escala dominante y porque el caso limite esperado sigue siendo reconocible. Dejaria de valer si una perturbacion creciera hasta competir con [[Routh]] o si el signo del resultado contradijera el comportamiento observable.
...
El resultado indica como cambia el sistema cuando cambia [[Routh]]. Si [[Routh]] aumenta y el regimen dominante se refuerza, el modelo predice mayor estabilidad, mayor flujo, mayor acoplamiento o mayor restriccion segun el contexto. Si disminuye hasta anularse, debe aparecer un limite simple.
...
Si la escala principal asociada a [[Routh]] vale aproximadamente 100 unidades y la correccion observable vale entre 3 y 8 unidades, el error relativo queda entre el 3 % y el 8 %. En ese rango la tendencia puede discutirse con el modelo ideal, pero no conviene prometer precision de diseno.
...
Para cerrar el ejemplo, el alumno debe escribir una frase de sensibilidad: si [[Routh]] aumenta un orden de magnitud, la conclusion cambia de forma cualitativa o solo cuantitativa. Esa frase obliga a mirar el resultado como modelo y no como cifra aislada. Tambien debe indicar que dato experimental pediria para confirmar la hipotesis principal.

La resolucion tambien debe incluir una lectura de sensibilidad. Si [[Routh]] se duplica, el alumno debe prever si el cambio esperado es lineal, cuadratico, inverso o cualitativo. Esa frase final es parte de la solucion, porque muestra si se comprende el mecanismo y no solo la sustitucion.

Para completar la validacion, se compara el resultado con dos escenarios: uno donde [[Routh]] se reduce a la mitad y otro donde se duplica. Si la respuesta no cambia de forma compatible con la teoria, el error no esta en la aritmetica sino en la eleccion del modelo. Esta prueba de sensibilidad debe quedar escrita porque muestra si el alumno entiende el control fisico de la solucion.
`;export{e as default};
