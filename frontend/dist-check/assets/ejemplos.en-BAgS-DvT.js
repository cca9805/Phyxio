...
## System definition\r
\r
Physical system consists of source mass [[M]] and test body [[m]] in a central field. Initial state uses [[ri]] with associated [[Ui]] and kinetic component. Final state uses [[rf]] with associated [[Uf]] and final kinetic component. Output quantities are [[DeltaU]], [[DeltaK]], [[E]], and [[Wg]]. SI consistency is enforced for direct joule interpretation and robust dimensional checks.\r
\r
The kinetic term [[K]] is tracked explicitly so that mechanical-state interpretation remains consistent with conservation statements.\r
\r
## Physical model\r
\r
Selected model is conservative central gravity with effective spherical symmetry. The process is represented through energetic states. Radial distance [[r]] controls the dominant behavior of [[U]]. This model is sufficient because the question targets two-state energy transfer, not multipole perturbations or drag effects. The local form using [[h]] and [[g]] is reserved only as a secondary order estimate.\r
\r
## Model justification\r
\r
The general model is physically explicit because process scale does not support safe local linearization. The exam question focuses on energetic transition between orbital states, and this is directly encoded with [[ri]] and [[rf]]. Conservative treatment is justified because no active thrust segment or non-conservative mechanism is included in the idealized transition stage. Therefore, the core energetic relations of this leaf are both relevant and sufficient.\r
\r
## Symbolic solution\r
\r
State potential energy is introduced through:\r
\r
{{f:energia_potencial_general}}\r
\r
Two-state variation is obtained through:\r
\r
{{f:cambio_energia_potencial}}\r
\r
Conservative exchange is linked with:\r
\r
{{f:conservacion_energia}}\r
\r
Work relation is enforced with:\r
\r
{{f:trabajo_gravitatorio}}\r
\r
Global state closure is tracked by:\r
\r
{{f:energia_mecanica}}\r
\r
State classification is completed with:\r
\r
{{f:condicion_ligadura_conceptual}}\r
\r
## Numerical substitution\r
\r
Insert values of [[G]], [[M]], [[m]], [[ri]], and [[rf]] into the [[DeltaU]] relation. The computed sign is positive, consistent with outward radial transfer. From that result, [[DeltaK]] is negative under conservation, and [[Wg]] is negative from the work relation. Substitution therefore confirms that raising orbital radius requires net energetic input, even when the derivation is framed as state transition rather than propulsion microdynamics.\r
\r
...
\r
The result is not only numeric. Positive [[DeltaU]] means the final state is less gravitationally bound than the initial state. Consequently, in conservative framework, [[DeltaK]] must decrease, so orbital speed cannot increase without external energy supply. The sign of [[Wg]] confirms gravity is not delivering energy in this segment. It opposes outward motion. This causal reading links configuration, work, and motion in one coherent narrative.\r
\r
# Real-world example\r
\r
## Context\r
\r
In an Earth-observation mission, a small satellite must change altitude to improve regional coverage. Mission control needs a quick but reliable energetic estimate before running high-cost simulations. The team evaluates the main transfer with the full radial model and uses a local approximation only for a short final adjustment segment. The goal is to decide whether fuel margin is sufficient with acceptable uncertainty.\r
\r
## Physical estimation\r
\r
Primary estimate computes [[DeltaU]] from [[ri]] and [[rf]] using the full radial model. For the short adjustment segment, the team tests local scaling with:\r
\r
{{f:aproximacion_superficial}}\r
\r
Comparing both orders of magnitude reveals whether local treatment is acceptable only as a secondary approximation. In this case, the major transfer requires the general model, while the fine adjustment remains inside local validity range. This quantitative check improves planning speed and reduces risk of underestimating mission energetic demand.\r
\r
## Interpretation\r
\r
