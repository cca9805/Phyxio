...
\r
Consider un plano inclinado de altura \`2 m\` y base \`5 m\` in a physics problem where one needs to use Pythagoras, similarity, scale, and angles to build the geometry required by a physics problem. The goal is not to press a trigonometric function at random, but to translate the situation into clear geometry. First decide what the angle represents, from which axis it is measured, and which quantity must be calculated.\r
\r
...
\r
The translation uses the relation \`a^2 + b^2 = c^2, razones iguales en triangulos semejantes\`. The known datum is placed in its geometric role: magnitude, radius, hypotenuse, leg, arc, component, or coordinate. If the angle is measured from another axis, the function used may change.\r
\r
...
## Given result\r
\r
Suppose an expression such as \`a^2 + b^2 = c^2, razones iguales en triangulos semejantes\` appears already written in a solution. The inverse question is: what geometry must have existed for that expression to be reasonable?\r
\r
