...
- [[E_young]]: 210 GPa (210 times ten to the ninth Pa)
- [[nu_poisson]]: 0.29

These two data form an independent pair sufficient for an isotropic solid. [[E_young]] sets the scale of longitudinal stiffness and [[nu_poisson]] sets how that stiffness is partitioned between shear response and volumetric response. Because 0.29 lies inside the physical interval (-1, 0.5), [[G_shear]], [[K_bulk]] and [[lambda_lame]] are expected to be positive and of the order of tens or hundreds of GPa, not MPa or TPa.
...
The model uses the algebraic relationships of isotropic elasticity. The core magnitudes are [[E_young]], [[nu_poisson]], [[G_shear]], [[K_bulk]] and [[lambda_lame]]. From E and nu the remaining constants are calculated using the conversion formulae.
...
[[E_young]] and [[nu_poisson]] are substituted in the formula to obtain [[G_shear]].

Then the bulk modulus:

{{f:relacion_K_E_nu}}

[[E_young]] and [[nu_poisson]] are substituted to obtain [[K_bulk]].

Then the first Lame parameter:

{{f:relacion_lambda_E_nu}}

[[E_young]] and [[nu_poisson]] are substituted to obtain [[lambda_lame]].

Finally verification with the nu-independent relationship:

{{f:relacion_E_G_K}}

The calculated values of [[G_shear]] and [[K_bulk]] are substituted and the result is checked to equal the original [[E_young]].

## Numerical substitution

Shear modulus: 210 divided by 2 times (1 plus 0.29) gives 210 divided by 2.58, which is 81.40 GPa. Therefore [[G_shear]] ≈ 81.4 GPa.

Bulk modulus: 210 divided by 3 times (1 minus 2 times 0.29) gives 210 divided by 3 times 0.42, which is 210 divided by 1.26, which is 166.67 GPa. Therefore [[K_bulk]] ≈ 166.7 GPa.
...
Verification: 9 times 166.67 times 81.40 divided by (3 times 166.67 plus 81.40) gives 122071 divided by 581.4, which is 209.97 GPa. Matches [[E_young]] of 210 GPa within rounding. Coherence confirmed.

## Dimensional validation

- [[G_shear]]: Pa divided by dimensionless gives Pa ✓
- [[K_bulk]]: Pa divided by dimensionless gives Pa ✓
...
The ratio [[ratio_G_E]] is 0.388, typical of metals with [[nu_poisson]] around 0.3. This means the material resists shear with a stiffness that is approximately 39% of its tensile stiffness because lateral contraction couples tensile deformation to shear response in an isotropic solid.

The ratio [[ratio_K_E]] is 0.794. The material is not especially incompressible: its bulk modulus is of the same order as Young modulus. Therefore compression and tension remain comparable resistance modes; if [[nu_poisson]] were closer to 0.5 (like rubber), K would be orders of magnitude greater than E and volumetric deformation would be strongly suppressed.
...
The shear modulus is obtained directly from the transverse velocity: [[G_shear]] is density times S velocity squared, i.e. 2700 times 3100 squared, giving 25.95 GPa.

To obtain K, the relationship between P velocity and the moduli is used: P velocity squared is (K plus four thirds of G) divided by density. Solving: K is density times P velocity squared minus four thirds of G. This gives 2700 times 6350 squared minus four thirds of 25.95 times ten to the ninth, which is 108.89 minus 34.60, giving 74.29 GPa. Therefore [[K_bulk]] ≈ 74.3 GPa.

Now E is calculated with the formula as a function of G and K: 9 times 74.3 times 25.95 divided by (3 times 74.3 plus 25.95) gives 17352 divided by 248.85, which is 69.7 GPa. Therefore [[E_young]] ≈ 69.7 GPa.
...
The quantitative reading is coherent even before consulting a table: [[ratio_G_E]] is about 25.95/69.7 = 0.37 and [[ratio_K_E]] is about 74.3/69.7 = 1.07. This describes a metal that resists compression slightly more than tension and whose shear stiffness is roughly a little over one third of its longitudinal stiffness. If the estimate gave E near 20 GPa or 200 GPa for aluminium, or nu outside the expected 0.30-0.36 range, the ultrasonic measurement should be checked.
