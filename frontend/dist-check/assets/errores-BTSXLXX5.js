const e=`# Errores comunes en el análisis de redes eléctricas domésticas
...
La red eléctrica está diseñada como fuente de tensión regulada. Los transformadores mantienen la tensión secundaria dentro de márgenes estrictos mediante regulación activa. La tensión permanece en [[V]] constante; lo que aumenta es la corriente total [[I_tot]] para mantener la potencia entregada.
...
La tensión de la red es independiente de la carga en condiciones normales. Lo que limita la capacidad de la red es la corriente máxima [[I_max]] que pueden soportar los cables sin sobrecalentarse. El magnetotérmico protege contra el exceso de corriente.
...
En una conexión en serie, la tensión se divide entre los receptores. En la instalación doméstica real, todos los enchufes están conectados directamente a los conductores de fase y neutro, formando derivaciones paralelas. Cada receptor ve la tensión completa de [[V]] y extrae su propia corriente.
...
La instalación doméstica utiliza conexión en paralelo exclusivamente. Cada receptor tiene sus propios terminales conectados directamente a la línea. La corriente total [[I_tot]] es la suma de todas las corrientes individuales.
...
El estudiante lee que un circuito tiene un magnetotérmico de [[I_max]] y calcula la potencia máxima como el producto de [[V]] por [[I_max]]. Luego suma las potencias de todos los aparatos de la casa y compara directamente con ese límite.

**Por qué es incorrecto**

Aunque la potencia máxima teórica es el producto de [[V]] por [[I_max]], en la práctica los aparatos no funcionan todos simultáneamente. Sumar potencias nominales sin ponderar el uso real produce estimaciones pesimistas. El cálculo correcto requiere estimar la potencia simultánea real usando el margen de uso [[margen_uso]].
...
*Correcto*: Con un coeficiente de simultaneidad típico, la potencia simultánea esperada es menor que la suma nominal. Calculando [[I_tot]] con la fórmula correspondiente y comparando con [[I_max]] mediante el margen de uso, se verifica que un interruptor estándar puede ser suficiente con margen.
...
1. **¿Estoy usando [[V]] constante?** Si tu respuesta implica que la tensión cambia con la carga, revisa el concepto de fuente de tensión.
...
3. **¿Mi resultado para [[I_tot]] tiene sentido físico?** Un horno suele estar entre 8 a 12 A; una lámpara LED entre 0,05 a 0,2 A. Si obtienes 50 A para un aparato doméstico, has cometido un error de unidades posiblemente confundiendo vatios con kilovatios.

4. **¿He verificado el margen de uso [[margen_uso]]?** Calcula siempre la relación entre [[I_tot]] e [[I_max]]. Si es mayor que 0,8, hay riesgo de sobrecarga. Si es mayor que 1,0, el magnetotérmico debería disparar.
`;export{e as default};
