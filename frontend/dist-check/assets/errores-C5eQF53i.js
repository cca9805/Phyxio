...
La [[q]] es una cantidad acumulada; la corriente es la tasa de flujo de esa cantidad. Son tan distintas como el volumen de agua almacenado en un depósito y el caudal que sale por el grifo. Un condensador cargado tiene [[q]] máxima y corriente nula; en el instante de descargarse, la corriente es máxima y [[q]] está disminuyendo. Confundirlas produce cálculos dimensionalmente incorrectos.
...
[[q]] responde a la pregunta "¿cuánta electricidad hay acumulada?"; la corriente responde a "¿a qué ritmo fluye la carga?". La relación entre ambas involucra el tiempo: una carga grande puede fluir lentamente (corriente pequeña) o rápidamente (corriente grande).
...
El alumno sabe que la carga está cuantizada y que [[n]] debe ser entero. Al calcular [[n]] dividiendo [[q]] entre [[e]], obtiene un resultado con muchos decimales (por ejemplo, n aproximadamente igual a 6.24 por diez a la doce) y decide que debe redondearse al entero más cercano. El error es creer que el redondeo cambia el resultado físico.

**Por qué es incorrecto**

Para cargas macroscópicas del orden de microcoulombios o mayores, [[n]] es del orden de billones. El redondeo al entero más cercano no altera el resultado físico en ninguna medida apreciable: la diferencia relativa es inferior a una parte en un billón. El modelo continuo de carga (sin cuantización explícita) es una aproximación perfectamente válida a esas escalas. Forzar el redondeo no añade precisión; en el extremo opuesto, si [[n]] da un número pequeño con parte decimal significativa (por ejemplo 2.5), hay un error real en los datos de entrada.

**Señal de detección**

El alumno reporta [[n]] con decimales como resultado final de un problema de cuantización de carga microscópica, o aplica la cuantización a cargas macroscópicas sin necesidad y comete errores de redondeo acumulativos.

**Corrección conceptual**

La cuantización de la carga es relevante cuando [[n]] es pequeño (menos de unos millones). Para cargas macroscópicas, el modelo continuo es indistinguible del discreto. Si [[n]] da un resultado no entero en un contexto microscópico, hay error en los datos; si es macroscópico, el resultado es correcto y el decimal es irrelevante.

**Mini-ejemplo de contraste**

Un electrón aislado tiene [[q]] igual a [[e]] negativo y [[n]] igual a menos uno exacto. Si un experimento de Millikan mide una gota con [[q]] equivalente a tres cargas elementales, [[n]] debe ser exactamente tres; cualquier desviación indica error experimental. Para un condensador de laboratorio con carga del orden de microcoulombios, [[n]] es de orden diez elevado a trece y el decimal es irrelevante.
...
En el lenguaje cotidiano, negativo suena a "menos", a "falta", a algo inferior. El alumno transfiere este significado cultural a la física y concluye que un objeto con [[q]] negativa tiene "menos electricidad" que uno con la misma [[q]] positiva. Esta interpretación lleva a errores en el análisis de fuerzas.

**Por qué es incorrecto**

La [[q]] positiva y negativa son simétricas en módulo: un electrón con [[q]] negativa de valor [[e]] tiene exactamente el mismo efecto en magnitud que un protón con [[q]] positiva del mismo valor. El signo solo indica el tipo de portador y el sentido de la fuerza (atracción o repulsión), no la intensidad. Dos cargas iguales en módulo pero opuestas en signo producen fuerzas de igual magnitud entre sí.
...
El módulo de [[q]] determina la intensidad de la interacción; el signo determina su dirección. Cargas de signos opuestos se atraen con la misma intensidad que se repelirían si tuvieran el mismo signo. Lo que cambia es solo el sentido de la fuerza, nunca su módulo si los módulos de carga son iguales.
...
Antes de dar por bueno cualquier resultado de [[q]] o [[n]], verificar estas cuatro condiciones:
...
2. **[[n]] entero**: si [[n]] es fraccionario en un contexto de carga elemental, hay un error de unidades o de datos.
3. **Conservación**: sumar algebraicamente [[q]] inicial y [[q]] final del sistema. Si no coinciden en un sistema aislado, hay un error de planteamiento.
