const e=`# Errores comunes en la formulación integral
...
#### Mini-ejemplo
*Incorrecto*: Calcular la circulación magnética [[cirB]] integrando sobre la superficie de una esfera.
*Correcto*: Calcular la circulación magnética [[cirB]] a lo largo de un camino circular que bordea un disco plano.
...
#### Corrección conceptual
La naturaleza es "conservadora": siempre intenta mantener el status quo magnético. Si el flujo magnético [[flux_B]] aumenta, la respuesta del sistema debe ser crear un campo que lo disminuya.

#### Mini-ejemplo
*Incorrecto*: La [[emf]] es igual a la variación del flujo magnético [[dPhiB_dt]].
*Correcto*: La [[emf]] es igual a la variación del flujo magnético [[dPhiB_dt]] cambiada de signo, como se define en:
...
#### Por qué parece correcto
Porque en la mayoría de los problemas de circuitos de corriente continua, no hay condensadores o estos están ya cargados, por lo que el término [[dPhiE_dt]] es cero y la ley de Ampère original funciona perfectamente.
...
#### Señal de detección
La circulación magnética [[cirB]] calculada en un cable no coincide con la calculada justo al lado de una placa de condensador. Existe una discontinuidad en el campo magnético que no debería estar ahí.

#### Corrección conceptual
El cambio en el flujo eléctrico [[flux_E]] es, a efectos magnéticos, idéntico a una corriente física. Maxwell lo llamó "corriente de desplazamiento" para enfatizar esta equivalencia.

#### Mini-ejemplo
*Incorrecto*: Afirmar que entre las placas de un condensador el campo magnético es nulo porque no hay electrones moviéndose.
*Correcto*: Usar el término de desplazamiento para hallar la circulación [[cirB]] en el espacio vacío mediante la relación:
...
#### Por qué es incorrecto
La relación entre el vector superficie y el sentido de la integral de línea está rígidamente definida por la regla de la mano derecha. Si cambias uno sin cambiar el otro, obtendrás una [[emf]] o una [[cirB]] con el signo invertido.

#### Señal de detección
Obtienes una fuerza contra-electromotriz que en realidad es motriz, o un campo magnético que gira en sentido contrario al esperado por la corriente [[I_enc]].
...
#### Por qué parece correcto
Porque en problemas con mucha simetría, como una esfera, el flujo [[flux_E]] y el campo E son proporcionales al área, lo que lleva a confundirlos como si fueran la misma cosa.

#### Por qué es incorrecto
El flujo [[flux_E]] es una magnitud escalar que resulta de sumar (integrar) todo el campo sobre una superficie. El campo eléctrico es una magnitud vectorial que existe en cada punto. Puedes tener un campo muy intenso en un punto pero un flujo neto nulo sobre una superficie cerrada.
...
#### Mini-ejemplo
*Incorrecto*: Decir que el campo eléctrico es nulo porque el flujo neto [[flux_E]] a través de una superficie que encierra un dipolo es cero.
