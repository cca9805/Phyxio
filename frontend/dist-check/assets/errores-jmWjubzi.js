...
La rigidez (modulo de Young) mide cuanto se deforma el material bajo carga, no cuanto aguanta antes de fallar. El vidrio tiene un [[E_young]] similar al del aluminio (ambos en torno a 70 GPa) pero su resistencia a la fractura es mucho menor. Un material puede ser muy rigido y simultaneamente muy fragil.
...
[[E_young]] describe la pendiente de la curva tension-deformacion en el tramo elastico. La resistencia a la fractura es una propiedad independiente que depende de defectos, microestructura y mecanismo de fallo.

**Mini-ejemplo de contraste**

Una barra de vidrio y una de aluminio tienen [[E_young]] similar. Al aplicar la misma carga, ambas se deforman practicamente igual. Sin embargo, la barra de vidrio se rompe a una tension mucho menor que la de aluminio. Si la rigidez fuera igual a la resistencia, ambas romperían juntas, lo cual no ocurre.
...
El alumno obtiene un valor de [[nu_poisson]] superior a 0.5 o inferior a menos uno y no cuestiona el resultado. Alternativamente, al sustituir en la formula de [[K_bulk]], obtiene un valor negativo y no detecta la incoherencia.

**Correccion conceptual**

Los limites de [[nu_poisson]] son consecuencia de la positividad de [[E_young]], [[G_shear]] y [[K_bulk]]. Si [[nu_poisson]] excede 0.5, [[K_bulk]] se vuelve negativo (material inestable bajo presion). Si [[nu_poisson]] es menor que menos uno, [[G_shear]] se vuelve negativo.
...
El alumno calcula [[G_shear]] a partir de [[E_young]] y [[nu_poisson]] medidos en una direccion y obtiene un valor que difiere notablemente del medido experimentalmente en otra direccion.
...
La madera tiene [[E_young]] de aproximadamente 12 GPa en la direccion de la fibra pero solo 0.6 GPa perpendicular a ella. Si se usa el valor de 12 GPa con la formula del isotropo para estimar [[G_shear]], se obtiene un valor muy diferente del experimental. El modelo isotropo falla porque la madera es claramente anisotropa.
...
Al calcular [[E_young]], el resultado tiene unidades de fuerza (N) en lugar de presion (Pa), o bien difiere del valor esperado por varios ordenes de magnitud.

**Correccion conceptual**

La deformacion es siempre el cociente entre el cambio de longitud y la longitud original. Verificar las dimensiones del resultado: [[E_young]] debe tener unidades de presion \`[M L⁻¹ T⁻²]\`.
...
El coeficiente de Poisson mide el acoplamiento entre deformaciones, no la rigidez absoluta. El oro tiene [[nu_poisson]] de 0.42 y sin embargo es un metal con [[E_young]] de 79 GPa. La incompresibilidad (Poisson cercano a 0.5) no implica baja rigidez a traccion o cizalla.
...
[[nu_poisson]] alto indica que el material conserva su volumen al deformarse (se acerca al comportamiento incompresible). La rigidez frente a traccion la da [[E_young]] y frente a cizalla la da [[G_shear]], que son magnitudes independientes de [[nu_poisson]] en terminos de orden de magnitud.

**Mini-ejemplo de contraste**

El oro tiene [[nu_poisson]] de 0.42 (alto) y [[E_young]] de 79 GPa. El corcho tiene [[nu_poisson]] cercano a cero (bajo) y [[E_young]] de apenas unos MPa. El material con Poisson mas bajo es mucho mas blando, contradiciendo la hipotesis de que Poisson alto significa blando.

## Regla de autocontrol rapido

Verificar siempre que los tres modulos elasticos calculados ([[E_young]], [[G_shear]], [[K_bulk]]) satisfacen la relacion fundamental del isotropo. Calcular [[G_shear]] a partir de [[E_young]] y [[nu_poisson]] usando la formula del factor dos y comparar con el valor tabulado. Si la discrepancia supera el 5 %, revisar los datos de entrada y la hipotesis de isotropia.
`;export{e as default};
