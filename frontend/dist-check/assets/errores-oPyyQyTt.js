...
Las fórmulas algebraicas del leaf exigen consistencia dimensional absoluta en unidades del Sistema Internacional. Si se introduce la longitud de onda directamente en nanómetros sin convertirla multiplicando por diez a la menos nueve, mientras que el ancho de la rendija se introduce en metros, el ángulo [[theta]] o el ancho [[w]] resultante estará multiplicado por un factor erróneo de mil millones, produciendo un resultado físicamente absurdo y carente de coherencia de escala.

**Señal de detección**

El alumno obtiene un ancho del máximo central [[w]] del orden de kilómetros en lugar de milímetros, o un ángulo [[theta]] que supera en varios órdenes de magnitud el rango físico admisible. También se detecta si la longitud de onda se anota en las operaciones como un número entero grande sin potencia de diez negativa asociada.
...
La fórmula simplificada para el ancho del máximo central [[w]] es lineal y sumamente fácil de aplicar. El alumno la generaliza como la ley definitiva del leaf y la aplica a cualquier rendija, incluyendo aberturas extremadamente pequeñas cuyos anchos se acercan o son menores que la propia longitud de onda de la luz.

**Por qué es incorrecto**

La relación lineal asume que el ángulo de difracción es lo suficientemente pequeño como para aproximar su seno y su tangente al propio ángulo en radianes. Si la rendija [[a]] es comparable a la longitud de onda, el ángulo real del primer mínimo de difracción se abre tanto que supera los límites paraxiales. En este régimen no lineal, la desviación angular real es mucho mayor que la aproximación lineal y la proyección sobre la pantalla plana se deforma, haciendo que la aproximación subestime significativamente el tamaño real de la franja luminosa.

**Señal de detección**

El alumno calcula un ancho [[w]] utilizando la aproximación paraxial lineal para una rendija cuyo ancho [[a]] es casi igual a [[lambda]], obteniendo un valor lineal en la pantalla que difiere notablemente del valor real obtenido mediante la función trigonométrica tangente del ángulo exacto.

**Corrección conceptual**

Comprobar siempre que el ancho de la rendija [[a]] sea significativamente mayor que la longitud de onda [[lambda]]. Si el cociente de la longitud de onda dividido entre el ancho de la rendija es mayor a una décima, se debe abandonar el modelo lineal y calcular el ángulo exacto mediante el arcoseno, proyectándolo luego en la pantalla plana mediante la tangente trigonométrica.
...
El máximo central luminoso es perfectamente simétrico y se extiende a ambos lados del eje de incidencia central, limitándose por el primer mínimo superior y el primer mínimo inferior. Por lo tanto, el ancho total del máximo central [[w]] es exactamente dos veces la distancia lineal que hay desde el centro de la pantalla al primer mínimo lateral. Olvidar este factor multiplicador de dos es un desliz aritmético muy común que reduce a la mitad el valor real del tamaño del patrón.

**Señal de detección**

El alumno calcula la posición del primer mínimo utilizando el producto de la longitud de onda por la distancia a la pantalla dividido entre el ancho de la rendija y presenta directamente ese número como el ancho total del máximo central [[w]].

**Corrección conceptual**

Siempre recordar que el patrón difractado se expande de forma bilateral. El ancho total del máximo central luminoso [[w]] comprende la distancia total entre las dos bandas oscuras de bloqueo lateral, lo que matemáticamente equivale a dos veces la distancia centro-mínimo.
...
El ángulo [[theta]] calculado a partir del cociente entre [[lambda]] y [[a]] es una magnitud pura en radianes. Si se desea expresar en grados sexagesimales para su visualización experimental, se debe multiplicar el valor por ciento ochenta y dividirlo por pi.
...
2. El ancho del máximo central [[w]] debe ser exactamente el doble de la distancia que separa el centro de la pantalla del primer mínimo de oscuridad lateral.
...
4. El ancho de la rendija [[a]] debe ser estrictamente mayor que la longitud de onda de la luz empleada. Si el cálculo da una rendija menor o igual a [[lambda]], revisar la viabilidad del primer mínimo en el espectro.
`;export{e as default};
