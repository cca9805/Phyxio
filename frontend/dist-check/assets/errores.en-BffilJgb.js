...
The student predicts higher indices for heavy materials without consulting optical tables, or tries to estimate [[n]] from the density expressed in kilograms per cubic metre.
...
Air has a density of roughly 1.2 kg/m³ and [[n]] close to 1.0003. Silica aerogel can have a density of 100 kg/m³ yet [[n]] close to 1.01. The relationship between the two quantities is neither direct nor proportional.
...
A red laser at 633 nm enters glass with [[n]] of 1.50. Inside the glass the wavelength shrinks to 422 nm, but the light remains red because the frequency stays unchanged. If the frequency changed, the perceived colour inside the glass would be different, contradicting observation.
...
In typical classroom problems a single value of [[n]] is given for the glass, so the student assumes that number applies to every colour.

**Why it is incorrect**

The index varies with wavelength (dispersion). For BK7 glass, [[n]] is approximately 1.5168 at 589 nm but 1.5224 at 486 nm. The difference seems small, but in a prism it produces an angular separation of several degrees between red and violet.
...
For polychromatic sources the dispersion curve of the material must be used, or at least the values of [[n]] for the extreme wavelengths of the visible spectrum.

**Contrast mini-example**

An equilateral BK7 glass prism with white light produces an angular fan of approximately 1.8 degrees between red and violet. Using a single average value of [[n]], the calculation predicts a single exit ray with no colour separation, clearly contradicting the observed prismatic rainbow.
...
A ray strikes the surface of glass ([[n]] of 1.50) at 30 degrees from the surface. The correct angle from the normal is 60 degrees. Using 30 degrees directly in Snell's law gives a refraction angle of 19.5 degrees; with the correct angle (60 degrees) the result is 35.3 degrees. The difference is highly significant.
...
The student knows that nothing can travel faster than light, and if [[n]] were less than one, [[v_medio]] would exceed [[c]]. The conclusion seems direct.

**Why it is incorrect**

An index below one refers to the **phase velocity**, not the group velocity or the energy-transport velocity. Relativity forbids **information** from travelling faster than [[c]], but the phase velocity does not carry information.
...
In bands close to atomic resonances, the phase velocity can exceed [[c]] without violating relativity. The key quantity is the group velocity, which remains below [[c]] under normal conditions.

**Contrast mini-example**

X-rays in many materials have an index slightly below one (for example, 0.9999 in glass). The phase velocity slightly exceeds [[c]], but X-ray pulses travel at a group velocity below it, and no signal is transmitted faster than light.

## Quick self-control rule

Before accepting a calculation involving the refractive index, check three things: that [[n]] is greater than or equal to one for conventional media, that the wavelength in the medium is shorter than in vacuum, and that the refraction angle decreases relative to the incidence angle when the ray enters a medium of higher index. If any of these conditions is not met, review the data and the procedure.
`;export{e as default};
