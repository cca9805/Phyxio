const e=`# Common errors in household electrical network analysis
...
The electrical network is designed as a regulated voltage source. Transformers maintain the secondary voltage within strict margins through active regulation. The voltage remains at [[V]] constant; what increases is the total current [[I_tot]] to maintain the delivered power.
...
The network voltage is independent of the load under normal conditions. What limits the network capacity is the maximum current [[I_max]] that the cables can withstand without overheating. The MCB protects against excess current.
...
In a series connection, the voltage is divided between the receptors. In the real domestic installation, all sockets are connected directly to the phase and neutral conductors, forming parallel branches. Each receptor sees the full voltage of [[V]] and extracts its own current.
...
The domestic installation uses exclusively parallel connection. Each receptor has its own terminals connected directly to the line. The total current [[I_tot]] is the sum of all individual currents.
...
The student reads that a circuit has an MCB of [[I_max]] and calculates the maximum power as the product of [[V]] by [[I_max]]. Then they sum the powers of all appliances in the house and compare directly with that limit.

**Why it is incorrect**

Although the theoretical maximum power is the product of [[V]] by [[I_max]], in practice appliances do not all operate simultaneously. Summing nominal powers without weighting real use produces pessimistic estimates. The correct calculation requires estimating the real simultaneous power using the usage margin [[margen_uso]].
...
*Correct*: With a typical simultaneity coefficient, the expected simultaneous power is less than the nominal sum. Calculating [[I_tot]] with the corresponding formula and comparing with [[I_max]] through the usage margin, it is verified that a standard breaker may be sufficient with margin.
...
1. **Am I using [[V]] constant?** If your answer implies that voltage changes with load, review the concept of voltage source.
...
3. **Does my result for [[I_tot]] make physical sense?** An oven is typically between 8 to 12 A; an LED lamp between 0.05 to 0.2 A. If you get 50 A for a domestic appliance, you have made a unit error possibly confusing watts with kilowatts.

4. **Have I checked the usage margin [[margen_uso]]?** Always calculate the ratio between [[I_tot]] and [[I_max]]. If it is greater than 0.8, there is overload risk. If it is greater than 1.0, the MCB should trip.
`;export{e as default};
