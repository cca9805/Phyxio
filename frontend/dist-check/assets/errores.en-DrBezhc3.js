...
[[q]] is an accumulated quantity; current is the rate of flow of that quantity. They are as different as the volume of water stored in a tank and the flow rate coming out of the tap. A charged capacitor has maximum [[q]] and zero current; at the instant of discharging, the current is maximum and [[q]] is decreasing. Confusing them produces dimensionally incorrect calculations.
...
[[q]] answers "how much electricity is accumulated?"; current answers "how fast is charge flowing?". The relationship between the two involves time: a large charge can flow slowly (small current) or quickly (large current).
...
The student knows charge is quantized and [[n]] must be an integer. When calculating [[n]] by dividing [[q]] by [[e]], the result has many decimal places (for example, n approximately 6.24 times ten to the twelve) and decides it must be rounded to the nearest integer. The error is believing this rounding changes the physical result.

**Why it is incorrect**

For macroscopic charges of the order of microcoulombs or larger, [[n]] is of the order of trillions. Rounding to the nearest integer does not affect the physical result in any appreciable way: the relative difference is less than one part in a trillion. The continuous charge model (without explicit quantization) is a perfectly valid approximation at those scales. Forcing the rounding adds no precision; at the other extreme, if [[n]] gives a small number with a significant decimal part (e.g., 2.5), there is a real error in the input data.

**Detection signal**

The student reports [[n]] with decimal places as the final result of a microscopic charge-quantization problem, or applies quantization to macroscopic charges unnecessarily and makes cumulative rounding errors.

**Conceptual correction**

Charge quantization is relevant when [[n]] is small (fewer than a few million). For macroscopic charges, the continuous model is indistinguishable from the discrete one. If [[n]] gives a non-integer result in a microscopic context, there is a data error; if macroscopic, the result is correct and the decimal is irrelevant.

**Contrast mini-example**

An isolated electron has [[q]] equal to negative [[e]] and [[n]] exactly minus one. If a Millikan experiment measures a drop with charge equivalent to three elementary charges, [[n]] must be exactly three; any deviation indicates experimental error. For a laboratory capacitor with charge of the order of microcoulombs, [[n]] is of order ten to the thirteen and the decimal is irrelevant.
...
In everyday language, negative sounds like "less", "lacking", something inferior. The student transfers this cultural meaning to physics and concludes that an object with negative [[q]] has "less electricity" than one with the same positive [[q]]. This leads to errors in force analysis.

**Why it is incorrect**

Positive and negative [[q]] are symmetric in magnitude: an electron with negative [[q]] of value [[e]] has exactly the same effect in magnitude as a proton with positive [[q]] of the same value. The sign only indicates the type of carrier and the direction of the force (attraction or repulsion), not its intensity. Two charges equal in magnitude but opposite in sign exert forces of equal magnitude on each other.
...
The magnitude of [[q]] determines the strength of the interaction; the sign determines its direction. Charges of opposite sign attract with the same strength as same-sign charges repel. What changes is only the direction of the force, never its magnitude if the charge magnitudes are equal.
...
Before accepting any result for [[q]] or [[n]], verify these four conditions:
...
2. **Integer [[n]]**: if [[n]] is fractional in an elementary-charge context, there is a units or data error.
3. **Conservation**: add algebraically the initial and final [[q]] of the system. If they differ in an isolated system, there is a setup error.
