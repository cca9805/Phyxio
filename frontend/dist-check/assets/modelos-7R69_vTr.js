...
El modelo estándar para el análisis cuantitativo de este leaf describe la **difracción de Fraunhofer de una sola rendija en aproximación paraxial**. En este modelo, el haz de luz se considera una onda electromagnética plana coherente y monocromática que incide perpendicularmente sobre una rendija rectangular infinitamente larga y uniforme de ancho micrómetro [[a]]. El patrón de difracción resultante se proyecta sobre una pantalla de observación plana paralela colocada a una distancia lejana [[L]].
...
- **Rendija rectangular unidimensional perfecta**: la rendija es perfectamente uniforme en su anchura [[a]], y su longitud vertical es infinitamente mayor que su anchura, permitiendo analizar la difracción de forma estrictamente unidimensional lateral.
...
2. **Criterio de campo lejano de Fraunhofer**: [[L]] debe ser mayor que el cociente entre el cuadrado de [[a]] y [[lambda]]. En términos operativos de laboratorio, esto exige que [[L]] sea mayor que 100 veces [[a]].
3. **Límite de difracción escalar**: [[a]] debe ser mayor que 10 veces [[lambda]]. El ancho de la rendija debe superar en al menos diez veces la longitud de onda de la luz para garantizar que las aproximaciones de la teoría ondulatoria escalar de la luz sigan siendo físicamente válidas sin necesidad de resolver las complejas ecuaciones vectoriales de Maxwell.

## Señales de fallo del modelo

- **El ancho del máximo central medido [[w]] es visiblemente mayor que el calculado**: esto ocurre cuando la rendija [[a]] es extremadamente estrecha. La dispersión real es tan grande que el seno del ángulo no puede aproximarse linealmente, requiriendo el uso de la relación trigonométrica exacta entre [[theta]], [[lambda]] y [[a]].
...
En el modelo de Fraunhofer exacto, se elimina la hipótesis de ángulos pequeños. La posición angular del primer mínimo se determina mediante la relación trigonométrica real entre [[theta]], [[lambda]] y [[a]], y su posición física proyectada sobre la pantalla plana a distancia [[L]] se calcula utilizando la tangente trigonométrica exacta.
Este modelo es esencial cuando la rendija es del orden de pocos micrómetros, provocando ángulos de dispersión amplios.

En el modelo de difracción de Fresnel, la pantalla se coloca en la zona de campo cercano, cuando [[L]] es menor o comparable con la distancia de Fresnel construida a partir de [[a]] y [[lambda]]. Las ondas secundarias de Huygens deben integrarse manteniendo los términos cuadráticos de la distancia, lo que da lugar a una distribución caracterizada por las integrales de Fresnel. El patrón en la pantalla ya no muestra un único máximo central definido, sino que varía entre un pico brillante y un mínimo oscuro en el eje central según la distancia exacta, dibujando una silueta con múltiples máximos secundarios muy oscilantes.

Cuando conviene pasar al modelo exacto no paraxial: cuando la relación entre [[lambda]] y [[a]] supera el umbral 0.05, lo que causa que el error cometido por la aproximación lineal en el cálculo del ancho del máximo central supere el cinco por ciento de error admisible.

Cuando conviene pasar al modelo de difracción de Fresnel: cuando [[L]] es inferior al límite de Fresnel o resulta menor que 100 veces [[a]] para la mayoría de rendijas de laboratorio típicas, haciendo inviable la asunción de ondas incidentes planas en la pantalla.
...
| Relación del ancho [[w]] con [[L]] | Perfectamente lineal | Ligeramente no lineal | No lineal y oscilatoria compleja |
| Geometría de frentes de onda | Onda plana ideal | Onda plana ideal | Onda esférica en pantalla |
| Rango típico en laboratorio | [[L]] mayor que 1.0 m, [[a]] cerca de 0.1 mm | [[L]] mayor que 1.0 m, [[a]] menor que 0.01 mm | [[L]] menor que 0.1 m, [[a]] cerca de 0.5 mm |
`;export{e as default};
