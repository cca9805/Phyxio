const e=`# Modelos del condensador

## Modelo ideal

El **modelo de condensador ideal** describe un dispositivo con dos armaduras conductoras planas paralelas de área [[C_plana]] efectiva uniforme, separadas por un dieléctrico perfectamente lineal, homogéneo e isotrópico, sin pérdidas ni corriente de fuga. En este modelo, la relación entre [[Q]], [[C]] y [[V]] es exactamente lineal, y la energía almacenada [[E_cap]] responde a la fórmula con el factor 1/2, sin pérdidas en ninguna etapa del ciclo de carga y descarga.

El modelo conserva las magnitudes [[C]], [[Q]], [[V]] y [[E_cap]] en su forma más simple. Ignora la resistencia parásita en serie (ESR), la inductancia parásita (ESL), las corrientes de fuga a través del dieléctrico, la variación de la permitividad [[epsilon]] con el campo eléctrico y la temperatura, y los efectos de borde en los extremos de las armaduras.

## Hipótesis

- **Campo eléctrico uniforme entre armaduras**: requiere que la separación d sea mucho menor que las dimensiones laterales de las placas (\\(d \\ll \\sqrt{A}\\)). Si se viola, el campo en los bordes no es uniforme y la capacidad real es ligeramente mayor que la calculada con la fórmula de [[C_plana]] a partir de [[epsilon]], el área y la separación.
- **Dieléctrico lineal e isotrópico**: la polarización es proporcional al campo eléctrico en todas las direcciones. Si se viola (materiales ferroeléctricos, cerámicas PZT), la capacidad depende de la tensión aplicada y la relación entre [[Q]] y [[V]] no es lineal.
...
- **Tensión por debajo de la tensión de ruptura**: [[V]] debe permanecer por debajo del umbral de ruptura del dieléctrico. Si se viola, el dieléctrico conduce y el condensador queda destruido de forma irreversible.
...
- **No linealidad de Q vs V**: si al representar [[Q]] frente a [[V]] el conjunto de puntos no cae sobre una recta, el modelo lineal ya no aplica. Indica que la permitividad [[epsilon]] del dieléctrico depende del campo (cerámica no lineal o material ferroelectrico).
...
- **Capacidad dependiente de la frecuencia**: si [[C]] medida con un puente LCR disminuye significativamente al subir la frecuencia, la inductancia parásita ESL es relevante y el modelo ideal ya no es suficiente.
