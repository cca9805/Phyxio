...
El modelo ideal del leaf supone un elemento resistivo equivalente, parametros estables y mediciones coherentes de [[V]] e [[I]] en el mismo componente. Bajo esas hipotesis, la potencia se obtiene de forma directa con:
...
Este marco es especialmente util en etapa de predimensionado porque ofrece interpretacion rapida: crecimiento lineal de [[P]] con [[I]] o [[V]] en forma base, y sensibilidad cuadratica en formas resistivas. Tambien permite enlazar de inmediato con energia cuando [[P]] se mantiene aproximadamente constante:

{{f:energia_desde_potencia}}

Como idealizacion, el modelo omite saturaciones de fuente, variacion termica de [[R]], dispersion de tolerancias y efectos de entorno. Aun asi, es el punto de partida correcto para entender causalidad fisica y para detectar inconsistencias evidentes en datos.

## Hipótesis
1. El elemento principal puede representarse por una [[R]] efectiva positiva y bien definida.
2. Las mediciones de [[V]] e [[I]] son simultaneas o representativas del mismo estado.
...
1. [[R]] > 0 y su deriva relativa en el intervalo cumple |Delta[[R]]|/[[R]] <= 0.05.
2. Diferencia entre potencia obtenida por via directa y forma resistiva <= 10% con datos consistentes.
3. Relacion pico/promedio de [[P]] <= 2 para usar promedio unico sin perder eventos criticos.
...
1. Resultados de [[P]] incompatibles con comportamiento observado (por ejemplo, calentamiento mayor al previsto).
2. Inconsistencia sistematica entre [[V]], [[I]] y [[R]] inferida en distintas mediciones.
3. Cambios de [[R]] por temperatura superiores al umbral definido.
...
El modelo extendido incorpora acoplamiento electrotermico y, cuando aplica, naturaleza no puramente resistiva de la carga. En este enfoque, [[R]] pasa a depender de temperatura y, en algunos casos, de frecuencia o punto de operacion. La potencia se analiza por tramos temporales o mediante perfil [[P]](t), y la energia se obtiene por acumulacion coherente.

La transicion recomendada es explicita: si |Delta[[R]]|/[[R]] > 0.05 o si la discrepancia entre formulas supera 10%, cambiar de modelo. En aplicaciones de electronica de potencia, tambien conviene migrar cuando existen picos repetidos de corriente que acercan el sistema a limites termicos aunque el promedio sea aceptable.

Como alternativa adicional, puede emplearse modelo de fuente no ideal con resistencia interna y limitacion de corriente. Ese enfoque explica por que la tension efectiva cae bajo carga y modifica [[P]] disponible, mejorando prediccion en escenarios reales de baterias y convertidores.

## Comparación operativa
Modelo ideal:
1. Ventaja: rapido, transparente y didactico para establecer causalidad entre [[V]], [[I]], [[R]] y [[P]].
