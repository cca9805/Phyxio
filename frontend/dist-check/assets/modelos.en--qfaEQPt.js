const e=`# Physical Models for Torque Calculation\r
\r
## Ideal model\r
...
\r
## Hypotheses\r
Five hypotheses support the ideal model of torque calculation:\r
\r
1. **Rigid body**: the solid does not deform when force is applied, so distances [[r]] and [[b]] remain constant throughout the calculation.\r
2. **Fixed axis**: the rotation axis neither translates nor rotates during the analysis interval, allowing a unique origin to measure [[r]] and declare [[tau_z]].\r
3. **Known point force**: the force [[F]] acts at a defined point with stable direction and magnitude; components [[F_x]] and [[F_y]] are determinate during the calculation.\r
4. **Planar or reducible-to-planar geometry**: the problem can be represented in a plane with z axis normal to it, making the planar component formula sufficient.\r
5. **No dissipative losses at first pass**: bearing friction, hysteresis, and compliance are neglected in the base model and introduced only if discrepancy with data is significant.\r
\r
## Quantitative validity domain\r
A practical threshold for this leaf is to use the ideal model when estimated relative error due to losses and clearances satisfies epsilon_rel < 0.10. Between 10 and 25 percent, ideal predictions can still support preliminary decisions but must be flagged as approximate. Beyond 25 percent, relying only on the ideal model is not adequate for design or safety analysis.\r
\r
...
\r
**When to move to the extended or alternative model:** when relative error exceeds 10 percent, when model failure signals appear, or when the system operates near material limits or control saturation.\r
\r
