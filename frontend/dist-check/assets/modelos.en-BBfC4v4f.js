const e=`# Models of the Biot-Savart law

## Ideal model

The ideal model is magnetostatic: current [[I]] is steady, the conductor is thin, and the medium is approximated as vacuum. In that framework, each oriented element [[dl]] produces a contribution [[dB]] that is then added vectorially.

The central simplification is treating current as a geometric line. Distance [[r]], orientation, and angular factor [[s_theta]] are preserved, while real conductor thickness, microscopic current structure, and electromagnetic delays are ignored.

## Hypotheses

- Steady current: if [[I]] changes rapidly, propagation effects appear and the local magnetostatic model becomes incomplete.
...
- Observation point outside the ideal conductor: if [[r]] approaches zero, the line model loses physical meaning.

## Quantitative validity domain

The long-wire model is reasonable when the straight length exceeds the distance [[r]] to the point by at least one order of magnitude. As an operational criterion, using it is defensible if straight-section length > 20 times the perpendicular distance.
...
A clear failure signal appears when points treated as symmetric by the model measure different fields. If [[B]] changes strongly when the probe is moved slightly, the real geometry does not match the ideal one.

Another signal is that the field does not grow linearly with [[I]]. That may indicate heating, nonlinear magnetic material, induced currents, or a source that does not maintain steady current.
