const e=`## Ideal model
The simplified model governing this leaf is the **ideal harmonic electromagnetic plane wave** propagating in absolute vacuum. In this model, we assume that the wavefronts are infinite parallel plane surfaces perpendicular to propagation, and that the electric field vector [[E0]] and magnetic field vector [[B0]] oscillate harmonically, maintaining constant amplitudes along the entire path. Any geometric decay of amplitude with distance, dissipation by absorption in attenuating media, and any distortion by chromatic dispersion are completely ignored.
...
- **Radiation intensity**: The power density or mean intensity [[I]] must be within standard ranges, typically below 10^12 W/m^2, where non-linear effects of quantum electrodynamics and non-linear polarization of the medium are non-existent.
...
- **Phase shift between fields**: If measuring the electric field [[E0]] and the magnetic field [[B0]] reveals a phase difference other than zero, this reveals the presence of a lossy conducting medium.
- **Deviation from the amplitude relation**: If calculating the ratio [[E0]] / [[B0]] yields a result that differs from the constant value of [[c]], it indicates that the wave is propagating in a dielectric material and not in vacuum.
...
| **Complexity** | Low (simple algebraic relations like [[E0]] = [[c]] [[B0]]). | High (complex differential equations and complex constants). |
| **Speed** | Constant, invariant [[c]]. | Variable depending on the refractive index, and dispersive. |
