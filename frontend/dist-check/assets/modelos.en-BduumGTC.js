const e=`# Models for General Rotating Frame

## Ideal model

The ideal model assumes that a general rotating frame separates relative acceleration, origin acceleration, and apparent terms. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[a_abs]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[a_abs]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by a general rotating frame separates relative acceleration, origin acceleration, and apparent terms and the correction exceeds the allowed order of magnitude.
