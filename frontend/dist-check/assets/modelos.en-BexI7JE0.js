...
The ideal model assumes **plane waves** propagating in a homogeneous, isotropic, perfectly elastic solid with constant cross-section. In this model, mechanical impedance [[Z_mec]] is real, constant and frequency-independent. The wave does not attenuate and the interface between two media is treated as an instantaneous discontinuity with no thickness or transition layers.

The central simplification is ignoring dispersion, attenuation and geometric diffraction effects. This allows reducing the transmission problem to a simple algebraic relation between [[R_Z]], [[T_Z]] and the impedances on both sides.

## Hypotheses

- The solid is linearly elastic: the stress-strain relationship is proportional without hysteresis. If violated, dissipation appears and [[Z_mec]] becomes complex.
...
- Constant cross-section: [[A_sec]] does not vary in the vicinity of the interface. If it changes, distributed partial reflections occur that the model does not capture.
...
- Calculated [[R_Z]] does not match measurement at real joints: oxide layers, surface roughness or trapped air layers act as additional unmodelled interfaces.
...
The extended model introduces **complex impedance**, adding an imaginary part to [[Z_mec]] for viscoelastic materials. The real part governs energy transmission and the imaginary part represents internal material dissipation. Additionally, for multilayer systems the **transfer matrix formulation** is used: each layer is described by a 2×2 matrix relating force and velocity on both faces, and the matrix product gives the total system response.
