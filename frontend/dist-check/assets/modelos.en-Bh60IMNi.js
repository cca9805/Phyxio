...
Under these conditions, propagation is described by a second-order scalar wave equation whose phase velocity depends exclusively on [[B]] and [[rho]]. The viscosity of the fluid, thermal conductivity, rotation of the medium, and any boundary or geometry effects are ignored.
...
- **Continuous medium**: the wavelength [[lambda]] is much greater than the mean free path of molecules. If violated, the concept of compression loses meaning and molecular kinetic theory must be used instead.

- **Homogeneity and isotropy**: [[B]] and [[rho]] are constant throughout the volume. If violated (gradients of temperature, salinity, or pressure), the wave undergoes refraction and speed [[v]] varies from point to point.

- **Adiabatic compression**: there is no appreciable heat exchange during the compression-rarefaction cycle. If violated (extremely low frequencies or fluids with high thermal conductivity), [[B]] must be replaced by the isothermal modulus, altering [[v]].
...
For ideal gases at temperatures between 200 K and 2000 K and pressures between 10 kPa and 10 MPa, the adiabatic modulus relates well to pressure and the adiabatic coefficient. Outside these ranges, the gas shows deviations from ideal behaviour that affect the prediction of [[v]].
...
- **Frequency-dependent speed**: in the ideal model [[v]] does not depend on [[f]]. If experimental measurements show dispersion (high frequencies travelling at different speeds), the medium exhibits molecular relaxation or viscoelastic effects.
...
For large amplitudes, the **Burgers model** or the **Westervelt equation** include nonlinearities that describe harmonic generation, profile distortion, and eventual shock wave formation. When to switch to the viscothermal model: when measured attenuation exceeds the geometric prediction by more than 10 %, or when [[f]] exceeds the megahertz range in liquids. Switch to a nonlinear model when the acoustic pressure amplitude exceeds 1 % of the static pressure of the medium.
...
| Propagation speed | Constant, frequency-independent | Slightly frequency-dependent (dispersion) |
