const e=`# Electric motor models

## Ideal model

The ideal direct-current motor model describes a flat coil of [[N_c]] active conductors rotating in a uniform magnetic field [[B]] produced by a permanent-magnet or wound-field stator. The commutator ensures that current [[I]] in the active conductors always maintains the same relative direction with respect to the field, so that the resulting torque always acts in the same rotation direction. Electrical losses are summarised by a constant internal resistance and all other dissipations are grouped into a global efficiency [[eta]].
...
The hypotheses of the ideal model are as follows. The magnetic field [[B]] in the air gap is uniform and constant, regardless of the load. The active conductors are straight and all have the same active length [[L_c]]. The armature radius [[r_a]] is constant for all conductors. The commutator operates without sparking and without losses, keeping [[sin_theta]] close to unity at all times. The internal winding resistance is constant and independent of temperature. Magnetic losses in the core (hysteresis and eddy currents) are negligible or are implicitly included in [[eta]]. Mechanical friction at the bearings is negligible or is included in [[eta]].
...
The magnetic saturation model replaces the constant field [[B]] with a nonlinear magnetisation curve that links flux to excitation current. This model should be used when the armature current exceeds twice the nominal value or when the air-gap field approaches 1.5 T, as core saturation causes the torque-current relationship to deviate substantially from linearity.
