...
The ideal model is a monochromatic electromagnetic wave propagating in a linear, homogeneous, isotropic medium. The medium is summarized by an effective index [[n]], and that index gives the phase speed [[v_m]] and the wavelength inside the material [[lambda_m]].

This model separates what the source contributes from what the medium contributes. The source fixes frequency; the medium changes speed and spatial scale. [[c]] remains the vacuum reference, not the material speed unless the index is practically one.

## Hypotheses

- The medium is treated as linear; if response is nonlinear, [[n]] may depend on intensity.
...
The typical range of many transparent dielectrics lies between 1 < [[n]] and [[n]] < 2.5. [[n]], [[v_m]], and [[lambda_m]] must also be positive; zero or negative values do not represent the ordinary regime of this leaf.

## Model failure signals

The model fails if it predicts a speed greater than [[c]] in an ordinary medium, if it attributes a frequency change to a stationary boundary, or if it uses the same index for widely separated spectral regions. It also fails when applied to metamaterials, plasmas, or anisotropic media without additional hypotheses.
...
Ideal model: uses one real [[n]], is fast and clear for estimating [[v_m]] and [[lambda_m]], and works for monochromatic waves with small losses. Its limit is that it does not describe absorption, strong dispersion, or pulse broadening.
