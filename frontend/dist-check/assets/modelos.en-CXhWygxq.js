const e=`## Ideal model
The ideal model for Maxwell's physical interpretation is that of the **harmonic plane wave in vacuum**. In this framework, it is assumed that the electric [[E]] and magnetic [[B]] fields are perpendicular to each other and also to the direction of propagation. Furthermore, the medium is considered a perfect vacuum, meaning there are no free charges or currents that can scatter or absorb energy. Under these conditions, the ratio between field amplitudes is constant and equal to the speed of light [[c]], and energy is transported without any loss of intensity as the wave advances. This model allows for the simplification of Maxwell's vector equations into direct scalar relationships between energy densities [[u]] and average intensities [[I]], providing a clear basis for understanding electromagnetic power transport.
...
2.  **Linearity of the medium**: The vacuum (or the medium analyzed) is considered linear, isotropic, and homogeneous, which guarantees that the speed [[c]] is constant and does not depend on frequency or direction.
...
4.  **Steady or harmonic regime**: It is assumed that the sources have been oscillating long enough for the fields to follow a regular sinusoidal pattern, facilitating the use of average intensity [[I]] as a measure of power.
...
This model is extremely accurate in vacuum and air for an immense range of frequencies, from radio waves to ultraviolet light. Quantitatively, the model is applicable as long as the spatial scale L of the system meets the condition **L >> 10^-10 meters**, ensuring that we stay outside the regime where space-time quantization or atomic effects dominate. Likewise, the field magnitude ratio [[EB]] is met with negligible error as long as the medium has near-zero conductivity. In terms of intensity, the average intensity [[I]] formula is valid as long as the electric field amplitude [[E0]] does not exceed the air ionization limit (approximately 3e6 V/m at atmospheric pressure).

## Model failure signals
An analyst can detect that Maxwell's ideal interpretation model is failing if they observe the following signals:
-   **Phase shift between fields**: If [[E]] and [[B]] fields are not in phase (their peaks do not coincide in time), it means the medium has significant losses or is conductive.
-   **Inconsistency in the E/B ratio**: If the ratio between field magnitudes significantly deviates from the speed of light [[c]], it indicates the presence of reactive near fields or reflection effects.
-   **Non-geometric attenuation**: If the average intensity [[I]] decreases faster than predicted by the inverse square law of distance, absorption exists in the medium.
