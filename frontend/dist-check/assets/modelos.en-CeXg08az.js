const e=`# Physical models of the inertia tensor\r
\r
## Ideal model\r
\r
...
## Hypotheses\r
\r
- The body is rigid and internal distances do not change appreciably.\r
- The origin and reference axes are explicitly defined.\r
- The mass distribution stays fixed relative to those axes.\r
- Off-diagonal terms represent genuine products of inertia, not sign or axis mistakes.\r
- The move to principal axes is performed by an orthogonal rotation, not by changing the physical quantities themselves.\r
\r
## Quantitative validity domain\r
\r
The matrix model is reasonable when the relative deformation satisfies\r
\r
{{f:rel_l_omega}}\r
\r
and the body orientation does not change so much during the interval that the tensor must be recomputed at every instant. If Delta r / R ge 0.01, or if mass redistribution makes the matrix time dependent, a single constant tensor is no longer enough. In laboratory work, a second useful criterion is that axis or mass-location uncertainty should stay below about 5% if one wants to decide reliably whether [[producto_inercia]] is truly zero.\r
\r
## Model failure signals\r
\r
- [[momento_angular]] looks parallel to [[omega]] on paper, but measurement or simulation shows clear precession or misalignment.\r
- [[producto_inercia]] changes sign only because axes were permuted incorrectly or because the origin changed without notice.\r
- The matrix stops being symmetric because components were assembled from inconsistent frames.\r
- Elastic corrections, hinges, or sliding masses modify the tensor during the process.\r
\r
## Extended or alternative model\r
\r
The first alternative model is the principal-axis frame. There the [[producto_inercia]] terms vanish and the tensor becomes diagonal. This does not change the physical body; it only chooses axes that reveal the natural structure of the mass distribution. One should switch to this model when \`rel_l_omega\` needs to be read cleanly as a componentwise relation between principal moments and principal angular velocities.\r
\r
The second extended model appears when the body cannot be treated as rigid or when the tensor changes in time. Then a constant matrix is no longer enough: one needs a time-dependent tensor, a deformable-body model, or a full rigid-body formulation in non-aligned rotation. The transition must be explicit: if the products of inertia depend on the evolving state, one has to switch to the time-dependent model before continuing as if nothing had changed.\r
\r
## Operational comparison\r
\r
The arbitrary-axis Cartesian model is best for seeing why [[momento_angular]] and [[omega]] are not aligned. The principal-axis model is better for interpreting stability, energy, and eigen-directions. The time-dependent or deformable model is the right one when geometry changes during the motion. In practice, one should switch models when the current representation hides the cause of the phenomenon: if the arbitrary frame hides the natural modes, move to principal axes; if the rigid model hides real geometric changes, move to a time-dependent tensor.`;export{e as default};
