const e=`# Models for Constrained Systems and Dirac Brackets

## Ideal model

The ideal model assumes that dirac formalism organizes hamiltonian constraints in phase space and modifies brackets when restrictions must be preserved. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[HT]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[HT]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by dirac formalism organizes hamiltonian constraints in phase space and modifies brackets when restrictions must be preserved and the correction exceeds the allowed order of magnitude.
