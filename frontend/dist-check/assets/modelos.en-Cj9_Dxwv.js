const e=`# Models for Effective Potential and Radial Equation

## Ideal model

The ideal model assumes that effective potential converts the geometry of a central orbit into a radial energy reading. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[Ueff]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[Ueff]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by effective potential converts the geometry of a central orbit into a radial energy reading and the correction exceeds the allowed order of magnitude.
