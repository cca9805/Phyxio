...
The ideal Faraday-law model treats the circuit as a well-defined loop or coil linking changing magnetic flux. Change of [[Phi_B]] during [[Deltat]] produces emf [[epsilon_ind]] around the circuit.
...
- Turns link similar flux changes; if this fails, [[N]] does not multiply simply.
- The interval [[Deltat]] is positive and measurable; if this fails, no finite average rate exists.
- The field can be summarized by [[B_perp]] if the uniform model is used; if this fails, spatial integration is needed.
...
As a practical scale, if [[Deltat]] falls to microseconds in extended circuits, parasitics and delays may alter measured emf. In that case, the law still guides interpretation, but the elementary lumped model is insufficient.

Quantitative use criterion: in small laboratory coils, [[Deltat]] > 1 ms and circuit dimensions below 0.1 m usually allow an average estimate without resolving propagation. If [[Deltat]] < 1 microsecond, parasitics and delays should be suspected.

## Model failure signals

A failure signal is that measured [[epsilon_ind]] does not grow when flux-change rate increases. Another is that changing orientation or area produces effects not explained by the effective [[B_perp]] used.
...
| Complexity | Uses [[DeltaPhi_B]], [[Deltat]], and [[N]] | Adds integrals, parasitics, and detailed geometry |
