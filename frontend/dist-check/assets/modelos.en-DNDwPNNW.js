...
The **point-charge model** is the central idealization of this leaf. In it, the entire source charge [[q_fuente]] is concentrated at a dimensionless geometric point, and the surrounding space is vacuum with constant permittivity. Under these hypotheses, the electric field [[E]] at any point is completely determined by [[q_fuente]], the constant [[k_e]], and the distance [[r]].
...
- **Point charge**: the physical size of the source charge is negligible compared to the evaluation distance [[r]]. If this hypothesis is violated (extended object at a distance comparable to its size), the real field diverges from the point model and elemental contributions must be integrated.
...
- The computed field grows without limit as [[r]] decreases: the point model predicts an infinite field as r tends to zero, which in practice indicates that the evaluation scale has reached the actual size of the charged object.
