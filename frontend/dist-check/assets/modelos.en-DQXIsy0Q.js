const e=`# Models for Nonholonomic Constraints

## Ideal model

The ideal model assumes that a nonholonomic constraint restricts admissible velocities and cannot always be integrated into a position constraint. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[Aai]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[Aai]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by a nonholonomic constraint restricts admissible velocities and cannot always be integrated into a position constraint and the correction exceeds the allowed order of magnitude.
