const e=`# Models and Limits of the Differential Formulation

## Ideal model
The ideal model of Maxwell's differential formulation describes the behavior of electric field [[E]] and magnetic field [[B]] in absolute vacuum, assuming that space is a mathematical continuum without atomic structure and that fields are infinitely differentiable functions. In this model, interactions occur locally and the speed of light [[c]] is an insurmountable universal constant.
...
1. **Infinite Divergences**: If when calculating [[divE]] the result tends to infinity at a point, it indicates that the point charge approximation has failed and a spatial distribution or quantum model is required.
...
3. **Loss of Energy Conservation**: If in a dynamic calculation the displacement current proportional to [[dE_dt]] is not included, the resulting system violates charge conservation, a critical signal of an incomplete model failure.
