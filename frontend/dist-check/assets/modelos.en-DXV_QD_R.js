const e=`# Models of Charge Distributions

## Ideal model

The ideal continuous distribution model treats charge as a smooth mathematical function defined over an idealised geometry (wire, plane, or volume of infinite extent). It replaces the discrete nature of electrons and protons with continuous densities ([[lambda]], [[sigma]], or [[rho_vol]]) and assumes the geometry has the perfect symmetry needed to obtain a closed-form expression for [[E]].

This model preserves the essential physical quantity — the resultant field [[E]] — while ignoring edge effects, non-uniform density, and the quantum nature of charge. Its strength is producing exact analytical results for canonical geometries that serve as reference or approximation for real objects.
...
- **Computed field [[E]] decays faster than expected**: if the [[E]] vs [[r_dist]] curve falls faster than the theoretical law (faster than 1/r for the wire, or begins to fall for the plane), edge effects are already relevant.
- **Result depends on orientation**: if [[E]] has unexpected components (axial for the wire, parallel for the plane) that are not zero, symmetry is not perfect and the ideal model does not apply.
...
The extended model for real distributions requires **integrating [[dE]] directly** over the finite geometry, without assuming infinite extent. For a wire of finite length \`L\`, the field at a point on the perpendicular bisector at distance [[r_dist]] is obtained by integrating the perpendicular component of [[dE]] from one end to the other.
