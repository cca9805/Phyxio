...
The ideal model of magnetic materials uses a linear, isotropic, homogeneous, reversible medium. In that framework, [[H]] acts as applied excitation, [[M]] as proportional internal response, and [[B]] as the resulting total field. The microscopic complexity of domains, spins, and bound currents is summarized by [[mu]], [[mu_r]], and [[chi_m]].
...
- The response is reversible: if this fails, magnetic memory appears and [[M]] depends on prior history.
- The medium is treated as isotropic: if this fails, the direction of [[B]] may not coincide with [[H]].
...
The model is reasonable when the used segment of the [[B]] versus [[H]] curve keeps a nearly constant slope. As a working rule, an error below 5 % in the experimental slope allows [[mu]] to be treated as effective for introductory calculations.
...
A first failure signal is visible curvature in the [[B]] versus [[H]] graph. If doubling the excitation produces much less total-field increase than expected, the effective slope is no longer constant.

Another signal is remanence: when the excitation is removed, [[M]] does not return to the initial state. Heating by magnetic losses, frequency dependence, different up and down cycles, or a different response after rotating the sample are also suspicious.
...
| Physical reading | Clearly separates [[H]], [[M]], and [[B]] | Adds remanence, coercivity, and losses |
