const e=`# Models for Vorticity and Circulation

## Ideal model

The ideal model assumes that vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[omega]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[omega]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour and the correction exceeds the allowed order of magnitude.
