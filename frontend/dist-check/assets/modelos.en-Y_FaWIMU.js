const e=`# Models for Euler Equations for a Rigid Body

## Ideal model

The ideal model assumes that euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[tau1]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[tau1]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two and the correction exceeds the allowed order of magnitude.
