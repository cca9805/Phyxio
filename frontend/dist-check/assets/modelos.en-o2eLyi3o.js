const e=`# Models for Routhian and Cyclic Coordinates

## Ideal model

The ideal model assumes that the routhian uses cyclic coordinates to reduce a lagrangian problem without abandoning the variational structure. It preserves the structure that makes the phenomenon readable and ignores secondary effects such as residual friction, external perturbations, deformation, experimental noise, or small couplings.

The central simplification is physical: it isolates the mechanism controlling [[Routh]] and allows the result to be judged before corrections are introduced.
...
The model fails if [[Routh]] changes sign without physical cause, if a limiting case does not recover a known expression, if units do not close, or if a correction grows until it dominates the term selected as principal.
...
The explicit transition is this: when to switch to the extended model is when the observable data are no longer explained by the routhian uses cyclic coordinates to reduce a lagrangian problem without abandoning the variational structure and the correction exceeds the allowed order of magnitude.
