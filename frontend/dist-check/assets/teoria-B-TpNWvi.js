...
Aquí [[C_0]] es la capacitancia geométrica del condensador en ausencia de material, determinada únicamente por el área de las placas y su separación. El factor [[epsilon_r]] es adimensional y caracteriza el material. Para un condensador de placas paralelas, [[C_0]] se calcula como el producto de la permitividad del vacío por el área dividido entre la separación.

El campo eléctrico dentro del dieléctrico se reduce según la segunda relación del leaf:

{{f:campo_en_dielectrico}}

donde \\(E_{\\text{ext}}\\) es el campo que existiría en vacío a las mismas condiciones de carga. La reducción es consecuencia directa de la **polarización eléctrica** [[P_pol]]: la alineación de los dipolos genera cargas superficiales de signo contrario a las cargas libres de las placas, que se oponen al campo externo. La polarización volúmica se relaciona con [[E_d]] y [[epsilon_r]] según:
...
**Mecanismo microscópico de la polarización.** La polarización [[P_pol]] puede originarse por tres mecanismos distintos, que coexisten con diferente peso según el material y la frecuencia de la señal:
...
**Condición de validez del modelo lineal.** El modelo [[C_d]] = [[epsilon_r]] · [[C_0]] asume que [[epsilon_r]] es constante: el material responde proporcionalmente al campo aplicado. Esta linealidad es válida para la mayoría de los materiales a campos moderados, pero los **ferroeléctricos** muestran histéresis (la polarización depende del historial del campo) y los **paraelástricos** presentan saturación a campos altos. Fuera del régimen lineal, la relación entre capacitancia y [[epsilon_r]] deja de ser proporcional.

## Interpretación física profunda

La constante dieléctrica [[epsilon_r]] no es una propiedad misteriosa del material: es la consecuencia medible de que los dipolos internos producen un campo que se opone al externo. Cuanto más fácilmente se polarizan los dipolos (mayor susceptibilidad), mayor es el campo opuesto y mayor es [[epsilon_r]]. El campo [[E_d]] que experimenta el material es el resultado de la competición entre el campo externo (que intenta desplazar cargas) y el campo de polarización (que resiste ese desplazamiento).

La reducción de [[E_d]] no significa que el dieléctrico absorba energía: la energía almacenada en el campo es mayor con dieléctrico que sin él, precisamente porque [[C_d]] es mayor. La energía adicional proviene de la fuente que mantuvo la tensión constante al insertar el dieléctrico. Si en cambio el condensador se cargó y se desconectó antes de insertar el dieléctrico, la energía disminuye al introducirlo (la carga no cambia, pero [[C_d]] aumenta y la tensión baja).
...
Para un condensador de placas paralelas de área un centímetro cuadrado y separación un milímetro con aire entre las placas, [[C_0]] es del orden de décimas de picofaradio. Si se rellena con vidrio ordinario ([[epsilon_r]] ≈ 7), [[C_d]] sube a unas pocas picofaradios. Con titanato de bario ([[epsilon_r]] ≈ 10 000), el mismo condensador alcanzaría capacitancias del orden del nanofaradio.
...
1. **Identificar [[C_0]]**: si no se da directamente, calcularlo a partir de la geometría del condensador (área y separación para el caso plano).
2. **Leer [[epsilon_r]]** del material dado. Verificar que es adimensional y mayor o igual a uno.
3. **Calcular [[C_d]]** multiplicando [[C_0]] por [[epsilon_r]]. Si el problema pregunta por [[epsilon_r]] a partir de medidas de capacitancia, dividir [[C_d]] entre [[C_0]].
4. **Calcular [[E_d]]** si se necesita saber el campo interior: dividir el campo externo entre [[epsilon_r]].
5. **Comparar [[E_d]] con la tensión de ruptura** del material para verificar que la solución es físicamente viable.
...
**¿Insertar el dieléctrico siempre aumenta la energía almacenada?** No. Depende de cómo se hace. Si la tensión entre placas se mantiene constante (condensador conectado a una batería), la carga aumenta y la energía almacenada [[C_d]] * V² / 2 sube. Si el condensador estaba cargado y aislado (carga constante), la tensión disminuye al entrar el dieléctrico y la energía disminuye también. La diferencia entre los dos casos es la energía intercambiada con la fuente externa.
