const e=`# Principio de superposicion
...
Lo mismo ocurre con el campo eléctrico: si quieres saber el campo en un punto del espacio, calculas el campo que crearía cada carga fuente por separado y sumas todos esos campos vectorialmente. El resultado es [[E_total]], que existe en ese punto del espacio independientemente de si colocas alguna carga de prueba allí.
...
El principio de superposición para la fuerza eléctrica afirma que la fuerza total [[F_total]] sobre una carga de prueba debida a [[N_cargas]] cargas fuente es la suma vectorial de las [[N_cargas]] fuerzas de Coulomb individuales:
...
De forma completamente paralela, el campo eléctrico total [[E_total]] en cualquier punto del espacio es la suma vectorial de los campos individuales de cada carga fuente:

{{f:superposicion_campo}}

La diferencia conceptual fundamental entre las dos fórmulas es que [[F_total]] depende de la carga de prueba (su valor aparece como factor multiplicativo), mientras que [[E_total]] es una propiedad del espacio que no depende de si hay carga de prueba ni de cuánto vale. [[E_total]] preexiste; [[F_total]] solo aparece cuando se coloca una carga en ese punto.
...
> [!WARNING]
> La suma de superposición es **vectorial**, no escalar. Sumar los módulos de las fuerzas individuales sin respetar las direcciones es el error más común y siempre sobreestima [[F_total]] salvo cuando todas las contribuciones son paralelas.
...
La estructura matemática de la suma de superposición tiene consecuencias geométricas importantes. Para dos cargas del mismo signo, [[F_total]] sobre una tercera carga siempre tiene un **punto de fuerza nula** entre ellas donde las contribuciones se cancelan exactamente. Para dos cargas de signo contrario, ese punto de fuerza nula no existe entre ellas sino fuera del segmento que las une. La posición de ese punto de equilibrio depende del cociente de magnitudes de las cargas y de sus distancias al punto de prueba.

La generalización a distribuciones continuas de carga reemplaza la suma discreta por una integral: el principio de superposición sigue siendo válido pero ahora cada diferencial de carga contribuye a [[E_total]] con su propio término. Esta continuación natural es el puente hacia la ley de Gauss y el potencial eléctrico, donde la superposición se expresa de forma global en lugar de punto a punto.
...
> [!TIP]
> Para simplificar una suma de superposición, identificar primero la simetría del sistema. Si hay simetría de reflexión, muchos términos se cancelan por pares y la suma efectiva tiene muchos menos términos que [[N_cargas]].

## Interpretación física profunda

El resultado [[F_total]] nulo no significa que no haya fuerzas; significa que las fuerzas se **equilibran exactamente**. Un punto de equilibrio electrostático es una posición donde la carga de prueba no experimenta fuerza neta, pero sí experimenta fuerzas individuales de cada carga fuente. Si la carga se desplaza ligeramente del equilibrio, la suma de superposición cambia: si [[F_total]] la devuelve a la posición original, el equilibrio es **estable**; si la aleja más, es **inestable**.
...
La independencia del campo respecto a la carga de prueba tiene una implicación profunda: [[E_total]] es una **descripción completa del efecto eléctrico** en una región del espacio, válida para cualquier carga que pase por allí, sea cual sea su valor. Esta independencia es la que justifica hablar de campo eléctrico como una entidad física real, no solo como un artificio de cálculo.

## Orden de magnitud

Para un sistema de dos cargas de 1 µC separadas 10 cm, el campo de cada una en el punto medio es del orden de 3.6×10⁵ N/C. Si las cargas son del mismo signo, los campos en el punto medio son iguales y opuestos y [[E_total]] es nulo allí. Si son de signo contrario, los campos en el punto medio son iguales y del mismo sentido, y [[E_total]] dobla el valor individual: del orden de 7.2×10⁵ N/C.

Un resultado de [[E_total]] mayor de 10⁷ N/C para cargas de laboratorio (del orden de microcoulombios) debe hacer sospechar un error de unidades o una distancia expresada en centímetros sin convertir a metros.

Si [[F_total]] sobre una carga de 1 nC en ese campo de 7.2×10⁵ N/C resulta ser del orden de 7.2×10⁻⁴ N, la verificación dimensional está bien: newtons por culombio multiplicado por culombios da newtons.

## Método de resolución personalizado

Para resolver cualquier problema de superposición:

1. **Etiquetar todas las cargas fuente** con subíndices del 1 al [[N_cargas]], anotando su valor con signo.
...
**Dos cargas iguales y del mismo signo:** las contribuciones al campo en el punto medio perpendicular al segmento que las une son iguales y paralelas (ambas apuntan en la misma dirección perpendicular), por lo que el campo en ese punto no es nulo sino el doble del campo individual. En cambio, en el punto medio del segmento las contribuciones son antiparalelas y se cancelan: [[E_total]] es nulo.

**Dos cargas iguales y de signo contrario (dipolo eléctrico):** en el punto medio [[E_total]] es máximo porque ambas contribuciones apuntan en el mismo sentido. A gran distancia el campo decae como la inversa del cubo, más rápido que el de una carga aislada, por cancelación parcial entre las dos contribuciones.
...
**¿Por qué [[E_total]] puede ser nulo sin que ninguna carga individual haya desaparecido?**
