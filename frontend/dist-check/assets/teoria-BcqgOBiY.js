const e=`# Riesgos y precauciones
...
La **potencia térmica disipada** [[P_joule]] en un punto de resistencia local:

{{f:potencia_joule}}

Esta relación establece que [[P_joule]] crece cuadráticamente con la corriente [[I]] y linealmente con la resistencia local [[R]]. Un pequeño aumento de corriente produce un gran incremento de calor; un contacto degradado con resistencia elevada genera calor significativo incluso con corrientes moderadas.

La **energía térmica acumulada** durante un intervalo de tiempo:

{{f:energia_joule}}

La energía [[E_joule]] representa el daño térmico total acumulado. Depende tanto de la potencia instantánea como del tiempo de exposición [[t]]. Una potencia moderada aplicada durante muchas horas puede producir más daño que un pico de alta potencia breve.

El **incremento de temperatura** aproximado en el punto de conexión:

{{f:incremento_termico}}

El incremento térmico [[DeltaT]] depende de la potencia disipada y de la resistencia térmica local [[R_th]], que caracteriza la capacidad del punto para disipar calor al ambiente. Es el indicador físico más directamente observable del riesgo.
...
El análisis de riesgos eléctricos domésticos presenta una arquitectura física donde **el origen es local pero la consecuencia puede ser global**. Un punto de conexión degradado —un simple tornillo aflojado en una regleta— genera calor localmente mediante disipación [[P_joule]], pero este calor no se queda confinado: aumenta la temperatura ambiente del cuadro eléctrico, acelera el envejecimiento de cables adyacentes, y puede propagarse a través de conducción térmica por los conductores metálicos. Físicamente, estamos ante un **sistema acoplado térmicamente** donde la resistencia de contacto local [[R]] determina un punto caliente que funciona como fuente térmica para todo el entorno inmediato.

La **propagación del riesgo** sigue mecanismos distintos en cada etapa. Inicialmente, el calor se disipa localmente mediante convección natural y radiación térmica desde el punto degradado. Si la potencia disipada [[P_joule]] supera la capacidad de disipación térmica local (determinada por [[R_th]] y las condiciones de ventilación), la temperatura del punto crece hasta alcanzar valores que degradan el material aislante circundante. Este proceso de degradación térmica acelerada reduce la resistencia de aislamiento de cables cercanos, creando potencialmente nuevos puntos de riesgo donde originalmente no existían. La consecuencia es que un único punto de fallo puede multiplicarse en varios si no se interviene a tiempo.

El **sistema de decisión mínimo** para gestionar este riesgo estructural requiere un conjunto reducido pero suficiente de magnitudes observables: corriente [[I]] que determina la potencia disponible para disipación, temperatura local [[DeltaT]] como indicador directo de sobrecalentamiento, frecuencia de eventos térmicos como señal de degradación progresiva, y evolución temporal del síntoma como predictor de cercanía al fallo crítico. Con estas cuatro variables, un operador puede clasificar cualquier punto de la instalación en categorías de riesgo sin necesidad de instrumentación sofisticada.

La **priorización objetiva** transforma la gestión de riesgos desde decisiones subjetivas ("me parece que está caliente") a criterios cuantitativos trazables. Cuando dos puntos compiten por recursos de mantenimiento limitados, la comparación de [[E_joule]] acumulada determina cuál representa mayor daño térmico proyectado anual. La comparación de [[DeltaT]] indica cuál está más próximo a umbrales críticos de seguridad. La comparación de tendencias temporales (¿está creciendo [[R]] con el tiempo?) señala cuál degradación es irreversible y requiere intervención definitiva versus cuál puede mantenerse con monitorización.

El **dominio de validez estructural** del protocolo está definido por sus hipótesis fundamentales. Asume conexiones metálicas en régimen de corriente continua o alterna con valor eficaz estable, válido para instalaciones domésticas típicas. Asume que la resistencia de contacto [[R]] varía lentamente comparado con la constante térmica del sistema, lo cual falla si hay transitorios térmicos rápidos. Asume comportamiento lineal de las resistencias térmicas, válido para el rango de temperaturas domésticas normales pero no para situaciones de arco eléctrico o cortocircuito. El protocolo es menos aplicable a certificación normativa profesional que requiere trazabilidad metrológica completa, y no sustituye la normativa eléctrica de obligado cumplimiento. Funciona como herramienta de mantenimiento preventivo, no como auditoría de cumplimiento normativo.

**Límites del modelo y transiciones de régimen** son críticos para aplicar el protocolo correctamente. Cuando [[DeltaT]] supera aproximadamente 80–100 K, el modelo lineal de resistencia térmica deja de ser válido porque la radiación térmica (proporcional a T⁴ según Stefan-Boltzmann) domina sobre la convección. Cuando la resistencia de contacto [[R]] crece por encima de ~1 Ω, el punto se comporta más como un elemento calefactor intencional que como una conexión eléctrica, generando calor extremo con corrientes moderadas. En ambos casos extremos, el protocolo sigue siendo útil como detector de "situación anómala que requiere intervención inmediata", aunque los valores cuantitativos de [[P_joule]] y [[DeltaT]] dejen de seguir las relaciones lineales simples del modelo básico.
...
- **Corriente [[I]]**: 0-25 A en circuitos estándar. Valores superiores a 16 A continuados en regletas domésticas merecen atención.

- **Resistencia de contacto [[R]]**: < 0.01 Ω en buen estado, 0.05-0.5 Ω en contacto degradado, > 0.5 Ω en riesgo alto.

- **Potencia térmica [[P_joule]]**: < 1 W despreciable, 1-5 W atención, 5-10 W riesgo moderado, > 10 W riesgo alto requiere intervención.

- **Energía acumulada diaria [[E_joule]]**: < 10 kJ negligible, 10-50 kJ bajo, 50-200 kJ moderado, > 200 kJ alto riesgo acumulado.

- **Incremento térmico [[DeltaT]]**: < 10 K normal, 10-40 K atención, > 40 K intervención inmediata.
...
2. **Registro**: Para cada punto, anotar carga habitual [[I]], síntomas observados (temperatura, olor, holgura), y tiempo de uso diario [[t]].

3. **Estimación**: Calcular [[P_joule]] = [[I]]²·[[R]] para conexiones sospechosas y estimar el incremento térmico [[DeltaT]].
...
5. **Ordenación**: Clasificar acciones en tres categorías:
   - **Inmediata**: Riesgo alto con síntomas activos ([[P_joule]] > 10 W o [[DeltaT]] > 40 K)
...
7. **Verificación**: Revisar de nuevo en servicio real para comprobar que los indicadores han mejorado comparando [[P_joule]] antes y después.
...
**Lectura técnica**: Una elevación de casi 47 K justifica actuación rápida aunque no haya disparos de protección. Reducir la corriente a 8 A baja [[P_joule]] a 7.04 W, casi la mitad, lo que demuestra cómo la distribución de cargas reduce el riesgo.
...
Riesgos y precauciones no es una lista genérica de consejos de seguridad. Es un **método técnico de priorización y control** basado en magnitudes físicas medibles —[[P_joule]], [[E_joule]], [[DeltaT]]— que permite cortar la cadena de fallo antes de que se produzca el incidente. La precaución eficaz interviene temprano, cuantifica el riesgo, y verifica que la mejora realmente ha reducido el peligro.
`;export{e as default};
