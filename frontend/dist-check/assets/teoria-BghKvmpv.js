const a=`# Líneas de campo eléctrico

## Contexto conceptual

El campo eléctrico [[E]] es una magnitud vectorial: en cada punto del espacio tiene módulo, dirección y sentido. Escribir un número en cada punto no es suficiente para comunicar esa información de forma rápida e intuitiva. Las líneas de campo son la solución gráfica: una familia de curvas orientadas que codifica simultáneamente la dirección, el sentido y la intensidad relativa del campo en todo el espacio.
...
La regla más importante es la de la **densidad**: donde las líneas están juntas, el campo [[E]] es intenso; donde están separadas, el campo es débil. En las inmediaciones de una carga puntual las líneas se aprietan porque el campo crece mucho al acercarse. Lejos de ella se separan porque el campo decae.
...
La definición precisa de una línea de campo es: **una curva cuya tangente en cada punto es paralela al vector [[E]] en ese punto**. Matemáticamente, si la curva se parametriza como r(s), la condición es dr/ds es paralelo a E(r).
...
donde la constante de proporcionalidad k depende del número total de líneas elegido para la carga de referencia. Esta relación es la que permite "leer" el módulo de [[E]] en un diagrama sin calcular nada: basta contar las líneas por unidad de área perpendicular.

El flujo eléctrico [[phi_E]] formaliza la idea de "cuántas líneas atraviesan una superficie". Para una superficie plana en un campo uniforme y con el ángulo [[angulo_campo]] entre [[E]] y la normal:
...
> [!TIP]
> Para calcular el flujo, lo primero es identificar el ángulo entre [[E]] y la **normal exterior** a la superficie, no la normal interior. Un error en este signo invierte el signo del flujo.
...
2. **Nunca se cruzan**: la unicidad de la solución de las ecuaciones de Laplace y Poisson garantiza que [[E]] tiene un único valor en cada punto regular del espacio. Dos líneas que se cruzaran implicarían dos valores distintos de [[E]] en ese punto.
...
La ley de Gauss establece que el flujo neto [[phi_E]] a través de cualquier superficie cerrada es proporcional a la carga encerrada: **el número neto de líneas que salen es proporcional a la carga interior**. Si el mismo número entra y sale, la carga neta es cero.

> [!WARNING]
> En campos variables en el tiempo (electrodinámica), [[E]] tiene una componente inducida que genera líneas que se **cierran sobre sí mismas** (no nacen ni terminan en cargas). En ese régimen, la interpretación estática de líneas de campo es incompleta y debe complementarse con las líneas del campo magnético (campo B) y la propagación ondulatoria.
...
6. **Leer la densidad**: contar las líneas por unidad de longitud perpendicular en dos regiones para comparar módulos de [[E]] relativos.

## Casos especiales y ejemplo extendido

**Campo uniforme entre placas paralelas.** Entre las placas de un condensador ideal, las líneas son rectas, paralelas y equidistantes. Esto significa que [[E]] es constante en módulo, dirección y sentido. La densidad de líneas es la misma en todo el interior. Este es el único caso práctico donde la densidad de líneas es literalmente uniforme en una región extensa.
...
**Punto de equilibrio entre dos cargas del mismo signo.** Existe un punto donde [[E]] es nulo: las contribuciones se cancelan. Las líneas rodean ese punto sin pasar por él. Una carga de prueba allí no siente fuerza neta.

## Preguntas reales del alumno

**¿Las líneas de campo son reales o son solo una representación?** Son una representación geométrica, no objetos físicos. El campo [[E]] existe en cada punto del espacio y las líneas son una forma de visualizarlo. No hay un número fijo de líneas; elegimos cuántas trazar para que el diagrama sea informativo.

**¿Por qué una carga positiva no siempre sigue la línea de campo si la fuerza sobre ella es proporcional a [[E]]?** Para una carga positiva, la fuerza es paralela a [[E]] y por tanto tangente a la línea en ese instante. Sin embargo, si la carga tiene velocidad inicial no tangente a la línea, su inercia la desvía y la trayectoria resultante no coincide con la línea de campo.
...
**¿Cómo interpreto las líneas de campo en una zona sin cargas?** En ese espacio, las líneas pasan sin nacer ni morir. El espaciado entre ellas indica la variación de [[E]]: si se aproximan al atravesar la zona, el campo crece; si se separan, el campo decrece.
...
En electrodinámica, las líneas del campo eléctrico y las del campo magnético forman patrones acoplados en las **ondas electromagnéticas**. Las líneas de [[E]] son transversales a la dirección de propagación y perpendiculares a las líneas del campo magnético (campo B).

## Síntesis final

Las líneas de campo eléctrico son la representación geométrica del vector [[E]]: su tangente indica la dirección y sentido del campo, su densidad [[rho_lineas]] es proporcional al módulo, nacen en cargas positivas y terminan en negativas, y nunca se cruzan. El flujo [[phi_E]] formaliza cuantitativamente la idea de "líneas que atraviesan una superficie", y la ley de Gauss convierte esa cuenta de líneas en una herramienta de cálculo precisa.
`;export{a as default};
