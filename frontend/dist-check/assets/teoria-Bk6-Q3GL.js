...
Dentro del magnetismo, esta ley es el puente entre corriente electrica y campo magnetico cuando existe simetria suficiente. Permite obtener [[B]] de forma directa en hilos rectos largos, solenoides y toroides, y tambien enseña cuando una integral cerrada no basta para resolver el problema.
...
La magnitud clave no es solo [[B]], sino la circulacion [[C_B]]. Esa circulacion mide si el campo acompaña el recorrido elegido o se opone a el. La corriente encerrada [[I_enc]] funciona como fuente neta: corrientes que atraviesan la superficie en sentidos opuestos pueden compensarse.
...
La forma integral de la ley relaciona la circulacion de [[B]] con la corriente neta encerrada por la curva:

{{f:ampere_integral}}

Aqui [[dl]] representa un elemento orientado de la curva cerrada. Si [[B]] es tangente a la curva y tiene modulo constante sobre ella, la integral se simplifica. Esa es la razon por la que la ley es tan potente en problemas con simetria cilindrica o traslacional.

Para un hilo recto largo con corriente [[I]], se elige una circunferencia de radio [[r]] centrada en el hilo. Por simetria, [[B]] es tangente y constante sobre esa circunferencia:
...
La lectura formal exige comprobar siempre que la longitud usada pertenece a la misma curva donde [[B]] mantiene la simetria asumida. Tambien exige fijar una orientacion antes de asignar signos a la corriente encerrada.

## 🔴 Nivel estructural

La estructura profunda de la ley de Ampere tiene tres capas. La primera es **topologica**: la curva cerrada encierra o no encierra corriente neta. La segunda es **geometrica**: la forma de la curva debe coincidir con la simetria del campo. La tercera es **fisica**: la corriente encerrada [[I_enc]] es la fuente de la circulacion [[C_B]].

Esta organizacion explica por que la ley puede ser verdadera y, aun asi, no resolver un problema de forma directa. Si [[B]] cambia de modulo o de direccion a lo largo de la trayectoria, la integral no se convierte en un producto simple. En ese caso, Ampere sigue imponiendo una condicion global, pero no determina por si sola el campo local.

> [!WARNING]
> Elegir una curva arbitraria suele ocultar el problema. La curva amperiana debe hacer que [[B]] sea tangente, nulo o constante por tramos; si no ocurre eso, la ley no produce un calculo simple.

Tambien hay que distinguir corriente encerrada de corriente cercana. Una corriente exterior a la curva puede modificar el campo en puntos de la trayectoria, pero no cambia la corriente neta que atraviesa la superficie. Por eso [[C_B]] puede ser cero aunque existan campos locales no nulos.
...
El signo de [[I_enc]] depende de la orientacion elegida para la curva. Si inviertes el sentido del recorrido, cambia el signo de [[C_B]]. Esto no cambia el montaje fisico; cambia la convencion usada para describirlo. La regla de la mano derecha conecta ambas elecciones.
...
Un solenoide con [[n]] de 1000 espiras por metro y corriente de 2 A produce un campo interior del orden de militeslas. Un resultado de varios teslas para un solenoide escolar sin nucleo ferromagnetico suele ser absurdo; normalmente indica que se uso mal [[n]], [[I]] o las unidades de longitud.

## Método de resolución personalizado

1. Identifica si el problema pide [[C_B]], [[B]], [[B_sol]] o [[I_enc]].
2. Dibuja una curva cerrada y marca su orientacion.
3. Decide que corrientes atraviesan la superficie limitada por esa curva.
4. Comprueba si la simetria permite sacar [[B]] de la integral.
...
El primer caso limite es una curva que no encierra corriente neta. La circulacion total [[C_B]] es nula, pero eso no obliga a que [[B]] sea cero en todos los puntos. Puede haber contribuciones locales que se compensan al recorrer la curva completa.
...
Ejemplo extendido: un hilo recto muy largo transporta 5 A. A una distancia de 2 cm, una curva circular centrada en el hilo encierra toda la corriente. La simetria permite leer que [[B]] es tangente y constante. El campo resultante queda del orden de decenas de microteslas, comparable al campo terrestre. Si duplicas [[r]], el campo se reduce a la mitad.
...
**¿Una corriente fuera de la curva no importa?** No cambia [[I_enc]], pero puede contribuir al campo local sobre la trayectoria. La ley habla de circulacion neta.
