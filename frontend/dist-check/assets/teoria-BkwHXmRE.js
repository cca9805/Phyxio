...
Cuando un haz de rayos paralelos atraviesa una lente convergente, los rayos se cruzan en un punto llamado **foco**. La distancia entre el centro de la lente y el foco es la **distancia focal** [[f]]. Si la lente es divergente, los rayos divergen y sus prolongaciones se cortan en un foco virtual.

La posicion de la imagen [[d_i]] depende de donde se coloque el objeto [[d_o]] respecto al foco. Si el objeto esta mas alla del foco de una lente convergente, la imagen es **real** (puede proyectarse en pantalla) e **invertida**. Si el objeto esta entre la lente y el foco, la imagen es **virtual**, **derecha** y **amplificada**. El **aumento lateral** [[m]] cuantifica cuanto mas grande o pequena es la imagen respecto al objeto y si esta invertida o no.
...
Esta ecuacion conecta la distancia focal [[f]] con la posicion del objeto [[d_o]] y la posicion de la imagen [[d_i]]. Los signos siguen la convencion: distancias positivas en el sentido de propagacion de la luz. Para una lente convergente [[f]] es positivo; para una divergente, negativo. Si [[d_i]] resulta positivo la imagen es real; si es negativo, virtual.

El tamano y orientacion de la imagen se obtienen mediante el aumento lateral:

{{f:aumento_lateral}}

El signo negativo indica que cuando [[d_i]] y [[d_o]] son ambos positivos, [[m]] es negativo: la imagen real esta invertida. El modulo de [[m]] mayor que uno significa imagen amplificada; menor que uno, reducida.
...
Aqui [[n]] es el indice de refraccion del material y [[R1]], [[R2]] son los radios de curvatura de las superficies. Esta ecuacion muestra que la potencia optica de la lente depende tanto del material como de la geometria de sus superficies.
...
La aproximacion paraxial exige que los rayos formen angulos pequenos con el eje optico. En la practica, esto se cumple razonablemente cuando la apertura de la lente es mucho menor que su distancia focal. Para lentes de gran apertura, aparecen **aberraciones esfericas** que desplazan la imagen real respecto a la prediccion paraxial. La aberracion cromatica surge adicionalmente cuando la luz no es monocromatica, porque [[n]] depende de la longitud de onda.

La dependencia de [[d_i]] con [[d_o]] es **hiperbolica**: cuando [[d_o]] se acerca al foco, [[d_i]] tiende a infinito (la imagen se forma muy lejos). Cuando [[d_o]] vale exactamente [[f]], la ecuacion predice imagen en el infinito y el modelo pierde sentido practico local. Este comportamiento asintotico delimita la zona de operacion estable de cualquier sistema optico basado en lentes.

La ecuacion del fabricante revela que lentes de igual forma geometrica pero distinto material producen focales diferentes. Un aumento de [[n]] reduce la distancia focal, incrementando la potencia optica. Esto tiene implicaciones directas en el diseno: materiales de alto indice permiten lentes mas delgadas y ligeras con la misma potencia. Sin embargo, materiales de alto indice suelen presentar mayor dispersion, lo que introduce aberracion cromatica.

El aumento lateral [[m]] conecta directamente con la conservacion de la relacion de tamanos. La inversion de la imagen (signo negativo de [[m]]) no es un artefacto matematico sino una consecuencia geometrica del cruce de rayos en un punto: todo par de rayos que converge en un punto imagen real invierte necesariamente la orientacion del objeto. Solo cuando la imagen es virtual (los rayos no convergen realmente) la imagen mantiene la orientacion original.
...
El aumento [[m]] tiene una lectura dual: su **modulo** indica la escala espacial de la imagen y su **signo** indica la paridad de la orientacion. Esta dualidad signo-modulo es comun en magnitudes opticas y conviene tratarla explicitamente para evitar errores de interpretacion.
...
- **Paso 1**: Identificar el tipo de lente (convergente si [[f]] positivo, divergente si negativo) y los datos disponibles.
...
- **Paso 4**: Calcular [[m]] para determinar tamano y orientacion.
- **Paso 5**: Verificar coherencia de signos entre [[d_i]] y [[m]].
- **Paso 6**: Validar el resultado con un trazado de rayos principales (rayo paralelo, rayo por el centro, rayo por el foco).

## Casos especiales y ejemplo extendido

**Objeto en el infinito**: cuando [[d_o]] tiende a infinito, la ecuacion de la lente da [[d_i]] igual a [[f]]. La imagen se forma exactamente en el plano focal. Este es el caso de una camara enfocando un objeto lejano.

**Objeto en el foco**: cuando [[d_o]] es exactamente igual a [[f]] en una lente convergente, [[d_i]] tiende a infinito. Los rayos emergen paralelos y no se forma imagen a distancia finita. Este caso se utiliza en proyectores colimados y faros.

**Lente divergente con objeto real**: [[d_i]] siempre resulta negativo, independientemente de la posicion del objeto. La imagen es siempre virtual, derecha y reducida. El modulo de [[m]] es siempre menor que uno.
...
En una lente convergente con objeto a [[d_o]] igual a dos veces [[f]], la imagen se forma a la misma distancia al otro lado, con [[m]] igual a menos uno: imagen real, invertida y del mismo tamano.
