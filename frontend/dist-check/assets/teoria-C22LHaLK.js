...
En esta expresión, [[theta]] representa el ángulo del primer mínimo de difracción, medido en radianes respecto al eje central de incidencia paraxial. El parámetro [[lambda]] es la longitud de onda de la luz en metros, y [[a]] es el ancho físico de la rendija difractora en metros.

Cuando proyectamos este patrón angular sobre una pantalla plana colocada a una distancia longitudinal [[L]], la separación lineal lateral entre el primer mínimo superior y el primer mínimo inferior define el ancho total del máximo central luminoso. Bajo la aproximación de ángulos pequeños, donde la tangente y el seno del ángulo se aproximan al ángulo en radianes, el ancho lineal del máximo central [[w]] se calcula mediante la relación derivada:

{{f:ancho_maximo_central}}

Donde [[w]] es el ancho del máximo central medido en metros sobre la pantalla, y [[L]] es la distancia desde la rendija a la pantalla en metros.
...
> [!WARNING]
> En todos los cálculos, la longitud de onda [[lambda]] debe convertirse estrictamente a metros. Expresar [[lambda]] en nanómetros sin conversión mientras [[a]] y [[L]] están en metros es el error más recurrente que invalida los cálculos.
...
**Aproximación paraxial de ángulos pequeños.** La fórmula lineal para [[w]] asume desviación angular pequeña. Es fiable cuando el ancho de la rendija [[a]] es mucho mayor que la longitud de onda [[lambda]]. Si la rendija se acerca a la escala de la longitud de onda, el patrón se abre demasiado y se requiere proyección trigonométrica exacta.
...
La relación inversa entre el ancho de la rendija [[a]] y el tamaño del patrón [[w]] recuerda a las ideas de incertidumbre ondulatoria. Cuanto más se confina espacialmente la luz, menos definida queda su dirección posterior. La energía no se pierde en la rendija; se redistribuye sobre la pantalla mediante superposición de fase.
...
**Rendija de ancho comparable a la longitud de onda.** Si intentamos fabricar una rendija extremadamente estrecha donde [[a]] es igual a la longitud de onda [[lambda]], el ángulo teórico del primer mínimo de difracción requeriría que el seno del ángulo fuera igual a uno. Esto desplaza el primer punto de oscuridad exactamente a noventa grados del eje central, por lo que el máximo central luminoso se expande lateralmente hasta llenar por completo todo el espacio de propagación hacia adelante. No se formará ningún mínimo lateral definido en la pantalla.
...
La difracción describe el desvío lateral y la dispersión angular de la luz al atravesar una rendija u obstáculo microscópico. La clave es la relación entre [[lambda]] y [[a]]. En el modelo paraxial de Fraunhofer, el ángulo del primer mínimo [[theta]] está controlado por ese cociente. Sobre una pantalla a distancia [[L]], el ancho central [[w]] crece con [[lambda]] y [[L]], y decrece con [[a]]. Estas relaciones permiten calibrar montajes ópticos cuando la rendija es mucho mayor que la longitud de onda y la pantalla está lejos.
`;export{e as default};
