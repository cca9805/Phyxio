const e=`# Proporcionalidad y escala\r
\r
## Que es\r
\r
La proporcionalidad describe como una cantidad responde al cambio de otra. En fisica se usa para leer una relacion antes de sustituir numeros. La pregunta central no es solo cuanto vale una cantidad, sino que cambia cuando se modifica una condicion del problema.\r
\r
...
\r
La idea visual es esta: imagina un control que multiplicas por dos y observa si el resultado se duplica, se reduce a la mitad, se cuadruplica o cambia con una potencia. Esa imagen evita tratar todas las formulas como listas de simbolos equivalentes.\r
\r
...
\r
En simbolos, si y es proporcional a x, duplicar x duplica y; si y es proporcional a 1/x, duplicar x reduce y a la mitad; si y es proporcional a x^2, duplicar x multiplica y por cuatro. Lo importante es leer la operacion como relacion entre magnitudes y no como decoracion algebraica.\r
\r
...
\r
<!-- algebra-depth-v2 -->\r
## Capa de profundidad: proporcionalidad y escala\r
\r
Este leaf no debe funcionar como una lista corta de reglas, sino como una herramienta para pensar problemas fisicos. El objetivo es distinguir dependencia directa, inversa, cuadratica, cubica y de potencia. Un alumno basico necesita reconocer la magnitud buscada y la unidad esperada; un alumno intermedio debe justificar la operacion elegida; un alumno avanzado debe anticipar limites, dependencias y casos donde el modelo deja de servir.\r
\r
La pregunta clave es que pasa con la salida cuando una entrada se multiplica por 2, por 10 o por 1/2.\r
\r
| Relacion | Si \`x\` se duplica | Ejemplo fisico |\r
|---|---|---|\r
| \`y proportional x\` | \`y\` se duplica | distancia en MRU |\r
| \`y proportional 1/x\` | \`y\` se reduce a la mitad | resistencia equivalente simple |\r
| \`y proportional x^2\` | \`y\` se cuadruplica | energia cinetica con masa fija |\r
| \`y proportional x^3\` | \`y\` se multiplica por 8 | volumen con escala lineal |\r
\r
