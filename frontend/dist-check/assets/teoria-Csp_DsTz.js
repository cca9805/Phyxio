const a=`# Routhiano y coordenadas ciclicas

## Contexto conceptual

Routhiano y coordenadas ciclicas responde a una pregunta fisica precisa: ¿Como usa el Routhiano las coordenadas ciclicas para reducir un problema lagrangiano? El tema no se introduce para sumar una formula mas, sino para dar al alumno una herramienta de lectura cuando los modelos elementales ya no separan bien causa, escala y dominio.

El Routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional. Su interes es operativo: conserva constantes y elimina variables que no aportan dinamica independiente. Por eso las magnitudes nucleares, [[Routh]], [[Lagr]], [[pa]], [[qa]], [[qdot_a]], [[qi]], [[E]], no son una lista decorativa. Cada una marca una parte de la estructura: una escala, una frontera, una variable dinamica, una condicion de validez o una cantidad conservada.

## 🟢 Nivel esencial

La idea esencial es que el routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional. Antes de calcular, el estudiante debe poder decir que mecanismo fisico se esta aislando y que aspecto del sistema queda deliberadamente fuera. Si esa lectura no existe, la formula puede producir un numero correcto para una pregunta equivocada.

En este nivel, [[Routh]] actua como magnitud dominante porque concentra la decision didactica del leaf. No basta nombrarla: hay que explicar si aumenta, disminuye, cambia de signo, se anula o deja de tener sentido cuando cambia el regimen. Esa lectura verbal prepara el paso formal.
...
Para aplicarlas, [[Routh]], [[Lagr]], [[pa]], [[qa]], [[qdot_a]], [[qi]], [[E]] deben pertenecer al mismo sistema. Tambien hay que revisar signo, unidades y dominio. La pregunta central sigue siendo: ¿Como usa el Routhiano las coordenadas ciclicas para reducir un problema lagrangiano? Si un despeje cambia esa pregunta, el procedimiento ha dejado de ser fisica y se ha convertido en manipulacion simbolica.
...
La estructura profunda aparece al comparar el modelo con sus limites. El Routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional. Su interes es operativo: conserva constantes y elimina variables que no aportan dinamica independiente. Esta frase debe funcionar como criterio de control: permite decidir si el tema se esta usando en su dominio natural o si se esta forzando fuera de rango.
...
La interpretacion profunda empieza preguntando que representa [[Routh]] dentro de la cadena causal. Si [[Routh]] se modifica, puede cambiar el regimen, desplazar una condicion critica o revelar que una hipotesis secundaria ya no es secundaria.
...
Esta comprobacion evita conclusiones artificiales. Las unidades de [[Routh]], [[Lagr]], [[pa]], [[qa]], [[qdot_a]], [[qi]], [[E]] deben sostener la misma dimension fisica y la misma escala experimental.

## Método de resolución personalizado

Primero se delimita el sistema y se nombran las hipotesis. Segundo se identifica [[Routh]] y se decide que formula responde a la pregunta central. Tercero se calcula simbolicamente antes de sustituir numeros.
...
Al terminar, el alumno debe explicar que el routhiano aprovecha coordenadas ciclicas para reducir un problema lagrangiano sin abandonar toda la estructura variacional, aplicar las formulas en su dominio y justificar que significa fisicamente el resultado. La competencia final es decidir si el modelo responde a la pregunta del sistema real, no solo obtener una expresion algebraica.
