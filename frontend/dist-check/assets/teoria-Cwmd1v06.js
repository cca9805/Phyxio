...
La relacion cuantitativa entre la distancia del objeto [[d_o]], la distancia de la imagen [[d_i]] y la distancia focal [[f]] viene dada por la ecuacion del espejo:

{{f:ecuacion_espejo}}

Donde [[d_o]] es la distancia desde el objeto hasta el vertice del espejo, [[d_i]] la distancia desde el vertice hasta la imagen y [[f]] la distancia focal. En la convencion de signos habitual, las distancias reales (delante del espejo) son positivas y las virtuales (detras del espejo) negativas. Para un espejo concavo [[f]] es positivo; para un espejo convexo [[f]] es negativo.

El tamano relativo de la imagen se describe mediante el aumento lateral:

{{f:aumento_lateral}}

El signo de [[M]] indica la orientacion: negativo significa imagen invertida y positivo imagen derecha. El valor absoluto indica el factor de escala.

La distancia focal se relaciona con la geometria del espejo a traves de:

{{f:distancia_focal_radio}}

Donde [[R]] es el radio de curvatura de la superficie esferica. Esta relacion es exacta dentro de la aproximacion paraxial.

> [!NOTE]
> La convencion de signos es el aspecto mas critico al resolver problemas de espejos. Un error de signo en [[f]] o [[d_o]] cambia completamente la naturaleza de la imagen (real frente a virtual, invertida frente a derecha).
...
La aproximacion paraxial supone que los rayos forman angulos pequenos con el eje optico, de modo que el seno y la tangente del angulo se aproximan al angulo en radianes. Esta simplificacion es valida mientras la apertura del espejo sea mucho menor que [[R]]. Cuando la apertura es grande, los rayos marginales no convergen en el mismo punto que los paraxiales, generando **aberracion esferica**. Para corregirla se usan espejos parabolicos, cuya superficie no es esferica sino un paraboloide de revolucion que garantiza convergencia perfecta para rayos paralelos al eje.
...
La dualidad entre imagenes reales y virtuales se refleja en el signo de [[d_i]]. En espejos concavos, la transicion entre imagen real e imagen virtual ocurre exactamente cuando el objeto se situa en el foco: para [[d_o]] mayor que [[f]], la imagen es real; para [[d_o]] menor que [[f]], es virtual. En espejos convexos no existe esa transicion porque la imagen es siempre virtual con [[d_i]] negativo.

## Interpretacion fisica profunda

El signo negativo en la definicion de [[M]] codifica la inversion geometrica de la imagen. Cuando [[d_i]] y [[d_o]] tienen el mismo signo (ambos positivos para objeto real e imagen real), [[M]] resulta negativo, indicando inversion. Este resultado no es arbitrario: la inversion se produce porque los rayos reflejados cruzan el eje optico antes de converger, invirtiendo la orientacion vertical de la imagen respecto al objeto.

En el limite donde [[d_o]] tiende al infinito, [[d_i]] tiende a [[f]]: los rayos procedentes de una fuente muy lejana convergen exactamente en el foco. Este principio es la base de los telescopios reflectores y de los concentradores solares. En el otro extremo, cuando [[d_o]] se aproxima a [[f]], [[d_i]] diverge hacia el infinito, produciendo un haz de rayos reflejados practicamente paralelos, como ocurre en una linterna con espejo parabolico.

## Orden de magnitud

Para un espejo concavo de laboratorio tipico, [[R]] esta entre 0.10 y 1.00 m, de modo que [[f]] varia entre 0.05 y 0.50 m. Retrovisores de vehiculos tienen radios de 1 a 3 m (convexos, [[f]] negativo). Telescopios astronomicos emplean espejos con [[R]] de varios metros. Un resultado donde [[d_i]] supere [[d_o]] por un factor de 10 o mas deberia revisarse salvo que el objeto este muy proximo al foco. Un aumento [[M]] de magnitud superior a 5 es infrecuente fuera de montajes opticos especializados.

## Metodo de resolucion personalizado

1. **Identificar el tipo de espejo**: concavo ([[f]] positivo) o convexo ([[f]] negativo).
2. **Obtener [[f]]**: si se da [[R]], aplicar la relacion focal-radio.
3. **Sustituir en la ecuacion del espejo**: con [[d_o]] y [[f]], despejar [[d_i]].
4. **Calcular el aumento**: con [[d_i]] y [[d_o]], obtener [[M]].
5. **Interpretar signos**: [[d_i]] positivo indica imagen real; negativo, virtual. [[M]] negativo indica imagen invertida; positivo, derecha.
...
- **Objeto en el centro de curvatura** ([[d_o]] igual a [[R]]): la imagen se forma en el mismo punto, [[d_i]] igual a [[R]], con [[M]] igual a menos uno. La imagen es real, invertida y del mismo tamano que el objeto.

- **Objeto en el foco** ([[d_o]] igual a [[f]]): la ecuacion del espejo da [[d_i]] infinito. No se forma imagen finita; los rayos reflejados salen paralelos. Este caso es la base de los reflectores de linternas y faros.

- **Espejo convexo**: sin importar la posicion del objeto, [[d_i]] es siempre negativo y el modulo de [[M]] siempre menor que uno. La imagen es virtual, derecha y reducida, lo que explica la advertencia "los objetos en el espejo estan mas cerca de lo que aparentan" en los retrovisores.
