...
Un objeto cotidiano es **eléctricamente neutro** porque tiene el mismo número de protones y electrones. Cuando frotas un globo contra la lana, el globo gana electrones y queda con [[q]] negativa; la lana pierde electrones y queda con [[q]] positiva. La carga total del sistema (globo más lana) no ha cambiado: simplemente se ha redistribuido.

La carga eléctrica no puede crearse ni destruirse: solo transferirse. Y se transfiere siempre en paquetes discretos de tamaño mínimo [[e]], la carga del protón. El [[n]] de paquetes transferidos es siempre un número entero exacto.

## 🔵 Nivel formal

La cuantización de la carga establece que toda carga observable es un múltiplo entero de la carga elemental [[e]]. Para un objeto con [[n]] cargas elementales:

{{f:cuantizacion_carga}}

Aquí [[q]] es la carga total, [[n]] es un número entero (positivo para déficit de electrones, negativo para exceso) y [[e]] es la carga del protón, aproximadamente 1.602 por diez elevado a menos diecinueve culombios.

Esta fórmula permite resolver dos tipos de problemas: dado [[n]], calcular [[q]]; dado [[q]] medida, determinar cuántos electrones han sido transferidos despejando [[n]]. El primer rearrangement es directo; el segundo exige que el cociente dé entero exacto, lo que sirve como verificación de coherencia física.
...
Las dos fórmulas no son independientes. La cuantización dice que cada aportación individual a [[q_pos]] o [[q_neg]] es un múltiplo de [[e]]; la carga neta del sistema es, por tanto, también múltiplo de [[e]].
...
En la física moderna, los quarks tienen cargas fraccionarias de [[e]] (un tercio y dos tercios), pero nunca se observan libres: están confinados en hadrones con carga total entera. Este confinamiento preserva la cuantización en múltiplos enteros de [[e]] a escala macroscópica.
...
La unidad SI de carga es el **culombio (C)**: desde 2019 se define como la carga transportada por 1 A durante 1 s, ligando [[q]] con la corriente. La corriente es la tasa de flujo de carga, no la carga misma.

Otra implicación estructural: la carga eléctrica es un **invariante relativista**: todos los observadores miden la misma [[q]] para un objeto, independientemente de su velocidad relativa. Esto contrasta con la masa o la longitud, que dependen del sistema de referencia.
...
> [!NOTE]
> Cuando dos objetos neutros se frotan, ninguna carga se crea: los electrones migran de uno al otro. La suma algebraica de [[q]] del sistema sigue siendo cero.

## Orden de magnitud

La carga elemental [[e]] es aproximadamente 1.6 por diez elevado a menos diecinueve culombios; un rayo descarga entre 1 y 5 culombios; un condensador pequeño de laboratorio almacena entre 1 y 100 microcoulombios.
...
4. **Aplicar cuantización si se pide [[n]]**: dividir [[q]] entre [[e]] y verificar que el resultado es entero.
5. **Revisar la escala**: comprobar que el orden de magnitud del resultado es razonable para el contexto físico del problema.

El paso 4 actúa como validación automática: si [[n]] no es entero, hay error en los datos o en las unidades.
...
**Caso 3 — Neutralización**: Si un objeto con [[q]] positiva y otro con [[q]] negativa del mismo módulo se ponen en contacto, los electrones del objeto negativo fluyen hacia el positivo hasta que ambos quedan neutros. La carga total del sistema sigue siendo cero: ninguna carga se destruyó.
...
**Si froto un globo y queda con [[q]] negativa, ¿de dónde vienen los electrones extra?**
...
Los quarks tienen cargas de un tercio y dos tercios de [[e]], pero nunca se observan libres: el confinamiento de color de la cromodinámica cuántica impide extraerlos de los hadrones. A escala macroscópica y en toda la física clásica, la respuesta es no: la carga mínima observable en partículas libres es [[e]].
...
La carga eléctrica conecta directamente con la [ley de Coulomb](leaf:fisica-clasica/electromagnetismo/fundamentos/ley-de-coulomb): [[q]] es el factor que determina la fuerza entre objetos cargados. El siguiente paso natural es el [campo eléctrico](leaf:fisica-clasica/electromagnetismo/fundamentos/campo-electrico), donde [[q]] actúa como factor de proporcionalidad entre campo y fuerza sobre una partícula de prueba.

En electromagnetismo avanzado, [[q]] es la fuente del campo eléctrico en la ley de Gauss, y la corriente (flujo de [[q]]) es la fuente del campo magnético. Comprender [[q]] es comprender las fuentes de todos los campos electromagnéticos.

## Síntesis final

La carga eléctrica es la propiedad fundamental de la materia que origina toda la interacción electromagnética. Su cuantización en múltiplos enteros de [[e]] y su conservación absoluta son las dos leyes más profundas del electromagnetismo clásico, con validez experimental sin excepción desde la escala atómica hasta la macroscópica. Todo cálculo eléctrico comienza por [[q]].
`;export{e as default};
