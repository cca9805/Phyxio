const e=`# Logaritmos y exponenciales\r
\r
## Que es\r
\r
Una exponencial describe cambios por factor y un logaritmo mide cuantos factores hay entre dos cantidades. En fisica se usa para leer una relacion antes de sustituir numeros. La pregunta central no es solo cuanto vale una cantidad, sino que cambia cuando se modifica una condicion del problema.\r
\r
...
\r
La idea visual es esta: imagina una cantidad que no suma lo mismo cada segundo, sino que conserva o pierde siempre la misma fraccion. Esa imagen evita tratar todas las formulas como listas de simbolos equivalentes.\r
\r
...
\r
En simbolos, en una ley y = y0 e^(kt), k indica ritmo relativo; el logaritmo permite despejar el tiempo cuando la incognita esta en el exponente. Lo importante es leer la operacion como relacion entre magnitudes y no como decoracion algebraica.\r
\r
...
\r
<!-- algebra-depth-v2 -->\r
## Capa de profundidad: logaritmos y exponenciales\r
\r
Este leaf no debe funcionar como una lista corta de reglas, sino como una herramienta para pensar problemas fisicos. El objetivo es leer crecimientos, decaimientos y escalas multiplicativas. Un alumno basico necesita reconocer la magnitud buscada y la unidad esperada; un alumno intermedio debe justificar la operacion elegida; un alumno avanzado debe anticipar limites, dependencias y casos donde el modelo deja de servir.\r
\r
El logaritmo responde a una pregunta inversa: cuantos factores hacen falta para llegar a una razon dada.\r
\r
| Modelo | Forma | Pregunta |\r
|---|---|---|\r
| exponencial | \`N=N0 e^(k t)\` | cuanto queda tras un tiempo |\r
| logaritmico | \`t=(1/k) ln(N/N0)\` | cuanto tarda en llegar a una razon |\r
| semilog | \`ln(y)\` frente a \`x\` | detectar exponenciales |\r
| log-log | \`ln(y)\` frente a \`ln(x)\` | detectar leyes de potencia |\r
\r
