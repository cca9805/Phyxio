...
Dividing [[F_C]] by the test charge yields the **electric field** [[E_C]] created by [[q1]] at that point in space:

{{f:campo_electrico_coulomb}}

This second relation is the entry point to the electric field concept: [[E_C]] does not depend on which charge is placed at the point, only on [[q1]] and [[r12]].
...
- **Inverse-square dependence on distance**: [[r12]] appears squared in the denominator. Doubling [[r12]] reduces [[F_C]] to one quarter; tripling it reduces it to one ninth.
...
- Two electrons separated by 1 Å (one tenth of a nanometre): [[F_C]] of the order of 10⁻⁸ N. Small, but enormous for the electron mass.
- Two laboratory spheres with 1 μC at 10 cm: [[F_C]] of the order of 0.9 N. Detectable with a precision balance.
- Two charges of 1 C at 1 m: [[F_C]] of the order of 9×10⁹ N. Catastrophic in any real system — the reason why 1 C of concentrated charge does not exist in practice.

If a result of [[F_C]] exceeds kiloNewtons for microcoulomb charges at centimetre distances, there is almost certainly a unit conversion error (using centimetres instead of metres for [[r12]]).
...
2. **Choose what to compute**: if [[F_C]] is requested, use the formula directly. If [[r12]] or a charge is requested, rearrange algebraically first.
...
**Limiting case — distance approaching zero**: when [[r12]] approaches zero, [[F_C]] diverges. In practice this never happens because objects have finite extent; the point charge model breaks down before the distance is comparable to the size of the objects. This limit marks the boundary of the model.

**Case of zero charge**: if [[q1]] or [[q2]] is zero, [[F_C]] is exactly zero regardless of the distance. A zero charge does not interact electrostatically. This result, though obvious, has important implications: the infinitesimal test charge used to define the electric field is an idealisation precisely because it must not disturb the field of the source charge.
...
Newton's third law is universal: the magnitude of [[F_C]] is unique for the pair. What changes is the resulting acceleration, which depends on each object's mass.

**If I separate the charges to double the distance, does the force drop to half?**
No. [[r12]] is squared in the denominator: doubling the distance reduces [[F_C]] to one quarter.

**What is the difference between [[F_C]] and [[E_C]]?**
[[F_C]] depends on both charges [[q1]] and [[q2]]. [[E_C]] is [[F_C]] divided by the test charge: it depends only on [[q1]] and position. The field is a property of space; the force is what a specific charge feels in it.

## Cross-cutting connections and study paths

Coulomb's law is the entry point to three branches:

- **Electric field [[E_C]]**: dividing [[F_C]] by the test charge defines [[E_C]]. This leads to the point charge field and, by superposition, to the field of distributions.
