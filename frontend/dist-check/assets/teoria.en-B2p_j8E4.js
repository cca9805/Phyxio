...
## 🔵 Formal level\r
\r
### One-dimensional law of restitution\r
\r
Formal analysis requires translating the above concepts into the language of vector magnitudes and dynamic equations. We define the initial velocity [[vi]] as the approach vector to the wall. After the impact, the body emerges with a final velocity [[vf]]. If the collision is head-on (one-dimensional), the relationship is given by Newton's law of restitution:\r
\r
{{f:vf}}\r
\r
This change in velocity requires the action of an external force. Although the collision seems instantaneous, it occurs during a very small contact time [[Delta_t]].\r
\r
### Impulse and impact force\r
\r
The integral of the force during this time is what we define as the linear impulse [[J]] received from the wall:\r
\r
{{f:impulse_wall}}\r
\r
From this impulse, we can estimate the intensity of the interaction by calculating the average impact force [[F_avg]], which represents the equivalent constant force that would produce the same change in momentum:\r
\r
{{f:average_force_wall}}\r
\r
In every real rebound, a loss of kinetic energy [[DeltaK]] occurs. This magnitude quantifies the work of internal deformation forces and the energy degraded into heat and sound waves. Since the initial kinetic energy [[K]] depends on the mass [[m]] and the square of the speed, the loss is expressed as:\r
\r
{{f:energy_loss_wall}}\r
\r
### Vector analysis in oblique collisions\r
\r
In oblique collisions (2D), the velocity vector is decomposed into a normal component [[vin]] and a tangential component [[vit]]. In the smooth wall model, only the normal component is affected by restitution, while the tangential component is fully preserved:\r
\r
{{f:vfn}}\r
{{f:vft}}\r
\r
This asymmetry causes the rebound angle [[theta_f]] to be greater than the incidence angle [[theta_i]] whenever there is dissipation, following the relationship:\r
\r
{{f:rebound_angle}}\r
\r
{{f:rango_e}}\r
...
\r
1. **Data evaluation**: Determine if the coefficient is a known value or if it must be obtained from kinematic observations such as measured heights or velocities.\r
2. **Calculating from heights**: If the problem involves a bounce against the ground, apply the square root of the ratio between the drop height and the bounce height.\r
...
5. **Energy analysis**: Use the mass of the projectile to calculate the kinetic loss and verify that the result is consistent with the type of collision.\r
\r
## Special cases and extended example\r
\r
**Collisions in fluid media.** When the collision occurs underwater, the medium's resistance (viscosity) affects both arrival and rebound, making the effective value of [[e]] appear much smaller due to drag.\r
\r
**Walls with elastic coatings.** Using rubber layers on concrete walls increases the contact time [[Delta_t]], which drastically reduces the average force [[F_avg]] without necessarily changing the total impulse [[J]].\r
\r
**Extended calibration example.** Suppose a sphere of mass [[m]] equal to 2 kg is dropped from an initial height [[h0]] of 10 meters in a lab. It rebounds to a rebound height [[hf]] of 6.4 meters. Applying the height relationship, we find that [[e]] is the root of 0.64, i.e., 0.8. The initial kinetic energy [[K]] was 196 J and the final was 125.4 J. The loss [[DeltaK]] of 70.6 J was dissipated mainly into the sound of the impact and sphere heating. If the collision lasted a contact time [[Delta_t]] of 5 milliseconds, the average force [[F_avg]] exceeded 5,000 N, demonstrating considerable impact forces.\r
\r
## Real student questions\r
\r
- **Why doesn't the ground move if I hit it hard?** Because its inertia is colossal. The Earth has a mass of 6*10^24 kg, so any human force produces an acceleration that is mathematically negligible.\r
- **Is energy conserved if e = 1?** Yes, macroscopic kinetic energy is conserved. But remember that in the real world, there is always some noise (sound energy), so [[e]] is always slightly less than 1.\r
- **What happens if the wall breaks?** The "fixed wall" model is no longer valid. The system becomes a collision with finite-mass fragments, and linear momentum conservation with multiple bodies must be used.\r
\r
