...
When an object is too small to be distinguished with the naked eye, a lens is needed that makes the light rays reach the eye at a larger angle. That larger angle causes the brain to interpret the object as bigger. The **angular magnification** [[M_angular]] quantifies how many times that angle is amplified.

The **magnifying glass** is the simplest optical instrument: a single converging lens of short focal length [[f_lupa]] placed so that the object lies between the lens and its focus. The resulting image is virtual, upright and magnified. To observe even smaller objects, the **compound microscope** chains two lenses: an objective with a very short focal length [[f_obj]] and an eyepiece [[f_oc]] that acts as a second magnifier. The result is a total magnification [[M_micro]] far larger than any single lens could provide.

## 🔵 Formal level

The thin-lens equation relates the object distance [[d_o]], the image distance [[d_i]] and the focal length [[f]]:
...
where [[d_pp]] is the conventional near-point distance (0.25 m). The shorter [[f_lupa]], the greater [[M_angular]].

In the compound microscope, the objective forms a magnified real intermediate image at a distance determined by the tube length [[L]]. The eyepiece then angularly magnifies that intermediate image. The total magnification is:
...
The **tube length** [[L]] of the microscope is the distance between the image focus of the objective and the object focus of the eyepiece. Increasing [[L]] proportionally enlarges the intermediate image, raising the total magnification. However, increasing [[L]] indefinitely is not feasible because image brightness drops and aberrations are amplified.
...
Angular magnification is not simply a number indicating how much larger the object appears. Physically, it expresses the **redistribution of the ray cone** arriving at the retina. A magnifying glass with [[M_angular]] equal to 10 makes the subtended angle ten times larger; this means the retinal image occupies ten times more area, activating more photoreceptors and allowing the observer to distinguish previously invisible details.
...
2. **Select the appropriate formula**: the magnifier uses the relationship between [[d_pp]] and [[f_lupa]]; the microscope uses the formula involving [[L]], [[f_obj]] and [[f_oc]].
...
- **Microscope with short tube**: if [[L]] is much less than the standard value (for example, half), the lateral magnification of the objective is proportionally reduced and the total magnification drops. A microscope with a tube length of 0.08 m instead of 0.16 m loses half its total magnification.

- **Magnifier with long focal length**: a lens with a focal length of 0.25 m gives [[M_angular]] of only 1, meaning no magnification; this corresponds to the lower useful limit of the instrument.
