const e=`# Superposition Principle
...
The same applies to the electric field: to find the field at a point in space, compute the field that each source charge would create on its own and add all those fields vectorially. The result is [[E_total]], which exists at that point in space regardless of whether a test charge is placed there.
...
The superposition principle for electric force states that the total force [[F_total]] on a test charge due to [[N_cargas]] source charges is the vector sum of the [[N_cargas]] individual Coulomb forces:
...
In a completely parallel way, the total electric field [[E_total]] at any point in space is the vector sum of the individual fields of each source charge:

{{f:superposicion_campo}}

The fundamental conceptual difference between the two formulas is that [[F_total]] depends on the test charge (its value appears as a multiplicative factor), whereas [[E_total]] is a property of space that does not depend on whether a test charge is present or on how large it is. [[E_total]] pre-exists; [[F_total]] only appears when a charge is placed at that point.
...
> [!WARNING]
> The superposition sum is **vectorial**, not scalar. Adding the magnitudes of individual forces without respecting their directions is the most common error and always overestimates [[F_total]] unless all contributions are parallel.
...
The mathematical structure of the superposition sum has important geometric consequences. For two charges of the same sign, [[F_total]] on a third charge always has a **zero-force point** between them where contributions cancel exactly. For two charges of opposite sign, that zero-force point does not exist between them but outside the segment connecting them. The position of that equilibrium point depends on the ratio of charge magnitudes and their distances to the test point.

The generalisation to continuous charge distributions replaces the discrete sum with an integral: the superposition principle remains valid but now each charge differential contributes to [[E_total]] with its own term. This natural continuation is the bridge to Gauss's law and electric potential, where superposition is expressed globally rather than point by point.
...
> [!TIP]
> To simplify a superposition sum, first identify the system's symmetry. If the system has reflection symmetry, many terms cancel in pairs and the effective sum has far fewer terms than [[N_cargas]].

## Deep physical interpretation

A zero result for [[F_total]] does not mean there are no forces; it means the forces **balance exactly**. An electrostatic equilibrium point is a position where the test charge experiences no net force, but does experience individual forces from each source charge. If the charge is displaced slightly from equilibrium, the superposition sum changes: if [[F_total]] returns it to the original position, the equilibrium is **stable**; if it drives it further away, the equilibrium is **unstable**.
...
The independence of the field from the test charge has a deep implication: [[E_total]] is a **complete description of the electric effect** in a region of space, valid for any charge passing through there, whatever its value. This independence is what justifies speaking of the electric field as a real physical entity, not merely a computational device.

## Order of magnitude

For a system of two charges of 1 µC separated by 10 cm, the field of each one at the midpoint is of order 3.6×10⁵ N/C. If the charges have the same sign, the fields at the midpoint are equal and opposite and [[E_total]] is zero there. If they have opposite signs, the fields at the midpoint point in the same direction and [[E_total]] doubles the individual value: of order 7.2×10⁵ N/C.

A result of [[E_total]] greater than 10⁷ N/C for laboratory charges (of order microcoulombs) should raise suspicion of a unit error or a distance expressed in centimetres without converting to metres.

If [[F_total]] on a charge of 1 nC in that field of 7.2×10⁵ N/C turns out to be of order 7.2×10⁻⁴ N, the dimensional check is correct: newtons per coulomb multiplied by coulombs gives newtons.

## Personalized resolution method

To solve any superposition problem:

1. **Label all source charges** with subscripts from 1 to [[N_cargas]], noting their signed values.
...
**Why can [[E_total]] be zero if none of the individual charges has disappeared?**
