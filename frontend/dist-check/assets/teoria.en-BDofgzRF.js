const e=`# Logarithms and exponentials\r
\r
## What it is\r
\r
An exponential describes change by factors, and a logarithm measures how many factors separate two quantities. In physics it is used to read a relation before substituting numbers. The central question is not only how large a quantity is, but what changes when one condition changes.\r
\r
...
\r
The visual idea is this: imagine a quantity that does not add the same amount each second but keeps or loses the same fraction. That image prevents treating every formula as a flat list of equivalent symbols.\r
\r
...
\r
Symbolically, in y = y0 e^(kt), k gives relative rate; the logarithm lets you solve for time when the unknown is in the exponent. The important point is to read the operation as a relation between quantities, not as algebraic decoration.\r
\r
...
\r
<!-- algebra-depth-v2 -->\r
## Depth layer: logarithms and exponentials\r
\r
...
\r
The logarithm answers an inverse question: how many factors are needed to reach a given ratio.\r
\r
| Model | Form | Question |\r
|---|---|---|\r
| exponential | \`N=N0 e^(k t)\` | what remains after a time |\r
| logarithmic | \`t=(1/k) ln(N/N0)\` | how long to reach a ratio |\r
| semilog | \`ln(y)\` versus \`x\` | detect exponentials |\r
| log-log | \`ln(y)\` versus \`ln(x)\` | detect power laws |\r
\r
