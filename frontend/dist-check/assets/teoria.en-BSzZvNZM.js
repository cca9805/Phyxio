...
When a beam of parallel rays passes through a converging lens, the rays cross at a point called the **focus**. The distance between the lens centre and the focus is the **focal length** [[f]]. If the lens is diverging, the rays spread out and their back-extensions meet at a virtual focus.

The image position [[d_i]] depends on where the object [[d_o]] is placed relative to the focus. If the object is beyond the focus of a converging lens, the image is **real** (projectable onto a screen) and **inverted**. If the object lies between the lens and the focus, the image is **virtual**, **upright** and **magnified**. The **lateral magnification** [[m]] quantifies how much larger or smaller the image is compared to the object and whether it is inverted.
...
This equation connects the focal length [[f]] with the object distance [[d_o]] and the image distance [[d_i]]. Signs follow the convention: positive distances in the direction of light propagation. For a converging lens [[f]] is positive; for a diverging one, negative. If [[d_i]] is positive the image is real; if negative, virtual.

Image size and orientation are obtained through the lateral magnification:

{{f:aumento_lateral}}

The negative sign means that when [[d_i]] and [[d_o]] are both positive, [[m]] is negative: the real image is inverted. A magnitude of [[m]] greater than one means the image is enlarged; less than one, reduced.
...
Here [[n]] is the refractive index of the material and [[R1]], [[R2]] are the radii of curvature of the surfaces. This equation shows that the optical power of the lens depends on both the material and the geometry of its surfaces.
...
The paraxial approximation requires that rays form small angles with the optical axis. In practice, this holds reasonably when the lens aperture is much smaller than its focal length. For wide-aperture lenses, **spherical aberrations** displace the real image from the paraxial prediction. Chromatic aberration additionally arises when the light is not monochromatic, because [[n]] depends on wavelength.

The dependence of [[d_i]] on [[d_o]] is **hyperbolic**: as [[d_o]] approaches the focus, [[d_i]] tends to infinity (the image forms very far away). When [[d_o]] equals [[f]] exactly, the equation predicts an image at infinity and the model loses local practical meaning. This asymptotic behaviour delimits the stable operating zone of any lens-based optical system.

The lensmaker's equation reveals that lenses of identical shape but different material produce different focal lengths. An increase in [[n]] reduces the focal length, increasing optical power. This has direct design implications: high-index materials allow thinner, lighter lenses with the same power. However, high-index materials usually exhibit greater dispersion, introducing chromatic aberration.

The lateral magnification [[m]] connects directly to the conservation of size ratios. The image inversion (negative sign of [[m]]) is not a mathematical artefact but a geometric consequence of ray crossing at a point: every pair of rays converging to a real image point necessarily inverts the object orientation. Only when the image is virtual (rays do not actually converge) does the image maintain the original orientation.
...
The magnification [[m]] carries a dual reading: its **magnitude** indicates the spatial scale of the image and its **sign** indicates the orientation parity. This sign-magnitude duality is common in optical quantities and should be treated explicitly to avoid interpretation errors.
...
- **Step 1**: Identify the lens type (converging if [[f]] positive, diverging if negative) and the available data.
...
- **Step 4**: Calculate [[m]] to determine size and orientation.
- **Step 5**: Verify sign coherence between [[d_i]] and [[m]].
- **Step 6**: Validate the result with a principal-ray trace (parallel ray, ray through the centre, ray through the focus).

## Special cases and extended example

**Object at infinity**: when [[d_o]] tends to infinity, the lens equation gives [[d_i]] equal to [[f]]. The image forms exactly in the focal plane. This is the case of a camera focusing on a distant object.

**Object at the focus**: when [[d_o]] exactly equals [[f]] in a converging lens, [[d_i]] tends to infinity. The rays emerge parallel and no image forms at a finite distance. This case is exploited in collimated projectors and headlamps.

**Diverging lens with a real object**: [[d_i]] is always negative regardless of the object position. The image is always virtual, upright and reduced. The magnitude of [[m]] is always less than one.
...
In a converging lens with [[d_o]] equal to twice [[f]], the image forms at the same distance on the other side, with [[m]] equal to minus one: a real, inverted image of the same size.
