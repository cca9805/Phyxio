const e=`# Euler Equations for a Rigid Body

## Conceptual context

Euler Equations for a Rigid Body answers a precise physical question: How do Euler equations describe three-dimensional rotation of a rigid body? The topic is not introduced to add one more equation, but to give the student a reading tool when elementary models no longer separate cause, scale, and domain clearly.

Euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two. The student learns that in three dimensions moment of inertia is not a single number. For that reason the core quantities, [[tau1]], [[tau2]], [[tau3]], [[I1]], [[I2]], [[I3]], [[omega1]], [[omega2]], [[omega3]], [[L]], are not decorative labels. Each one marks part of the structure: a scale, a boundary, a dynamic variable, a validity condition, or a conserved quantity.

## 🟢 Essential level

The essential idea is that euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two. Before calculating, the student must state which physical mechanism is being isolated and which part of the system is deliberately left outside the model. Without that reading, an equation may produce the right number for the wrong question.

At this level, [[tau1]] acts as the dominant quantity because it concentrates the didactic decision of the leaf. It is not enough to name it: the student must explain whether it increases, decreases, changes sign, vanishes, or loses meaning when the regime changes.
...
It is also useful to separate description from decision. Description lists quantities; decision explains which of them controls the observed change. In this leaf, that decision is supported by [[tau1]] and by the comparison between dominant term, correction, and limiting case. This distinction prevents the student from confusing a long solution with a deep solution.
...
To apply them, [[tau1]], [[tau2]], [[tau3]], [[I1]], [[I2]], [[I3]], [[omega1]], [[omega2]], [[omega3]], [[L]] must belong to the same system. Sign, units, and domain must also be checked. The central question remains: How do Euler equations describe three-dimensional rotation of a rigid body? If an algebraic rearrangement changes that question, the procedure has stopped being physics and has become symbolic manipulation.
...
The deeper structure appears when the model is compared with its limits. Euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two. The student learns that in three dimensions moment of inertia is not a single number. This sentence must work as a control criterion: it decides whether the topic is being used in its natural domain or forced outside its range.
...
Deep interpretation starts by asking what [[tau1]] represents in the causal chain. If [[tau1]] is modified, the regime may change, a critical condition may move, or a secondary assumption may stop being secondary.
...
This check prevents artificial conclusions. The units of [[tau1]], [[tau2]], [[tau3]], [[I1]], [[I2]], [[I3]], [[omega1]], [[omega2]], [[omega3]], [[L]] must support the same physical dimension and the same experimental scale.

## Personalized resolution method

First, delimit the system and state the assumptions. Second, identify [[tau1]] and decide which relation answers the central question. Third, calculate symbolically before substituting numbers.
...
At the end, the student should explain that euler equations describe rigid-body rotation in principal axes, where each angular component couples to the other two, apply the formulas in their domain, and justify the physical meaning of the result. The final competence is deciding whether the model answers the question of the real system, not merely obtaining an algebraic expression.
