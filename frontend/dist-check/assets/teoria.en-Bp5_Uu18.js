...
The essential idea is **local and vectorial**. Current matters, the oriented element [[dl]] matters, distance [[r]] to the point matters, and fragment orientation also matters. An element almost aligned with the point contributes little; an almost perpendicular element contributes much more.
...
The result sought is the total field, but the natural path starts with [[dB]]. This difference avoids a common mistake: thinking that one part of the conductor already represents the whole field.
...
Here [[dB]] grows with [[I]], with [[dl]], and with [[s_theta]], but decreases with [[r]]. The constant [[mu0]] sets the magnetic coupling of the vacuum model. Direction is not read from the magnitude alone; it follows from the right-hand rule applied to the current element and the direction toward the point.

When the geometry is a very long straight wire, integrating all contributions gives a simple relation for the field at perpendicular distance [[r]]:
...
The formal reading requires not confusing the integration variable with the final result. [[dl]] belongs to the conductor, [[r]] connects that element with the observation point, and [[B]] appears only after coherent contributions are summed. If that separation is lost, an integrated formula is used outside its geometry.
...
2. Draw the element [[dl]] and the point where [[B]] is required.
3. Decide distance [[r]] and angular factor [[s_theta]] for each type of element.
4. Use symmetry to know which components of [[dB]] cancel and which add.
...
**Can I add only magnitudes of [[dB]]?** Only if you have shown that all contributions point in the same direction. Otherwise, vectors must be added.
...
The Biot-Savart law calculates [[B]] by adding contributions [[dB]] produced by current elements. Its strength is connecting current, distance, orientation, and geometry; its difficulty is respecting vector addition and the magnetostatic domain.
`;export{e as default};
