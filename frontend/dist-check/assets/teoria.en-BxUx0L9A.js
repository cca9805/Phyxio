const e=`# Household Electrical Network
...
This equation establishes that the **total current** [[I_tot]] demanded by the installation is directly proportional to the **total connected power** [[P_tot]] and inversely proportional to the **network voltage** [[V]], which remains constant at 230 V in the European standard.
...
Where [[I_max]] represents the **nominal rating** of the MCB (16 A, 20 A, 25 A, etc.). When [[margen_uso]] reaches or exceeds the value 1, the circuit is overloaded and the protection must trip.
...
The **MCB** acts as a **reusable fuse sensitive to current**. When the total current [[I_tot]] exceeds the limit [[I_max]] for a sufficient time, the heat generated by the Joule effect in a bimetallic element causes the mechanical opening of the circuit. The parameter [[margen_uso]], obtained by dividing [[I_tot]] by [[I_max]], is a direct indicator of the **thermal risk** of the wiring.
...
1. **Power inventory**: Sum the nominal powers [[P_tot]] of all receptors that could operate simultaneously on the circuit.

2. **Current calculation**: Apply the current formula using [[V]] fixed at 230 V. You obtain the theoretical maximum current.
...
4. **Rating selection**: Choose an MCB with [[I_max]] higher than the calculated current, selecting the immediately higher commercial value (16, 20, 25 A).

5. **Margin verification**: Calculate the margin by dividing total current by [[I_max]]. If it is greater than 0.8, distribute the load or increase the rating if the installation permits.

6. **Cable cross-section verification**: Consult electrical regulations to confirm the cable can safely carry the selected [[I_max]].
...
The maximum total power [[P_tot]] reaches 5200 W.
...
A: Motors have a starting current much higher than the nominal current (can be 6 times higher) for a few seconds. If the circuit is already loaded, that peak can momentarily exceed [[I_max]].
