...
An electric **generator** is a direct application of electromagnetic induction. It does not create energy from nothing; it converts **mechanical work** into electrical energy by producing a controlled change in [[Phi_B]]. This is why the topic links Faraday's law with turbines, alternators, bicycle dynamos, emergency generators, and laboratory coils.

The useful way to think about a generator is to connect three readings: coil geometry, rate of motion, and useful electrical output. The central question is what must change for [[epsilon_0]] to appear, and what part of that conversion finally reaches the external circuit as [[P_elec]] when real losses are present.
...
The intuition is direct: more magnetic field through the coil, more linked area, and faster change produce a larger induced voltage. [[N]] acts as a multiplier because each turn contributes to the linked flux.

> [!WARNING]
> Do not confuse [[epsilon_0]] with [[P_elec]]. The first describes induced voltage; the second describes the rate at which energy is delivered to a circuit.

In an ideal generator, no energy is lost to friction, heating, or internal resistance. In a real generator, mechanical input enters as [[P_mec]], and only a fraction becomes useful electrical output. That fraction is summarized by [[eta]].

## 🔵 Formal level

The formal description first separates the **available magnetic linkage** from the rate at which that linkage changes. In an ideal coil with [[N]] turns, average field [[B]], and effective area [[A]], the maximum linked flux is set by geometry and field strength:
...
Faraday's law can also be read as a mean value when a flux change [[DeltaPhi_B]] occurs during a time interval [[Delta_t]]. This is useful for switching events, partial turns, or estimates without a full sinusoidal waveform:

{{f:fem_media_generador}}

To connect induction with energy, efficiency is introduced. A real generator receives [[P_mec]] and delivers a smaller or equal useful output because mechanical, thermal, magnetic, and electrical losses are present:
...
The leaf's Coord graph represents a quantitative reading of that chain: the alternating [[epsilon]] changes sign while [[Phi_B]] changes phase. When the flux is near an extreme value, its instantaneous change is small; when it passes through the central region, the change is faster and the emf is large. This link between waveform shape and time variation matters more than memorizing a sinusoidal picture.

The ideal model requires an approximately uniform magnetic field, a rigid coil, steady rotation, a well-defined area, and no magnetic saturation. If the ferromagnetic core saturates, [[B]] no longer grows according to the ideal design. If the speed varies during a turn, [[omega]] no longer describes the whole process. If the load demands too much current, terminal voltage may fall even when internal emf remains high.
...
In an educational coil with [[N]] of about one hundred turns, [[B]] of a few tenths of a tesla, [[A]] of hundredths of a square metre, and [[omega]] of about one hundred radians per second, [[epsilon_0]] can be in the tens of volts. A dynamic microphone works with millivolt-scale signals, while a grid alternator can handle hundreds of volts. Millions of volts from a small hand-turned coil would be physically absurd unless the data were extreme.

## Personalized resolution method

First identify what the problem asks for: [[Phi_B]], [[epsilon]], [[epsilon_0]], [[eta]], or [[P_elec]]. Then separate geometric data from time data. Convert [[omega]] if it is given in revolutions per minute, check that [[A]] is in square metres, and confirm whether [[N]] multiplies the flux of one turn.

Next decide whether the problem uses ideal sinusoidal rotation or a mean flux change. For a rotating ideal generator, use the amplitude; for a specified change during an interval, use the mean emf. Finally, if useful energy is requested, do not mix voltage with power: include [[P_mec]] and [[eta]].
...
Another limiting case is an open circuit. A measurable [[epsilon_0]] may exist at the terminals, but [[P_elec]] delivered to an external load is zero because no useful current flows. With a very low-resistance load, current may be large, internal losses increase, and terminal voltage can fall.
...
**Why does voltage appear if there is no chemical battery?** Mechanical work changes the linkage of [[Phi_B]] and separates charges through induction. The energy source is the motion supplied to the shaft.

**Does more turns always mean a better generator?** Increasing [[N]] raises flux linkage, but it can also increase resistance, size, mass, and inductance. Real design balances emf, losses, and heating.
...
**Why does a generator become harder to turn when a load is connected?** Delivering [[P_elec]] creates an electromagnetic reaction that requires more [[P_mec]] to keep the same speed. It is not mysterious friction; it is energy conversion.
