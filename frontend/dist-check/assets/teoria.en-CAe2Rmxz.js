...
In this expression, [[theta]] represents the angle of the first diffraction minimum, measured in radians relative to the central paraxial axis of incidence. The parameter [[lambda]] is the wavelength of light in meters, and [[a]] is the physical width of the diffracting slit in meters.

When we project this angular pattern onto a flat observation screen placed at a longitudinal distance [[L]], the lateral linear separation between the first upper minimum and the first lower minimum defines the total width of the bright central maximum. Under the paraxial approximation of small angles, where the tangent and the sine of the angle are approximately equal to the angle in radians, the linear width of the central maximum [[w]] is calculated using the derived relation:

{{f:ancho_maximo_central}}

Where [[w]] is the width of the central maximum measured in meters on the screen, and [[L]] is the distance from the slit to the screen in meters.
...
> [!WARNING]
> In all calculations, the wavelength [[lambda]] must be strictly converted to meters. Expressing [[lambda]] in nanometers without conversion while [[a]] and [[L]] are in meters is the most common error that invalidates calculations.
...
**Paraxial Small-Angle Approximation.** The simplified linear formula for [[w]] assumes a small angular deviation. This is reliable when the slit width [[a]] is significantly larger than the wavelength [[lambda]]. If the slit approaches the wavelength scale, the pattern spreads widely and exact trigonometric projection is needed.
...
The inverse relationship between slit width [[a]] and pattern size [[w]] echoes wave uncertainty ideas. The more tightly the light path is confined, the less sharply its later direction is defined. The beam energy is not lost in the slit; it is redistributed across the screen by phase superposition.
...
In a typical school or university physics laboratory setup, the wavelength [[lambda]] of a red helium-neon laser takes a characteristic value of six hundred thirty-three nanometers. If the light passes through a slit of width [[a]] equal to one-tenth of a millimeter, the resulting angle of the first minimum [[theta]] is of the order of six milliradians.

If the projection screen is placed at a longitudinal distance [[L]] of two meters, the linear width of the central maximum [[w]] measured with a ruler on the screen is approximately twenty-five millimeters. This is a highly practical and comfortable scale for visually observing the bands and recording the experimental positions of the diffracted pattern.

## Personalized resolution method

When solving a quantitative single-slit diffraction problem, use this sequence:

1. **Convert to SI units**: express [[lambda]], [[a]], [[L]], and [[w]] in meters.
2. **Assess angular consistency**: compare [[lambda]] with [[a]]. If the ratio is below one-tenth, the paraxial formulas are appropriate.
3. **Calculate the unknown**: solve for [[theta]], [[w]], or [[a]] as requested.
4. **Verify physical limits**: check that the screen distance is large enough for Fraunhofer diffraction.

## Special Cases and Extended Example

**Slit width comparable to the wavelength.** If we attempt to create an extremely narrow slit where [[a]] is equal to the wavelength [[lambda]], the theoretical angle of the first minimum would require the sine of the angle to equal one. This shifts the first point of darkness exactly to ninety degrees from the central axis, causing the bright central maximum to expand laterally and fill the entire forward-propagating space. No defined lateral minima will form on the screen.
...
Diffraction describes the lateral bending and angular spreading of light through a microscopic slit or obstacle. The key quantity is the ratio between [[lambda]] and [[a]]. In the paraxial Fraunhofer model, the first dark minimum angle [[theta]] is controlled by that ratio. On a screen at distance [[L]], the central maximum width [[w]] scales with [[lambda]] and [[L]], and inversely with [[a]]. These relations allow optical calibration when the slit is much wider than the wavelength and the screen is far away.
`;export{e as default};
