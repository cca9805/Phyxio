const e=`# Reference frames [[frame_sigma]] \r
\r
...
\r
The central relationship of the topic is the **Galilean position transformation**. Written with the leaf's notation, it expresses that the position measured in one frame is obtained by adding the position measured in the other frame and the displacement accumulated by the moving frame itself:\r
\r
{{f:trans_galileo_pos}}\r
\r
This equation is not used to decorate a deduction, but to answer a specific question: how a single trajectory is translated when the observer changes. If the student knows that [[r_vec]] belongs to the chosen frame and that [[v_frame]] belongs to the relationship between frames, they stop adding velocities and positions without criteria.\r
\r
Velocity is interpreted with the same logic. If a passenger walks inside the carriage, the carriage observer measures [[v_prime]] and the platform observer measures [[v]] . The velocity transformation adds the carriage's velocity [[v_frame]] to the internal reading. The important thing is not to memorize the sum, but to distinguish two levels: the internal motion of the body within the frame and the motion of the entire frame relative to another:\r
\r
{{f:trans_galileo_vel}}\r
\r
There are also two classical invariances that order the interpretation. The first states that two Galilean frames compare the same **spatial separation** [[r_vec]] when they measure the same segment simultaneously:\r
\r
{{f:invarianza_distancia}}\r
\r
The second states that both share the same [[t_global]] . These invariances are not philosophical ornaments: they are the reason why the problem can be closed with a single common chronology:\r
\r
{{f:invarianza_tiempo}}\r
\r
The correct use of formal expressions requires reading discipline. First, the frame is chosen. Then, the location of [[O]] is declared. Next, [[r_vec]] is written with the appropriate sign. Only at the end is it decided how [[v_frame]] enters. This order avoids the typical confusion of putting a number in an equation before knowing whom it belongs to. Algebra does not correct a poorly chosen frame; it only propagates the error with more precision.\r
\r
---\r
\r
## 🔴 Structural level\r
\r
The Galilean model works because it simplifies the world in a very specific way. It assumes inertial or quasi-inertial frames, parallel axes, constant relative velocities, and a common time for all observers in the problem. As long as these hypotheses hold, changing [[frame_sigma]] reorders the description without breaking physical coherence. The result may change sign, value, or even transition from rest to motion, but it does not stop referring to the same event.\r
\r
The structure begins to strain when any of these hypotheses break. If the moving frame accelerates appreciably, if one observer rotates relative to the other, or if the comparison mixes different instants, the simple transformation is no longer enough. It also fails when two different origins are used without declaration: the student thinks they are comparing positions, but in reality, they are mixing [[r_vec]] from incompatible geometric scenes. At that point, the equation is not wrong because of an algebraic detail; it is wrong because it no longer corresponds to the mechanism described by the leaf.\r
\r
...
\r
It is also advisable to review the order of magnitude of [[r_vec]] . In a laboratory, a few meters are usually reasonable; in a track or road problem, tens or hundreds of meters; in a temporal graph animation, massive displacements in minimal times often reveal that the origin or the clock was mixed up. The order of magnitude does not solve the problem, but it avoids accepting a translation between frames that is already physically misadjusted.\r
\r
---\r
\r
## Personalized resolution method\r
\r
1. Explicitly name the [[frame_sigma]] of each observer and decide which one simplifies the physical question.\r
2. Fix the origin [[O]] and the positive sign in each description.\r
3. Write [[r_vec]] and, if necessary, the body's own velocity within the accompanying frame.\r
4. Introduce [[v_frame]] as the velocity of the moving frame relative to the fixed frame.\r
5. Check at the end if the change of observer only alters the description and not the event.\r
\r
This method matters because it attacks the most frequent error of the topic: comparing legitimate magnitudes that belong to different observers. When each symbol is assigned to its frame before substituting data, the resolution becomes much shorter and more defensible.\r
\r
---\r
\r
...
\r
### Shifted origins\r
Another useful case is two observers who share [[t_global]] but not spatial origin. It is not enough to say "it is at una coordenada de 5 m": one must say relative to which [[O]] .\r
\r
---\r
\r
## Real student questions\r
\r
- **If the motion is the same, why does a different velocity come out?** The physical motion has not changed, only the partition between own motion and frame motion.\r
- **How do I know which is the best frame?** Choose the one that makes the dominant relationship simplest. If you want to know how a passenger moves inside the train, the carriage frame clarifies first; for a post, the ground frame.\r
\r
---\r
\r
## Cross-cutting connections and study paths\r
\r
This leaf connects directly with relative velocity, meeting and pursuit problems, graphical analysis, and, later, with the frontier where classical mechanics is no longer sufficient. It also prepares the reading for many dynamics problems: before deciding on forces or accelerations, it is useful to know from which frame they will be interpreted.\r
\r
---\r
\r
## Final synthesis\r
\r
Mastering reference frames means choosing a [[frame_sigma]] , fixing an origin [[O]] , and interpreting [[r_vec]] unambiguously. Changing the observer changes the description of the motion, not the physical event described.\r
\r
Remember:\r
1. The frame [[frame_sigma]] is the tool, not the phenomenon.\r
2. The [[v_frame]] is the "translation velocity" between descriptions.\r
3. The time [[t_global]] is the only common thread shared by all Galilean observers.\r
4. A result without a declared frame is physically incomplete.\r
`;export{e as default};
