...
Where [[K_bulk]] is the bulk modulus, [[G_shear]] the shear modulus, and [[rho_medio]] the density. The presence of both moduli reflects that P waves involve both volume change and shape change.
...
The fundamental difference between P and S lies in the type of deformation they mobilize. P waves produce **volumetric dilation** (compression-rarefaction), so their velocity depends on both [[K_bulk]] and [[G_shear]]. S waves produce **shape distortion** without volume change, and depend only on [[G_shear]].

> [!WARNING]
> In a fluid, [[G_shear]] is exactly zero. This means [[v_s]] vanishes and S waves do not exist. P waves, however, still propagate because [[K_bulk]] remains finite.
...
The velocity [[v_p]] combines two elastic restoration mechanisms: resistance to compression ([[K_bulk]]) and resistance to shape change ([[G_shear]]). This means that even in a material very soft in shear but incompressible (like water), P waves propagate effectively.

The velocity [[v_s]], depending only on [[G_shear]], is a pure indicator of structural rigidity. A medium that supports S waves is necessarily a solid. This dichotomy makes the P/S pair a diagnostic tool for the physical state of matter.
...
1. **Identify the medium**: determine [[K_bulk]], [[G_shear]], and [[rho_medio]] of the material (geophysical tables or laboratory tests).
2. **Calculate [[v_p]]**: apply the square root formula with ([[K_bulk]] plus 4/3 times [[G_shear]]) divided by [[rho_medio]].
3. **Calculate [[v_s]]**: apply the square root of [[G_shear]] divided by [[rho_medio]].
...
**Fluid case**: setting [[G_shear]] to zero, the [[v_p]] formula reduces to the speed of sound in the fluid (square root of [[K_bulk]] over [[rho_medio]]). S waves disappear completely.
...
**Partially saturated medium**: partial saturation produces a sharp increase in [[ratio_vp_vs]] because [[K_bulk]] increases with water while [[G_shear]] barely changes. This effect is the basis for hydrocarbon reservoir detection using seismic methods.
...
- **Why is P always faster than S?** Because the [[v_p]] formula includes [[K_bulk]] added to [[G_shear]], while [[v_s]] only includes [[G_shear]]. Adding a positive term in the numerator makes [[v_p]] necessarily larger.

- **How do we know the outer core is liquid?** Because S waves do not traverse it: [[G_shear]] is zero in a liquid, which makes [[v_s]] zero. Only P waves continue (with finite [[K_bulk]]).
...
This leaf connects directly to [Longitudinal waves in bars](leaf:fisica-clasica/ondas/ondas-mecanicas/ondas-en-solidos/ondas-longitudinales-en-barras), where propagation in confined geometry is studied. The main difference is that in a slender bar only Young's modulus is involved (with free lateral deformation), while in an infinite medium [[K_bulk]] and [[G_shear]] enter as independent parameters.
