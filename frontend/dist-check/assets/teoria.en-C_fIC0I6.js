...
The mathematical treatment of double-slit interference begins with the geometry of the experiment. Consider two slits separated by a distance [[d]] that act as coherent secondary sources. A point on the screen, located at angle [[theta]] relative to the central direction, receives contributions from both slits.
...
Where [[m]] is an integer that identifies the order of the maximum. Positive and negative values of [[m]] correspond to fringes on either side of the centre. The zero order corresponds to the central maximum where both paths are equal.
...
The linear position [[y_m]] of each fringe on the screen, at distance [[L]] from the slits, is obtained using the small angle approximation:

{{f:posicion_franja}}

This formula results from combining the maximum condition with the small-angle approximation, valid when [[y_m]] is much smaller than [[L]]. The separation between consecutive fringes is constant:
...
The four equations above form a chained system that completely describes the interference pattern in the small-angle regime. The path difference [[delta]] depends directly on separation [[d]] and angle [[theta]]. The maximum or minimum condition establishes discrete values of [[delta]] for each order [[m]]. The position [[y_m]] translates these angular conditions into measurable linear positions on the screen.
...
The interference pattern establishes a fundamental duality: the larger the slit separation [[d]], the more compressed the pattern, with fringes closer together. This spatial reciprocity relationship is analogous to the time-frequency relationship in Fourier analysis. Increasing [[d]] is equivalent to increasing the angular resolution of the system, allowing sources with smaller angular separation to be resolved. This principle is the basis of long-baseline astronomical interferometry.
...
Typical scales in double-slit interference experiments are the following. Slit separation [[d]] is of the order of 0.1 mm to 0.5 mm. Distance to screen [[L]] is of the order of 1 m to 2 m. Visible wavelength [[lambda]] lies between 400 nm and 700 nm. The resulting fringe separation [[Delta_y]] is typically 1 mm to 10 mm.

The dimensionless factor [[lambda]]/[[d]], of the order of 5 × 10⁻³, determines the angular aperture of the pattern. A typical pattern contains between 10 and 20 visible bright fringes before diffraction from the finite slit width attenuates the intensity. The maximum observable number of orders limits the precision with which [[lambda]] can be measured from the pattern.

## Personalized resolution method

To solve double-slit interference problems, follow this procedure. First, identify the values of [[d]], [[L]] and [[lambda]], verifying that all units are consistent in metres. Second, calculate the path difference for the given position or angle. Third, apply the constructive or destructive interference condition depending on whether maxima or minima are sought. Fourth, solve for the unknown: [[y_m]], [[m]], [[lambda]], [[d]] or [[L]]. Fifth, verify physical consistency: the order [[m]] must be integer, the position [[y_m]] must be reasonable, and fringe separation should be in the millimetre range for typical parameters. Sixth, validate by computing the separation between consecutive fringes as [[lambda]] × [[L]] / [[d]] and verify coherence with partial results.

## Special cases and extended example

The case [[m]] = 0 corresponds to the central maximum, where the path difference is zero and waves always arrive in phase regardless of [[lambda]]. This maximum is white when polychromatic light is used.

Extended example: an experiment uses a helium-neon laser with [[lambda]] of 632.8 nm, slit separation [[d]] of 0.20 mm and screen distance [[L]] of 2.0 m. Substituting into the fringe separation formula yields approximately 6.3 mm. The position of the third lateral maximum is approximately 18.9 mm. If [[d]] doubles, fringes move closer by half. If [[lambda]] increases by 10%, fringes spread by 10%. This analysis demonstrates that the pattern allows measuring [[lambda]] with 0.1% precision by knowing the geometric parameters.
...
Can the small-angle formula be used for any order? No. The approximation is valid only when [[y_m]] is much smaller than [[L]]. For high orders where the angle exceeds 0.1 radians, the exact expression with arcsine must be used.
