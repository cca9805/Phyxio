...
An everyday object is **electrically neutral** because it has equal numbers of protons and electrons. When you rub a balloon against wool, the balloon gains electrons and acquires negative [[q]]; the wool loses electrons and acquires positive [[q]]. The total charge of the system (balloon plus wool) has not changed: it has simply been redistributed.

Electric charge cannot be created or destroyed, only transferred. And it transfers always in discrete packets of minimum size [[e]], the proton charge. The number [[n]] of packets transferred is always an exact integer.

## 🔵 Formal level

Charge quantization states that every observable charge is an integer multiple of the elementary charge [[e]]. For an object with [[n]] elementary charges:

{{f:cuantizacion_carga}}

Here [[q]] is the total charge, [[n]] is an integer (positive for electron deficit, negative for excess), and [[e]] is the proton charge, approximately 1.602 times ten to the minus nineteen coulombs.

This formula solves two types of problems: given [[n]], calculate [[q]]; given a measured [[q]], determine how many electrons were transferred by solving for [[n]]. The second rearrangement requires the quotient to be an exact integer, which serves as a coherence check.
...
The two formulas are not independent. Quantization states that each individual contribution to [[q_pos]] or [[q_neg]] is a multiple of [[e]]; the net charge of the system is therefore also a multiple of [[e]].
...
In modern physics, quarks carry fractional charges of [[e]] (one-third and two-thirds), but are never observed free: they are confined in hadrons with integer total charge. This confinement preserves quantization in integer multiples of [[e]] at macroscopic scale.
...
The SI unit of charge is the **coulomb (C)**: since 2019 it is defined as the charge transported by 1 A in 1 s, linking [[q]] with current. Current is the rate of charge flow, not the charge itself.

Another structural implication: electric charge is a **relativistic invariant** — all observers measure the same [[q]] for a given object regardless of their relative velocity. This contrasts with mass or length, which depend on the reference frame.
...
> [!NOTE]
> When two neutral objects are rubbed together, no charge is created: electrons migrate from one to the other. The algebraic sum of [[q]] for the system remains zero.

## Order of magnitude

The elementary charge [[e]] is approximately 1.6 times ten to the minus nineteen coulombs; a lightning bolt discharges between 1 and 5 coulombs; a small laboratory capacitor stores between 1 and 100 microcoulombs.
...
4. **Apply quantization when [[n]] is requested**: divide [[q]] by [[e]] and verify the result is an integer.
5. **Check the scale**: confirm the order of magnitude is reasonable for the physical context.

Step 4 acts as automatic validation: if [[n]] is not an integer, there is an error in the data or units.
...
**Case 3 — Neutralization**: If an object with positive [[q]] and another with negative [[q]] of the same magnitude are put in contact, electrons flow from the negative to the positive object until both are neutral. The system's total charge remains zero: no charge was destroyed.
...
**If I rub a balloon and it acquires negative [[q]], where do the extra electrons come from?**
...
Quarks carry charges of one-third and two-thirds of [[e]], but are never observed free: quantum chromodynamics confinement prevents extracting them from hadrons. At macroscopic scale and throughout classical physics, the answer is no: the minimum observable charge in free particles is [[e]].
...
Electric charge connects directly with [Coulomb's law](leaf:fisica-clasica/electromagnetismo/fundamentos/ley-de-coulomb): [[q]] is the factor that determines the force between charged objects. The natural next step is the [electric field](leaf:fisica-clasica/electromagnetismo/fundamentos/campo-electrico), where [[q]] acts as the proportionality factor between field and force on a test particle.

In advanced electromagnetism, [[q]] is the source of the electric field in Gauss's law, and current (flow of [[q]]) is the source of the magnetic field. Understanding [[q]] means understanding the sources of all electromagnetic fields.

## Final synthesis

Electric charge is the fundamental property of matter that originates all electromagnetic interaction. Its quantization in integer multiples of [[e]] and its absolute conservation are the two deepest laws of classical electromagnetism, experimentally valid without exception from atomic to macroscopic scales. Every electrical calculation begins with [[q]].
`;export{e as default};
