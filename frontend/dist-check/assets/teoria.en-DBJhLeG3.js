const e=`# Types of Waves in Solids
...
The longitudinal wave velocity [[v_L]] in an isotropic three-dimensional solid is calculated from Young's modulus [[E_young]] and Poisson's ratio [[nu]]:

{{f:vel_longitudinal}}

The factor in parentheses combines [[E_young]] and [[nu]] to form the **confined modulus**, which is always larger than [[E_young]] for any positive [[nu]]. This is why [[v_L]] in a 3D solid is larger than the longitudinal velocity in a thin bar of the same material.

The transverse wave velocity [[v_T]] depends exclusively on the shear modulus [[G_corte]] and density:

{{f:vel_transversal}}

The simplicity of this expression is revealing: [[v_T]] does not depend on how the material responds to compression, only on how it responds to shear. Therefore, when [[G_corte]] tends to zero (fluid limit), [[v_T]] tends to zero and the S-wave disappears.

The Rayleigh wave velocity [[v_R]] is obtained as a fraction of [[v_T]] that depends gently on [[nu]] through Viktorov's approximation:

{{f:vel_rayleigh}}

This formula shows that [[v_R]] is always between 87 and 96% of [[v_T]], regardless of the material. The velocity hierarchy is invariant: [[v_L]] greater than [[v_T]] greater than [[v_R]] in any isotropic elastic solid.
...
The transverse wave, by contrast, is **purely shape-dissipative**: it moves the material angularly without changing its volume. It activates only [[G_corte]], which is why its speed relation is simpler and slower than the longitudinal one. This purity makes [[v_T]] measurements the best estimators of the shear modulus in non-destructive testing.

The **velocity hierarchy** is not accidental: it can be proved that for any stable isotropic material (with [[nu]] between zero and 0.5), [[v_L]] always exceeds [[v_T]]. The relative difference depends exclusively on [[nu]] and becomes very large when [[nu]] approaches 0.5 (nearly incompressible materials such as rubber), because in that limit the confined modulus diverges while [[G_corte]] remains finite.
...
This has immediate practical consequences: a material soft in shear (low G) but stiff in compression (high K) — such as rubber — has a moderate [[v_L]] but a very low [[v_T]]. A material stiff in shear but light — such as aluminium — has a high [[v_T]] for its density. The combination of [[v_L]] and [[v_T]] measured experimentally allows simultaneous determination of two of the five elastic moduli of the material without any macroscopic deformation.
...
These values contrast with those of soft materials: in soft geotechnical soil, [[v_T]] can be as low as 50 m/s, while [[v_L]] is around 300 m/s. The separation between [[v_L]] and [[v_T]] is doubly large in relative terms, because [[nu]] of saturated soil approaches 0.5.

A physically absurd result would be obtaining [[v_T]] greater than [[v_L]] in the same material: that would imply a Poisson's ratio outside the physically admissible range, or an error in the input moduli.

## Personalized resolution method

The analysis process for wave types in a solid follows a reproducible protocol:

1. **Identify the material parameters**: [[E_young]], [[G_corte]], [[rho]] and [[nu]]. If only two of the four are known, verify the elastic relation linking them before proceeding.

2. **Calculate [[v_L]]** with the full 3D solid formula if the medium is three-dimensionally homogeneous; or with the thin-bar version if the solid is geometrically confined.

3. **Calculate [[v_T]]** using [[G_corte]] and [[rho]]. This step always precedes the calculation of [[v_R]].

4. **Calculate [[v_R]]** using Viktorov's approximation if [[nu]] is between 0 and 0.5.

5. **Verify the hierarchy**: [[v_L]] greater than [[v_T]] greater than [[v_R]]. If not satisfied, there is an error in the data or the model.

6. **Interpret**: The ratio [[v_T]]/[[v_L]] determines [[nu]]; the ratio [[v_R]]/[[v_T]] is an additional indicator of [[nu]]. If all three velocities are mutually consistent, the material characterisation can be trusted.

## Special cases and extended example

**Nearly incompressible material ([[nu]] close to 0.5)**: in rubber, [[v_L]] grows strongly because the confined modulus increases, while [[v_T]] remains moderate. The P-S separation becomes large: the P-wave looks fluid-like, but the S-wave reveals solid shear stiffness.

**Auxetic material (negative [[nu]])**: in re-entrant foams the P-S separation is reduced, and Viktorov's approximation for [[v_R]] becomes less reliable.

**Stainless steel 304**: with E of 193 GPa, G of 77 GPa, density of 7900 kg/m³ and [[nu]] of 0.25, the approximate speeds are [[v_L]] = 5700 m/s, [[v_T]] = 3120 m/s and [[v_R]] = 2900 m/s. The hierarchy holds and [[v_T]]/[[v_L]] is consistent with [[nu]].
...
Its surface mode combines compression and shear without maximising propagation speed. The solution for [[v_R]] lies slightly below [[v_T]].

**What information does the ratio [[v_T]]/[[v_L]] give me about the material?**
...
Elastic solids host longitudinal, transverse and Rayleigh surface waves. Their speeds depend on different combinations of elastic moduli and [[rho]]. The hierarchy [[v_L]] greater than [[v_T]] greater than [[v_R]] is the diagnostic summary: S-waves are the direct dynamic signature of shear resistance.
`;export{e as default};
