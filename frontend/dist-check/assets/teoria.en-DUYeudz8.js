const e=`# Reflection and Transmission in Solids
...
[[Z_1]] and [[Z_2]] are the two numbers being compared. Each is calculated as the product of medium density and wave velocity in that medium; the same product can be read as [[Z_medio]] when referring to a generic medium impedance. A dense and stiff medium (like steel) has high impedance. A light and flexible medium (like air) has low impedance.

[[R_amp]] indicates what fraction of the amplitude bounces back. If impedances match, [[R_amp]] is zero: no reflection. If they differ greatly, [[R_amp]] approaches unity in absolute value: nearly all the wave bounces back.

[[T_amp]] indicates what fraction of the amplitude passes to the second medium. For normal incidence, reflected and transmitted amplitudes add up to the incident amplitude in terms of displacement continuity.
...
Energy conservation requires that [[R_E]] plus [[T_E]] equals exactly unity. This condition serves as an immediate check for any calculation.
...
The algebraic structure shows that only the impedance ratio matters, not their absolute values. Two interfaces with the same [[Z_2]]/[[Z_1]] ratio produce exactly the same reflection, regardless of the specific materials.

The sign of [[R_amp]] has physical meaning: if [[Z_2]] is greater than [[Z_1]], the reflected wave maintains the phase of the incident one. If [[Z_2]] is less than [[Z_1]], the reflected wave inverts its phase. This phase change is analogous to what a string experiences at a fixed rigid end versus a free end.
...
At steel-aluminium interfaces, the amplitude reflection coefficient is of the order of 0.35 and the energy coefficient of the order of 0.12. At steel-air interfaces, reflection is practically total ([[R_E]] close to 1) because air impedance is about one hundred thousand times less than steel.
...
A quick check: if [[R_E]] comes out greater than 1 or negative, there is an error. If [[R_E]] plus [[T_E]] does not sum to 1, there is an error.

## Personalized resolution method

First calculate the impedance of each medium as the product of density and velocity. Second, calculate the amplitude reflection coefficient as difference divided by sum. Third, square to obtain the energy reflection. Fourth, verify that [[R_E]] plus [[T_E]] equals 1.
...
If [[Z_2]] tends to infinity (ideal rigid wall), R approaches 1 and T approaches 2 in amplitude. The amplitude at the interface doubles (constructive interference between incident and reflected), but transmitted energy is zero because there is no net displacement in the wall.

If [[Z_2]] tends to zero (ideal free surface), R approaches -1 and T approaches 0 in amplitude. The reflected wave inverts its phase and all energy returns.
