const e=`# General Rotating Frame

## Conceptual context

General Rotating Frame answers a precise physical question: How is absolute acceleration decomposed in a general rotating frame? The topic is not introduced to add one more equation, but to give the student a reading tool when elementary models no longer separate cause, scale, and domain clearly.

A general rotating frame separates relative acceleration, origin acceleration, and apparent terms. The important reading is distinguishing which forces come from real interaction and which appear because the observer rotates. For that reason the core quantities, [[a_abs]], [[a_rel]], [[a_O]], [[Omega]], [[alpha]], [[r]], [[v_rel]], [[F_E]], [[m]], are not decorative labels. Each one marks part of the structure: a scale, a boundary, a dynamic variable, a validity condition, or a conserved quantity.

## 🟢 Essential level

The essential idea is that a general rotating frame separates relative acceleration, origin acceleration, and apparent terms. Before calculating, the student must state which physical mechanism is being isolated and which part of the system is deliberately left outside the model. Without that reading, an equation may produce the right number for the wrong question.

At this level, [[a_abs]] acts as the dominant quantity because it concentrates the didactic decision of the leaf. It is not enough to name it: the student must explain whether it increases, decreases, changes sign, vanishes, or loses meaning when the regime changes.
...
It is also useful to separate description from decision. Description lists quantities; decision explains which of them controls the observed change. In this leaf, that decision is supported by [[a_abs]] and by the comparison between dominant term, correction, and limiting case. This distinction prevents the student from confusing a long solution with a deep solution.
...
The deeper structure appears when the model is compared with its limits. A general rotating frame separates relative acceleration, origin acceleration, and apparent terms. The important reading is distinguishing which forces come from real interaction and which appear because the observer rotates. This sentence must work as a control criterion: it decides whether the topic is being used in its natural domain or forced outside its range.
...
Deep interpretation starts by asking what [[a_abs]] represents in the causal chain. If [[a_abs]] is modified, the regime may change, a critical condition may move, or a secondary assumption may stop being secondary.
...
This check prevents artificial conclusions. The units of [[a_abs]], [[a_rel]], [[a_O]], [[Omega]], [[alpha]], [[r]], [[v_rel]], [[F_E]], [[m]] must support the same physical dimension and the same experimental scale.

## Personalized resolution method

First, delimit the system and state the assumptions. Second, identify [[a_abs]] and decide which relation answers the central question. Third, calculate symbolically before substituting numbers.
...
At the end, the student should explain that a general rotating frame separates relative acceleration, origin acceleration, and apparent terms, apply the formulas in their domain, and justify the physical meaning of the result. The final competence is deciding whether the model answers the question of the real system, not merely obtaining an algebraic expression.
