const e=`# Differential Formulation of Maxwell's Equations
...
## 🟢 Essential level
To understand the local description without complex calculations, imagine that the electric field [[E]] and magnetic field [[B]] are like invisible fluids filling space. At each point, however small, we can ask two questions that define reality: Is there something here creating or absorbing the fluid? and Is the fluid spinning around this point?

The first question analyzes expansion. If at a point the electric field [[E]] tends to open outward, there is a source expelling lines. If it tends to close, there is a sink. This indicates that the opening tendency at a point is proportional to the electric charge density [[rho]] located exactly there. It is an immediate neighborhood connection: the charge perturbs the medium at its own point, and that perturbation then spreads through the universe. It is like watching water springing from a source; the flow tells us where the tap is. In magnetism, lines are neither born nor die, meaning that no isolated creation points exist for the [[B]] field.

The second question analyzes local spin. If the field seems to "whirl" around a point, a rotation exists. The [[E]] field starts to spin if the nearby [[B]] field changes over time. Similarly, the [[B]] field can spin if there is a charge flow [[J]] or if the [[E]] field varies. This dance of local spins and flows keeps the electromagnetic interaction alive even in vacuum, allowing energy to propagate as light. It is this point-to-point rotation that allows the interaction to "jump" from one place to the next without the need for a material support, creating an infinite chain of mutual inductions that crosses the cosmos.

## 🔵 Formal level
The differential formulation is based on the nabla operator (\\(\\nabla\\)), which acts on vector fields [[E]] and [[B]]. This operator condenses information on spatial derivatives, allowing physical laws to be written compactly. In addition to the fundamental laws, there are structural consequences governing conservation and propagation. The fundamental equations and their direct derivatives that define the leaf core are:

1. **Gauss's Law for electricity**: Relates [[divE]] to [[rho]] and [[epsilon0]].
{{f:gauss_local_e}}

2. **Gauss's Law for magnetism**: Establishes that [[divB]] is zero.
{{f:gauss_local_b}}

3. **Faraday-Lenz Law**: Relates [[curlE]] to the time variation [[dB_dt]].
{{f:faraday_local}}

4. **Ampère-Maxwell Law**: Relates [[curlB]] to [[J]] and the time variation [[dE_dt]], mediated by [[mu0]].
{{f:ampere_local}}

5. **Continuity equation (Charge conservation)**: Relates [[divJ]] to the variation of [[rho]].
{{f:continuidad_carga}}

6. **Wave equation (Propagation in vacuum)**: Relates [[laplacianE]] to the second time variation of the field.
{{f:onda_electromagnetica}}

7. **Total current definition**: Sum of real and displacement currents.
{{f:definicion_corriente_maxwell}}

These expressions form a system of coupled partial differential equations. Solving them allows finding the fields in any region if we know the sources and boundary conditions. The fact that charge conservation and light propagation at speed [[c]] arise directly from the four main laws is one of the greatest achievements of theoretical physics, demonstrating the internal consistency and mathematical beauty of Maxwell's building. Each term has a precise physical interpretation: the source, the induction, the displacement current, and conservation.
...
2. **Apply Local Gauss's Law**: Calculate field divergence [[divE]] to determine the spatial distribution of charge sources [[rho]] in the region of interest.
3. **Analyze the Dynamic Regime**: Check if time derivatives [[dE_dt]] and [[dB_dt]] are significant compared to static terms.
...
2. **What is "displacement current"?** It is not a flow of electrons, but a term proportional to [[dE_dt]] indicating that a varying electric field has magnetic effects.
3. **Do magnetic monopoles exist?** To date, there is no experimental evidence; divergence [[divB]] is always zero.
