const e=`# Vorticity and Circulation

## Conceptual context

Vorticity and Circulation answers a precise physical question: How do vorticity and circulation connect local fluid rotation with global lift effects? The topic is not introduced to add one more equation, but to give the student a reading tool when elementary models no longer separate cause, scale, and domain clearly.

Vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour. The student learns not to confuse a curved path with local rotation of a fluid element. For that reason the core quantities, [[omega]], [[v]], [[Gamma]], [[C]], [[rho]], [[Vinf]], [[Lprime]], are not decorative labels. Each one marks part of the structure: a scale, a boundary, a dynamic variable, a validity condition, or a conserved quantity.

## 🟢 Essential level

The essential idea is that vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour. Before calculating, the student must state which physical mechanism is being isolated and which part of the system is deliberately left outside the model. Without that reading, an equation may produce the right number for the wrong question.

At this level, [[omega]] acts as the dominant quantity because it concentrates the didactic decision of the leaf. It is not enough to name it: the student must explain whether it increases, decreases, changes sign, vanishes, or loses meaning when the regime changes.
...
It is also useful to separate description from decision. Description lists quantities; decision explains which of them controls the observed change. In this leaf, that decision is supported by [[omega]] and by the comparison between dominant term, correction, and limiting case. This distinction prevents the student from confusing a long solution with a deep solution.
...
To apply them, [[omega]], [[v]], [[Gamma]], [[C]], [[rho]], [[Vinf]], [[Lprime]] must belong to the same system. Sign, units, and domain must also be checked. The central question remains: How do vorticity and circulation connect local fluid rotation with global lift effects? If an algebraic rearrangement changes that question, the procedure has stopped being physics and has become symbolic manipulation.
...
Formally, vorticity requires thinking about local rotation of a material element, whereas circulation requires following a contour. That double scale is specific to the leaf and prevents high velocity from being confused with real fluid spin.

## 🔴 Structural level

The deeper structure appears when the model is compared with its limits. Vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour. The student learns not to confuse a curved path with local rotation of a fluid element. This sentence must work as a control criterion: it decides whether the topic is being used in its natural domain or forced outside its range.
...
The specific reading of vorticity separates local spin from curved path. A fluid element may move along a curved trajectory without rotating about itself, and a global circulation may reveal lift even when the exterior region is almost irrotational. This leaf trains the local-global view of fluid motion.

## Deep physical interpretation

Deep interpretation starts by asking what [[omega]] represents in the causal chain. If [[omega]] is modified, the regime may change, a critical condition may move, or a secondary assumption may stop being secondary.
...
This check prevents artificial conclusions. The units of [[omega]], [[v]], [[Gamma]], [[C]], [[rho]], [[Vinf]], [[Lprime]] must support the same physical dimension and the same experimental scale.

## Personalized resolution method

First, delimit the system and state the assumptions. Second, identify [[omega]] and decide which relation answers the central question. Third, calculate symbolically before substituting numbers.
...
At the end, the student should explain that vorticity measures local spin of the fluid and circulation summarizes accumulated swirl around a contour, apply the formulas in their domain, and justify the physical meaning of the result. The final competence is deciding whether the model answers the question of the real system, not merely obtaining an algebraic expression.
...
This leaf requires local-rotation language: curl, contour, circulation, and lift. Its identity is lost if it becomes a generic balance, because the central question is not what enters or leaves, but where material spin appears and how it accumulates around a curve.
`;export{e as default};
