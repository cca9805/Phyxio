...
The quantitative relationship between the object distance [[d_o]], the image distance [[d_i]] and the focal length [[f]] is given by the mirror equation:

{{f:ecuacion_espejo}}

Here [[d_o]] is the distance from the object to the mirror vertex, [[d_i]] the distance from the vertex to the image and [[f]] the focal length. In the standard sign convention, real distances (in front of the mirror) are positive and virtual distances (behind the mirror) are negative. For a concave mirror [[f]] is positive; for a convex mirror [[f]] is negative.

The relative size of the image is described by the lateral magnification:

{{f:aumento_lateral}}

The sign of [[M]] indicates orientation: negative means an inverted image and positive means an upright image. The absolute value gives the scale factor.

The focal length relates to the mirror geometry through:

{{f:distancia_focal_radio}}

Here [[R]] is the radius of curvature of the spherical surface. This relationship is exact within the paraxial approximation.

> [!NOTE]
> The sign convention is the most critical aspect when solving mirror problems. A sign error in [[f]] or [[d_o]] completely changes the nature of the image (real versus virtual, inverted versus upright).
...
The paraxial approximation assumes that rays form small angles with the optical axis, so the sine and tangent of the angle approximate the angle in radians. This simplification holds as long as the mirror aperture is much smaller than [[R]]. When the aperture is large, marginal rays do not converge at the same point as paraxial ones, producing **spherical aberration**. To correct it, parabolic mirrors are used, whose surface is a paraboloid of revolution that guarantees perfect convergence for rays parallel to the axis.
...
The duality between real and virtual images is encoded in the sign of [[d_i]]. In concave mirrors the transition occurs exactly when the object sits at the focus: for [[d_o]] greater than [[f]] the image is real; for [[d_o]] less than [[f]] it is virtual. In convex mirrors there is no such transition because the image is always virtual with negative [[d_i]].

## Deep physical interpretation

The negative sign in the definition of [[M]] encodes the geometric inversion of the image. When [[d_i]] and [[d_o]] share the same sign (both positive for a real object and a real image), [[M]] is negative, indicating inversion. This result is not arbitrary: inversion occurs because the reflected rays cross the optical axis before converging, flipping the vertical orientation of the image relative to the object.

In the limit where [[d_o]] tends to infinity, [[d_i]] tends to [[f]]: rays from a very distant source converge exactly at the focus. This principle underlies reflecting telescopes and solar concentrators. At the other extreme, when [[d_o]] approaches [[f]], [[d_i]] diverges to infinity, producing a nearly parallel beam of reflected rays, as in a flashlight with a parabolic reflector.

## Order of magnitude

For a typical laboratory concave mirror, [[R]] lies between 0.10 and 1.00 m, so [[f]] ranges from 0.05 to 0.50 m. Vehicle rear-view mirrors have radii of 1 to 3 m (convex, negative [[f]]). Astronomical telescopes use mirrors with [[R]] of several metres. A result where [[d_i]] exceeds [[d_o]] by a factor of 10 or more should be checked unless the object is very close to the focus. A magnification [[M]] with magnitude above 5 is uncommon outside specialised optical setups.

## Personalized resolution method

1. **Identify the mirror type**: concave ([[f]] positive) or convex ([[f]] negative).
2. **Obtain [[f]]**: if [[R]] is given, apply the focal-length–radius relation.
3. **Substitute into the mirror equation**: with [[d_o]] and [[f]], solve for [[d_i]].
4. **Calculate the magnification**: with [[d_i]] and [[d_o]], obtain [[M]].
5. **Interpret signs**: positive [[d_i]] indicates a real image; negative, virtual. Negative [[M]] indicates an inverted image; positive, upright.
...
- **Object at the centre of curvature** ([[d_o]] equal to [[R]]): the image forms at the same point, [[d_i]] equal to [[R]], with [[M]] equal to minus one. The image is real, inverted and the same size as the object.

- **Object at the focus** ([[d_o]] equal to [[f]]): the mirror equation yields infinite [[d_i]]. No finite image forms; reflected rays emerge parallel. This case underlies flashlight and headlight reflectors.

- **Convex mirror**: regardless of the object position, [[d_i]] is always negative and the magnitude of [[M]] is always less than one. The image is virtual, upright and reduced, which explains the warning "objects in mirror are closer than they appear" on vehicle mirrors.
