...
This leaf explains why induction does not oppose magnetic field merely by existing, but opposes linked flux change. The central quantity is [[epsilon_ind]], but its reading depends on [[DeltaPhi_B]], [[r_Phi]], and the chosen sign convention.
...
The rule can also be written as a conceptual sign relation:

{{f:regla_signos_lenz}}

These relations say the same thing at different levels. [[r_Phi]] contains the sign of flux change, while [[epsilon_ind]] appears with opposite sign if [[N]] is positive. The sign is not algebraic punishment: it encodes physical polarity of the induced emf.
...
On a graph of [[epsilon_ind]] versus [[r_Phi]], Lenz appears as a negative slope. That slope does not represent dissipation; it represents that induced response changes sign when the sign of flux variation changes.
...
To apply the law, first fix a surface and a normal. That choice gives sign to [[Phi_B]] and [[DeltaPhi_B]]. Then decide whether flux increases or decreases. Only then assign induced polarity. Without that sequence, phrases like "it opposes the field" become ambiguous.
...
> [!TIP]
> Safe procedure: fix normal, decide whether [[Phi_B]] increases or decreases, reverse the sign for the induced response, and then translate that sign into current or polarity.
...
An absurd result appears if induced polarity reinforces the initial change, if [[epsilon_ind]] is predicted without [[DeltaPhi_B]], or if [[Deltat]] is used in milliseconds without conversion. Scale depends on magnitude; direction depends on sign.

## Personalized resolution method

First draw the positive normal and decide the sign of [[Phi_B]]. Second compare initial and final states to find whether [[DeltaPhi_B]] increases or decreases. Third apply opposition: induced current or emf must produce an effect contrary to that change.

Then compute magnitude with [[Deltat]] and [[N]] if needed. Finally translate the sign into a physical sentence: the loop tries to sustain flux, reduce its increase, or resist the motion changing it.
...
Lenz's law states that induction is oriented against the flux change causing it. [[epsilon_ind]] has physical meaning only if the sign of [[DeltaPhi_B]] or [[r_Phi]] is preserved and induced polarity is interpreted.
`;export{e as default};
