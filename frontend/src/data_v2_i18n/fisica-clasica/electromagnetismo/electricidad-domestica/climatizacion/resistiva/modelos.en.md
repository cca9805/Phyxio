...
The ideal stationary model of resistive heating assumes that [[consumo_electrico]] is calculated exclusively as the product of [[potencia_del_emisor]] by [[tiempo_efectivo_de_uso]], with a constant [[conversion efficiency|eta]] close to 1 and a [[coeficiente_de_rendimiento]] = 1. This model represents heating as a purely linear process: doubling [[potencia_del_emisor]] or [[tiempo_efectivo_de_uso]] doubles [[consumo_electrico]] and cost.

This model is the canonical starting point for any first-approximation residential estimate. Its strength lies in transparency: it only requires knowing [[potencia_del_emisor]] and [[tiempo_efectivo_de_uso]] to obtain [[consumo_electrico]] with high reliability when the system operates in quasi-stationary regime. It is the appropriate tool for comparing usage scenarios and evaluating the impact of behavioral changes.

## Hypotheses

For the ideal model to be applicable, the following simplifying conditions are assumed:
1. **Constant power**: [[potencia_del_emisor]] remains at its nominal value throughout the analyzed period; there are no variations due to grid voltage or degradation of the resistive element.
2. **Known effective time**: [[tiempo_efectivo_de_uso]] represents the real hours with the resistance actually active, not the hours with the device switched on. The thermostat cycle is assumed to be correctly accounted for in the average.
...
- [[potencia_del_emisor]] is known with accuracy of ±5%.
- [[tiempo_efectivo_de_uso]] is estimated or measured with error below 15%.
...
- **Bill-to-calculation discrepancy > 20%**: If consumption measured on the bill differs by more than 20% from that calculated with the main consumption formula, [[tiempo_efectivo_de_uso]] is poorly estimated or there are uncounted additional loads.
...
- **Comparative model with heat pump**: Replaces the resistive [[coeficiente_de_rendimiento]] = 1 with the seasonal SCOP of the alternative (typically 3–5), enabling comparison of annual [[consumo_electrico]] for the same [[calor_util]] delivered.
...
| **Required data** | [[potencia_del_emisor]] and [[tiempo_efectivo_de_uso]] | Envelope + climate | Full hourly data |
