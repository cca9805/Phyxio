...
- **Moderate loads**: Total current [[corriente_total_de_linea]] less than 80 % of the MCB rating [[corriente_maxima_del_circuito]]. Above this threshold, heat in the cables begins to be appreciable and the resistance of copper increases slightly.
...
- **Abnormal heating of cables**: If conductors heat visibly without [[corriente_total_de_linea]] reaching [[corriente_maxima_del_circuito]], the cable resistance has increased due to aging or insufficient cross-section.
...
- **Current measurement higher than calculated**: If an ammeter shows [[corriente_total_de_linea]] systematically higher than that predicted by the formula, the power factor is different from 1 or there are leakage currents.
...
- It is calculated that [[corriente_maxima_del_circuito]] would produce a voltage drop exceeding 10 V in the line
- Working with significant inductive or capacitive receptors (motors, transformers)

The extended model also contemplates the **apparent power**, different from the active power [[potencia_total_conectada]] when the power factor is less than 1. In this case, the total current results greater than that predicted by the ideal model, explaining why it underestimates the actual current.
...
| **Required variables** | [[potencia_total_conectada]], [[tension_de_red]] | [[potencia_total_conectada]], [[tension_de_red]], power factor, cable data |
...
4. **Verify the safety margin**: Check that the current calculated with the extended model maintains a safe usage margin with respect to [[corriente_maxima_del_circuito]].