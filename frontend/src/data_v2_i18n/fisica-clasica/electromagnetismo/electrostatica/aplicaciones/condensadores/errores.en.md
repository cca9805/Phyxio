...
### Error 1: Believing capacitance [[capacidad_electrica]] changes when voltage [[diferencia_de_potencial]] changes

**Why it seems correct**

The student observes that as voltage increases, stored charge [[carga_almacenada]] also increases. By analogy with Ohm's law (where current changes with both voltage and resistance), they conclude that capacitance must also change, because "something must change in the capacitor" to store more charge.

**Why it is incorrect**

Capacitance [[capacidad_electrica]] is a geometric and material property of the capacitor: it depends on plate area, separation, and dielectric permittivity. None of those factors change when the applied voltage changes (in the ideal model). The only thing that changes is [[carga_almacenada]], which increases proportionally with [[diferencia_de_potencial]] for constant [[capacidad_electrica]].

**Detection signal**

The student calculates the ratio [[carga_almacenada]]/[[diferencia_de_potencial]] for two different voltages and obtains different values, then claims [[capacidad_electrica]] changed. If the values obtained are identical (as they should be), the error is absent.

**Conceptual correction**

Capacitance [[capacidad_electrica]] is constant for a given capacitor in the linear regime. [[carga_almacenada]] varies with [[diferencia_de_potencial]], but the proportionality coefficient [[capacidad_electrica]] is fixed. [[capacidad_electrica]] only changes if the capacitor is physically modified (area, separation, dielectric).
...
Check 1: In Capacitors, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
The definition of [[capacidad_electrica]] uses [[carga_almacenada]] as a positive value. The student interprets "the capacitor has charge Q" as applying to its entire structure, without distinguishing between the two plates. Also, the battery "injects" charge into the circuit, suggesting that all charge goes uniformly into the capacitor.
...
Before applying the [[capacidad_geometrica_del_condensador_plano]] formula with [[epsilon]], area, and separation, verify that d is at least 10–20 times smaller than the smaller lateral dimension of the plate. If not, the formula underestimates actual capacitance and a model with fringe correction or numerical simulation must be used.
...
The 1/2 factor in [[energia_almacenada_en_el_condensador]] is not optional: it arises from integrating charging work from zero voltage to the final value. During charging, voltage grows progressively; work at each instant depends on accumulated charge divided by [[capacidad_electrica]], and the integral gives [[carga_almacenada]]²/(2·[[capacidad_electrica]]). Omitting the 1/2 doubles the calculated energy.
...
The correct formula for [[energia_almacenada_en_el_condensador]] includes the 1/2 factor multiplying [[capacidad_electrica]] by [[diferencia_de_potencial]]². Three equivalent expressions exist: using [[capacidad_electrica]] and [[diferencia_de_potencial]], using [[carga_almacenada]] and [[diferencia_de_potencial]], or using [[carga_almacenada]] and [[capacidad_electrica]]. The 1/2 factor reflects that average voltage during charging is half the final value.
...
1. **1/2 factor in energy**: Does the [[energia_almacenada_en_el_condensador]] formula include the 1/2 factor multiplying [[capacidad_electrica]] by [[diferencia_de_potencial]]² or [[carga_almacenada]] by [[diferencia_de_potencial]]? If the factor is absent, there is a definite error.
...
3. **Consistency between [[carga_almacenada]], [[capacidad_electrica]] and [[diferencia_de_potencial]]**: does the product of [[capacidad_electrica]] by [[diferencia_de_potencial]] equal [[carga_almacenada]]? If not, one of the three is incorrectly calculated or entered.
