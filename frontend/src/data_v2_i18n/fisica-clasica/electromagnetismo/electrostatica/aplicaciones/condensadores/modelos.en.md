...
The **ideal capacitor model** describes a device with two flat parallel conducting plates of uniform effective area [[capacidad_geometrica_del_condensador_plano]], separated by a perfectly linear, homogeneous, and isotropic dielectric, with no losses or leakage current. In this model, the relation between [[carga_almacenada]], [[capacidad_electrica]] and [[diferencia_de_potencial]] is exactly linear, and stored energy [[energia_almacenada_en_el_condensador]] follows the formula with the 1/2 factor, with no losses at any stage of the charge–discharge cycle.

The model preserves the quantities [[capacidad_electrica]], [[carga_almacenada]], [[diferencia_de_potencial]], and [[energia_almacenada_en_el_condensador]] in their simplest form. It ignores series parasitic resistance (ESR), parasitic inductance (ESL), leakage currents through the dielectric, variation of permittivity [[epsilon]] with electric field and temperature, and fringe effects at the edges of the plates.

## Hypotheses

- **Uniform electric field between plates**: requires that separation d be much smaller than the lateral plate dimensions (\(d \ll \sqrt{A}\)). If violated, the field at the edges is non-uniform and actual capacitance is slightly higher than calculated with the [[capacidad_geometrica_del_condensador_plano]] formula from [[epsilon]], area, and separation.
- **Linear and isotropic dielectric**: polarization is proportional to the electric field in all directions. If violated (ferroelectric materials, PZT ceramics), capacitance depends on applied voltage and the relation between [[carga_almacenada]] and [[diferencia_de_potencial]] is nonlinear.
...
- **Voltage below breakdown voltage**: [[diferencia_de_potencial]] must remain below the dielectric breakdown threshold. If violated, the dielectric conducts and the capacitor is irreversibly destroyed.
...
- **Nonlinearity of Q vs V**: if plotting [[carga_almacenada]] against [[diferencia_de_potencial]] the data points do not fall on a straight line, the linear model no longer applies. This indicates that the permittivity [[epsilon]] of the dielectric depends on the field (nonlinear ceramic or ferroelectric material).
...
- **Frequency-dependent capacitance**: if [[capacidad_electrica]] measured with an LCR bridge decreases significantly as frequency rises, parasitic inductance ESL is relevant and the ideal model is no longer sufficient.
