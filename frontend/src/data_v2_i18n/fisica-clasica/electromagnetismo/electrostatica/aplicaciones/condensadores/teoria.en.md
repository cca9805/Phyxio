...
What defines a capacitor as a better or worse charge storage device is its **capacitance** [[capacidad_electrica]]. A high-capacitance capacitor accumulates a lot of charge for a small voltage; a low-capacitance one needs high voltages to accumulate the same charge. Capacitance does not depend on the applied voltage or the stored charge: it is a fixed property of the device, determined by its geometry and its dielectric material.

The **voltage** [[diferencia_de_potencial]] between the plates is the direct cause of everything that happens in the capacitor. Higher voltage means more accumulated charge [[carga_almacenada]] and more stored energy [[energia_almacenada_en_el_condensador]]. The relationship is simple and elegant: charge grows proportionally with voltage, and energy grows quadratically.

## 🔵 Formal level

The **quantitative definition of capacitance** is obtained by measuring how much charge [[carga_almacenada]] the capacitor accumulates per volt [[diferencia_de_potencial]] of applied voltage:

{{f:capacidad_definicion}}

This equation has three useful readings: it calculates [[capacidad_electrica]] by measuring Q and V, predicts [[carga_almacenada]] knowing C and V, or finds [[diferencia_de_potencial]] knowing C and Q. All three forms are exactly equivalent and derive from the same linear relationship.
...
The relationship between [[carga_almacenada]] and [[diferencia_de_potencial]] is linear (with [[capacidad_electrica]] as the slope), but the consequences of that linearity are profound. In the [[carga_almacenada]] vs [[diferencia_de_potencial]] graph, a capacitor with higher [[capacidad_electrica]] produces a steeper line. The **stored energy** [[energia_almacenada_en_el_condensador]] corresponds to the area of the triangle below the line, between zero voltage and the value of interest. This geometric interpretation visually explains why the 1/2 factor appears and why energy grows quadratically rather than linearly with voltage.
...
**Frequency dependence.** The relations among [[capacidad_electrica]], [[carga_almacenada]], and [[diferencia_de_potencial]] are valid in static or quasi-static regime. In AC, the capacitor has a **reactive impedance** inversely proportional to frequency and [[capacidad_electrica]]. At very high frequencies, parasitic inductance dominates and the device no longer behaves as a pure capacitance.

## Deep physical interpretation

The energy stored in a capacitor **resides in the electric field**, not in the conductors. This is a profound result of electromagnetic field theory: energy is distributed in the space between the plates with a volumetric density proportional to [[epsilon]] and the square of the electric field. Integrating that density over the volume between the plates reproduces exactly the [[energia_almacenada_en_el_condensador]] formula with the 1/2 factor.
...
To compare with everyday energies: a 1000 µF capacitor at 230 V stores approximately 26.5 J (applying the [[energia_almacenada_en_el_condensador]] formula with the 1/2 factor), energy equivalent to lifting an apple (100 g) about 27 meters. A 1.5 V AA battery with 2500 mAh stores 13,500 J — 500 times more — illustrating the energy density difference between capacitors and batteries.

A physically absurd result would be obtaining a capacitance of 10 F for a capacitor with 1 cm² plates separated by 1 mm with air. Applying the [[capacidad_geometrica_del_condensador_plano]] formula with ε₀, 1 cm², and 1 mm gives approximately 0.9 pF. Any calculation giving farads for that geometry with air implies a unit error of 10¹³ or more.

## Personalized resolution method

To solve any capacitor problem:

1. **Identify known quantities**: determine which of [[capacidad_electrica]], [[carga_almacenada]], [[diferencia_de_potencial]], and [[energia_almacenada_en_el_condensador]] are given and which is sought.
2. **Select the appropriate relation**: if [[capacidad_electrica]] is sought from geometry, use the parallel-plate formula with [[epsilon]], area, and separation. If energy or charge is sought, use the [[energia_almacenada_en_el_condensador]] formula and the relation between [[carga_almacenada]], [[capacidad_electrica]], and [[diferencia_de_potencial]].
...
5. **Verify consistency**: confirm that the product [[capacidad_electrica]]·[[diferencia_de_potencial]] equals [[carga_almacenada]], and that the [[energia_almacenada_en_el_condensador]] formula is consistent whether using [[capacidad_electrica]] and [[diferencia_de_potencial]] or [[carga_almacenada]] and [[diferencia_de_potencial]].
...
**Capacitor with inserted dielectric.** If a dielectric with εᵣ = 5 is inserted between the plates of a capacitor previously charged to 100 V and then the source is disconnected, charge [[carga_almacenada]] remains constant (it cannot escape). Then capacitance [[capacidad_geometrica_del_condensador_plano]] increases 5 times and [[diferencia_de_potencial]] drops 5 times (from 100 V to 20 V), since [[carga_almacenada]] is fixed and [[capacidad_electrica]] quintupled. [[energia_almacenada_en_el_condensador]] also drops 5 times: the energy difference was absorbed as the mechanical work of inserting the dielectric (the capacitor pulled the dielectric inward). If instead the source remained connected, [[diferencia_de_potencial]] stays fixed, [[carga_almacenada]] increases 5 times and energy increases 5 times — the source supplied additional energy.

**Dielectric breakdown.** Reducing d between plates increases [[capacidad_geometrica_del_condensador_plano]] (the formula shows [[capacidad_geometrica_del_condensador_plano]] grows as d decreases). But d also determines the maximum admissible voltage: reducing d from 10 µm to 1 µm increases [[capacidad_geometrica_del_condensador_plano]] ten times but reduces maximum voltage from 100 V to 10 V. The maximum storable [[energia_almacenada_en_el_condensador]] grows with d, so there is a fundamental trade-off between high capacitance (small d) and high working voltage (large d).
...
No. [[capacidad_electrica]] is a geometric and material property, independent of voltage. What changes is the stored [[carga_almacenada]] and [[energia_almacenada_en_el_condensador]]; [[capacidad_electrica]] itself does not change (except in nonlinear dielectrics).

**Why do capacitors in series have lower capacitance than either one alone?**
In series they share the same [[carga_almacenada]] but voltage is distributed between them. Equivalent capacitance measures charge per total volt; distributing voltage reduces the charge accumulated for a given total voltage below what either capacitor alone would store.
...
The capacitor is the culmination of electrostatics study in this block. It depends directly on the concepts of electric field and potential difference studied in the previous electrostatics leaves; without understanding them, the relation between [[capacidad_electrica]], [[carga_almacenada]], and [[diferencia_de_potencial]] is only arithmetic without physical substance.
...
The capacitor stores energy in the electric field between its plates. Its **capacitance** [[capacidad_electrica]] is the central property: fixed for a given geometry and material, it determines how much charge [[carga_almacenada]] and energy [[energia_almacenada_en_el_condensador]] the device stores for each applied voltage [[diferencia_de_potencial]]. Energy grows with the square of voltage, making [[diferencia_de_potencial]] the most critical design parameter. The **geometry** of the parallel-plate capacitor links [[capacidad_geometrica_del_condensador_plano]] with [[epsilon]], area, and separation.