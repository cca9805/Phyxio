...
## 1. Film capacitors in consumer electronics
...
In a power supply, the filtering capacitor converts a pulsating rectified signal into stable DC voltage. The required capacitance determines the residual ripple. Using a dielectric with [[epsilon_r]] three times higher allows the capacitor volume to be reduced to one-third at equal capacitance [[capacitancia_con_dielectrico]]. However, the higher dielectric loss of polyester at high frequency raises thermal dissipation and can reduce the component's service life.
...
Validity limit: the LHI model is valid as long as the operating frequency is below the relaxation frequency of the polymer and the field [[campo_electrico_en_el_dielectrico]] does not exceed 20 % of the dielectric strength of the material.

## 2. Insulation of high-voltage cables

High-voltage transmission cables (100–500 kV) are surrounded by a layer of cross-linked polyethylene (XLPE) with [[epsilon_r]] ≈ 2.3. The primary objective is not to maximise capacitance but to electrically insulate the conductor and control the distribution of the field [[campo_electrico_en_el_dielectrico]] between the conductor and the outer screen.

A low [[epsilon_r]] keeps [[campo_electrico_en_el_dielectrico]] relatively uniform radially and reduces dielectric losses under alternating current. The dielectric acts here as a shield: it reduces the field in the region closest to the conductor, where the field is most intense in cylindrical geometry. If [[epsilon_r]] were higher, the cable capacitance would increase and with it the reactive charging current, which can exceed the cable's thermal capacity in long lines.

Dominant variable: [[epsilon_r]] of the insulator, which determines the capacitance per unit length of the cable and the radial [[campo_electrico_en_el_dielectrico]] field profile.
Validity limit: the model is valid as long as [[campo_electrico_en_el_dielectrico]] at the highest-field point (conductor surface) is below the dielectric strength of XLPE, approximately 20 MV/m.

## 3. Capacitive sensors for humidity and composition

Capacitive humidity sensors use a hygroscopic material as the dielectric. As the material absorbs moisture from the environment, the composite [[epsilon_r]] increases because liquid water has [[epsilon_r]] ≈ 80. The change in [[capacitancia_con_dielectrico]] is directly proportional to the change in [[epsilon_r]] of the composite, and measuring [[capacitancia_con_dielectrico]] allows the moisture content to be inferred.

In industrial control applications, the same principle is used to detect the level of a liquid in a tank: a partially submerged capacitor has an effective [[epsilon_r]] intermediate between that of the liquid and that of air, and [[capacitancia_con_dielectrico]] varies linearly with the level. Calibration requires knowing [[capacitancia_en_vacio]] with the tank empty and [[capacitancia_con_dielectrico]] with the tank full to determine the dynamic range of the sensor.

Dominant variable: the variation in [[epsilon_r]] of the active dielectric (hygroscopic composite or liquid-air mixture), which modifies [[capacitancia_con_dielectrico]] proportionally and measurably.
