...
Check 1: In Field Lines, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
If two lines crossed at a point, the field [[campo_electrico]] would have two distinct directions at that point simultaneously. The uniqueness of the electric field (guaranteed by Poisson's equation with well-defined boundary conditions) prevents this from occurring. At the neutral point between two equal same-sign charges, the field is zero, but no lines pass through that point; lines curve to avoid it.
...
For two equal positive charges, the midpoint has [[campo_electrico]] equal to zero. If the student draws lines crossing there, they can verify by computing the field vectorially at that exact point (sum of two equal opposite vectors) the result is zero: there is no vector tangent to those crossed lines, because there is no vector at that point.

## Model errors

**Why it seems correct**

Check 6: In Field Lines, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 6.
...
Before drawing, establish the scale: for example, 4 lines per microcoulomb. A 2 μC charge has 8 lines, a 1 μC charge has 4 lines, and a −3 μC charge has 12 lines entering it. This consistency ensures that line density correctly reflects the magnitude of [[campo_electrico]] in each region.
...
Check 11: In Field Lines, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 11.
...
### Error 4: Computing flux [[flujo_electrico]] using only the field magnitude without the cosine factor

**Por qué parece correcto**

The student remembers that flux has to do with "how much field passes" and directly substitutes [[campo_electrico]] for [[flujo_electrico]] by multiplying field by area, without considering the relative orientation between [[campo_electrico]] and the surface.

**Por qué es incorrecto**

Flux [[flujo_electrico]] depends on how many lines cross the surface, not on how many are in the region. If the field is parallel to the surface, no lines cross it and the flux is zero even if the field is strong. The [[angulo_campo]] between [[campo_electrico]] and the outward normal is essential: only the component perpendicular to the plane contributes to the flux.
...
Flux is the product of the field by the area of the perpendicular projection of the surface onto the wave front (equivalently, area multiplied by the cosine of the angle between [[campo_electrico]] and the normal). Always ask: "how many field lines cross this surface?" The answer is zero if the field is tangent to it.

**Mini-ejemplo de contraste**

A vertical uniform field of magnitude [[campo_electrico]] points upward. A horizontal surface of area A has flux [[flujo_electrico]] equal to [[campo_electrico]] times A (all lines cross it). A vertical surface of the same area has zero flux (no lines cross it). If the student gets the same value for both, their calculation ignores the angle and is incorrect.

## Interpretation errors

**Why it seems correct**

Check 16: In Field Lines, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 16.
...
Field lines give the field direction only at points belonging to them. At a point between two lines, the actual direction of [[campo_electrico]] is a vector interpolation between the tangents to neighbouring lines, weighted by distance. Discrete lines are a sample of the continuous field, not the field itself.
...
Check 21: In Field Lines, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 21.
