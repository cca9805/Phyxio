...
Para dos cargas positivas iguales, el punto medio tiene [[campo_electrico]] nulo. Si el alumno dibuja líneas que se cruzan allí, puede comprobar que calculando el campo vectorialmente en ese punto exacto (suma de dos vectores opuestos e iguales) el resultado es cero: no hay ningún vector que sea tangente a esas líneas cruzadas, porque no hay vector en ese punto.
...
Antes de dibujar, establecer la escala: por ejemplo, 4 líneas por cada microcoulombio. Una carga de 2 μC tendrá 8 líneas, una de 1 μC tendrá 4 y una de −3 μC tendrá 12 líneas que entran. Esta coherencia garantiza que la densidad de líneas refleje correctamente el módulo de [[campo_electrico]] en cada zona.
...
### Error 4: Calcular el flujo [[flujo_electrico]] usando solo el módulo del campo sin el factor coseno

**Por qué parece correcto**

El alumno recuerda que el flujo tiene que ver con "cuánto campo pasa" y sustituye directamente [[campo_electrico]] por [[flujo_electrico]] multiplicando campo por área, sin considerar la orientación relativa entre [[campo_electrico]] y la superficie.

**Por qué es incorrecto**

El flujo [[flujo_electrico]] depende de cuántas líneas atraviesan la superficie, no de cuántas hay en la zona. Si el campo es paralelo a la superficie, ninguna línea la atraviesa y el flujo es nulo aunque el campo sea intenso. El factor [[angulo_campo]] entre [[campo_electrico]] y la normal exterior es imprescindible: solo la componente perpendicular al plano contribuye al flujo.
...
El flujo es el producto del campo por el área de la proyección perpendicular de la superficie sobre el frente de onda (o equivalentemente, el área multiplicada por el coseno del ángulo entre [[campo_electrico]] y la normal). Siempre preguntar: "¿cuántas líneas de campo cruzan esta superficie?" La respuesta es cero si el campo es tangente a ella.

**Mini-ejemplo de contraste**

Un campo uniforme vertical de módulo [[campo_electrico]] apunta hacia arriba. Una superficie horizontal de área A tiene flujo [[flujo_electrico]] igual a [[campo_electrico]] por A (todas las líneas la atraviesan). Una superficie vertical del mismo área tiene flujo nulo (ninguna línea la atraviesa). Si el alumno obtiene el mismo valor para ambas, su cálculo ignora el ángulo y es incorrecto.
...
Las líneas de campo dan la dirección del campo solo en los puntos que pertenecen a ellas. En un punto entre dos líneas, la dirección real de [[campo_electrico]] es una interpolación vectorial entre las tangentes a las líneas vecinas, ponderada por la distancia. Las líneas discretas son una muestra del campo continuo, no el campo mismo.
