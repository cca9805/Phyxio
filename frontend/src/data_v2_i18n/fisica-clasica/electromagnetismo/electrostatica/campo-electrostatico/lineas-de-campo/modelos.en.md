...
The electrostatic field line model describes the field [[campo_electrico]] through continuous oriented curves in a homogeneous isotropic space, under the assumption that the source charges are static and the space between them is vacuum or a uniform linear medium. In this model, the line density [[rho_lineas]] is strictly proportional to the magnitude of [[campo_electrico]] and lines never branch or intersect at any regular point in space.
...
- **Homogeneous linear medium**: the permittivity of the space is constant. If the medium is inhomogeneous, lines refract at interfaces (their density changes when crossing boundaries between different media) and the simple model of density proportional to [[campo_electrico]] in vacuum no longer applies.
...
- **Uniqueness of field at each point**: at every point in space (except at the source charge position), [[campo_electrico]] has a unique value. This uniqueness guarantees that lines do not intersect.
- **Discrete representation as an approximation of a continuous field**: the real field [[campo_electrico]] is continuous; discrete lines are a visual approximation. Local density is interpreted in a statistical sense.
...
- **Field [[campo_electrico]] that does not coincide with the tangent to the drawn lines**: if the analytically computed direction of [[campo_electrico]] at a point is not tangent to the drawn line, the line is incorrectly drawn.
- **Line density that does not reflect the known variation of [[campo_electrico]]**: in a region where [[campo_electrico]] analytically decays as 1/r², if lines do not separate in that proportion, the diagram is incorrect.
...
The most advanced model is that of **flux lines in electrodynamics**: in that regime the lines of [[campo_electrico]] can close upon themselves (rotational field induced by varying magnetic field) and must be considered together with the magnetic field lines to obtain a complete picture of the electromagnetic wave.
