...
Flux is a property of the surface-source system, not of the local field. The shape of the Gaussian surface is a computational tool, not a physical parameter. Changing the surface without crossing charges does not change [[flujo_electrico]].
...
Check 1: In Gauss's Law, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.


### Error 2: Including external charges in [[carga_encerrada]]
...
The student adds an exterior charge when computing [[carga_encerrada]] and obtains a flux that does not correspond to the physical situation. Comparing with Coulomb's law for a simple geometry, the results do not match.

**Conceptual correction**

Only charges whose position lies strictly inside the Gaussian surface belong to [[carga_encerrada]]. Charges on the surface itself present an ambiguity resolved by convention (usually not included). Exterior charges never contribute to the net flux under any circumstances.

**Contrast mini-example**

There are two charges: 2 μC inside the surface and 5 μC outside. If the student computes [[carga_encerrada]] as 7 μC, they obtain a flux of 7.9×10⁵ N·m²·C⁻¹. The correct value, with only the interior charge, is 2.26×10⁵ N·m²·C⁻¹. The difference is a factor of 3.5, a serious error that cannot be attributed to numerical imprecision.
...
Gauss's law is always valid, leading the student to think it can always be used to compute the field directly. After learning to obtain [[campo_electrico_gaussiano]] with spherical symmetry, they generalise the procedure to any geometry.
...
When applying the formula that divides flux by area to obtain [[campo_electrico_gaussiano]], the student uses the total surface area, including parts where the field is parallel to the surface (zero flux contribution). For a cylinder, this means including the end caps even when the field is tangential there.

**Why it is incorrect**

Only the part of the surface where the field is perpendicular contributes to the flux. If the field is parallel to a portion of the surface, the dot product of the field with the normal vector is zero and that portion contributes nothing to the flux. Including that area in the denominator gives an incorrect [[campo_electrico_gaussiano]], systematically smaller than the true value.
...
When designing the Gaussian surface, deliberately ensure that parts where the field is not perpendicular are not part of the active area [[area_de_la_superficie_gaussiana]] calculation. Only the area where the field is perpendicular enters the denominator of the field expression.

**Contrast mini-example**

A long wire with linear charge density: the cylindrical Gaussian surface of length L has lateral area 2πrL and two end caps of area πr² each. Since the field is radial and perpendicular only to the lateral surface, using the total area gives an incorrect [[campo_electrico_gaussiano]]. Using only 2πrL gives the correct result that matches the integrated Coulomb result.
...
Zero flux only indicates that [[carga_encerrada]] is zero: the positive and negative contributions of the field to the flux cancel each other. The field may be non-zero at every point on the surface, as long as the inward and outward contributions balance. A clear example: an electric dipole inside the surface produces zero flux because [[carga_encerrada]] is zero, but the field is intense near the charges.
...
Zero flux implies zero [[carga_encerrada]], not zero field. The field on the surface depends on all charges—interior and exterior. Only the net flux, which integrates the field's contribution over the entire surface, is zero when [[carga_encerrada]] is zero.
...
1. **Interior charges only**: confirm that [[carga_encerrada]] includes only the charges inside the Gaussian surface, with their correct signs.
...
3. **Sign consistency**: the sign of [[flujo_electrico]] must match the sign of [[carga_encerrada]]. Positive flux with negative charge is impossible; if it occurs, there is a sign error in the charges or in the orientation of the normal vector.
