...
Electric flux [[flujo_electrico]] quantifies that net number of lines. If [[carga_encerrada]] is positive, [[flujo_electrico]] is positive: the field exits. If [[carga_encerrada]] is negative, [[flujo_electrico]] is negative: the field enters. If [[carga_encerrada]] is zero, there is no net flux even though the field is not necessarily zero at every point on the surface.
...
where [[flujo_electrico]] is the total electric flux through the closed surface, [[carga_encerrada]] is the net electric charge enclosed inside it, and [[epsilon_0]] is the permittivity of free space, with a value of approximately 8.854×10⁻¹² C²·N⁻¹·m⁻².
...
where [[campo_electrico_gaussiano]] is the field magnitude, [[area_de_la_superficie_gaussiana]] is the total area of the Gaussian surface, [[carga_encerrada]] is the enclosed charge, and [[epsilon_0]] is the permittivity of free space.
...
3. **Compute [[carga_encerrada]]**: sum algebraically all charges—with their signs—that lie inside the chosen surface. External charges do not contribute.
4. **Apply Gauss's law**: compute the flux and, if symmetry allows the field to be factored out, divide by the area [[area_de_la_superficie_gaussiana]] to obtain [[campo_electrico_gaussiano]].
5. **Check the direction**: the direction of the field is given by the sign of [[carga_encerrada]] and the geometry of the chosen surface.

## Special cases and extended example

**Conductor in electrostatic equilibrium**: inside a conductor, any interior field would drive charge movement until it cancelled itself. Gauss's law over a Gaussian surface fully embedded in the conductor gives zero [[campo_electrico_gaussiano]] and zero [[carga_encerrada]]: all free charges redistribute on the surface of the conductor, not in its interior.

**Zero net enclosed charge**: if there are two equal and opposite charges inside the surface, [[carga_encerrada]] is zero and [[flujo_electrico]] is zero. This does not mean the field is zero at every point on the surface—there is field, but the inward and outward contributions cancel globally.

**Uniformly charged sphere**: for a sphere of radius R with total charge [[carga_encerrada]] distributed uniformly, Gauss's law gives an exterior field that falls off as the inverse square of the distance, identical to that of a point charge at the centre. Inside the sphere, only the enclosed charge contributes to the field, which grows linearly with the distance from the centre.

## Real student questions

**Why does the shape of the Gaussian surface not matter if the field varies from point to point on it?**

Because flux is an integral that sums all contributions. Even if the field is strong at one point and weak at another, the global sum depends only on the interior sources. The field varies from point to point, but the integral over the complete surface always gives the same value for the same [[carga_encerrada]].
...
The most direct check is to verify that [[carga_encerrada]] includes only interior charges, that [[area_de_la_superficie_gaussiana]] corresponds to the active part of the surface, and that the sign of the flux is consistent with the sign of [[carga_encerrada]].

## Cross-cutting connections and study paths

Gauss's law is a gateway to electric potential: once [[campo_electrico_gaussiano]] has been computed, it can be integrated to obtain the potential, a path especially efficient for symmetric geometries. Within electrostatics, it is the fundamental tool for capacitors and charged conductors; the capacitors leaf uses the Gauss's law result for the field between parallel plates directly.
...
Gauss's law links the cause—the enclosed charge [[carga_encerrada]]—to the collective effect—the electric flux [[flujo_electrico]]—through the fundamental constant [[epsilon_0]]. Its operational power lies in the independence of flux from the geometry of the Gaussian surface, which allows the most convenient surface to be chosen for each problem. For symmetric geometries, this choice turns a complex integration problem into a straightforward algebraic computation of the field [[campo_electrico_gaussiano]].