...
El flujo eléctrico [[flujo_electrico]] cuantifica ese número de líneas neto. Si [[carga_encerrada]] es positiva, [[flujo_electrico]] es positivo: el campo sale. Si [[carga_encerrada]] es negativa, [[flujo_electrico]] es negativo: el campo entra. Si [[carga_encerrada]] es cero, no hay flujo neto aunque el campo no sea necesariamente cero en cada punto de la superficie.
...
donde [[flujo_electrico]] es el flujo eléctrico total a través de la superficie cerrada, [[carga_encerrada]] es la carga eléctrica neta encerrada en su interior, y [[epsilon_0]] es la permitividad del vacío, cuyo valor es aproximadamente 8.854×10⁻¹² C²·N⁻¹·m⁻².
...
donde [[campo_electrico_gaussiano]] es el módulo del campo, [[area_de_la_superficie_gaussiana]] es el área total de la superficie gaussiana, [[carga_encerrada]] es la carga encerrada y [[epsilon_0]] es la permitividad del vacío.
...
3. **Calcular [[carga_encerrada]]**: sumar algebraicamente todas las cargas —con sus signos— que están dentro de la superficie elegida. Las cargas exteriores no contribuyen.
4. **Aplicar la ley de Gauss**: calcular el flujo y, si la simetría permite extraer el campo, dividir entre el área [[area_de_la_superficie_gaussiana]] para obtener [[campo_electrico_gaussiano]].
5. **Verificar el sentido**: la dirección del campo viene dada por el signo de [[carga_encerrada]] y la geometría de la superficie elegida.

## Casos especiales y ejemplo extendido

**Conductor en equilibrio electrostático**: Dentro de un conductor, cualquier campo interior causaría movimiento de carga hasta anularse. La ley de Gauss sobre una superficie gaussiana completamente inmersa en el conductor da [[campo_electrico_gaussiano]] nulo y [[carga_encerrada]] nulo: todas las cargas libres se redistribuyen en la superficie del conductor, no en su interior.

**Carga neta cero encerrada**: Si hay dos cargas iguales y opuestas dentro de la superficie, [[carga_encerrada]] es cero y [[flujo_electrico]] es cero. Esto no significa que el campo sea cero en cada punto de la superficie —hay campo, pero las contribuciones de entrada y salida se cancelan globalmente.

**Esfera uniformemente cargada**: Para una esfera de radio R con carga total [[carga_encerrada]] distribuida uniformemente, la ley de Gauss da un campo exterior que decrece como el inverso del cuadrado de la distancia, idéntico al que produciría una carga puntual en el centro. En el interior de la esfera, solo la carga encerrada contribuye al campo, que crece linealmente con la distancia al centro.

## Preguntas reales del alumno

**¿Por qué la forma de la superficie gaussiana no importa si el campo cambia de un punto a otro de ella?**

Porque el flujo es una integral que suma todas las contribuciones. Aunque en un punto el campo sea fuerte y en otro débil, la suma global solo depende de las fuentes interiores. El campo varía de punto a punto, pero la integral sobre la superficie completa da siempre el mismo valor para la misma [[carga_encerrada]].
...
La verificación más directa es comprobar que [[carga_encerrada]] solo incluye las cargas interiores, que [[area_de_la_superficie_gaussiana]] corresponde a la parte activa de la superficie, y que el signo del flujo es coherente con el signo de [[carga_encerrada]].

## Conexiones transversales y rutas de estudio

La ley de Gauss es puerta de entrada al potencial eléctrico: calculado [[campo_electrico_gaussiano]], se puede integrar para obtener el potencial, camino especialmente eficiente en geometrías simétricas. En la electrostática, es la herramienta fundamental para condensadores y conductores cargados: el leaf de condensadores usa directamente el campo entre placas derivado por Gauss.
...
La ley de Gauss vincula la causa —la carga encerrada [[carga_encerrada]]— con el efecto colectivo —el flujo eléctrico [[flujo_electrico]]— a través de la constante fundamental [[epsilon_0]]. Su potencia operativa reside en la independencia del flujo respecto a la geometría de la superficie gaussiana, lo que permite elegir la superficie más conveniente para cada problema. En geometrías simétricas, esta elección convierte un problema de integración complejo en un cálculo algebraico directo del campo [[campo_electrico_gaussiano]].