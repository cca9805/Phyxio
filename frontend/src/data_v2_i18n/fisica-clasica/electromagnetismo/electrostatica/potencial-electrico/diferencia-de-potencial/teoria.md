...
La diferencia de potencial [[diferencia_de_potencial]] entre dos puntos A y B mide exactamente esa "cuesta" por unidad de carga: cuánto trabajo realiza el campo al mover una carga positiva de A a B. Si [[diferencia_de_potencial]] es positiva, el campo impulsa a la carga de A a B (A está "arriba" en el paisaje energético). Si [[diferencia_de_potencial]] es negativa, el campo frena ese movimiento.
...
donde [[diferencia_de_potencial]] es la diferencia de potencial en voltios, [[trabajo_electrico]] es el trabajo realizado por la fuerza eléctrica en julios, y [[carga_de_prueba]] es la carga eléctrica en coulombios.

El voltio es la unidad del SI para la diferencia de potencial, equivalente a un julio por culombio (J·C⁻¹). Esta definición muestra que [[diferencia_de_potencial]] no depende de [[carga_de_prueba]]: es una propiedad del campo en esos dos puntos, no de la carga que se mueve.
...
donde [[campo_electrico_uniforme]] es el módulo del campo en N/C y [[distancia_entre_puntos]] es la proyección del desplazamiento A→B sobre la dirección del campo en metros.
...
> [!TIP]
> La diferencia de potencial tiene signo: invertir el orden de los subíndices cambia el signo, por lo que [[diferencia_de_potencial]] y [[diferencia_de_potencial_inversa]] son opuestos. Convención habitual: el primero indica el punto de llegada respecto al de partida.
...
La diferencia de potencial [[diferencia_de_potencial]] equivale a la diferencia de los potenciales absolutos en A y en B. El potencial absoluto requiere elegir un punto de referencia con potencial cero (usualmente el infinito o tierra), pero la diferencia de potencial es independiente de esa elección.
...
La relación entre [[campo_electrico_uniforme]] y [[diferencia_de_potencial]] en campo uniforme refleja esta idea: el campo es mayor cuando la misma diferencia de potencial está comprimida en una distancia [[distancia_entre_puntos]] menor. Este principio explica por qué los condensadores pequeños con la misma tensión tienen campos más intensos que los grandes, con consecuencias directas para la ruptura dieléctrica.

> [!WARNING]
> La fórmula del campo uniforme (proporcionalidad entre [[diferencia_de_potencial]], [[campo_electrico_uniforme]] y [[distancia_entre_puntos]]) solo es válida para campo uniforme. Cerca de una carga puntual, el campo no es uniforme y la diferencia de potencial requiere integración.
...
Para calcular [[diferencia_de_potencial]] o [[trabajo_electrico]] de forma sistemática:
...
3. **Elegir la fórmula apropiada**: campo uniforme → multiplicar [[campo_electrico_uniforme]] por [[distancia_entre_puntos]]; caso general → calcular trabajo o usar potenciales absolutos.
4. **Calcular [[distancia_entre_puntos]]**: si el campo es uniforme, proyectar el desplazamiento sobre la dirección del campo.
5. **Obtener [[diferencia_de_potencial]]** y luego el trabajo [[trabajo_electrico]] multiplicando [[carga_de_prueba]] por [[diferencia_de_potencial]] si se necesita.
6. **Verificar el signo**: ¿el campo facilita o dificulta el movimiento? El signo de [[diferencia_de_potencial]] debe ser coherente con la geometría.

## Casos especiales y ejemplo extendido

**Movimiento perpendicular al campo**: si la carga se mueve perpendicularmente al campo eléctrico, la proyección [[distancia_entre_puntos]] es cero y [[diferencia_de_potencial]] es nulo. La carga se mueve sobre una superficie equipotencial y el campo no hace trabajo. Este principio explica por qué las cargas en un conductor se distribuyen en la superficie: una redistribución que generara diferencias de potencial en el interior provocaría corrientes hasta anularse.

**Carga negativa en un campo**: si [[diferencia_de_potencial]] es positivo y [[carga_de_prueba]] es negativo, [[trabajo_electrico]] es negativo: el campo frena a la carga negativa cuando se mueve de A a B (en el sentido del campo). La carga negativa sería acelerada si se moviera de B a A, en sentido contrario al campo.
...
La diferencia de potencial [[diferencia_de_potencial]] cuantifica el trabajo del campo eléctrico por unidad de carga al desplazar una carga de A a B, siendo independiente de la trayectoria por el carácter conservativo del campo. En campo uniforme, [[diferencia_de_potencial]] es proporcional a [[campo_electrico_uniforme]] y a [[distancia_entre_puntos]], lo que convierte la geometría del problema en el factor clave. El trabajo sobre cualquier carga concreta se obtiene multiplicando [[diferencia_de_potencial]] por [[carga_de_prueba]], conectando la propiedad del campo con el efecto dinámico medible.