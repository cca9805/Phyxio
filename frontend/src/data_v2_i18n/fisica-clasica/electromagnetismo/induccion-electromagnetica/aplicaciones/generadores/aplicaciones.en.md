...
In a wind turbine, the wind supplies [[potencia_mecanica_de_entrada]] to the shaft and the generator transforms that input into [[potencia_electrica_util]]. The design seeks stable conversion even when wind speed varies. Induction physics appears inside the generator, but global efficiency also depends on blades, gearbox, power electronics, and control.
...
Dominant variable: the mechanical input power [[potencia_mecanica_de_entrada]] sets the energetic ceiling for useful electrical power.
...
The design variable is not only induced voltage. Current delivery, [[eta]], cooling, and magnetic-circuit losses matter. This is why a plant can handle huge [[potencia_electrica_util]] without requiring absurd fields: scale, turns, conductor section, and conversion quality all increase.
...
This case is pedagogically useful because it corrects the idea that induced emf is free. Lamp brightness depends on [[potencia_electrica_util]], not only on [[epsilon_0]]. At low speeds, induction may be insufficient for proper lighting; at high speeds, electronics limit voltage.
...
The dominant hypothesis is linearity: small displacements, stable field, and light coil. The physical interpretation differs from a power plant; the priority is not [[potencia_electrica_util]], but having the emf follow the motion variation without significant distortion.

This application shows that “generator” does not always mean power machine. The same physics can sense motion. The induced signal encodes speed and waveform of the vibration, so fidelity depends on keeping [[campo_magnetico]], geometry, and mechanical response stable.

Dominant variable: the flux change [[cambio_de_flujo_magnetico]] associated with diaphragm motion determines the induced signal.
