...
Energy comes from [[potencia_mecanica_de_entrada]]. Induction converts mechanical work into charge separation and then into [[potencia_electrica_util]] if a load is connected.
...
Check 1: In Generators, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
The student remembers that flux appears in Faraday's law and substitutes any [[flujo_magnetico_maximo_enlazado]] value without checking the interval. This is mechanical symbol substitution.

**Why it is incorrect**

Mean emf depends on [[cambio_de_flujo_magnetico]] during [[intervalo_temporal]], not on an isolated maximum value. Without flux change, mean emf vanishes.
...
Before closing a generator problem, check four questions: what changes in [[flujo_magnetico_maximo_enlazado]], over what time it changes, which electrical quantity is requested, and whether the answer is emf or power. Then verify that [[eta]] does not exceed unity and that [[potencia_electrica_util]] is not greater than [[potencia_mecanica_de_entrada]].
...
The student computes [[potencia_electrica_util]] by squaring the amplitude and multiplying by the load conductance without the one-half factor, obtaining twice the correct power.
