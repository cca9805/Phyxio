...
The ideal generator model uses a rigid coil with [[numero_de_espiras]] turns rotating at constant [[omega]] in a uniform [[campo_magnetico]] field. The effective geometry is summarized by [[area_de_la_espira]], and the main electrical result is a sinusoidal [[epsilon_0]]. The model preserves the link between mechanical work, changing [[flujo_magnetico_maximo_enlazado]], and induced emf, but it ignores internal resistance, friction, magnetic saturation, and heating.
...
- Approximately uniform magnetic field: if this fails, [[campo_magnetico]] no longer represents the field crossing the whole coil.
- Steady rotation: if [[omega]] fluctuates strongly, the signal no longer has a well-defined amplitude and frequency.
- Fixed effective area: if the coil deforms, [[area_de_la_espira]] changes and linked flux varies from turn to turn.
- Negligible losses in emf estimation: if heating or friction is significant, [[potencia_electrica_util]] falls below ideal output.
...
The ideal model is reasonable when speed variations are below 5 %, internal losses are below 10 % of [[potencia_mecanica_de_entrada]], and the magnetic field remains far from saturation. In educational coils, it is often usable with [[campo_magnetico]] between 0.01 T and 0.5 T, [[omega]] below 500 rad/s, and areas smaller than 0.1 m².
