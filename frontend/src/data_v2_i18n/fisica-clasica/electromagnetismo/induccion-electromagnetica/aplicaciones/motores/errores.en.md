...
Torque is force multiplied by the armature radius [[r_a]]. Once the Laplace force [[fuerza_de_laplace]] is calculated, torque is obtained by multiplying by the distance from the conductor to the axis. Only then does it make sense to compare it with the load torque.
...
Check 1: In Motors, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
At steady state, armature current is calculated by subtracting back-EMF [[epsilon_c]] from supply voltage and dividing by the internal winding resistance. [[epsilon_c]] increases with speed: at full speed, [[epsilon_c]] can account for 90 % of the supply voltage.
...
Physics formulas use SI units. Angular speed [[omega]] must be in radians per second. A value in rpm is numerically about one hundred times larger than the value in rad/s for typical speeds, producing errors of the same order in [[epsilon_c]] and [[potencia_mecanica]]. A speed of 1500 rpm equals 157 rad/s, not 1500 rad/s.
...
The student measures or is given the total winding length of the motor and enters that value as [[longitud_activa_del_conductor]] in the Laplace force formula.

**Why it is incorrect**

Only the part of the conductor inside the air-gap magnetic field contributes to the Laplace force. End windings — the sections connecting active conductors at the ends of the rotor — are outside the field and produce no useful force. Using total length instead of active length [[longitud_activa_del_conductor]] overestimates motor force and torque.
...
The active length [[longitud_activa_del_conductor]] is the stator axial length: the distance between the two edges of the magnetic core lamination stack. End windings extend beyond that zone and, although they carry the same current, are not in the field and generate no force.
