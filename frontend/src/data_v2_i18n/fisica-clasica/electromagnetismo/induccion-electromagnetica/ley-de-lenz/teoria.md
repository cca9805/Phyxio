...
Este leaf explica por que la induccion no se opone al campo magnetico por existir, sino al cambio de flujo enlazado. La magnitud central es [[epsilon_ind]], pero su lectura depende de [[cambio_de_flujo_magnetico]], [[rapidez_media_de_cambio_de_flujo]] y de la convencion de signo elegida.
...
La regla puede escribirse tambien como relacion conceptual entre el signo inducido [[s_ind]] y el signo del cambio de flujo [[s_flujo]]:

{{f:regla_signos_lenz}}

Estas relaciones dicen lo mismo desde niveles distintos. [[rapidez_media_de_cambio_de_flujo]] contiene el signo del cambio de flujo, mientras [[epsilon_ind]] aparece con signo contrario si [[numero_de_espiras]] es positivo. El signo no es un castigo algebraico: codifica la polaridad fisica de la fem inducida.
...
En una grafica de [[epsilon_ind]] frente a [[rapidez_media_de_cambio_de_flujo]], Lenz aparece como una pendiente negativa. Esa pendiente no representa disipacion; representa que la respuesta inducida cambia de signo cuando cambia el signo de la variacion de flujo.
...
Para aplicar la ley, primero se fija una superficie y una normal. Esa eleccion da signo a [[flujo_magnetico]] y a [[cambio_de_flujo_magnetico]]. Despues se decide si el flujo aumenta o disminuye. Solo entonces se asigna la polaridad inducida. Sin esa secuencia, frases como "se opone al campo" se vuelven ambiguas.
...
> [!TIP]
> Procedimiento seguro: fija normal, decide si [[flujo_magnetico]] aumenta o disminuye, invierte el signo para la respuesta inducida y despues traduce ese signo a corriente o polaridad.
...
Un resultado absurdo aparece si la polaridad inducida refuerza el cambio inicial, si [[epsilon_ind]] se predice sin [[cambio_de_flujo_magnetico]], o si [[intervalo_de_tiempo]] se usa en milisegundos sin convertir. La escala depende del modulo; el sentido depende del signo.

## Método de resolución personalizado

Primero dibuja la normal positiva y decide el signo de [[flujo_magnetico]]. Segundo compara el estado inicial y final para encontrar si [[cambio_de_flujo_magnetico]] aumenta o disminuye. Tercero aplica oposicion: la corriente o fem inducida debe producir un efecto contrario a ese cambio.

Despues calcula el modulo con [[intervalo_de_tiempo]] y [[numero_de_espiras]] si hace falta. Finalmente traduce el signo a una frase fisica: la espira intenta sostener el flujo, reducir su aumento o resistir el movimiento que lo cambia.
...
La ley de Lenz afirma que la induccion se orienta contra el cambio de flujo que la causa. [[epsilon_ind]] tiene sentido fisico solo si se conserva el signo de [[cambio_de_flujo_magnetico]] o [[rapidez_media_de_cambio_de_flujo]] y se interpreta la polaridad inducida.