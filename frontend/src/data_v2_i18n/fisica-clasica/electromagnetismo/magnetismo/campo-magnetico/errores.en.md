...
The student knows that the magnetic field [[campo_magnetico]] exists everywhere in space and exerts forces. The incorrect generalization is that those forces act on any charge, moving or at rest, just like the electric field.

**Why it is incorrect**

The magnetic Lorentz force is proportional to the velocity [[velocidad_de_la_carga]] of the charge. If [[velocidad_de_la_carga]] is zero, the force is exactly zero regardless of how large [[campo_magnetico]] is. The magnetic field does not act on static charges; that is the fundamental difference from the electric field.
...
The necessary condition for a magnetic force to exist is that the charge has nonzero velocity with a component perpendicular to [[campo_magnetico]]. If the charge is at rest or moves parallel to [[campo_magnetico]], the force is zero.
...
Check 1: In Magnetic Field, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
For the straight wire: doubling [[distancia_al_hilo_conductor]] halves [[campo_magnetico]]. For the point-charge electric field: doubling r quarters the field. The difference originates in the geometry of the source distribution: point (spherical) versus linear (cylindrical).
...
Before applying the straight-wire formula, verify that the conductor length is at least ten times larger than the evaluation distance [[distancia_al_hilo_conductor]]. For conductors with length comparable to [[distancia_al_hilo_conductor]], use the Biot-Savart law.
...
The formula structure has several numerical factors: [[mu_0]], the current [[intensidad_de_corriente]], and the distance [[distancia_al_hilo_conductor]]. The 2π factor in the denominator is easy to omit if the student memorizes the formula incompletely.
...
Kinetic energy depends on the square of the **speed** (velocity magnitude), not its direction. The magnetic force [[fuerza_magnetica]] is always perpendicular to [[velocidad_de_la_carga]], so it does not change the magnitude of [[velocidad_de_la_carga]]. Speed stays constant; only direction changes. The net work of [[fuerza_magnetica]] along any trajectory is always zero.
...
A proton entering a uniform [[campo_magnetico]] field at 10⁶ m/s traces a perfect circle and exits with exactly the same speed of 10⁶ m/s. Its kinetic energy before and after the field is identical. If the field had changed its energy, it would be a perpetual motion machine, violating energy conservation.
...
4. Is the magnitude of [[campo_magnetico]] positive?
