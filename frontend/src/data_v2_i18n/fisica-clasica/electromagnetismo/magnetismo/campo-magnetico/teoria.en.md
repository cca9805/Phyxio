...
This formula has three immediate implications: the force is maximum when [[velocidad_de_la_carga]] is perpendicular to [[campo_magnetico]], zero when [[velocidad_de_la_carga]] is parallel or antiparallel to [[campo_magnetico]], and always perpendicular to the plane formed by [[velocidad_de_la_carga]] and [[campo_magnetico]]. It also explains why a magnetic field can bend a trajectory without increasing the kinetic energy of the particle.
...
The two relations play different roles. The Lorentz relation defines how a test charge detects the field, while the straight-wire relation gives one concrete source model for producing [[campo_magnetico]]. Keeping those roles separate prevents a common error: using the force law as if it created the field, instead of using it to measure the effect of a field already present.

This formal split also fixes the order of work: first identify the source current and geometry, then evaluate [[campo_magnetico]], and only afterwards infer the magnetic force on the moving charge.
