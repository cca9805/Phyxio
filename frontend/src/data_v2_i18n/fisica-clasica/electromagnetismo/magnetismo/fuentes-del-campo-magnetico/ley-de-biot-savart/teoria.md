...
La idea esencial es **local y vectorial**. Importa la corriente, importa el elemento orientado [[dl]], importa la distancia [[distancia_al_punto_de_observacion]] al punto y tambien importa la orientacion del fragmento. Un elemento casi alineado con el punto aporta poco; uno casi perpendicular aporta mucho mas.
...
El resultado buscado es el campo total, pero el camino natural empieza por [[contribucion_diferencial_de_campo]]. Esa diferencia evita un error comun: creer que una sola parte del conductor ya representa todo el campo.
...
Aqui [[contribucion_diferencial_de_campo]] crece con [[corriente_electrica]], con [[dl]] y con [[s_theta]], pero disminuye con [[distancia_al_punto_de_observacion]]. La constante [[mu0]] fija el acoplamiento magnetico del modelo de vacio. La direccion no se lee solo en el modulo; se obtiene con la regla de la mano derecha aplicada al elemento de corriente y a la direccion hacia el punto.

Cuando la geometria es un hilo recto muy largo, la integracion de todas las contribuciones da una relacion simple para el campo a distancia perpendicular [[distancia_al_punto_de_observacion]]:
...
La lectura formal exige no confundir variable de integracion con resultado final. [[dl]] pertenece al conductor, [[distancia_al_punto_de_observacion]] une ese elemento con el punto de observacion, y [[campo_magnetico]] solo aparece despues de sumar las contribuciones coherentes. Si esa separacion se pierde, una formula integrada se usa fuera de su geometria.
...
2. Dibuja el elemento [[dl]] y el punto donde quieres calcular [[campo_magnetico]].
3. Decide la distancia [[distancia_al_punto_de_observacion]] y el factor angular [[s_theta]] para cada tipo de elemento.
4. Usa simetria para saber que componentes de [[contribucion_diferencial_de_campo]] se cancelan y cuales se suman.
...
**¿Puedo sumar solo modulos de [[contribucion_diferencial_de_campo]]?** Solo si ya demostraste que todas las contribuciones apuntan en la misma direccion. Si no, debes sumar vectores.
...
Este leaf conecta hacia atras con [Campo magnetico](leaf:fisica-clasica/electromagnetismo/magnetismo/campo-magnetico), donde se define que representa [[campo_magnetico]]. Tambien prepara [Ley de Ampere](leaf:fisica-clasica/electromagnetismo/magnetismo/fuentes-del-campo-magnetico/ley-de-ampere), que resuelve casos muy simetricos desde una lectura global.
...
La ley de Biot Savart calcula [[campo_magnetico]] sumando contribuciones [[contribucion_diferencial_de_campo]] producidas por elementos de corriente. Su fuerza esta en conectar corriente, distancia, orientacion y geometria; su dificultad esta en respetar la suma vectorial y el dominio magnetostatico.