...
In a motor, coil conductors carry [[intensidad_de_corriente]] inside a [[campo_magnetico]] created by magnets or electromagnets. Forces on opposite sides of the coil can form a torque that produces rotation.

Dominant variable: [[fuerza_magnetica_sobre_el_conductor]] as total mechanical force on each active segment.
...
Real design also controls how much active length remains inside the field. More coil turns increase the number of contributing segments, but also raise resistance and heating. Useful force comes from balancing [[intensidad_de_corriente]], [[longitud_efectiva]], [[campo_magnetico]], and cooling.

This leaf separates design choices. Increasing [[campo_magnetico]] requires stronger magnets or cores, increasing [[intensidad_de_corriente]] requires supply and dissipation capacity, and increasing [[longitud_efectiva]] requires more copper inside the useful region. An efficient motor optimizes the full set rather than one isolated variable.

## 2. Dynamic loudspeakers

A loudspeaker uses a coil placed in the gap of a permanent magnet. The electrical signal changes [[intensidad_de_corriente]], and magnetic force moves the diaphragm forward or backward.

Dominant variable: [[intensidad_de_corriente]] as time control of force.
Validity limit: at large amplitudes, the coil may leave the uniform-field region.

The force must follow the signal without too much distortion. Therefore [[campo_magnetico]] must be stable, effective coil length must be known, and heating must not alter the current.

The reading of [[fuerza_magnetica_por_unidad_de_longitud]] is useful for comparing magnets and coils before fixing the number of turns. If two designs give the same force per metre, the one with greater active length pushes more, but it may be heavier or dissipate more heat.
...
Dominant variable: [[longitud_efectiva]] as calibrated effective segment.
Validity limit: the conductor must remain straight with stable orientation.

This application is pedagogical because it converts an invisible electrical magnitude into a weight or displacement reading. If force does not change linearly with [[intensidad_de_corriente]], the setup reveals alignment or uniformity failure.

It can also calibrate fields. If [[intensidad_de_corriente]] and [[longitud_efectiva]] are known, measuring force helps estimate [[campo_magnetico]]. Orientation must be locked, because a small rotation changes [[s_theta]] and alters the reading even if the field has not changed.
...
Dominant variable: [[fuerza_magnetica_por_unidad_de_longitud]] as available force per active metre.
...
Force per length helps estimate stresses before building the full device. If [[fuerza_magnetica_por_unidad_de_longitud]] exceeds what the support can tolerate, the issue is not solved only by reducing mass; field, current, or travel must be redesigned.
...
Dominant variable: [[campo_magnetico]] as intensity of the magnetic environment.
