...
El sistema es el tramo de conductor dentro de la region magnetica. La corriente [[intensidad_de_corriente]] atraviesa la longitud efectiva [[longitud_efectiva]], mientras el campo [[campo_magnetico]] actua sobre las cargas moviles del metal. Las magnitudes buscadas son [[fuerza_magnetica_sobre_el_conductor]] y [[fuerza_magnetica_por_unidad_de_longitud]].
...
La resolucion usa [[fuerza_magnetica_sobre_el_conductor]], [[intensidad_de_corriente]], [[longitud_efectiva]], [[campo_magnetico]], [[s_theta]] y [[fuerza_magnetica_por_unidad_de_longitud]]. El sentido de la fuerza no se obtiene del signo numerico, sino de la regla de la mano derecha aplicada a la corriente convencional y al campo.

## Sustitución numérica

Para [[fuerza_magnetica_sobre_el_conductor]], multiplicar 4.0 A por 0.30 m, por 0.50 T y por el factor angular maximo da aproximadamente 0.60 N. Por tanto, el tramo recibe una fuerza lateral de escala claramente medible en laboratorio.

Para [[fuerza_magnetica_por_unidad_de_longitud]], multiplicar 4.0 A por 0.50 T y por el factor angular maximo da aproximadamente 2.0 N*m^-1. Esa lectura significa que cada metro efectivo de conductor soportaria dos newtons de fuerza magnetica en las mismas condiciones.
...
La diferencia dimensional confirma que [[fuerza_magnetica_sobre_el_conductor]] y [[fuerza_magnetica_por_unidad_de_longitud]] no son la misma magnitud. Una tiene unidad de fuerza total y la otra describe distribucion lineal.
...
Si se duplicara [[intensidad_de_corriente]], tanto [[fuerza_magnetica_sobre_el_conductor]] como [[fuerza_magnetica_por_unidad_de_longitud]] aumentarian en la misma proporcion. Si se duplicara [[longitud_efectiva]], solo aumentaria [[fuerza_magnetica_sobre_el_conductor]], porque [[fuerza_magnetica_por_unidad_de_longitud]] ya esta normalizada por longitud. Esta distincion permite separar intensidad magnetica local y tamano del tramo activo.
...
Tambien se observa que la longitud efectiva no es un detalle administrativo. Si el mismo cable tuviera solo 0.10 m dentro del campo, la fuerza total seria tres veces menor, mientras [[fuerza_magnetica_por_unidad_de_longitud]] permaneceria igual. Esa comparacion separa con claridad lo que pertenece al entorno magnetico y lo que pertenece a la geometria del montaje.

Si el conductor estuviera inclinado, la fuerza no podria estimarse copiando el caso perpendicular. El factor [[s_theta]] reduciria el empuje porque solo la parte perpendicular del tramo respecto a [[campo_magnetico]] contribuye de forma eficaz. Por eso, antes de sustituir datos, conviene dibujar el conductor y el campo: el calculo numerico depende de esa lectura geometrica.

La diferencia entre [[fuerza_magnetica_sobre_el_conductor]] y [[fuerza_magnetica_por_unidad_de_longitud]] tambien ayuda a revisar el resultado experimental. Si una balanza mide una fuerza total, debe compararse con [[fuerza_magnetica_sobre_el_conductor]]. Si el informe busca una carga mecanica distribuida para diseñar soportes, la lectura relevante es [[fuerza_magnetica_por_unidad_de_longitud]]. Mezclar ambas magnitudes puede llevar a sobredimensionar o infradimensionar el montaje.
...
Si la corriente instantanea aumenta, [[fuerza_magnetica_sobre_el_conductor]] aumenta y el diafragma recibe mayor empuje. Si se aumenta la longitud efectiva de hilo dentro del entrehierro, tambien crece la fuerza total. La magnitud [[fuerza_magnetica_por_unidad_de_longitud]] permite comparar disenos sin depender todavia de cuantas vueltas tiene la bobina.
...
Si el altavoz calienta demasiado, la resistencia de la bobina cambia y la corriente real deja de coincidir con la esperada. Si la bobina sale parcialmente del campo uniforme, [[longitud_efectiva]] efectiva cambia durante el movimiento. Por eso un buen diseño no solo busca un campo grande, sino mantener constante la region activa donde se produce [[fuerza_magnetica_sobre_el_conductor]].
