...
In an interferometer, two beams travel along different paths and recombine to form fringes. Coherence decides whether those fringes remain stable during measurement. If [[diferencia_de_camino_optico]] exceeds the scale [[longitud_de_coherencia]], the pattern loses contrast even though both beams reach the detector.

The dominant assumption is that source and optical table remain stable. In that case, [[coherencia_relativa]] anticipates whether path mismatch is tolerable, and [[visibilidad_de_franjas]] checks the real experimental contrast. Low visibility is not automatically interpreted as low light.
...
Coherence thus becomes an alignment test: changing path, observing [[visibilidad_de_franjas]], and comparing with [[longitud_de_coherencia]] helps locate the dominant cause of fringe loss.

Dominant variable: [[coherencia_relativa]] summarizes whether path difference lies inside the coherence scale.
...
Choosing a laser for interferometry does not depend only on power or color. A source with enough [[longitud_de_coherencia]] is required for the path difference of the setup. A narrow-line laser can sustain fringes with long arms, while a noisier one may lose contrast.

The ideal model helps compare sources before building the experiment. If the manufacturer provides [[tau_c]] or approximate coherence length, path tolerance can be estimated. Then measuring [[visibilidad_de_franjas]] confirms whether source, setup, and detector work as expected.
...
Dominant variable: [[longitud_de_coherencia]] fixes the approximate maximum acceptable path mismatch.
...
This application reverses the classroom intuition. Maximizing [[longitud_de_coherencia]] is not always desirable. If coherence were too long, reflections from many depths would mix. The design seeks a coherence window matched to the layer thickness to be distinguished.
...
Dominant variable: [[longitud_de_coherencia]] controls the depth window capable of producing interferometric signal.
...
Coherence is not a technical luxury here, but the condition for recording phase information. [[visibilidad_de_franjas]] acts as a quality indicator of the pattern: high-contrast fringes allow cleaner reconstructions; washed-out fringes produce images with less useful information.
...
Dominant variable: [[visibilidad_de_franjas]] measures the experimental quality of fringe recording.
...
The application shows why brightness and coherence are different design decisions. Filtering can make previously washed-out fringes visible, but it may require a more sensitive detector or longer exposure time. The physical reading compares [[longitud_de_coherencia]], [[diferencia_de_camino_optico]], and [[visibilidad_de_franjas]].
