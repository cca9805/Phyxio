...
The quantity [[longitud_de_coherencia]] summarizes a useful distance of stability. If the path mismatch [[diferencia_de_camino_optico]] is small compared with that scale, interference can be seen with good contrast. If the mismatch becomes too large, the source no longer keeps phase memory between the two paths.

The visibility [[visibilidad_de_franjas]] measures real contrast between bright and dark fringes. High visibility does not necessarily mean more total light; it means a better relative difference between maxima and minima. This distinction is essential for reading experiments.
...
The first relation converts coherence time [[tau_c]] into coherence length [[longitud_de_coherencia]]. Coherence time measures how long phase remains usefully correlated; multiplying it by propagation speed gives a spatial scale comparable with optical paths.

{{f:longitud_coherencia}}

This length is not the size of the source or the maximum distance light travels. It is a comparison scale: if two paths differ much more than [[longitud_de_coherencia]], relative phases change too much to sustain stable fringes.

To represent simply how coherence falls as path difference grows, the dimensionless quantity [[coherencia_relativa]] is used. The model is not meant to replace a full correlation function; it gives a monotonic reading of coherence loss as [[diferencia_de_camino_optico]] increases.
...
Experimental visibility is obtained by comparing maximum intensity [[intensidad_maxima]] with minimum intensity [[intensidad_minima]] in the pattern. This relation measures normalized contrast and removes direct dependence on total source power.
...
The deep structure of coherence combines three levels: source, propagation, and detection. The source determines [[tau_c]] and therefore [[longitud_de_coherencia]]. The setup introduces [[diferencia_de_camino_optico]] through different paths. The detector measures [[visibilidad_de_franjas]] as the visible consequence of that relation. Mixing these levels confuses a source property with a setup failure.
...
Coherence is not all or nothing. In practice, fringes degrade gradually. It is therefore useful to work with indicators such as [[coherencia_relativa]] or [[visibilidad_de_franjas]] rather than only deciding whether interference exists. A low-contrast pattern may still contain useful information if detector and noise allow it.

The structure also forces a distinction between prediction and diagnosis. [[coherencia_relativa]] predicts expected loss from path mismatch; [[visibilidad_de_franjas]] diagnoses what was actually measured. If the two readings do not agree, the setup is adding another physical or instrumental mechanism.
...
The fine physical reading separates contrast from energy. [[visibilidad_de_franjas]] does not say how much power arrives, but how distinguishable maxima and minima are. A dim image can have high visibility if the detector is sensitive; a bright image can have low visibility if phases are mixed.
...
A broad-spectrum lamp may have [[longitud_de_coherencia]] of micrometres, while a stabilized laser can reach metres or more. This many-order-of-magnitude difference explains why a simple double slit may work with filtered light, but a long-arm interferometer usually needs a much more coherent source.

An absurd result appears if an ordinary white lamp is used with centimetre path differences and high contrast is expected, or if almost zero visibility is interpreted as total absence of light. The correct question is to compare [[diferencia_de_camino_optico]] with [[longitud_de_coherencia]] and then read [[visibilidad_de_franjas]].

## Personalized resolution method

First identify what is being evaluated: source temporal scale, path difference, or measured contrast. If [[tau_c]] is known, calculate [[longitud_de_coherencia]]. If the setup is known, compare [[diferencia_de_camino_optico]] with that scale. If a fringe image is available, use [[intensidad_maxima]] and [[intensidad_minima]] to estimate [[visibilidad_de_franjas]].

Then decide whether the problem belongs to the source or the setup. If [[diferencia_de_camino_optico]] is much larger than [[longitud_de_coherencia]], temporal coherence is missing. If [[visibilidad_de_franjas]] is low even with well-matched paths, check intensities, alignment, background, or spatial coherence.
...
Coherence measures whether a source and a setup preserve enough phase to produce observable fringes. [[longitud_de_coherencia]], [[diferencia_de_camino_optico]], [[coherencia_relativa]], and [[visibilidad_de_franjas]] translate that idea into physical reading: temporal stability, path mismatch, and experimental contrast.