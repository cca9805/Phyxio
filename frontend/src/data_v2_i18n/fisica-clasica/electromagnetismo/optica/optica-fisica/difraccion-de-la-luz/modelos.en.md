...
- **Perfect One-Dimensional Rectangular Slit**: the slit is perfectly uniform in its width [[ancho_de_la_rendija]], and its vertical length is infinitely larger than its width, allowing the diffraction to be analyzed as a strictly one-dimensional lateral phenomenon.
...
2. **Fraunhofer Far-Field Criterion**: [[distancia_a_la_pantalla]] must be greater than the quotient between the square of [[ancho_de_la_rendija]] and [[lambda]]. In practical laboratory terms, this requires [[distancia_a_la_pantalla]] to be greater than 100 times [[ancho_de_la_rendija]].
3. **Scalar Diffraction Limit**: [[ancho_de_la_rendija]] must be greater than 10 times [[lambda]]. The slit width must exceed the wavelength of light by at least ten times to guarantee that the approximations of scalar wave theory remain physically valid without needing to solve complex vector Maxwell equations.

## Model failure signals

- **The measured central maximum width [[ancho_del_maximo_central]] is noticeably larger than calculated**: this occurs when the slit [[ancho_de_la_rendija]] is extremely narrow. The actual spread is so large that the sine of the angle cannot be approximated linearly, requiring the use of the exact trigonometric relation between [[theta]], [[lambda]], and [[ancho_de_la_rendija]].
...
In the exact Fraunhofer model, the paraxial small-angle hypothesis is eliminated. The angular position of the first minimum is determined by the actual trigonometric relationship between [[theta]], [[lambda]], and [[ancho_de_la_rendija]], and its physical position projected onto the flat screen at a distance [[distancia_a_la_pantalla]] is calculated using the exact trigonometric tangent.
This model is essential when the slit is on the scale of a few micrometers, causing wide dispersion angles.

In the Fresnel diffraction model, the screen is placed in the near-field zone, when [[distancia_a_la_pantalla]] is less than or comparable to the Fresnel distance built from [[ancho_de_la_rendija]] and [[lambda]]. The Huygens secondary wavelets must be integrated keeping the quadratic terms of the distance, which yields a distribution characterized by Fresnel integrals. The pattern on the screen no longer displays a single, well-defined central maximum; instead, it oscillates between a bright peak and a dark minimum along the central axis depending on the exact distance, drawing a profile with multiple highly oscillating secondary maxima.

When to transition to the exact non-paraxial model: when the ratio of [[lambda]] to [[ancho_de_la_rendija]] exceeds the threshold 0.05, causing the error committed by the paraxial linear approximation in calculating the central maximum width to exceed the five percent acceptable error threshold.

When to transition to the Fresnel diffraction model: when [[distancia_a_la_pantalla]] is less than the Fresnel limit or becomes lower than 100 times [[ancho_de_la_rendija]] for most typical laboratory slits, making the assumption of plane waves at the screen invalid.
...
| Relationship of width [[ancho_del_maximo_central]] with [[distancia_a_la_pantalla]] | Perfectly linear | Slightly non-linear | Non-linear and highly oscillating |
| Wavefront geometry | Ideal plane wave | Ideal plane wave | Spherical wave at the screen |
| Typical laboratory range | [[distancia_a_la_pantalla]] greater than 1.0 m, [[ancho_de_la_rendija]] near 0.1 mm | [[distancia_a_la_pantalla]] greater than 1.0 m, [[ancho_de_la_rendija]] smaller than 0.01 mm | [[distancia_a_la_pantalla]] smaller than 0.1 m, [[ancho_de_la_rendija]] near 0.5 mm |