...
The magnitudes that are preserved are [[intensidad_incidente]] as the maximum-intensity reference, [[theta_p]] as the angular control variable and [[n_1]] together with [[n_2]] as the medium parameters. The transmitted intensity [[intensidad_transmitida]] and the Brewster angle [[angulo_de_brewster]] are the principal model outputs.
...
- The polariser is **ideal**: no residual absorption, no internal reflections and no scattering. A real polariser always absorbs a small fraction of the aligned component, reducing the maximum transmission below [[intensidad_incidente]].
