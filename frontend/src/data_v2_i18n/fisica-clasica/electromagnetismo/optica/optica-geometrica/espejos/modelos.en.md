...
The ideal mirror model assumes that the reflecting surface is **perfectly spherical**, that all incident rays are **paraxial** (forming small angles with the optical axis) and that reflection is **specular** with no scattering or absorption. Within this framework, the mirror equation relates [[d_o]], [[d_i]] and [[distancia_focal_del_espejo]] exactly, and the lateral magnification [[aumento_lateral]] describes the size and orientation relationship between image and object.

The conserved magnitudes are [[d_o]] as the input parameter, [[distancia_focal_del_espejo]] and [[radio_de_curvatura_del_espejo]] as system parameters and [[d_i]] together with [[aumento_lateral]] as the principal outputs. The focal-length–radius relation remains exact within the paraxial approximation.
