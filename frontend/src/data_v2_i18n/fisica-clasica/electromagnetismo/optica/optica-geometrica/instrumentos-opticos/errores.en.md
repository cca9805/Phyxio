...
The total magnification of a multi-stage instrument is always the **product** of the partial magnifications. For the microscope, [[aumento_total_del_microscopio]] results from multiplying the lateral magnification of the objective by the angular magnification of the eyepiece.
...
Check 1: In Optical Instruments, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
The objective factor is [[longitud_del_tubo]] divided by [[f_obj]] and the eyepiece factor is [[d_pp]] divided by [[f_oc]]. Swapping the focal lengths drastically changes the result because the objective focal length is typically an order of magnitude shorter than the eyepiece focal length.
...
Remember that the **objective** divides the tube length [[longitud_del_tubo]] by its focal length [[f_obj]], and the **eyepiece** divides the near-point distance [[d_pp]] by its focal length [[f_oc]]. The objective produces lateral magnification; the eyepiece produces angular magnification.

**Contrast mini-example**

With [[f_obj]] of 0.004 m, [[f_oc]] of 0.025 m, [[longitud_del_tubo]] of 0.16 m and [[d_pp]] of 0.25 m, the correct result is (0.16/0.004) times (0.25/0.025), equal to 40 times 10, giving 400. Swapping the focal lengths gives (0.16/0.025) times (0.25/0.004), equal to 6.4 times 62.5, giving 400 as well by algebraic coincidence. But if the focal lengths were 0.004 m and 0.050 m, the swap would yield an incorrect result.
...
Check 6: In Optical Instruments, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 6.
...
- The objective and eyepiece focal lengths were correctly assigned: [[f_obj]] in the factor with [[longitud_del_tubo]] and [[f_oc]] in the factor with [[d_pp]].
