...
The system consists of a ray travelling through vacuum (or air, with index approximately one) and striking a flat vacuum-glass interface. The plane of incidence contains the ray, the surface normal and the refracted ray. The relevant quantities are [[indice_de_refraccion]] (glass index), [[velocidad_de_la_luz_en_el_vacio]], [[v_medio]], [[lambda_0]], [[lambda_medio]], [[theta_1]] and [[theta_2]].

## Physical model

The geometrical-optics model with a flat interface between a medium of index one (vacuum) and a medium of index [[indice_de_refraccion]] (glass) is applied. The light is monochromatic, the medium is homogeneous and isotropic, and the interface is ideal. The definition formula gives [[indice_de_refraccion]], the wavelength relation gives [[lambda_medio]] and Snell's law gives [[theta_2]].
...
The complete sequence yields the three requested quantities in a chain: first [[indice_de_refraccion]], then [[lambda_medio]] and finally [[theta_2]].

## Numerical substitution

For the index: dividing 2.998 times ten to the eighth by 1.97 times ten to the eighth gives approximately 1.522. Therefore [[indice_de_refraccion]] ≈ 1.522.
...
The refraction angle of 27.7 degrees, smaller than the 45 degrees of incidence, confirms that the ray bends towards the normal upon entering a medium of higher index. If we increased [[theta_1]] up to the critical angle (calculated as the arcsine of the inverse of [[indice_de_refraccion]], approximately 41.1 degrees from inside the glass), we would observe total internal reflection. This phenomenon delimits the boundary of the model: beyond the critical angle no refracted ray exists.
...
A telecommunications technician must verify that a doped-silica optical fibre correctly guides an infrared laser signal at 1550 nm. The fibre has a core with index [[indice_de_refraccion]] of 1.468 and a cladding with a lower index. The technician needs to estimate the propagation speed of the signal and the wavelength of the light inside the core in order to check that the conditions for constructive interference are met within the waveguide.

## Physical estimation

Dividing [[velocidad_de_la_luz_en_el_vacio]] (2.998 times ten to the eighth metres per second) by [[indice_de_refraccion]] of 1.468 gives a speed in the core of approximately 2.042 times ten to the eighth metres per second. Therefore [[v_medio]] ≈ 2.042 times ten to the eighth m/s.
...
The signal speed in the fibre (roughly two thirds of [[velocidad_de_la_luz_en_el_vacio]]) implies a latency of about 4.9 microseconds per kilometre of fibre. For a 6000 km transatlantic link, the minimum theoretical propagation latency would be around 29 milliseconds, a crucial figure for high-frequency applications in financial markets.
