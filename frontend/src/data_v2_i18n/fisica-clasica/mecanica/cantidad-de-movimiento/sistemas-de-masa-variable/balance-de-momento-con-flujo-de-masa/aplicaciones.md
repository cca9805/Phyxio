## 1. Cinta transportadora que recibe arena
...
Variable dominante: el termino [[u]]·[[mdot]] frente a [[Fext]], porque el frenado nace del momento que la arena debe ganar al entrar.

Limite de validez: el modelo deja de bastar si la arena rebota, se amontona o aparece rozamiento interno dominante.

## 2. Manguera de bomberos y retroceso
...
Variable dominante: la rapidez con la que el flujo exporta momento y no solo la masa total de agua movida.

Limite de validez: la lectura se degrada si el chorro cambia de direccion, pulsa en el tiempo o el conducto flexa mucho.

## 3. Vagon con fuga vertical
...
Variable dominante: la componente horizontal de u, porque sin ella el flujo no extrae momento en esa direccion.

Limite de validez: hay que cambiar de modelo si el liquido sale inclinado, si hay rozamiento apreciable o si el carro rota.

## 4. Tolva dosificadora instrumentada

En una linea industrial con sensores de caudal y fuerza, el balance de momento con flujo de masa deja de ser abstracto porque permite comprobar si las lecturas de fuerza cierran con el flujo de masa medido antes de ajustar el control. El problema practico no consiste solo en obtener un numero, sino en decidir si la simplificacion elegida sigue siendo defendible cuando aparecen tiempos finitos de contacto, rotaciones, apoyos o distribuciones reales de masa. Para responder bien hay que conectar el modelo con una escena medible y con una consecuencia tangible: aprobar un ensayo, corregir un diseno, interpretar un video o rechazar una conclusion precipitada.
...
Variable dominante: la coherencia entre [[dPdt]], [[Fext]] y [[u]]·[[mdot]] en una misma ventana temporal.

Limite de validez: la auditoria falla si sensores y video no miran el mismo tramo o si el volumen de control se mueve.

## 5. Aspersor con dos salidas
...
Variable dominante: la suma vectorial de los flujos de salida y no cada chorro aislado.

Limite de validez: la simplificacion se rompe si las ramas no son equivalentes o si el flujo cambia de regimen.
