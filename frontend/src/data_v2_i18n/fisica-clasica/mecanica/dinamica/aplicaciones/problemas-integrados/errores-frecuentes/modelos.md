# Modelos físicos: Errores frecuentes
...
- **Ángulo de inclinación** ([[angulo_plano]]): 0° ≤ θ ≤ 60°. Para θ > 60°, la [[componente_normal_erronea]] se vuelve tan pequeña que el contacto con la superficie puede perderse antes del análisis.
- **Coeficiente de rozamiento**: 0,01 ≤ μs ≤ 1,5. Valores fuera de este rango no corresponden a materiales físicos comunes en condiciones secas.
- **Aceleración del sistema** ([[aceleracion_sistema]]): [[aceleracion_sistema]] ≤ [[aceleracion_gravedad]] (9,81 m/s²). Una aceleración superior a la caída libre indica que se ha incluido erróneamente la tensión como fuerza externa, o que hay una fuerza propulsora adicional no declarada.
- **Masa total**: M_tot > 0 kg y M_tot ≤ 10³ kg para sistemas manejados manualmente. Masas superiores requieren considerar deformación de cables y poleas.
- **Velocidad**: \(v \ll 10^7\ \mathrm{m/s}\); efectos relativistas despreciables para toda la mecánica clásica de taller.
...
Para transicionar al modelo extendido de dinámica con efectos secundarios, el criterio operativo es verificar si alguna hipótesis del modelo ideal ha sido violada; cuando eso ocurre, el siguiente paso es incluir esos efectos en el análisis:
