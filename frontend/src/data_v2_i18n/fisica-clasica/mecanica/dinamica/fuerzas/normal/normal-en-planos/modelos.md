# Modelos físicos: Normal en planos
...
- Criterio de error: error_relativo <= 0.10 en ensayos repetidos.
- Criterio geométrico: 0 <= [[theta]] <= 60 grados dentro del rango operativo calibrado.
- Criterio dinámico: abs([[aperp]]) <= 1 m/s^2 para mantener validez del esquema lineal usado.
- Criterio de forzamiento: abs([[Fextn]]) <= 0.3*[[m]]*[[g]] en el régimen de decisión rápida.
- Criterio de contacto: [[N]] >= 0 en todos los estados donde se observa apoyo continuo.
...
Transición explícita y cuándo conviene cambiar de modelo: si error_relativo > 0.10 durante varios ensayos consecutivos o si aparece [[N]] < 0 en estados de contacto observado, conviene pasar al modelo extendido con recalibración.
