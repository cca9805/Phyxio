...
The extended model introduces corrections when degrees and radians are mixed or angular and linear quantities are used without conversion. In that broader description, terms linked to real geometry, changes in the medium, nonlinear effects, or internal redistribution become relevant. The goal is not to make the algebra heavier, but to identify which refinement actually changes the prediction. A practical criterion is to compare how much the searched quantity changes when each correction is activated separately.
