...
The transition to the extended model becomes necessary when a central hypothesis stops holding and the leaf's dominant magnitude changes appreciably.

The extended model introduces corrections when relations valid for one point are extrapolated to another radius without control. In that broader description, terms linked to real geometry, changes in the medium, nonlinear effects, or internal redistribution become relevant. The goal is not to make the algebra heavier, but to identify which refinement actually changes the prediction. A practical criterion is to compare how much the searched quantity changes when each correction is activated separately.
