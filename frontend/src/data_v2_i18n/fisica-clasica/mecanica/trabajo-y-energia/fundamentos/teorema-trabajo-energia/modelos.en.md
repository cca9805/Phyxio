...
The practical criterion is not to abandon the theorem too early, but to recognize when it stops being sufficient on its own. When it is useful to switch models is when the global balance no longer preserves the dominant physical cause that the task is asking the student to interpret.
