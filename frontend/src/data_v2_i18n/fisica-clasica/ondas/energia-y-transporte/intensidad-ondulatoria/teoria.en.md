...
**Wave intensity** measures that energy concentration: power transported per unit area perpendicular to propagation. Energy flux describes local transport, while intensity [[intensidad_ondulatoria]] summarizes how much power crosses a measuring surface.
...
Intensity [[intensidad_ondulatoria]] measures how much wave power crosses each square meter. If you move twice as far from an ideal sound source, intensity does not fall to one half, but to one quarter. The reason is not that the source emits less, but that the available spherical surface grows much faster.
...
For a point source emitting power [[potencia_de_la_fuente]] isotropically in three dimensions, the intensity at distance [[r_distancia]] is obtained by dividing the total power by the area of the sphere centered on the source:

{{f:intensidad_desde_potencia}}

The spreading area [[area_de_la_onda_esferica]] follows spherical geometry:
...
Intensity [[intensidad_ondulatoria]] is positive and scalar, while energy flux keeps directional information. If only the power crossing a surface is needed, intensity is enough. If the direction of energy motion matters, the vector flux is the better model.
...
2. **Determine emitted power**: use [[potencia_de_la_fuente]] in watts and check whether it is total power, useful power, or power emitted in a specific direction.
