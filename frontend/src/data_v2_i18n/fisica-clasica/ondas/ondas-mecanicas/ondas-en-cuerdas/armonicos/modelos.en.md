...
- **Frequencies not exactly integer multiples**: If the ratio between [[f_n]] and [[f_1]] differs significantly from [[numero_de_armonico]], **inharmonicity** appears due to string stiffness.
...
The **stiff string model** (Rayleigh, 1877) incorporates the bending moment of the string. In that reading, [[f_n]] lies above the ideal value through a correction factor that increases with the square of [[numero_de_armonico]].

The stiffness parameter controls how far upper harmonics move away from the ideal series. This model predicts increasing inharmonicity with [[numero_de_armonico]].
