...
The boundary condition for a string with fixed ends requires nodes at both points of its vibrating length. The standing wave solution satisfying this restriction imposes that [[longitud_de_la_cuerda]] must contain exactly [[numero_de_armonico]] half-wavelengths:

{{f:longitud_onda_armonica}}

Where [[numero_de_armonico]] is the harmonic number (positive integer) and [[lambda_n]] the corresponding wavelength. Solving for wavelength, we obtain the discrete set of allowed values.

The general relation between frequency, propagation velocity and wavelength indicates that [[velocidad_de_propagacion]] equals the product of [[f_n]] and [[lambda_n]], allowing derivation of harmonic frequencies. Substituting the previous expression:

{{f:frecuencia_armonica}}

This equation reveals that frequencies are integer multiples of a base frequency. The particular case where [[numero_de_armonico]] is 1 defines the **fundamental frequency**:
...
> [!WARNING]
> The velocity [[velocidad_de_propagacion]] in these formulas is the propagation velocity of the disturbance along the string, not the transverse oscillation velocity of points on the string. These magnitudes are related but conceptually distinct.
...
The round-trip travel time is obtained from traveled length and propagation speed. The resonance condition requires this time to coincide with [[numero_de_armonico]] periods associated with [[f_n]]. Reorganizing that reading gives the harmonic frequency formula, demonstrating that normal modes are **stationary resonance states**.
...
> [!NOTE]
> The linear character of the harmonic spectrum, where [[f_n]] scales with [[numero_de_armonico]] and [[f_1]], is a direct consequence of the system's one-dimensionality and the homogeneity of the string. Higher-dimensional systems exhibit more complex spectra.
...
The **limits of the ideal model** appear when string stiffness is not negligible. In real strings, especially thick piano strings, upper harmonics do not exactly follow the ideal proportionality between [[f_n]], [[numero_de_armonico]] and [[f_1]], but exhibit **inharmonicity**: frequencies are slightly higher than predicted, deviating more as [[numero_de_armonico]] increases. This effect obliges piano tuners to "stretch" the tuning, compensating for deviations.
...
4. **Derive frequencies**: obtain [[f_n]] by combining [[numero_de_armonico]], [[velocidad_de_propagacion]], [[longitud_de_la_cuerda]] and [[lambda_n]] through the formal-level formulas.
