...
El sistema fisico es una cuerda tensa que actua como medio mecanico. La fuente impone el ritmo [[frecuencia]], pero la rapidez [[velocidad_de_propagacion]] debe leerse desde las propiedades de la cuerda: [[tension_de_la_cuerda]] y [[mu]]. La longitud efectiva [[longitud_efectiva]] no es necesaria para hallar la rapidez local, aunque seria importante si se pidieran modos estacionarios.

## Modelo físico

Usamos el modelo de cuerda ideal: tension uniforme, masa lineal constante, amplitud pequena y ausencia de dispersion apreciable. Bajo esas condiciones, [[velocidad_de_propagacion]] depende del equilibrio entre restauracion por [[tension_de_la_cuerda]] e inercia por [[mu]]. Una vez conocida la rapidez, se conecta con [[frecuencia]] para obtener [[lambda]].
...
Esta lectura muestra que [[tension_de_la_cuerda]] acelera la propagacion y [[mu]] la ralentiza. Despues se relaciona la rapidez calculada con la [[frecuencia]] impuesta:
...
Si se sustituye la cuerda por otra con doble [[mu]] y la misma [[tension_de_la_cuerda]], la rapidez disminuye porque aumenta la inercia por metro. No se modifica la [[frecuencia]] de la fuente, por lo que el cambio visible principal sera una longitud de onda menor.

## Sustitución numérica

Con [[tension_de_la_cuerda]] de 45 N y [[mu]] de 0.0050 kg/m, dividir la tension entre la densidad lineal da 9000 en unidades compatibles. La raiz cuadrada de ese valor produce una rapidez aproximada de 95 m/s. Por tanto [[velocidad_de_propagacion]] ≈ 95 m/s.

Con una [[frecuencia]] de 60 Hz, dividir la rapidez por la [[frecuencia]] da una longitud de onda aproximada de 1.6 m. Por tanto [[lambda]] ≈ 1.6 m.

Si el tramo activo [[longitud_efectiva]] mide 1.5 m y sus extremos se comportan como fijos, dividir 95 m/s entre el doble de esa longitud da una fundamental cercana a 32 Hz. Esa lectura no cambia la rapidez ya calculada; solo traduce la misma propagacion a un modo compatible con la longitud disponible.
...
Longitud de onda: dividir una rapidez `[L T⁻¹]` entre una magnitud temporal de dimensión `[T⁻¹]` deja dimensión `[L]`.
Frecuencia: dividir una rapidez `[L T⁻¹]` entre una longitud `[L]` deja dimensión `[T⁻¹]`.
...
Las tres comprobaciones son coherentes con las unidades esperadas: m/s para [[velocidad_de_propagacion]], m para [[lambda]] y Hz para [[frecuencia]].
...
Al duplicar [[mu]], la cuerda se vuelve mas dificil de acelerar en cada metro. La tension sigue siendo la misma, pero debe mover mas inercia distribuida. Por eso la rapidez disminuye y, al mantenerse [[frecuencia]], tambien disminuye [[lambda]]. La comparacion aisla la idea clave del leaf: una cuerda mas pesada por metro no necesita una fuente mas lenta para propagar mas despacio; su propia inercia ya reduce [[velocidad_de_propagacion]].

La lectura de [[longitud_efectiva]] anade una consecuencia fisica distinta. Si el tramo esta fijado, la misma rapidez mecanica queda obligada a encajar en un recorrido de ida y vuelta; por eso la [[frecuencia]] fundamental no mide una nueva causa, sino una restriccion geometrica sobre una propagacion ya decidida por la cuerda. En un laboratorio, esto ayuda a no mezclar decisiones: cambiar la masa lineal modifica el medio, cambiar la longitud activa modifica las condiciones de frontera, y cambiar la fuente solo cambia el ritmo impuesto.
