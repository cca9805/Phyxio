...
Check 1: In Ultrasound and Testing Applications, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.


### Error 2: Believing that higher [[coeficiente_de_reflexion_acustica]] always implies greater defect danger

**Why it seems correct**

A higher-amplitude echo in the A-scan seems to indicate a more severe defect. Since [[coeficiente_de_reflexion_acustica]] governs echo amplitude, the mental equivalence is established: more [[coeficiente_de_reflexion_acustica]] equals more danger.

**Why it is incorrect**

[[coeficiente_de_reflexion_acustica]] measures the acoustic contrast at the defect interface, not its mechanical danger. A very small air-filled crack produces [[coeficiente_de_reflexion_acustica]] close to 1 (maximum echo) but may be mechanically irrelevant. A hard material inclusion with [[impedancia_acustica]] similar to steel may have [[coeficiente_de_reflexion_acustica]] close to zero (minimum echo) and still be dangerous due to its brittleness or the stress concentration it generates.
...
Defect danger is assessed using fracture mechanics criteria (critical size, stress intensity factor) that require knowing the defect size, geometry, and position, not just its acoustic contrast. [[coeficiente_de_reflexion_acustica]] informs about the interface nature, not structural criticality.

**Contrast mini-example**

A spherical 0.1 mm diameter air-filled pore has [[coeficiente_de_reflexion_acustica]] close to 1 but is undetectable in practice because it is smaller than the wavelength. A flat 5 mm thick ceramic material inclusion may have [[coeficiente_de_reflexion_acustica]] of 0.3 and be structurally critical by delaminating under load.
...
### Error 5: Interpreting the sign of [[coeficiente_de_reflexion_acustica]] as propagation direction

**Why it seems correct**

In physics, the sign of a quantity often indicates direction (positive to the right, negative to the left). The student applies this convention and concludes that negative [[coeficiente_de_reflexion_acustica]] means the echo travels in the opposite direction or that the wave is transmitted into the interior instead of being reflected.

**Why it is incorrect**

The sign of [[coeficiente_de_reflexion_acustica]] indicates whether the reflected wave is in phase or in antiphase with the incident wave, i.e., whether there is phase inversion in the reflection. A negative [[coeficiente_de_reflexion_acustica]] means the second medium is acoustically softer than the first (lower [[impedancia_acustica]]), and the reflected wave has opposite-sign pressure. It does not indicate propagation direction: the reflected wave always travels back towards the transducer regardless of the sign of [[coeficiente_de_reflexion_acustica]].

**Detection signal**

The student writes that with negative [[coeficiente_de_reflexion_acustica]] the wave "does not return to the transducer" or that "energy is only transmitted and not reflected". The student may also calculate the reflected energy fraction using negative [[coeficiente_de_reflexion_acustica]] directly instead of its squared magnitude.

**Conceptual correction**

The squared magnitude of [[coeficiente_de_reflexion_acustica]] gives the fraction of reflected intensity; the magnitude of [[coeficiente_de_reflexion_acustica]] (always positive) gives the fraction of reflected amplitude. The sign only indicates echo polarity. The reflected wave always exists when [[coeficiente_de_reflexion_acustica]] is non-zero, regardless of its sign.

**Contrast mini-example**

At the water-air interface (the second medium being air, which is softer), [[coeficiente_de_reflexion_acustica]] is negative and close to -1. This does not imply there is no reflection; on the contrary, almost all energy is reflected. The observable difference is that the echo arrives with opposite-sign pressure to the original pulse, something the equipment can detect by inverting the peak polarity in the A-scan.
...
3. **The magnitude of [[coeficiente_de_reflexion_acustica]] is between 0 and 1**: a value outside this range indicates an algebraic error in the [[impedancia_acustica]] calculation for the two media or use of different units for each.
