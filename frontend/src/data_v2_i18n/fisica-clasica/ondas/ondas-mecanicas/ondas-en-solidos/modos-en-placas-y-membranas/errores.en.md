...
Check 1: In Modes in Plates and Membranes, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 1.
...
The restoring mechanisms are different. A plate is restored by bending stiffness [[rigidez_a_la_flexion_de_la_placa]], which depends strongly on [[h_placa]] and on elastic constants such as [[modulo_de_young]] and [[nu_p]]. A membrane is restored mainly by surface tension [[tension_superficial_de_la_membrana]]. An ideal membrane has negligible bending stiffness; an unstressed membrane cannot behave like a plate simply because its material has a Young modulus.

**Detection signal**

The student applies the plate formula to a drum head, or tries to tune a membrane by changing [[modulo_de_young]] instead of [[tension_superficial_de_la_membrana]]. Another sign is using [[rigidez_a_la_flexion_de_la_placa]] and [[tension_superficial_de_la_membrana]] in the same calculation without declaring which regime dominates.
...
Increasing [[h_placa]] in a plate raises [[rigidez_a_la_flexion_de_la_placa]] strongly and shifts the spectrum upward. Tightening a drum head raises [[tension_superficial_de_la_membrana]] and shifts the membrane spectrum upward. These are not interchangeable controls.
...
A panel clamped on all sides can have a much higher fundamental frequency than the same panel simply supported on all sides, even with identical [[rigidez_a_la_flexion_de_la_placa]], [[rho_s]], [[a_dim]] and [[b_dim]].
...
Students remember beam bending stiffness as a product of Young's modulus and a geometric inertia. They then write the plate stiffness as if it were only [[modulo_de_young]] times a cubic thickness factor.

**Why it is incorrect**

A plate bends in a two-dimensional stress state. The lateral coupling described by [[nu_p]] changes the effective bending stiffness. Omitting the factor involving [[nu_p]] gives a systematic error in [[rigidez_a_la_flexion_de_la_placa]] and therefore in all plate frequencies.
...
Check 6: In Modes in Plates and Membranes, this error appears when the result is treated as a number detached from the circuit model; Check the declared quantities, the sign convention, and the physical role of the computed value revision 6.
