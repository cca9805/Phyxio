...
## 1. Defect detection by guided waves in rails
...
Dominant variable: the second moment of area [[momento_de_inercia_de_la_seccion_transversal]] of the section, which when varied along the bar (arched profile) allows independent control of the fundamental frequency and upper partials.

**Physical interpretation:** The ratio between successive mode frequencies in a uniform bar is not harmonic (partials are not integer multiples of the fundamental). By carving the bar to reduce [[momento_de_inercia_de_la_seccion_transversal]] at the centre, the builder lowers the second partial frequency until it coincides with a consonant ratio (e.g. 4:1 relative to the fundamental).
