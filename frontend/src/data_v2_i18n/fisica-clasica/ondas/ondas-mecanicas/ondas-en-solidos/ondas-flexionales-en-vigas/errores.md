...
Un golpe en un raíl de tren produce un pulso breve. A 10 m del punto de impacto, las componentes de 5 kHz llegan en 6 ms ([[v_fase_flex]] ≈ 1700 m/s) mientras que las de 500 Hz llegan en 19 ms ([[v_fase_flex]] ≈ 530 m/s). El observador percibe primero las frecuencias agudas y después las graves, fenómeno imposible en un medio no dispersivo.
...
Una barra de aluminio de 2 m de longitud se golpea en un extremo. La velocidad de fase a 1 kHz es 150 m/s (tiempo aparente de 13.3 ms) pero la energía llega realmente en 6.7 ms porque [[v_grupo_flex]] vale 300 m/s. Si se cronometra la llegada del impacto, se mide la velocidad de grupo, no la de fase.
...
La velocidad de fase flexional requiere incluir explícitamente [[momento_de_inercia_de_la_seccion_transversal]], [[rho_lin]] y [[omega]]. No se puede obtener a partir de una sola propiedad del material. A 1 kHz en una barra de acero de 5 mm, la flexional es unas 40 veces más lenta que la longitudinal.
...
En ondas flexionales de Euler-Bernoulli, [[v_grupo_flex]] es siempre exactamente el doble de [[v_fase_flex]]. La energía viaja más rápido que el patrón de fase. Esto es una consecuencia directa de que la curva de [[omega]] frente al número de onda es una parábola convexa.

**Mini-ejemplo de contraste**

A 1 kHz en acero con sección de 5 mm, [[v_fase_flex]] es 130 m/s y [[v_grupo_flex]] es 260 m/s. Un cronómetro midiendo la llegada de un pulso indicaría 260 m/s, no 130 m/s. Si el alumno asume [[v_grupo_flex]] menor que [[v_fase_flex]] (digamos 65 m/s), predice un tiempo de llegada 4 veces mayor que el real.
...
3. **Comparación con longitudinal:** [[v_fase_flex]] debe ser siempre menor que la velocidad longitudinal sqrt([[modulo_de_young]]/[[rho_vol]]). Si no lo es, el modelo está fuera de rango.
...
6. **Unidades de entrada:** [[omega]] en rad/s (no Hz), [[momento_de_inercia_de_la_seccion_transversal]] en m⁴ (no cm⁴), [[modulo_de_young]] en Pa (no GPa).