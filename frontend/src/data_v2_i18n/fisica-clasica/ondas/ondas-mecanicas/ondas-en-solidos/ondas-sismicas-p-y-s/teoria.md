...
Donde [[modulo_de_compresibilidad]] es el modulo de compresibilidad, [[modulo_de_cizalla]] el modulo de cizalla y [[rho_medio]] la densidad. La presencia de ambos modulos refleja que la onda P involucra tanto cambio de volumen como cambio de forma.
...
La diferencia fundamental entre P y S reside en el tipo de deformacion que movilizan. La onda P produce **dilatacion volumetrica** (compresion-rarefaccion), por lo que su velocidad depende tanto de [[modulo_de_compresibilidad]] como de [[modulo_de_cizalla]]. La onda S produce **distorsion de forma** sin cambio de volumen, y solo depende de [[modulo_de_cizalla]].

> [!WARNING]
> En un fluido, [[modulo_de_cizalla]] es exactamente cero. Esto implica que [[v_s]] se anula y la onda S no existe. Las ondas P, en cambio, siguen propagandose porque [[modulo_de_compresibilidad]] permanece finito.
...
La velocidad [[v_p]] combina dos mecanismos de restitucion elastica: la resistencia a la compresion ([[modulo_de_compresibilidad]]) y la resistencia al cambio de forma ([[modulo_de_cizalla]]). Esto significa que incluso en un material muy blando en cizalla pero incompresible (como el agua), las ondas P se propagan eficazmente.

La velocidad [[v_s]], al depender solo de [[modulo_de_cizalla]], es un indicador puro de la rigidez estructural. Un medio que permite ondas S es necesariamente un solido. Esta dicotomia convierte al par P/S en una herramienta de diagnostico del estado fisico de la materia.
...
1. **Identificar el medio**: determinar [[modulo_de_compresibilidad]], [[modulo_de_cizalla]] y [[rho_medio]] del material (tablas geofisicas o ensayos de laboratorio).
2. **Calcular [[v_p]]**: aplicar la formula con raiz cuadrada de ([[modulo_de_compresibilidad]] mas 4/3 de [[modulo_de_cizalla]]) dividido entre [[rho_medio]].
3. **Calcular [[v_s]]**: aplicar la raiz cuadrada de [[modulo_de_cizalla]] entre [[rho_medio]].
...
**Caso del fluido**: al hacer [[modulo_de_cizalla]] nulo, la formula de [[v_p]] se reduce a la velocidad del sonido en fluido (raiz de [[modulo_de_compresibilidad]] entre [[rho_medio]]). Las ondas S desaparecen por completo.
...
**Medio parcialmente saturado**: la saturacion parcial produce un aumento brusco de [[ratio_vp_vs]] porque [[modulo_de_compresibilidad]] se incrementa con el agua mientras [[modulo_de_cizalla]] apenas cambia. Este efecto es la base de la deteccion de reservorios de hidrocarburos mediante sismica.
...
- **¿Por que la P siempre es mas rapida que la S?** Porque la formula de [[v_p]] incluye [[modulo_de_compresibilidad]] sumado con [[modulo_de_cizalla]], mientras la de [[v_s]] solo incluye [[modulo_de_cizalla]]. Al sumar un termino positivo en el numerador, [[v_p]] es necesariamente mayor.

- **¿Como se sabe que el nucleo externo es liquido?** Porque las ondas S no lo atraviesan: [[modulo_de_cizalla]] es cero en un liquido, lo que anula [[v_s]]. Solo las P continuan (con [[modulo_de_compresibilidad]] finito).
