...
## 1. Geotechnical soil profiling via MASW for seismic classification
...
Since penetration depth [[d_pen]] at these frequencies is on the order of micrometres (comparable to coating thickness), the measurement is exclusively sensitive to film properties and the interface with the substrate. From the measured [[velocidad_de_onda_de_rayleigh]], the Young modulus and Poisson ratio of the coating are estimated. Local variations in [[velocidad_de_onda_de_rayleigh]] reveal adhesion defects, incipient delaminations, or composition gradients in the film. The technique is non-destructive with micrometre lateral resolution.
