...
## 1. Seismic geotechnics and soil classification

Dominant variable: [[v_s]]

Validity limit: [[lambda_s]] must be larger than the layer heterogeneity scale.

In geotechnics, near-surface S-wave velocity helps classify soil and estimate its ability to amplify seismic motion. Low values indicate soft material, low dynamic stiffness and larger expected deformation during an earthquake. The reading is not just a number: engineers compare the trend with depth, look for velocity inversions and check whether each layer can be treated as roughly homogeneous.
...
## 3. Industrial shear ultrasonics

Dominant variable: [[lambda_s]]

Validity limit: The part must behave as a continuum and attenuation must not hide the echo.

Shear transducers are used to inspect welds, laminates and metallic parts when a longitudinal wave does not illuminate certain defects well. Wavelength controls resolution: higher frequencies detect smaller details, but also increase attenuation and may disperse if the microstructure is no longer small.
...
Dominant variable: [[impedancia_transversal]]

Validity limit: Interfaces require mode conversion when incidence is not normal.

In seismology, S waves reveal where the planet retains shear rigidity. Their absence through the outer core is physical evidence for a liquid state. At internal discontinuities, contrasts in transverse impedance control reflections, conversions and amplitude changes observed at distant stations.

Global interpretation does not use a single homogeneous material. Radial or three-dimensional models are built and adjusted to arrival times. The elementary idea of positive [[modulo_de_cizalla]] remains the key separating solid regions from regions unable to carry shear.
...
## 5. Materials engineering and curing control

Dominant variable: [[modulo_de_cizalla]]

Validity limit: The response must remain in the linear elastic regime.

During curing of concretes, polymers or adhesives, S-wave velocity tracks increasing stiffness without destroying the sample. As the internal network consolidates, [[modulo_de_cizalla]] grows and the shear wave travels faster. The technique is sensitive to early changes that are not always visible in static tests.
