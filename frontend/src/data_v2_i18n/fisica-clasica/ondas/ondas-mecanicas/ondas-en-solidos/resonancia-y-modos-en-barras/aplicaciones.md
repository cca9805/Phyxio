...
## 1. Medida del modulo de Young por excitacion por impulso (ASTM E1876)

Una barra rectangular o cilindrica se apoya en sus puntos nodales del primer modo flexional y se golpea en el centro. Se mide la frecuencia fundamental longitudinal con un microfono. La frecuencia libre-libre permite obtener [[velocidad_de_onda_longitudinal_en_barra]] y la relacion de velocidad longitudinal permite obtener [[modulo_de_young]]. El metodo es no destructivo, rapido (segundos por muestra) y preciso (error menor del 1 %). Es el estandar industrial para ceramicas, metales sinterizados y materiales fragiles.

Variable dominante: [[f_n]] medida, de la que se extrae [[velocidad_de_onda_longitudinal_en_barra]] y finalmente [[modulo_de_young]].
...
Variable dominante: [[longitud_de_la_barra]], que se ajusta para obtener la [[f_n]] deseada con el [[velocidad_de_onda_longitudinal_en_barra]] del material elegido.
...
Variable dominante: [[longitud_de_la_barra]] y [[velocidad_de_onda_longitudinal_en_barra]] del material (titanio, aluminio, acero), ajustados para que [[f_n]] coincida con la frecuencia del generador.
Limite de validez: a potencias altas, la temperatura sube y cambia [[modulo_de_young]] y [[rho]] ligeramente, desplazando la frecuencia de resonancia. Se requiere control de frecuencia activo.
