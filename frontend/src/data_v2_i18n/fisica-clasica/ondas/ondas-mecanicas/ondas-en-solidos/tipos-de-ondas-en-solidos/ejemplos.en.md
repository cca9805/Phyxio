...
The chosen model is **linear isotropic elasticity in a 3D medium**, which relates wave velocities to the elastic moduli [[modulo_de_young]], [[modulo_de_corte]], [[rho]] and [[nu]] through the three formulas of the leaf. For the longitudinal wave, the model uses the effective confined modulus, which includes the effect of the hindered lateral compression. For the transverse wave, the model uses only [[modulo_de_corte]] and [[rho]]. For the Rayleigh wave, Viktorov's approximation is applied.
...
Rayleigh wave velocity using Viktorov's approximation from the result of [[velocidad_de_onda_transversal_s]] and Poisson's ratio [[nu]]:

{{f:vel_rayleigh}}

The symbolic hierarchy to be verified is that [[velocidad_de_onda_longitudinal_p]] is greater than [[velocidad_de_onda_transversal_s]] and [[velocidad_de_onda_transversal_s]] is greater than [[velocidad_de_onda_de_rayleigh]].

## Numerical substitution

For the longitudinal wave, inserting the numerical values into the P-wave speed formula: the numerator uses the product of 114×10⁹ Pa by (1 minus 0.34), giving 75.24×10⁹ Pa. The denominator uses 4430 kg/m³ multiplied by (1 plus 0.34) multiplied by (1 minus 2 times 0.34), yielding 4430 times 1.34 times 0.32, approximately 1902. The quotient 75.24×10⁹ divided by 1902 gives approximately 39.56×10⁶ m²/s². The square root of that quantity is approximately 6290 m/s. Therefore [[velocidad_de_onda_longitudinal_p]] is approximately 6290 m/s.

For the transverse wave, inserting into the shear relation: the quotient of 44×10⁹ Pa divided by 4430 kg/m³ gives approximately 9.93×10⁶ m²/s². The square root is approximately 3152 m/s. Therefore [[velocidad_de_onda_transversal_s]] is approximately 3150 m/s.

For the Rayleigh wave, inserting into the surface-wave approximation: the numerator is 0.862 plus the product of 1.14 times 0.34, which is 0.862 plus 0.388, giving 1.250. The denominator is 1 plus 0.34, equal to 1.34. The quotient 1.250 divided by 1.34 is approximately 0.933. Multiplying by [[velocidad_de_onda_transversal_s]] of 3150 m/s gives [[velocidad_de_onda_de_rayleigh]] approximately 2939 m/s, rounded to 2940 m/s.

## Dimensional validation

All three formulas share the same dimensional structure: the square root of a pressure (Pa) divided by a density (kg/m³). Expressing Pa in fundamental units, Pa is kg/(m·s²), and kg/m³ is kg/m³. The ratio Pa/(kg/m³) yields m²/s². The square root of m²/s² is m/s. Dimensional verification for [[velocidad_de_onda_longitudinal_p]] and [[velocidad_de_onda_transversal_s]]: `[M L⁻¹ T⁻²] / [M L⁻³] = [L² T⁻²]` → square root `[L T⁻¹]` ✓. The [[velocidad_de_onda_de_rayleigh]] formula is a dimensionless ratio times [[velocidad_de_onda_transversal_s]], so it carries the same units as [[velocidad_de_onda_transversal_s]]: m/s ✓.

## Physical interpretation

The three values obtained — [[velocidad_de_onda_longitudinal_p]] of approximately 6290 m/s, [[velocidad_de_onda_transversal_s]] of approximately 3150 m/s and [[velocidad_de_onda_de_rayleigh]] of approximately 2940 m/s — satisfy the mandatory hierarchy and are consistent with values tabulated in the literature for Ti-6Al-4V (typically 6100 to 6300 m/s, 3100 to 3200 m/s and 2900 to 3000 m/s respectively). The small difference from some reference values is due to compositional and heat treatment variation between batches of the same material.

The longitudinal wave travels approximately twice as fast as the transverse wave, reflecting that the confined modulus of titanium is approximately four times larger than [[modulo_de_corte]]. This is typical of metals with moderate [[nu]]. For the inspection technician, this means that in a pulse-echo test, the first echo to arrive corresponds to the P-wave, and if an S-wave is also emitted simultaneously (with a shear transducer), the second echo will arrive approximately twice as late for the same defect depth.

The Rayleigh wave is only 6.7% below [[velocidad_de_onda_transversal_s]], which is typical of metals with [[nu]] around 0.30 to 0.35. This result has direct implications for surface inspection: the time window between S-wave and Rayleigh arrival is very short, so surface inspection systems must have sufficient temporal resolution to distinguish the two arrivals if used in transmission mode.
