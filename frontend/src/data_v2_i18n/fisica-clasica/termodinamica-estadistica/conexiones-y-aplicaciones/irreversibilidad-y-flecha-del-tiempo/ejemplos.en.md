...
The physical model considers fundamental system quantities such as [[constante_boltzmann]], [[entropia]], [[entropia_producida]].
...
First, apply the total second-law balance: add the system and surroundings contributions to obtain [[variacion_entropia_universo]]. If the total result is positive, the evolution is compatible with the arrow of time.

{{f:segunda_ley_delta_s_universo}}

Then interpret [[entropia_producida]] as a measure of irreversibility: how far the process departs from the reversible ideal limit.

{{f:criterio_flecha_temporal}}

{{f:entropia_producida_irreversible}}

Finally, the statistical basis connects [[entropia]] with the number of compatible microscopic configurations through [[constante_boltzmann]] and [[numero_microestados]]. This explains why the macroscopic J/K scale is immensely larger than any individual microscopic contribution.
...
[[variacion_entropia_sistema]], [[variacion_entropia_entorno]], [[variacion_entropia_universo]], and [[entropia_producida]] have units of J/K. Adding the first two keeps J/K. If [[grado_irreversibilidad]] is calculated by dividing by [[constante_boltzmann]], the result is dimensionless. That dimensionless ratio connects the macroscopic J/K balance with the microscopic [[numero_microestados]] scale: [[variacion_entropia_universo]] divided by [[constante_boltzmann]] estimates the logarithm of the extra microstates gained by the total system. For 1.2 J/K divided by 1.38 × 10⁻²³ J/K, that ratio is roughly 10²³, an astronomical number that explains why spontaneous reversal is unobservable on any practical time scale.
...
The cup does not spontaneously warm by taking energy from cooler air because that path would correspond to macrostates with vastly fewer [[numero_microestados]]. The arrow of time appears as a macroscopic statistical asymmetry: the compatible process produces or maintains non-decreasing [[variacion_entropia_universo]]. Heat flows from hot to cold not because an extra law prohibits the reverse, but because the accessible macrostates are overwhelmingly more numerous in the forward direction. Every irreversible everyday process — cooling, mixing, friction — is an instance of the same statistical principle. The scale comparison with [[constante_boltzmann]] confirms that, at the macroscopic level, the probability of spontaneous reversal is negligible for any practical purpose.