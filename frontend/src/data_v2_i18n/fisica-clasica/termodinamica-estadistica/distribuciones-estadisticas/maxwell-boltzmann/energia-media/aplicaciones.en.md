...
One way to check the model is to compare predictions at different wall temperatures: doubling the wall temperature must double [[energia_media]] and, if [[numero_particulas]] stays constant, also double [[energia_total]]. This internal coherence test does not require knowing the molecular mass, only that the gas is classical, dilute, and equilibrated with the walls. When the measurement deviates from that scaling, the cause is typically non-equilibrium outgassing, multiple temperature zones, or a gas mixture where light and heavy species have different relaxation times. [[k_boltzmann]] remains the same in all these cases; only the effective [[temperatura]] may differ across regions. Identifying which zone sets the dominant [[energia_media]] is the first diagnostic step before invoking more complex models.
