...
If the occupied energies are tightly clustered, [[fluctuacion_energia]] is small. If many energy states compete with noticeable probability, [[varianza_energia]] grows. Physically, a fluctuation is not a failure of equilibrium; it is the allowed spread of energy inside that equilibrium description.
...
A worked benchmark helps: consider a two-level system where the ground state has degeneracy one and the excited state has degeneracy g. As [[temperatura]] rises, the excited state contributes more to both [[funcion_particion]] and [[varianza_energia]]. If g is large, even a modest thermal scale can shift substantial statistical weight upward, making [[varianza_energia]] peak earlier than in the case where both levels are nondegenerate. The ratio of [[fluctuacion_energia]] to [[energia_media]] captures that relative importance and shrinks toward zero in the large-system limit, confirming that bulk thermodynamics suppresses individual fluctuations relative to the mean without eliminating their macroscopic imprint through [[capacidad_calorifica]].
...
Can [[fluctuacion_energia]] exceed [[energia_media]]? In small systems or near excited levels, yes: the relative spread is a physical signature that the average is not a reliable single-value description. In that regime the full distribution must be reported, not only the mean.

## Cross-cutting connections and study paths

This leaf connects to mean energy, Boltzmann distributions, heat capacity, and phase transitions. It also prepares the broader idea of susceptibility: many macroscopic responses are microscopic fluctuations translated into observable language. The same pattern appears in magnetism, gases, solids, and statistical information models. The language of fluctuations extends further into Brownian motion and signal processing, where noise power spectra carry the same information that [[varianza_energia]] carries in the canonical ensemble. Recognizing this cross-domain pattern makes it possible to read a measured noise floor as a thermodynamic fingerprint and to design experiments that extract [[capacidad_calorifica]] from fluctuation measurements rather than from direct heat exchange.
