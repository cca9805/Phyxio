...
Para cuantificar la dispersión estadística de la distribución de equilibrio, la [[entropia]] de Gibbs sintetiza el reparto de ocupaciones entre los estados accesibles:

{{f:entropia_gibbs_equilibrio}}

Con probabilidades 0,60 y 0,40, la [[entropia]] de la distribución indica cuánto se concentra la ocupación hacia el estado A. Si ambas probabilidades fueran 0,50, la [[entropia]] sería máxima para un sistema de dos estados. El valor de [[constante_boltzmann]] convierte el argumento adimensional en julios por kelvin cuando se trabaja con [[temperatura]] física real.
...
El resultado indica equilibrio estadístico porque las transiciones continúan, pero están compensadas. En cada segundo estadístico sale de A hacia B la misma fracción que entra desde B hacia A. Eso no significa que cada copia permanezca quieta; significa que el conjunto no cambia de forma macroscópica. La [[probabilidad_estado]] mayor de A no contradice el equilibrio, porque la tasa de salida desde A es menor. La compensación entre ocupación y [[tasa_transicion]] elimina el [[flujo_probabilidad]] neto. Nótese que el sistema podría tener la misma distribución de [[probabilidad_estado]] con tasas distintas y el equilibrio seguiría siendo válido, siempre que el producto p_A k_AB igualara p_B k_BA. Eso ilustra que el equilibrio estadístico no fija las tasas individuales, sino solo su relación ponderada con las ocupaciones.
...
Si se quiere estimar la distribución de equilibrio directamente desde la física del material, conviene conocer la diferencia de [[energia_estado]] entre las dos configuraciones del defecto, la [[temperatura]] del entorno y la [[constante_boltzmann]], para calcular [[peso_boltzmann]] mediante el factor de Boltzmann. Si la diferencia de energías es del orden de kT (aproximadamente 25 meV a 300 K), ambas configuraciones tienen occupaciones similares. Si supera varias unidades de kT, la configuración de menor [[energia_estado]] domina y [[probabilidad_estado]] de la configuración excitada puede caer por debajo de 0,05.

En metales y semiconductores, el equilibrio estadístico de defectos puntuales controla la conductividad eléctrica a temperaturas moderadas. La misma lógica de [[flujo_probabilidad]] nulo se aplica a la distribución de portadores entre bandas de conducción y de valencia cuando la [[temperatura]] del cristal es estable. Detectar el desbalance con precisión requiere muestras estadísticas amplias: con 1000 observaciones independientes, el error relativo en [[probabilidad_estado]] es del orden de 3%.
