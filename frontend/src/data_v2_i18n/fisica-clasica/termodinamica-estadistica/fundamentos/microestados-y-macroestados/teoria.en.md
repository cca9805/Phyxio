...
A [[microestado]] gives the fine microscopic detail of the system. A [[macroestado]] gives only the observable or controlled description, defined by constraints such as total energy, volume, and particle number. Many different [[microestado]] can share the same macroscopic description without being identical at the microscopic level. The number of microscopic configurations that all produce the same [[macroestado]] is the [[multiplicidad_macroestado]], which determines how statistically prominent that macrostate is within the accessible set.

When one [[macroestado]] has larger [[multiplicidad_macroestado]], it usually has larger [[probabilidad_macroestado]], not because nature chooses it with intention, but because there are more microscopic ways to realize it. [[entropia]] translates that abundance of compatible configurations into a thermodynamic quantity. So the visible state is not a single hidden picture. It is a whole crowd of microscopic pictures wearing the same macroscopic mask.
...
This leaf connects directly to the fundamental hypothesis, where accessible [[microestado]] are treated as equally likely and the dominant [[macroestado]] at equilibrium is the one with the largest [[multiplicidad_macroestado]]. It also prepares the partition function, where [[microestado]] are no longer weighted equally but carry exponential energy factors. [[entropia]] links this counting language to equilibrium, fluctuations, and thermal models. The distinction between [[microestado]] and [[macroestado]] underlies every major ensemble in statistical mechanics.

## Final synthesis

A visible [[macroestado]] can be supported by many hidden [[microestado]]. [[multiplicidad_macroestado]] counts those microscopic routes, [[probabilidad_macroestado]] compares their statistical weight, and [[entropia]] translates their abundance into thermodynamic language. The core insight is that counting compatible microscopic configurations, and grouping them into observable classes, is the conceptual foundation that unifies all of statistical mechanics.