...
The model works well when [[numero_copias_ensemble]] is large, for example greater than 10³ for classroom estimates and far larger in thermodynamic interpretation. Each [[probabilidad_estado]] must satisfy 0 ≤ [[probabilidad_estado]] ≤ 1, and the total sum must be 1 within numerical tolerances below 10⁻⁶. If frequencies are used, [[conteo_sistemas]] must be large enough so [[conteo_sistemas]]/[[numero_copias_ensemble]] is not controlled by one copy. The [[fluctuacion_ensemble]] must be compared with the [[valor_medio_ensemble]]: if [[fluctuacion_ensemble]]/[[valor_medio_ensemble]] is of order 1, the mean needs a spread statement.
