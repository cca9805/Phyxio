...
The following quantities define the calculation. The chain contains four binary spin variables with a ferromagnetic coupling constant, a positive external field, and a nonzero temperature that introduces thermal competition between ordered and disordered configurations.

- [[espin]] configuration: +1, +1, -1, +1
- [[numero_espines]]: 4
- [[acoplamiento]]: positive (ferromagnetic, favors alignment)
- [[campo_externo]]: positive (biases toward positive spins)
- [[temperatura]]: nonzero (finite thermal agitation)
- [[constante_boltzmann]]: thermal conversion scale between energy and temperature
...
The practical value of the model is that it shows how a simple microscopic rule can generate collective behavior. A falling [[magnetizacion_media]] with increasing [[temperatura]] indicates a shift in statistical weights, not a mechanical force that randomly pushes arrows. Configurations with larger [[energia_ising]] become less suppressed. The Ising model is therefore a compact laboratory for magnetism, thermal disorder, and phase-transition reasoning. At low [[temperatura]], statistical weight concentrates on aligned configurations and [[magnetizacion_media]] saturates near its extreme values, because [[beta_termica]] is large and [[probabilidad_configuracion]] strongly favors low-energy states. At high [[temperatura]], many configurations carry comparable statistical weight and [[magnetizacion_media]] fluctuates near zero. The curve connecting these two limits traces the complete statistical story of the lattice under the competition between [[acoplamiento]] and thermal agitation measured by [[constante_boltzmann]] times [[temperatura]].