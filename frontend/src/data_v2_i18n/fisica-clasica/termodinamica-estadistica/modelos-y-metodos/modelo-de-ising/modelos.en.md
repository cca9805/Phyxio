...
One may extend Ising with random couplings, local fields, three-dimensional lattices, or specific boundary conditions. If spins have continuous orientations, an XY or Heisenberg model is more appropriate. If time evolution matters, a kinetic rule or Monte Carlo dynamics is required. The model can also include second-neighbor interactions, which extend the range over which [[energia_ising]] accumulates. In all these extensions, the core reading of [[magnetizacion_media]] and [[probabilidad_configuracion]] is preserved structurally, but the [[temperatura]] scale where collective order appears can shift or split relative to the nearest-neighbor ideal. The [[beta_termica]] comparison between coupling strength and thermal energy remains the primary scale for deciding which regime the system occupies and whether the extended coupling creates competing ordered phases or merely shifts the critical point.
