...
export const DCL_SCENE_IDS = Object.freeze({
  ...
  TWO_BODIES_HORIZONTAL: "two_bodies_horizontal",
...
export const DCL_SCENES = Object.freeze({
  ...
  [DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL]: {
    ...
    label: makeBilingual("Dos cuerpos en superficie horizontal", "Two bodies on horizontal surface"),
    description: makeBilingual(
      "Dos cuerpos sobre la misma superficie horizontal, ideal para colisiones e interacciones laterales.",
      "Two bodies on the same horizontal surface, ideal for collisions and lateral interactions."
    ),
    bodyKinds: [DCL_BODY_KIND_IDS.BLOCK, DCL_BODY_KIND_IDS.MASS],
    supportsSurface: true,
    supportsRope: false,
...
function normalizeOptions(options = {}) {
  ...
  return {
    ...
    resolveComponents: ensureBoolean(
      safe.resolveComponents,
      DEFAULT_OPTIONS.resolveComponents
    ),
    resolveResultant: ensureBoolean(
      safe.resolveResultant,
      DEFAULT_OPTIONS.resolveResultant
    ),
    showStepControls: ensureBoolean(
      safe.showStepControls,
      DEFAULT_OPTIONS.showStepControls
    ),
    resolutionStep: ensureNumber(
      safe.resolutionStep,
      DEFAULT_OPTIONS.resolutionStep
    ),
    snapVectorDirections: ensureBoolean(
      safe.snapVectorDirections,
      DEFAULT_OPTIONS.snapVectorDirections
    ),

    displayMode: ensureEnum(
      safe.displayMode,
      ["student", "teacher"],
      DEFAULT_OPTIONS.displayMode
    ),
    enableVectorSelection: ensureBoolean(
      safe.enableVectorSelection,
      DEFAULT_OPTIONS.enableVectorSelection
    ),
    showCompactLegend: ensureBoolean(
      safe.showCompactLegend,
      DEFAULT_OPTIONS.showCompactLegend
    ),
    showTypicalErrors: ensureBoolean(
      safe.showTypicalErrors,
      DEFAULT_OPTIONS.showTypicalErrors
    ),
    showTheoryLink: ensureBoolean(
      safe.showTheoryLink,
      DEFAULT_OPTIONS.showTheoryLink
    ),
  };
}
...
function normalizeInterpretation(interpretation = {}) {
  ...
  return {
    ...
    physical_insight: normalizeBilingualText(safe.physical_insight, "", ""),
    ...
    step_descriptions: Array.isArray(safe.step_descriptions) ? safe.step_descriptions : [],
    next_steps: isPlainObject(safe.next_steps)
      ? {
          es: Array.isArray(safe.next_steps.es) ? safe.next_steps.es : [],
          en: Array.isArray(safe.next_steps.en) ? safe.next_steps.en : [],
        }
      : {
          es: [],
          en: [],
...
function normalizeBody(body = {}) {
  ...
  const normalized = {
    ...
    // El rol se asigna automáticamente en la normalización de perfil para escenas de dos cuerpos
    role: ensureString(body.role),
    anchor: ensureEnum(
      body.anchor,
    ...
    labelYOffset: ensureNumber(body.labelYOffset ?? body.style?.labelYOffset, 0),
    style: {
      ...DEFAULT_BODY_STYLE,
...
function normalizeVector(vector = {}) {
  ...
  const normalized = {
    ...
    anchorYOffset: ensureNumber(vector.anchorYOffset, 0),
...
export function normalizeDclProfile(input = {}) {
  ...
  let bodies = ensureArray(input.bodies).map(normalizeBody);
  // Asignación automática de roles para escenas de dos cuerpos horizontales
  if (scene === DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL && bodies.length === 2) {
    bodies = [
      { ...bodies[0], role: "main" },
      { ...bodies[1], role: "secondary" },
    ];
  }
  const vectors = ensureArray(input.vectors).map(normalizeVector);
  const labels = ensureArray(input.labels).map(normalizeLabel);

  // Opciones automáticas para escenas de dos cuerpos horizontales
  let options = normalizeOptions(input.options);
  if (scene === DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL) {
    options = {
      ...options,
      enableVectorSelection: true,
      showCompactLegend: true,
  ...
  const profile = {
    ...
    theory_link: isPlainObject(input.theory_link)
      ? {
          sectionId: ensureString(input.theory_link.sectionId),
          conceptId: ensureString(input.theory_link.conceptId),
          label: normalizeBilingualText(input.theory_link.label, "", ""),
        }
      : null,
    options,
  };
