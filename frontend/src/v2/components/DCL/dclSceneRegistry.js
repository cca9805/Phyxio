...
function getSceneSurfaceBasis(sceneId, environment = {}) {
  if (sceneId === DCL_SCENE_IDS.BLOCK_ON_INCLINE) {
    ...
    // Tangente: paralelo al plano, hacia arriba-derecha en SVG
    // Dirección del plano: (cos(θ), -sin(θ))
    const tangentPositive = norm(Math.cos(theta), -Math.sin(theta));
    const tangentNegative = scaleVec(tangentPositive, -1);
    
    // Normal: perpendicular al plano (rotación 90° horaria del tangente)
    // Rotación horaria: (x, y) → (y, -x)
    // (cos(θ), -sin(θ)) → (-sin(θ), -cos(θ))
    // Verificación dot product: cos(θ)*(-sin(θ)) + (-sin(θ))*(-cos(θ)) = -cos(θ)sin(θ) + sin(θ)cos(θ) = 0 ✓
...
function getBodyGeometryKey(sceneId, body) {
  ...
  if (sceneId === DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL) {
    if (body.role === "main" || body.id === "body_left") return "left";
    if (body.role === "secondary" || body.id === "body_right") return "right";
  }
...
function buildBlockOnInclineGeometry({ width, height, environment }) {
  ...
  // Calcular desplazamiento perpendicular para que el bloque apoye sobre el plano
  // Vector normal hacia afuera del plano: (-sin(θ), -cos(θ))
  const blockHeight = 48;
  const perpOffset = blockHeight / 2;
  
  const block = {
    cx: center.x + perpOffset * (-Math.sin(theta)),
    cy: center.y + perpOffset * (-Math.cos(theta)),
    w: 92,
    h: blockHeight,
    angleDeg: -thetaDeg,  // Negativo para alinear con el plano en SVG
  };
...
function buildTwoBodiesHorizontalGeometry({ width, height }) {
  const tableY = height * 0.68;
  const bodyW = 104;
  const bodyH = 52;
  const gap = 60;
...
function resolveBodyAnchor({ sceneId, body, geometry, anchor }) {
  ...
  if (sceneId === DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL) {
    const g = getBodyGeometry(sceneId, body, geometry);
    if (!g) return { x: 0, y: 0 };
    return resolveRectAnchor(g, anchor);
  }
...
function getSemanticRoleText(vector, lang) {
  ...
  const map = {
    ...
    fictitious_cause: {
      es: "Fuerza ficticia (sin agente físico)",
      en: "Fictitious force (no physical agent)",
    },
    absolute_motion: {
      es: "Aceleración absoluta (sistema S)",
      en: "Absolute acceleration (frame S)",
    },
    frame_motion: {
      es: "Aceleración del marco (sistema S')",
      en: "Frame acceleration (frame S')",
    },
    relative_motion: {
    ...
    state_descriptor: {
      es: "Descriptor de estado cinemático",
      en: "Kinematic state descriptor",
    },
  };
...
function buildContactConsistency(profile) {
  const hasContactScene =
    ...
    profile.scene === DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL ||
...
function buildGraphInterpretationPayload({
  ...
  const buildLegendItem = (vectorLike, resolved = false) => {
    const label =
      vectorLike.localizedLabel ||
      getLocalizedText(vectorLike.label, lang, vectorLike.symbol || vectorLike.id);
    const bodyLabel = bodyRuntimeMap?.[vectorLike.bodyId]
      ? getBodyName(profile.bodies.find((b) => b.id === vectorLike.bodyId), lang)
      : null;

    return {
      id: vectorLike.id,
      label,
      symbol: vectorLike.symbol || null,
      bodyId: vectorLike.bodyId || null,
      bodyLabel,
      magnitudeId: vectorLike.magnitudeId || null,
      scalarValue: resolved ? vectorLike.scalarValue : null,
      displayUnit: resolved ? vectorLike.displayUnit || null : null,
      roleText: vectorLike.roleText || getSemanticRoleText(vectorLike, lang),
      derived: !!vectorLike.derived,
      derivedKind: vectorLike.derivedKind ?? null,
    };
  };

  // Visibles: desde el modelo ya resuelto para render
  const visibleVectors = (vectorsResolved || []).filter(
    (v) => v.visible !== false && v.style?.visible !== false
  );

  // Ocultas: desde el profile original
  const hiddenVectors = (profile.vectors || []).filter(
    (v) => v.visible === false || v.style?.visible === false
  );

  const activeForces = unique(
    visibleVectors.map((v) =>
      v.localizedLabel ||
      getLocalizedText(v.label, lang, v.symbol || v.id)
    )
  );

  const inactiveForces = unique(
    hiddenVectors.map((v) =>
      getLocalizedText(v.label, lang, v.symbol || v.id)
    )
  );

  const activeForceDetails = visibleVectors.map((v) => buildLegendItem(v, true));
  const inactiveForceDetails = hiddenVectors.map((v) => buildLegendItem(v, false));
  const equilibriumHints = buildEquilibriumHints(vectorsResolved, profile, lang);
  const graphWarnings = buildGraphWarnings(profile, geometry, vectorsResolved);
  ...
  return {
    ...
    compactLegend: {
      ...
      visibleForces: activeForceDetails,
      hiddenForces: inactiveForceDetails,
      equilibriumSummary: equilibriumHints,
      graphWarnings,
    },
    resolutionSteps,
    currentStep,
...
export const DCL_SCENE_REGISTRY = {
  ...
  [DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL]: {
    id: DCL_SCENE_IDS.TWO_BODIES_HORIZONTAL,
    label: {
      es: "Dos cuerpos en superficie horizontal",
      en: "Two bodies on horizontal surface",
    },
    buildGeometry: buildTwoBodiesHorizontalGeometry,
  },
...
export function createSceneModel(profile, runtime = {}) {
  ...
  let vectorsResolved = profile.vectors
    ...
    .map((vector) => {
      ...
      if (vector.anchorYOffset) {
        anchor = { x: anchor.x, y: anchor.y + vector.anchorYOffset };
      }
