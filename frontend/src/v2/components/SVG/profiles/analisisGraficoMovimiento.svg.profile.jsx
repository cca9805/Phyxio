...
const profile = {
  meta: {
    id: "svg-analisis-grafico-del-movimiento",
    leafId: "analisis-grafico-del-movimiento",
    title: {
      es: "Análisis gráfico del movimiento",
  ...
  graph_identity: {
    pregunta_principal: {
      es: "¿Cómo se leen posición, velocidad y aceleración a partir de las gráficas cinemáticas?",
      en: "How are position, velocity and acceleration read from kinematic graphs?",
    },
    magnitud_estrella: "velocidad",
    magnitud_secundaria: "aceleracion",
    variable_control: "tiempo",
    tipo_lectura: "cuantitativa",
  },
  graph_contract: createSvgProfileContract({
    leafId: "analisis-grafico-del-movimiento",
    graphId: "svg-analisis-grafico-del-movimiento",
    purpose: {
      es: "Leer posición, velocidad y aceleración a partir de pendientes y áreas en gráficos cinemáticos.",
      en: "Read position, velocity and acceleration from slopes and areas on kinematic graphs.",
    },
    level: {
      es: "ESO | bachillerato | universidad inicial",
      en: "secondary | high school | early undergraduate",
    },
    expectedInputs: ["x0", "v0", "a", "t"],
    derivedMagnitudes: ["x", "v"],
    targetCandidates: ["x", "v"],
    equations: ["x = x0 + v0*t + 0.5*a*t^2", "v = v0 + a*t"],
    hypotheses: ["aceleración constante", "movimiento rectilíneo"],
    validityDomain: ["velocidades no relativistas"],
    visualSimplifications: ["partícula puntual sobre un eje", "un solo grado de libertad"],
    alwaysVisible: ["x", "v"],
  }),
  ...
  params: {
    ...
    derivations: {
      x: ({ x0, v0, a, t }) =>
        safeNum(x0, 0) +
        safeNum(v0, 4) * safeNum(t, 3) +
        0.5 * safeNum(a, 0.8) * safeNum(t, 3) * safeNum(t, 3),
