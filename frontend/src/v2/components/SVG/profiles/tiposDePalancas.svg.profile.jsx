...
const profile = {
  meta: {
    id: "svg-tipos-de-palancas",
    leafId: "tipos-de-palancas",
    title: {
      es: "Tipos de palancas",
      en: "Types of Levers",
    },
    subtitle: {
      es: "Orden entre apoyo, potencia y resistencia",
      en: "Order among fulcrum, effort, and load",
    },
  },
  graph_identity: {
    pregunta_principal: {
      es: "¿Cómo clasifica el orden de apoyo, potencia y resistencia la clase de palanca y su ventaja mecánica?",
      en: "How does the order of fulcrum, effort, and load classify the lever class and its mechanical advantage?",
    },
    magnitud_estrella: "VM",
    magnitud_secundaria: "clase_palanca",
    variable_control: "clase_palanca",
  ...
  infoCards: [
    ...
    },
    {
      ...
      lines: ({ p, lang }) => [
        `${lang === "en" ? "Class" : "Clase"} = ${classText(Math.round(safeNum(p?.clase_palanca, 1)), lang)}`,
  ...
  render: ({ p, lang = "es", toNum }) => {
    const isEs = lang !== "en";
    const clase = clamp(Math.round(toNum(p?.clase_palanca, 1)), 1, 3);
  ...
  interpretation_binding: {
    paramsIn: ["clase_palanca", "d_P", "d_R", "R"],
    stateOut: ["VM", "P"],
  },
};
