...
const graphIdentity = {
  id: "tornillo",
  type: "Coord",
  magnitudes: ["p", "F", "ventaja_mecanica", "R", "r", "tau", "eta"],
  pregunta_principal: {
...
function TornilloGraphsCoordView(props) {
  void graphIdentity;
  ...
  const schema = useMemo(
    () => [
      ...
      { key: "p", aliases: ["paso"], default: toNum(p.p ?? p.paso, 0.006) },
  ...
  const p_val = clamp(toNum(ep.p, 0.006), 0.0005, 0.05);
  ...
  const Fideal = forceIdeal(R, p_val, r);
  const Freal = Fideal / eta;
  const VM = vmIdeal(r, p_val);
  const tau = Freal * r;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Tornillo (Coord)", "Screw (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>p</span><input className="form-control" style={{ width: 95 }} value={fmt(p_val, 4).replace(",", ".")} onChange={(e) => setValue("p", e.target.value)} /></div>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>{tt("eta", "eta")}</span><input className="form-control" style={{ width: 95 }} value={fmt(eta, 3).replace(",", ".")} onChange={(e) => setValue("eta", e.target.value)} /></div>
            <button
              type="button"
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(2,132,199,0.22)", background: "rgba(2,132,199,0.10)" }}>
            <Pill active={showIdeal} onClick={() => setValue("showIdeal", !showIdeal)}>{tt("F ideal", "F ideal")}</Pill>
            <Pill active={showReal} onClick={() => setValue("showReal", !showReal)}>{tt("F real", "F real")}</Pill>
            <Pill active={showVM} onClick={() => setValue("showVM", !showVM)}>{tt("VM ideal", "VM ideal")}</Pill>
          </div>

          <div className="v2-panel" style={{ padding: 12, fontSize: 13, opacity: 0.9 }}>
            F_ideal = <b>{fmt(Fideal, 4)}</b> N | F_real = <b>{fmt(Freal, 4)}</b> N | VM_ideal = <b>{fmt(VM, 3)}</b> {tt("| torque =", "| torque =")} <b>{fmt(tau, 4)}</b> N*m
          </div>
        </div>
      </div>

      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Efecto del paso de rosca", "Thread-pitch effect")}</div>
          ...
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showVM ? <Line yAxisId="right" type="monotone" dataKey="VM" name="VM ideal" stroke="#22c55e" strokeWidth={3} dot={false} /> : null}
               {showReal ? <ReferenceDot yAxisId="left" x={p_val} y={Freal} r={5} fill="#fb923c" stroke="none" /> : null}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
...
const TornilloGraphsCoord = createCoordGraph({
  Component: TornilloGraphsCoordView,
  displayName: "TornilloGraphsCoord",
});

TornilloGraphsCoord.graphContract = TornilloGraphsCoordView.graphContract;
TornilloGraphsCoord.graph_identity = TornilloGraphsCoordView.graph_identity;
TornilloGraphsCoord.axes = TornilloGraphsCoordView.axes;
TornilloGraphsCoord.formula = TornilloGraphsCoordView.formula;
TornilloGraphsCoord.profile = TornilloGraphsCoordView.profile;

export default TornilloGraphsCoord;
