...
export default function CollisionGraphsCoordBase({ title, params, type = "elastic" }) {
  ...
  const data = [
    { s: 0, p1: m1 * v1i, p2: m2 * v2i, pTot: pI, K: kI },
    { s: 1, p1: m1 * v1f, p2: m2 * v2f, pTot: pF, K: kF },
  ];
  ...
  const formula = type === "elastic"
    ? "\\sum p_i=\\sum p_f\\;\\wedge\\;K_i=K_f"
    : type === "inelastic"
      ? "\\sum p_i=\\sum p_f,\\;K_f<K_i"
      : "\\sum p_i=\\sum p_f,\\;v_{1f}=v_{2f}";

  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title}</div>
        <div className="muted" style={{ marginTop: 6 }}><MathInline latex={formula} /></div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            ...
            {type !== "tot" ? (
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <span style={{ fontSize: 13, opacity: 0.85 }}>e</span>
                <input className="form-control" style={{ width: 80 }} value={e} onChange={(e2) => setValue("e", e2.target.value)} />
              </div>
            ) : null}
          ...
          <div className="v2-panel" style={{ marginTop: 10, padding: 10, fontSize: 13 }}>
            v1f=<b>{fmt(v1f, 4)}</b>, v2f=<b>{fmt(v2f, 4)}</b> | p_i=<b>{fmt(pI, 4)}</b>, p_f=<b>{fmt(pF, 4)}</b> | K_i=<b>{fmt(kI, 4)}</b>, K_f=<b>{fmt(kF, 4)}</b>
          </div>
        </div>
      </div>

      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">Estados inicial y final</div>
        <div style={{ width: "100%", height: 340 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showK ? <Line type="linear" dataKey="K" stroke="#ef4444" name="K_total" strokeWidth={2.4} dot /> : null}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
