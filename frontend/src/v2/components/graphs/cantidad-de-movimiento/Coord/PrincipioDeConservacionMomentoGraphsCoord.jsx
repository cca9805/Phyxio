...
const _graph_meta = {
  pregunta_principal: {
    es: "¿Cuáles son las condiciones para que el momento lineal total se conserve en un sistema con fuerzas externas?",
    en: "What are the conditions for total linear momentum to be conserved in a system with external forces?",
  },
};
...
function PrincipioDeConservacionMomentoGraphsCoordView({ title, lang = "es", params, sharedParams }) {
  const uiLang = lang === "en" ? "en" : "es";
  const t = (es, en) => (uiLang === "en" ? en : es);

  const schema = useMemo(
    () => [
      { key: "m1", default: toNum((params ?? sharedParams)?.m1, 2) },
      { key: "m2", default: toNum((params ?? sharedParams)?.m2, 3) },
      { key: "v1i", default: toNum((params ?? sharedParams)?.v1i, 4) },
      { key: "v2i", default: toNum((params ?? sharedParams)?.v2i, -1) },
      { key: "v1f", default: toNum((params ?? sharedParams)?.v1f, 1.5) },
      { key: "showP1", default: (params ?? sharedParams)?.showP1 ?? true },
      { key: "showP2", default: (params ?? sharedParams)?.showP2 ?? true },
      { key: "showPTot", default: (params ?? sharedParams)?.showPTot ?? true },
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || t("Principio de conservación (momento lineal)", "Conservation principle (linear momentum)")}</div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t("Comparación inicial/final", "Initial/final comparison")}</div>
...
const PrincipioDeConservacionMomentoGraphsCoord = createCoordGraph({
  Component: PrincipioDeConservacionMomentoGraphsCoordView,
  displayName: "PrincipioDeConservacionMomentoGraphsCoord",
});

PrincipioDeConservacionMomentoGraphsCoord.graphContract = PrincipioDeConservacionMomentoGraphsCoordView.graphContract;
PrincipioDeConservacionMomentoGraphsCoord.graph_identity = PrincipioDeConservacionMomentoGraphsCoordView.graph_identity;
PrincipioDeConservacionMomentoGraphsCoord.axes = PrincipioDeConservacionMomentoGraphsCoordView.axes;
PrincipioDeConservacionMomentoGraphsCoord.formula = PrincipioDeConservacionMomentoGraphsCoordView.formula;
PrincipioDeConservacionMomentoGraphsCoord.profile = PrincipioDeConservacionMomentoGraphsCoordView.profile;

export default PrincipioDeConservacionMomentoGraphsCoord;
