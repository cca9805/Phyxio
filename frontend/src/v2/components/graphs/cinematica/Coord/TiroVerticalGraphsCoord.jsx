...
function TiroVerticalGraphsCoordView({ title, params }) {
  ...
  const yObs = Math.max(0, y0 + v0 * tEff - 0.5 * g * tEff * tEff);
  const vObs = v0 - g * tEff;
  const tPeak = Math.max(0, v0 / g);
  const yPeak = y0 + (v0 * v0) / (2 * g);
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i <= N; i += 1) {
      ...
      out.push({
        t,
        y: Math.max(0, y0 + v0 * tc - 0.5 * g * tc * tc),
        v: v0 - g * tc,
        a: -g,
      });
    }
    return out;
  }, [tMax, tFlight, y0, v0, g]);
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || "Tiro vertical (Coord)"}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"y(t)=y_0+v_0t-\\tfrac{1}{2}gt^2,\\; v(t)=v_0-gt"} />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              t_vuelo = <b>{fmt(tFlight, 3)}</b> s | t_pico = <b>{fmt(tPeak, 3)}</b> s | y_max = <b>{fmt(yPeak, 3)}</b> m | y(t) = <b>{fmt(yObs, 3)}</b> m | v(t) = <b>{fmt(vObs, 3)}</b> m/s
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">Evolucion temporal (tiro vertical)</div>
        <div style={{ width: "100%", height: 360, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showY ? <Line type="monotone" dataKey="y" name="y(t)" stroke="#34d399" strokeWidth={3} dot={false} /> : null}
              {showV ? <Line type="monotone" dataKey="v" name="v(t)" stroke="#60a5fa" strokeWidth={2.8} dot={false} /> : null}
              {showA ? <Line type="monotone" dataKey="a" name="a(t)" stroke="#f97316" strokeWidth={2.6} dot={false} /> : null}
              {showPoint && showY ? <ReferenceDot x={tEff} y={yObs} r={5} fill="#34d399" stroke="none" /> : null}
              {showPoint && showV ? <ReferenceDot x={tEff} y={vObs} r={5} fill="#60a5fa" stroke="none" /> : null}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
...
const TiroVerticalGraphsCoord = createCoordGraph({
  Component: TiroVerticalGraphsCoordView,
  displayName: "TiroVerticalGraphsCoord",
});

TiroVerticalGraphsCoord.graphContract = TiroVerticalGraphsCoordView.graphContract;
TiroVerticalGraphsCoord.graph_identity = TiroVerticalGraphsCoordView.graph_identity;
TiroVerticalGraphsCoord.axes = TiroVerticalGraphsCoordView.axes;
TiroVerticalGraphsCoord.formula = TiroVerticalGraphsCoordView.formula;
TiroVerticalGraphsCoord.profile = TiroVerticalGraphsCoordView.profile;

export default TiroVerticalGraphsCoord;
