...
function VelocidadRelativaGraphsCoordView({ title, params }) {
  const schema = useMemo(
    () => [
      ...
      { key: "showVA", default: params?.showVA ?? true },
      { key: "showVB", default: params?.showVB ?? true },
      { key: "showVRel", default: params?.showVRel ?? true },
  ...
  const showVA = !!ep.showVA;
  const showVB = !!ep.showVB;
  const showVRel = !!ep.showVRel;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || "Velocidad relativa (Coord)"}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"\\vec v_{A/B}=\\vec v_A-\\vec v_B"} />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(2,132,199,0.22)", background: "rgba(2,132,199,0.10)" }}>
            <Pill active={showVA} onClick={() => setValue("showVA", !showVA)}>vA(t)</Pill>
            <Pill active={showVB} onClick={() => setValue("showVB", !showVB)}>vB(t)</Pill>
            <Pill active={showVRel} onClick={() => setValue("showVRel", !showVRel)}>vA/B(t)</Pill>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              t_obs = <b>{fmt(tObs, 2)}</b> s | vA = <b>{fmt(vA, 3)}</b> m/s | vB = <b>{fmt(vB, 3)}</b> m/s | vA/B = <b>{fmt(vRel, 3)}</b> m/s
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">Comparacion de velocidades</div>
        <div style={{ width: "100%", height: 340, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showVA ? <Line type="monotone" dataKey="vA" name="vA" stroke="#60a5fa" strokeWidth={3} dot={false} /> : null}
              {showVB ? <Line type="monotone" dataKey="vB" name="vB" stroke="#34d399" strokeWidth={2.6} dot={false} /> : null}
              {showVRel ? <Line type="monotone" dataKey="vRel" name="vA/B" stroke="#f97316" strokeWidth={2.6} dot={false} /> : null}
              {showPoint && showVA ? <ReferenceDot x={tObs} y={vA} r={5} fill="#60a5fa" stroke="none" /> : null}
              {showPoint && showVB ? <ReferenceDot x={tObs} y={vB} r={5} fill="#34d399" stroke="none" /> : null}
              {showPoint && showVRel ? <ReferenceDot x={tObs} y={vRel} r={5} fill="#f97316" stroke="none" /> : null}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
...
const VelocidadRelativaGraphsCoord = createCoordGraph({
  Component: VelocidadRelativaGraphsCoordView,
  displayName: "VelocidadRelativaGraphsCoord",
});

VelocidadRelativaGraphsCoord.graphContract = VelocidadRelativaGraphsCoordView.graphContract;
VelocidadRelativaGraphsCoord.graph_identity = VelocidadRelativaGraphsCoordView.graph_identity;
VelocidadRelativaGraphsCoord.axes = VelocidadRelativaGraphsCoordView.axes;
VelocidadRelativaGraphsCoord.formula = VelocidadRelativaGraphsCoordView.formula;
VelocidadRelativaGraphsCoord.profile = VelocidadRelativaGraphsCoordView.profile;

export default VelocidadRelativaGraphsCoord;
