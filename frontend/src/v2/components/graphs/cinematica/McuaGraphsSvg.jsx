...
export default function McuaGraphsSvg({ title, params }) {
  ...
  const radialNorm = Math.hypot(cx - x, cy - y) || 1;
  const nx = (cx - x) / radialNorm;
  const ny = (cy - y) / radialNorm;

  const velLen = Math.min(160, 35 + Math.abs(omega * R) * 14);
  const atLen = Math.min(120, 25 + Math.abs(alpha * R) * 14);
  const acLen = Math.min(140, 30 + Math.abs(omega * omega * R) * 8);
    ...
    <div style={{ display: "grid", gap: 10 }}>
      ...
      <div className="v2-panel m-1" style={{ background: "rgba(0,0,0,0.45)" }}>
        ...
        <div className="v2-toggle-row">
          ...
          <Toggle checked={showAt} onChange={(e) => setValue("showAt", e.target.checked)} label="Tangencial at" />
          <Toggle checked={showAc} onChange={(e) => setValue("showAc", e.target.checked)} label="Centripeta ac" />
      ...
      <div className="v2-panel m-1" style={{ background: "rgba(0,0,0,0.35)" }}>
        <div style={{ fontSize: 13, lineHeight: 1.6 }}>
          ...
          <div>at: <b>{fmt(Math.abs(alpha) * R, 3)} m/s2</b></div>
  ...
  return (
    ...
    >
      ...
      {showVel ? <Arrow x={x} y={y} dx={tx * velLen} dy={ty * velLen} label={`v=${fmt(Math.abs(omega) * R, 2)}`} labelDy={-12} /> : null}
      {showAt ? <Arrow x={x} y={y} dx={tx * atLen * Math.sign(alpha || 1)} dy={ty * atLen * Math.sign(alpha || 1)} label={`at=${fmt(Math.abs(alpha) * R, 2)}`} labelDy={-12} /> : null}
      {showAc ? <Arrow x={x} y={y} dx={nx * acLen} dy={ny * acLen} label={`ac=${fmt(omega * omega * R, 2)}`} labelDy={-12} /> : null}
    </SvgFrame>
  );
}
