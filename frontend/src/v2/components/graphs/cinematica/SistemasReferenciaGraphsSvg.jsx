...
export default function SistemasReferenciaGraphsSvg({ title, params }) {
    ...
    <div style={{ display: "grid", gap: 10 }}>
      ...
      <div className="v2-panel m-1" style={{ background: "rgba(0,0,0,0.35)" }}>
        <div style={{ fontSize: 13, lineHeight: 1.6 }}>
          <div><MathInline latex={"x'_{A/B}=x_A-x_B"} /></div>
          <div>t: <b>{fmt(tCurrent, 2)} s</b></div>
          <div>xA: <b>{fmt(xA, 3)}</b> m</div>
          <div>xB: <b>{fmt(xB, 3)}</b> m</div>
          <div>xA/B: <b>{fmt(xRel, 3)}</b> m</div>
  ...
  return (
    <SvgFrame
      title={title || "Sistemas de referencia (SVG)"}
      subtitle="Comparacion de marcos A y B"
      badgeLabel="RELACION"
