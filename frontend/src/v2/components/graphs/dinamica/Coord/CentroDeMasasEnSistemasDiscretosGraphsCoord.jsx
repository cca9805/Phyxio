...
function CentroDeMasasEnSistemasDiscretosGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Centro de masas en sistemas discretos (Coord)", "Center of mass in discrete systems (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"x_{CM}=\\frac{\\sum m_i x_i}{\\sum m_i}"} /> {tt("para tres masas puntuales.", "para tres masses puntuales.")}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            x_CM = <b>{fmt(xcm, 3)}</b> m (con m_total = <b>{fmt(m1 + m2 + m3, 2)}</b> {tt("kg).", "kg).")}
          </div>
        </div>
      </div>

      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Barrido de la tercera masa", "Third-mass sweep")}</div>
...
const CentroDeMasasEnSistemasDiscretosGraphsCoord = createCoordGraph({
  Component: CentroDeMasasEnSistemasDiscretosGraphsCoordView,
  displayName: "CentroDeMasasEnSistemasDiscretosGraphsCoord",
});

CentroDeMasasEnSistemasDiscretosGraphsCoord.graphContract = CentroDeMasasEnSistemasDiscretosGraphsCoordView.graphContract;
CentroDeMasasEnSistemasDiscretosGraphsCoord.graph_identity = CentroDeMasasEnSistemasDiscretosGraphsCoordView.graph_identity;
CentroDeMasasEnSistemasDiscretosGraphsCoord.axes = CentroDeMasasEnSistemasDiscretosGraphsCoordView.axes;
CentroDeMasasEnSistemasDiscretosGraphsCoord.formula = CentroDeMasasEnSistemasDiscretosGraphsCoordView.formula;
CentroDeMasasEnSistemasDiscretosGraphsCoord.profile = CentroDeMasasEnSistemasDiscretosGraphsCoordView.profile;

export default CentroDeMasasEnSistemasDiscretosGraphsCoord;
