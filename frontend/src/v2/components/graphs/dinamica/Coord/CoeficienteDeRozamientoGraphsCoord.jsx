...
const graph_identity = {
  pregunta_principal: {
    ...
    en: "How are friction, the normal force, and the friction coefficient related?",
  },
  magnitud_estrella: "mu_s",
  variable_control: "N",
};
void graph_identity;
...
function CoeficienteDeRozamientoGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Coeficiente de rozamiento (Coord)", "Coefficient of friction (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("Identifica", "Identifica")} <MathInline latex={"\\mu = \\frac{f}{N}"} /> {tt("o el umbral en plano inclinado", "o el umbral en plano inclinado")} <MathInline latex={"\\mu_{min}=\\tan\\theta"} />.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={mode === "measure"} onClick={() => setMode("measure")}>{tt("f vs N", "f vs N")}</Pill>
          ...
          >
            <Pill active={showModel} onClick={() => setShowModel(!showModel)}>{tt("Curva", "Curve")}</Pill>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="muted" style={{ marginBottom: 10 }}>
          {mode === "measure" ? (
            <>{tt("mu(modelo) =", "mu(modelo) =")} <b>{fmt(mu, 3)}</b>{tt(", mu(f/N) =", ", mu(f/N) =")} <b>{fmt(muFromPair, 3)}</b></>
          ) : (
...
const CoeficienteDeRozamientoGraphsCoord = createCoordGraph({
  Component: CoeficienteDeRozamientoGraphsCoordView,
  displayName: "CoeficienteDeRozamientoGraphsCoord",
});

CoeficienteDeRozamientoGraphsCoord.graphContract = CoeficienteDeRozamientoGraphsCoordView.graphContract;
CoeficienteDeRozamientoGraphsCoord.graph_identity = CoeficienteDeRozamientoGraphsCoordView.graph_identity;
CoeficienteDeRozamientoGraphsCoord.axes = CoeficienteDeRozamientoGraphsCoordView.axes;
CoeficienteDeRozamientoGraphsCoord.formula = CoeficienteDeRozamientoGraphsCoordView.formula;
CoeficienteDeRozamientoGraphsCoord.profile = CoeficienteDeRozamientoGraphsCoordView.profile;

export default CoeficienteDeRozamientoGraphsCoord;
