...
function buildData(mu, g, thetaMaxRad, n = 220) {
  const out = [];
  for (let i = 0; i <= n; i += 1) {
    const theta = (thetaMaxRad * i) / n;
    out.push({ theta, a: g * (Math.sin(theta) - mu * Math.cos(theta)) });
  }
  return out;
}
...
const graphIdentity = {
  pregunta_principal: {
    es: "Como cambia la aceleracion con el angulo theta en la curva a(theta) y donde aparece la linea del angulo critico theta_c con rozamiento",
    en: "How acceleration changes with angle theta on the a(theta) curve and where the critical-angle line theta_c appears with friction",
  },
};

function ConRozamientoGraphsCoordView(props) {
  ...
  const schema = useMemo(
    ...
    [p.mu, p.coef, p.coeficiente, p.g, p.gravedad, p.thetaDeg, p.theta, p.angulo, p.thetaMaxDeg, p.thetaMax, p.anguloMax, p.showCurve, p.showPoint, p.showRef]
  );
  ...
  const aNow = g * (Math.sin(theta) - mu * Math.cos(theta));
  const thetaCrit = mu >= 0 ? Math.atan(mu) : 0;
  const data = useMemo(() => buildData(mu, g, thetaMax), [mu, g, thetaMax]);
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Plano inclinado con rozamiento (Coord)", "Inclined plane with friction (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <input className="form-control" style={{ width: 86 }} value={fmt(mu, 3).replace(",", ".")} onChange={(e) => setValue("mu", e.target.value)} />
            </div>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              ...
              <br />
              Angulo critico (a=0): theta_c = arctan(mu) = <b>{fmt((thetaCrit * 180) / Math.PI, 3)}</b> {tt("deg.", "deg.")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Aceleracion vs angulo con rozamiento", "Acceleration vs angle with friction")}</div>
        <div style={{ width: "100%", height: 350, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showRef && thetaCrit <= thetaMax ? <ReferenceLine x={thetaCrit} stroke="rgba(245,158,11,0.9)" strokeDasharray="4 4" /> : null}
...
const ConRozamientoGraphsCoord = createCoordGraph({
  Component: ConRozamientoGraphsCoordView,
  displayName: "ConRozamientoGraphsCoord",
});

ConRozamientoGraphsCoord.graphContract = ConRozamientoGraphsCoordView.graphContract;
ConRozamientoGraphsCoord.graph_identity = ConRozamientoGraphsCoordView.graph_identity;
ConRozamientoGraphsCoord.axes = ConRozamientoGraphsCoordView.axes;
ConRozamientoGraphsCoord.formula = ConRozamientoGraphsCoordView.formula;
ConRozamientoGraphsCoord.profile = ConRozamientoGraphsCoordView.profile;

export default ConRozamientoGraphsCoord;
