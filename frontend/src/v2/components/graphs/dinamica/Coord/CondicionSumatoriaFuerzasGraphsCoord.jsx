...
const graphIdentity = {
  pregunta_principal: {
    es: "¿Cómo verificar en Coord la condición de sumatoria de fuerzas leyendo sumFx, sumFy y la resultante R para confirmar equilibrio traslacional?",
    en: "How can we verify in Coord the force-balance condition by reading sumFx, sumFy, and the resultant R to confirm translational equilibrium?",
  },
};

function CondicionSumatoriaFuerzasGraphsCoordView(props) {
  void graphIdentity;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Condicion de sumatoria de fuerzas (Coord)", "Net-force condition (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("Equilibrio traslacional:", "Equilibrio traslacional:")} <MathInline latex={"\\sum F_x=0,\\;\\sum F_y=0"} />.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.85 }}>{tt("theta", "theta")}</span>
          ...
          >
            ...
            <Pill active={showComponents} onClick={() => setValue("showComponents", !showComponents)}>{tt("sumFx/sumFy", "sumFx/sumFy")}</Pill>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.85, lineHeight: 1.5 }}>
              ...
              <br />
              {tt("Equilibrante:", "Equilibrante:")} <MathInline latex={"\\vec F_3=-\\vec R"} /> {"->"} F3x = <b>{fmt(F3x, 3)}</b> N, F3y = <b>{fmt(F3y, 3)}</b> N.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Variacion con el angulo theta", "Variation with angle theta")}</div>
...
const CondicionSumatoriaFuerzasGraphsCoord = createCoordGraph({
  Component: CondicionSumatoriaFuerzasGraphsCoordView,
  displayName: "CondicionSumatoriaFuerzasGraphsCoord",
});

CondicionSumatoriaFuerzasGraphsCoord.graphContract = CondicionSumatoriaFuerzasGraphsCoordView.graphContract;
CondicionSumatoriaFuerzasGraphsCoord.graph_identity = CondicionSumatoriaFuerzasGraphsCoordView.graph_identity;
CondicionSumatoriaFuerzasGraphsCoord.axes = CondicionSumatoriaFuerzasGraphsCoordView.axes;
CondicionSumatoriaFuerzasGraphsCoord.formula = CondicionSumatoriaFuerzasGraphsCoordView.formula;
CondicionSumatoriaFuerzasGraphsCoord.profile = CondicionSumatoriaFuerzasGraphsCoordView.profile;

export default CondicionSumatoriaFuerzasGraphsCoord;
