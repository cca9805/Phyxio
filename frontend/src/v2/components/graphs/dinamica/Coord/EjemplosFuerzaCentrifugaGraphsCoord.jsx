...
export const graphContract = {
  meta: {
    title: {
      es: "Ejemplos de fuerza centrifuga",
      en: "Examples of centrifugal force",
    },
  },
  graph_identity: {
    pregunta_principal: {
      es: "Como se resuelven y comparan problemas de fuerza centrifuga cuando los datos llegan en rpm, radio o velocidad angular, y que cambia fisicamente al variar omega?",
      en: "How are centrifugal-force problems solved and compared when data are given in rpm, radius, or angular speed, and what changes physically when omega varies?",
    },
...
function EjemplosFuerzaCentrifugaGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Ejemplos de fuerza centrifuga (Coord)", "Examples of centrifugal force (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>{PRESETS[example].desc}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt(
            "Pregunta principal: como pasar de rpm a omega, calcular F_cf y comparar dos escenarios de giro sin perder la lectura fisica del cambio.",
            "Main question: how to move from rpm to omega, calculate F_cf, and compare two spin scenarios without losing the physical reading of the change."
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            <Pill active={example === "carrusel"} onClick={() => setExample("carrusel")}>{tt("Carrusel", "Carousel")}</Pill>
            <Pill active={example === "centrifuga"} onClick={() => setExample("centrifuga")}>{tt("Centrifuga", "Centrifuge")}</Pill>
            <Pill active={example === "curva"} onClick={() => setExample("curva")}>{tt("Curva", "Curve")}</Pill>
            <Pill active={example === "rotor"} onClick={() => setExample("rotor")}>{tt("Rotor", "Rotor")}</Pill>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.85 }}>{tt("omega", "omega")}</span>
...
const EjemplosFuerzaCentrifugaGraphsCoord = createCoordGraph({
  Component: EjemplosFuerzaCentrifugaGraphsCoordView,
  displayName: "EjemplosFuerzaCentrifugaGraphsCoord",
});

EjemplosFuerzaCentrifugaGraphsCoord.graphContract = EjemplosFuerzaCentrifugaGraphsCoordView.graphContract;
EjemplosFuerzaCentrifugaGraphsCoord.graph_identity = EjemplosFuerzaCentrifugaGraphsCoordView.graph_identity;
EjemplosFuerzaCentrifugaGraphsCoord.axes = EjemplosFuerzaCentrifugaGraphsCoordView.axes;
EjemplosFuerzaCentrifugaGraphsCoord.formula = EjemplosFuerzaCentrifugaGraphsCoordView.formula;
EjemplosFuerzaCentrifugaGraphsCoord.profile = EjemplosFuerzaCentrifugaGraphsCoordView.profile;

export default EjemplosFuerzaCentrifugaGraphsCoord;
