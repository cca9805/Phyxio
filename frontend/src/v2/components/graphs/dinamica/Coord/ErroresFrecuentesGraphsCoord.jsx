...
const GRAPH_META = { pregunta_principal: { es: "¿Cómo varía la aceleración del sistema según el error en el cálculo de la normal?", en: "How does system acceleration vary depending on the error in the normal calculation?" } };
...
function ErroresFrecuentesGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t(lang, "Errores frecuentes (Coord)", "Common mistakes (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"a_{ok}=g(\\sin\\theta-\\mu\\cos\\theta)"} /> {t(lang, "vs errores comunes.", "vs common mistakes.")}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>{tt("mu", "mu")}</span><input className="form-control" style={{ width: 88 }} value={fmt(mu, 3).replace(",", ".")} onChange={(e) => setValue("mu", e.target.value)} /></div>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>{tt("theta", "theta")}</span><input className="form-control" style={{ width: 88 }} value={fmt(theta, 1).replace(",", ".")} onChange={(e) => setValue("theta", e.target.value)} /></div>
            <button
              type="button"
            ...
            >
              {t(lang, "Seguir calculadora", "Follow calculator")}
            </button>
          </div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(2,132,199,0.22)", background: "rgba(2,132,199,0.10)" }}>
            <Pill active={showOk} onClick={() => setValue("showOk", !showOk)}>{t(lang, "Correcta", "Correct")}</Pill>
            <Pill active={showSwap} onClick={() => setValue("showSwap", !showSwap)}>{t(lang, "Sin/Cos invertidos", "Sin/Cos swapped")}</Pill>
            <Pill active={showNoFric} onClick={() => setValue("showNoFric", !showNoFric)}>{t(lang, "Sin rozamiento", "No friction")}</Pill>
            <Pill active={showPoint} onClick={() => setValue("showPoint", !showPoint)}>{t(lang, "Punto", "Point")}</Pill>
          </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t(lang, "Comparacion de resultados", "Result comparison")}</div>
        <div style={{ width: "100%", height: 340, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 8, bottom: 10 }}>
              ...
              {showOk ? <Line type="monotone" dataKey="a_ok" name={t(lang, "a_correcta", "a_correct")} stroke="#22c55e" strokeWidth={3} dot={false} /> : null}
              {showSwap ? <Line type="monotone" dataKey="a_swap" name={t(lang, "a_error_sin_cos", "a_sin_cos_error")} stroke="#f97316" strokeWidth={3} dot={false} /> : null}
              {showNoFric ? <Line type="monotone" dataKey="a_nofric" name={t(lang, "a_sin_rozamiento", "a_no_friction")} stroke="#3b82f6" strokeWidth={3} dot={false} /> : null}
...
const ErroresFrecuentesGraphsCoord = createCoordGraph({
  Component: ErroresFrecuentesGraphsCoordView,
  displayName: "ErroresFrecuentesGraphsCoord",
});

ErroresFrecuentesGraphsCoord.graphContract = ErroresFrecuentesGraphsCoordView.graphContract;
ErroresFrecuentesGraphsCoord.graph_identity = ErroresFrecuentesGraphsCoordView.graph_identity;
ErroresFrecuentesGraphsCoord.axes = ErroresFrecuentesGraphsCoordView.axes;
ErroresFrecuentesGraphsCoord.formula = ErroresFrecuentesGraphsCoordView.formula;
ErroresFrecuentesGraphsCoord.profile = ErroresFrecuentesGraphsCoordView.profile;

export default ErroresFrecuentesGraphsCoord;
