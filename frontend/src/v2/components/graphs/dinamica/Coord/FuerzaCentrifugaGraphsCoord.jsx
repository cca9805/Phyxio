...
export const graphContract = {
  meta: {
    title: {
      es: "Definicion de fuerza centrifuga",
      en: "Definition of centrifugal force",
    },
  },
  graph_identity: {
    pregunta_principal: {
      es: "Que significa realmente la fuerza centrifuga dentro de un marco rotatorio, de que magnitudes depende y por que no debe confundirse con la fuerza centripeta real?",
      en: "What does centrifugal force really mean inside a rotating frame, which magnitudes control it, and why must it not be confused with the real centripetal force?",
    },
    magnitud_estrella: "F_cf",
    variable_control: "sharedParams",
  },
  axes: {
    x: "radio r",
    y: "fuerza centrifuga F_cf",
  },
  formula: "F_cf = m omega^2 r = m v^2 / r",
};
...
function FuerzaCentrifugaGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Definicion de fuerza centrifuga (Coord)", "Definition of centrifugal force (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"F_{cf}=\\frac{mv^2}{r}=m\\omega^2r"} />.
        </div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt(
            "Pregunta principal: que significa realmente la fuerza centrifuga en un marco rotatorio, de que magnitudes depende y por que no debe confundirse con la centripeta real.",
            "Main question: what does centrifugal force really mean in a rotating frame, which magnitudes control it, and why must it not be confused with the real centripetal force."
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>{tt("omega", "omega")}</span><input className="form-control" style={{ width: 90 }} value={fmt(omega, 2).replace(",", ".")} onChange={(e) => setValue("omega", e.target.value)} /></div>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              En r = <b>{fmt(rObs, 3)}</b> m: F(v) = <b>{fmt(FvObs, 4)}</b> {tt("N | F(omega) =", "N | F(omega) =")} <b>{fmt(FwObs, 4)}</b> N.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Fuerza centrifuga vs radio", "Centrifugal force vs radius")}</div>
...
const FuerzaCentrifugaGraphsCoord = createCoordGraph({
  Component: FuerzaCentrifugaGraphsCoordView,
  displayName: "FuerzaCentrifugaGraphsCoord",
});

FuerzaCentrifugaGraphsCoord.graphContract = FuerzaCentrifugaGraphsCoordView.graphContract;
FuerzaCentrifugaGraphsCoord.graph_identity = FuerzaCentrifugaGraphsCoordView.graph_identity;
FuerzaCentrifugaGraphsCoord.axes = FuerzaCentrifugaGraphsCoordView.axes;
FuerzaCentrifugaGraphsCoord.formula = FuerzaCentrifugaGraphsCoordView.formula;
FuerzaCentrifugaGraphsCoord.profile = FuerzaCentrifugaGraphsCoordView.profile;

export default FuerzaCentrifugaGraphsCoord;
