...
const graph_identity = {
  pregunta_principal: {
    es: "Como se interpreta la fuerza elastica de un muelle ideal como respuesta restauradora cuando cambia la deformacion x?",
    en: "How do we interpret the elastic force of an ideal spring as a restoring response when deformation x changes?",
  },
  magnitud_estrella: "Fel",
  variable_control: "x",
};
void graph_identity;

function LeyDeHookeGraphsCoordComponent(props) {
  ...
  const derived = useMemo(
    () => ({
      source: "ley-de-hooke",
      target: "Fel",
      question: "restoring_response",
      k,
      x,
      Fel: Fnow,
      Fm: Math.abs(Fnow),
      Uel: Unow,
      criterio_restaurador: Fnow * x <= 0,
      formula: "Fel = -k*x",
    }),
    [Fnow, Unow, k, x]
  );
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Ley de Hooke (Coord)", "Hooke's law (Coord)")}</div>
...
export default createCoordGraph({
  Component: LeyDeHookeGraphsCoordComponent,
  displayName: "LeyDeHookeGraphsCoord",
});
