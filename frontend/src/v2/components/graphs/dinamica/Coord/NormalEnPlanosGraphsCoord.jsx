...
function NormalEnPlanosGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Normal en planos (Coord)", "Normal force on planes (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("En un plano inclinado sin aceleracion perpendicular:", "En un plano inclinado sin acceleration perpendicular:")} <MathInline latex={"N = m g \\cos\\theta"} />.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            ...
            <Pill active={showW} onClick={() => setShowW(!showW)}>{tt("W = mg", "W = mg")}</Pill>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div style={{ fontWeight: 900, opacity: 0.95 }}>{tt("Curvas de fuerza vs angulo", "Force curves vs angle")}</div>
              <div style={{ fontSize: 12, opacity: 0.75 }}>
                theta = <b>{fmt(current.theta, 0)}</b> {tt("deg | N =", "deg | N =")} <b>{fmt(current.N, 2)}</b> N | W = <b>{fmt(current.W, 2)}</b> N
              </div>
            </div>
...
const NormalEnPlanosGraphsCoord = createCoordGraph({
  Component: NormalEnPlanosGraphsCoordView,
  displayName: "NormalEnPlanosGraphsCoord",
});

NormalEnPlanosGraphsCoord.graphContract = NormalEnPlanosGraphsCoordView.graphContract;
NormalEnPlanosGraphsCoord.graph_identity = NormalEnPlanosGraphsCoordView.graph_identity;
NormalEnPlanosGraphsCoord.axes = NormalEnPlanosGraphsCoordView.axes;
NormalEnPlanosGraphsCoord.formula = NormalEnPlanosGraphsCoordView.formula;
NormalEnPlanosGraphsCoord.profile = NormalEnPlanosGraphsCoordView.profile;

export default NormalEnPlanosGraphsCoord;
