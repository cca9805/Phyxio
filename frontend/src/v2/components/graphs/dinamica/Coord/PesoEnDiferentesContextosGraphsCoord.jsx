...
function PesoEnDiferentesContextosGraphsCoordView(props) {
  ...
  const fmt = (n, d = 2) => {
    ...
    return n.toFixed(d).replace(".", decSep);
  };
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Peso en diferentes contextos (Coord)", "Weight in different contexts (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("Compara", "Compara")} <MathInline latex={"W=mg"} /> y <MathInline latex={"N=m(g+a)"} /> {tt("por contexto.", "por contexto.")}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div style={{ fontWeight: 900, opacity: 0.95 }}>{tt("Comparacion por contexto", "Comparison by context")}</div>
              <div style={{ fontSize: 12, opacity: 0.75 }}>
                {tt("Contexto actual:", "Contexto actual:")} <b>{currentCtx.label}</b> | W = <b>{fmt(currentW, 2)}</b> N | N = <b>{fmt(currentN, 2)}</b> N
              </div>
            </div>
          ...
          <div className="v2-panel" style={{ padding: 12, background: "rgba(0,0,0,0.40)" }}>
            <div style={{ fontWeight: 900, opacity: 0.95 }}>{tt("Lectura rapida", "Quick reading")}</div>
            <div style={{ marginTop: 8, fontSize: 13, opacity: 0.84, lineHeight: 1.45 }}>
              ...
              {context === "elevator" ? (
                ...
                <> {tt("Fuera del ascensor, a = 0 y N coincide con W en reposo.", "Fuera del ascensor, a = 0 y N coincide con W en reposo.")}</>
              )}
            </div>
          </div>
        </div>
...
PesoEnDiferentesContextosGraphsCoordView.meta = {
  title: {
    es: "Comparativa de pesos por contexto",
    en: "Weight Comparison by Context",
  },
  pregunta_principal: {
  ...
  magnitud_estrella: "Pap",
};

const PesoEnDiferentesContextosGraphsCoord = createCoordGraph({
  Component: PesoEnDiferentesContextosGraphsCoordView,
  displayName: "PesoEnDiferentesContextosGraphsCoord",
});

PesoEnDiferentesContextosGraphsCoord.graphContract = PesoEnDiferentesContextosGraphsCoordView.graphContract;
PesoEnDiferentesContextosGraphsCoord.graph_identity = PesoEnDiferentesContextosGraphsCoordView.graph_identity;
PesoEnDiferentesContextosGraphsCoord.axes = PesoEnDiferentesContextosGraphsCoordView.axes;
PesoEnDiferentesContextosGraphsCoord.formula = PesoEnDiferentesContextosGraphsCoordView.formula;
PesoEnDiferentesContextosGraphsCoord.profile = PesoEnDiferentesContextosGraphsCoordView.profile;

export default PesoEnDiferentesContextosGraphsCoord;
