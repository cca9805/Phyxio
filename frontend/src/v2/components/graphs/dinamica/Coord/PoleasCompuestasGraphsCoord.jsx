...
function PoleasCompuestasGraphsCoordView(props) {
  const uiLang = props.lang === "en" ? "en" : "es";
  const decSep = uiLang === "en" ? "." : ",";
  const fmt = (x, d = 3) => (Number.isFinite(x) ? x.toFixed(d).replace(".", decSep) : "-");
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Poleas compuestas (Coord)", "Compound pulleys (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.85 }}>{tt("eta", "eta")}</span>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Fuerza necesaria en funcion de n", "Required force as a function of n")}</div>
...
const PoleasCompuestasGraphsCoord = createCoordGraph({
  Component: PoleasCompuestasGraphsCoordView,
  displayName: "PoleasCompuestasGraphsCoord",
});

PoleasCompuestasGraphsCoord.graphContract = PoleasCompuestasGraphsCoordView.graphContract;
PoleasCompuestasGraphsCoord.graph_identity = PoleasCompuestasGraphsCoordView.graph_identity;
PoleasCompuestasGraphsCoord.axes = PoleasCompuestasGraphsCoordView.axes;
PoleasCompuestasGraphsCoord.formula = PoleasCompuestasGraphsCoordView.formula;
PoleasCompuestasGraphsCoord.profile = PoleasCompuestasGraphsCoordView.profile;
