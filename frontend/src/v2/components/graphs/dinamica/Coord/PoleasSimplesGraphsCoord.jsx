...
function PoleasSimplesGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Poleas simples (Coord)", "Simple pulleys (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              {tt("Para", "For")} n = <b>{nSel}</b>, F = <b>{fmt(Fsel, 4)}</b> N | dx_pull = <b>{fmt(dxPull, 4)}</b> m.
            </div>
          </div>
        </div>
      </div>
...
const PoleasSimplesGraphsCoord = createCoordGraph({
  Component: PoleasSimplesGraphsCoordView,
  displayName: "PoleasSimplesGraphsCoord",
});

PoleasSimplesGraphsCoord.graphContract = PoleasSimplesGraphsCoordView.graphContract;
PoleasSimplesGraphsCoord.graph_identity = PoleasSimplesGraphsCoordView.graph_identity;
PoleasSimplesGraphsCoord.axes = PoleasSimplesGraphsCoordView.axes;
PoleasSimplesGraphsCoord.formula = PoleasSimplesGraphsCoordView.formula;
PoleasSimplesGraphsCoord.profile = PoleasSimplesGraphsCoordView.profile;

export default PoleasSimplesGraphsCoord;
