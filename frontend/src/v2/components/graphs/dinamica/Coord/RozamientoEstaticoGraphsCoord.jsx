...
function RozamientoEstaticoGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Rozamiento estatico (Coord)", "Static friction (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("El estatico se adapta hasta su limite:", "El static se adapta hasta su limite:")} <MathInline latex={"f_s \\le \\mu_s N"} />.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={mode === "incline"} onClick={() => setMode("incline")}>{tt("Plano inclinado", "Inclined plane")}</Pill>
            <Pill active={mode === "push"} onClick={() => setMode("push")}>{tt("Empuje horizontal", "Horizontal push")}</Pill>
        ...
        <div className="muted" style={{ marginBottom: 10 }}>
          {mode === "incline" ? (
            <>
              ...
              {current.slips ? <b style={{ color: "#ef4444" }}>{tt("desliza", "slips")}</b> : <b style={{ color: "#22c55e" }}>{tt("sin deslizamiento", "sin deslizamiento")}</b>}
            </>
          ) : (
            <>
              F = <b>{fmt(current.x, 1)}</b> N, f_s = <b>{fmt(current.yReq, 2)}</b> N, f_max = <b>{fmt(current.yMax, 2)}</b> N{" "}
              {current.slips ? <b style={{ color: "#ef4444" }}>{tt("supera estatico", "exceeds static friction")}</b> : <b style={{ color: "#22c55e" }}>{tt("estatico activo", "static friction active")}</b>}
            </>
          )}
        </div>
          ...
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 8, bottom: 10 }}>
              ...
              <YAxis
                ...
                label={{ value: tt("Fuerza (N)", "Force (N)"), angle: -90, position: "insideLeft" }}
              />
              <Tooltip
...
const RozamientoEstaticoGraphsCoord = createCoordGraph({
  Component: RozamientoEstaticoGraphsCoordView,
  displayName: "RozamientoEstaticoGraphsCoord",
});

RozamientoEstaticoGraphsCoord.graphContract = RozamientoEstaticoGraphsCoordView.graphContract;
RozamientoEstaticoGraphsCoord.graph_identity = RozamientoEstaticoGraphsCoordView.graph_identity;
RozamientoEstaticoGraphsCoord.axes = RozamientoEstaticoGraphsCoordView.axes;
RozamientoEstaticoGraphsCoord.formula = RozamientoEstaticoGraphsCoordView.formula;
RozamientoEstaticoGraphsCoord.profile = RozamientoEstaticoGraphsCoordView.profile;

export default RozamientoEstaticoGraphsCoord;
