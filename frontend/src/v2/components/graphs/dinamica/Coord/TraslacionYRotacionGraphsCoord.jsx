...
const GRAPH_META = {
  pregunta_principal: {
    es: "¿Cómo se acoplan la aceleración lineal y la angular cuando un cuerpo rígido traslada y rota simultáneamente?",
    en: "How do linear and angular acceleration couple when a rigid body simultaneously translates and rotates?",
  },
};
...
function TraslacionYRotacionGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t(lang, "Traslacion y rotacion (Coord)", "Translation and rotation (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.08)" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 13, opacity: 0.85 }}>{tt("omega", "omega")}</span><input className="form-control" style={{ width: 90 }} value={fmt(omega, 3).replace(",", ".")} onChange={(e) => setValue("omega", e.target.value)} /></div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t(lang, "Energia vs velocidad angular", "Energy vs angular speed")}</div>
        <div style={{ width: "100%", height: 340, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 8, bottom: 10 }}>
              ...
              <YAxis tickFormatter={(e) => fmt(e, 2)} label={{ value: t(lang, "Energia (J)", "Energy (J)"), angle: -90, position: "insideLeft" }} />
              <Tooltip formatter={(value, name) => [fmt(value, 5), name]} labelFormatter={(x) => `${t(lang, "omega", "omega")} = ${fmt(x, 3)} rad/s`} />
...
const TraslacionYRotacionGraphsCoord = createCoordGraph({
  Component: TraslacionYRotacionGraphsCoordView,
  displayName: "TraslacionYRotacionGraphsCoord",
});

TraslacionYRotacionGraphsCoord.graphContract = TraslacionYRotacionGraphsCoordView.graphContract;
TraslacionYRotacionGraphsCoord.graph_identity = TraslacionYRotacionGraphsCoordView.graph_identity;
TraslacionYRotacionGraphsCoord.axes = TraslacionYRotacionGraphsCoordView.axes;
TraslacionYRotacionGraphsCoord.formula = TraslacionYRotacionGraphsCoordView.formula;
TraslacionYRotacionGraphsCoord.profile = TraslacionYRotacionGraphsCoordView.profile;

export default TraslacionYRotacionGraphsCoord;
