...
export default function DiagramasCuerpoLibreGraphsSvg(props) {
  ...
  const schema = useMemo(
    ...
    [p.scene, pickNum(p, ["theta", "angulo", "th"], 25), p.showForces, p.showMotion, p.showGuides]
  );
  ...
  const motion = showMotion ? 80 * Math.sin(t * 0.95) : 0;
  const tableObj = { x: clamp(baseX + motion, 140, W - 140), y: floorY - 38 };
  const inclineOrigin = { x: 290, y: floorY - 18 };
  const inclineObj = {
    x: clamp(inclineOrigin.x + planeUx * 170 + planeUx * motion * 0.45, 120, W - 120),
    y: clamp(inclineOrigin.y + planeUy * 170 + planeUy * motion * 0.45, 90, floorY - 35),
  };
  const hangingTop = { x: 430, y: 78 };
  const hangingObj = { x: hangingTop.x, y: clamp(150 + motion * 0.3, 110, 265) };
  ...
  const setShowGuides = useCallback((val) => {
    exp.startExperiment();
    exp.setValue("showGuides", val);
  }, [exp]);
  ...
  return (
    <SvgFrame
      ...
      subtitle="Del objeto real al DCL: identifica fuerzas sobre el cuerpo aislado"
      badgeLabel="MODO"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div className="v2-panel m-3 mb-1">
            ...
            <div style={{ marginTop: 10, display: "grid", gap: 2 }}>
              <div className="v2-panel m-1 mt-1" style={{ background: "rgba(0, 0, 0, 1)" }}>
                <div style={{ fontSize: 12, opacity: 0.75 }}>DCL</div>
              ...
              <div className="v2-panel m-1 mt-1" style={{ background: "rgba(0, 0, 0, 1)" }}>
                ...
                <div style={{ marginTop: 2, color: "#86efac" }}>
                  <MathInline latex={"W_{\\parallel}=mg\\sin\\theta,\\;W_{\\perp}=mg\\cos\\theta"} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="v2-panel m-2 mt-0" style={{ background: "rgba(0, 0, 0, 0.7)" }}>
            ...
            <div className="v2-toggle-row" style={{ marginTop: 10 }}>
              ...
              <label className="v2-toggle">
                <input className="v2-toggle-input" type="checkbox" checked={showGuides} onChange={(e) => setShowGuides(e.target.checked)} />
                <span className="v2-toggle-label">Guias</span>
              </label>
              <label className="v2-toggle">
          ...
          <div className="v2-panel m-2 mt-0" style={{ background: "rgba(0, 0, 0, 0.40)" }}>
            ...
            <div style={{ marginTop: 8, fontSize: 13, opacity: 0.82, lineHeight: 1.45 }}>
              t = <b>{fmt(t, 2)}</b> s
              <br />
              Escena activa: <b style={{ color: "#93c5fd" }}>{scene === "table" ? "mesa" : scene === "incline" ? "plano inclinado" : "masa colgante"}</b>
            </div>
          </div>
        </div>
      }
    >
      <defs>
        <linearGradient id="bgDclSvg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#c4d3ff" />
          <stop offset="100%" stopColor="#a4b8f7" />
        </linearGradient>
      </defs>

      <rect x="0" y="0" width={W} height={H} fill="url(#bgDclSvg)" />
      ...
      {scene === "table" ? (
        <g>
          <rect x={tableObj.x - 52} y={tableObj.y - 26} width="104" height="52" rx="12" fill="rgba(15,23,42,0.95)" stroke="rgba(255,255,255,0.4)" strokeWidth="2" />
          <circle cx={tableObj.x} cy={tableObj.y} r="4" fill="rgba(255,255,255,0.85)" />
          {showForces ? (
            <>
              <Arrow x1={tableObj.x} y1={tableObj.y - 26} x2={tableObj.x} y2={tableObj.y - 126} color="#16a34a" label="N" />
              <Arrow x1={tableObj.x} y1={tableObj.y + 26} x2={tableObj.x} y2={tableObj.y + 126} color="#dc2626" label="mg" />
              <Arrow x1={tableObj.x - 52} y1={tableObj.y} x2={tableObj.x - 148} y2={tableObj.y} color="#f59e0b" label="f" labelNormalOffset={-10} />
            </>
          ) : null}
        </g>
      ) : null}
      ...
      {scene === "incline" ? (
        <g>
          ...
          {showGuides ? (
            <g>
              <line x1={inclineObj.x} y1={inclineObj.y} x2={inclineObj.x + planeUx * 100} y2={inclineObj.y + planeUy * 100} stroke="rgba(255,255,255,0.25)" strokeDasharray="5 4" />
              <line x1={inclineObj.x} y1={inclineObj.y} x2={inclineObj.x + normalUx * 100} y2={inclineObj.y + normalUy * 100} stroke="rgba(255,255,255,0.25)" strokeDasharray="5 4" />
            </g>
          ) : null}
          {showForces ? (
            <>
              <Arrow x1={inclineObj.x} y1={inclineObj.y} x2={inclineObj.x} y2={inclineObj.y + 118} color="#dc2626" label="mg" />
              <Arrow x1={inclineObj.x} y1={inclineObj.y} x2={inclineObj.x + normalUx * 110} y2={inclineObj.y + normalUy * 110} color="#16a34a" label="N" labelNormalOffset={-10} />
              <Arrow x1={inclineObj.x} y1={inclineObj.y} x2={inclineObj.x + planeUx * 100} y2={inclineObj.y + planeUy * 100} color="#f59e0b" label="f" labelNormalOffset={10} />
            </>
          ) : null}
        </g>
      ) : null}
      ...
      {scene === "hanging" ? (
        <g>
          ...
          {showForces ? (
            <>
              <Arrow x1={hangingObj.x} y1={hangingObj.y + 30} x2={hangingObj.x} y2={hangingObj.y + 130} color="#dc2626" label="mg" />
              <Arrow x1={hangingObj.x} y1={hangingObj.y - 30} x2={hangingObj.x} y2={hangingObj.y - 130} color="#6366f1" label="T" />
            </>
          ) : null}
        </g>
      ) : null}
