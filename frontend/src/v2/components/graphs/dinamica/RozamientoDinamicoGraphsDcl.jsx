...
export default function RozamientoDinamicoGraphsDcl(props) {
  ...
  const W = m * g;
  const N = scene === "incline" ? W * Math.cos(rad) : W;
  const fk = mu * N;

  const FnetH = F - fk;
  const aH = FnetH / m;
  const Fdown = W * Math.sin(rad);
  const FnetI = Fdown - fk;
  const aI = FnetI / m;

  const Fmax = Math.max(1, W, N, fk, F);
  ...
  const fkDir = scene === "incline" ? { x: -Math.cos(rad), y: -Math.sin(rad) } : { x: -1, y: 0 };
  const fkEnd = { x: origin.x + fkDir.x * toLen(fk), y: origin.y + fkDir.y * toLen(fk) };
  const fEnd = { x: origin.x + toLen(F), y: origin.y };
  ...
  const setFv = useCallback((v) => { exp.startExperiment(); exp.setValue("F", v); }, [exp]);
  ...
  const setShowF = useCallback((v) => { exp.startExperiment(); exp.setValue("showF", v); }, [exp]);
  const setShowAxes = useCallback((v) => { exp.startExperiment(); exp.setValue("showAxes", v); }, [exp]);

  return (
    <SvgFrame
      title="Rozamiento dinamico (DCL)"
      subtitle="Solo vectores en regimen de deslizamiento"
      badgeLabel="MODO"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div className="v2-panel m-3 mb-1">
            ...
            <div style={{ marginTop: 10, display: "grid", gap: 2 }}>
              <div className="v2-panel m-1 mt-1" style={{ background: "rgba(0,0,0,1)" }}>
                <div style={{ fontSize: 12, opacity: 0.75 }}>Rozamiento dinamico</div>
                <div style={{ marginTop: 2, color: "#f59e0b" }}>
                  <MathInline latex={"f_k = \\mu_k N"} />
                </div>
              </div>
              <div className="v2-panel m-1 mt-1" style={{ background: "rgba(0,0,0,1)" }}>
                <div style={{ fontSize: 12, opacity: 0.75 }}>Aceleracion</div>
                <div style={{ marginTop: 2, color: "#22c55e" }}>
                  <MathInline latex={scene === "horizontal" ? "a=\\frac{F-\\mu_k mg}{m}" : "a=g(\\sin\\theta-\\mu_k\\cos\\theta)"} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="v2-panel m-2 mt-0" style={{ background: "rgba(0,0,0,0.7)" }}>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <Pill active={scene === "horizontal"} onClick={() => setScene("horizontal")}>Mesa + F</Pill>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 900, opacity: 0.9 }}>mu_k</div>
              ...
              <div style={{ opacity: scene === "horizontal" ? 1 : 0.55 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 900, opacity: 0.9 }}>F</div>
                  <div style={{ fontSize: 12, opacity: 0.7 }}>{fmt(F, 1)} N</div>
                </div>
                <div style={{ marginTop: 8 }}><Slider value={F} min={0} max={300} step={1} onChange={setFv} /></div>
              </div>
              ...
              <div className="v2-toggle-row" style={{ marginTop: 6 }}>
                ...
                <label className="v2-toggle"><input className="v2-toggle-input" type="checkbox" checked={showFk} onChange={(e) => setShowFk(e.target.checked)} /><span className="v2-toggle-label">f_k</span></label>
                <label className="v2-toggle" style={{ opacity: scene === "horizontal" ? 1 : 0.5 }}><input className="v2-toggle-input" type="checkbox" checked={showF} onChange={(e) => setShowF(e.target.checked)} disabled={scene !== "horizontal"} /><span className="v2-toggle-label">F</span></label>
          ...
          <div className="v2-panel m-2 mt-0" style={{ background: "rgba(0,0,0,0.40)" }}>
            ...
            <div style={{ marginTop: 8, fontSize: 13, opacity: 0.82, lineHeight: 1.45 }}>
              f_k = <b style={{ color: "#f59e0b" }}>{fmt(fk, 2)}</b> N.
              {scene === "horizontal" ? (
                <> F_net = <b>{fmt(FnetH, 2)}</b> N, a = <b style={{ color: "#22c55e" }}>{fmt(aH, 2)}</b> m/s2.</>
              ) : (
                <> F_par = <b>{fmt(Fdown, 2)}</b> N, a = <b style={{ color: "#22c55e" }}>{fmt(aI, 2)}</b> m/s2.</>
              )}
            </div>
          </div>
        </div>
    ...
    >
      <defs>
        <linearGradient id="bgRozDinDcl" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#cae2ff" />
          <stop offset="100%" stopColor="#b7d2ff" />
        </linearGradient>
      </defs>

      <rect x="0" y="0" width="850" height="330" fill="url(#bgRozDinDcl)" />
      ...
      {showFk ? <Arrow x1={origin.x} y1={origin.y} x2={fkEnd.x} y2={fkEnd.y} color="#f59e0b" label="f_k" labelNormalOffset={10} /> : null}
      {showF && scene === "horizontal" ? <Arrow x1={origin.x} y1={origin.y} x2={fEnd.x} y2={fEnd.y} color="#3b82f6" label="F" labelNormalOffset={-10} /> : null}
    </SvgFrame>
  );
}
