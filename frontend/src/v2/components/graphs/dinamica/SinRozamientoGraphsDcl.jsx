...
export default function SinRozamientoGraphsDcl(props) {
  ...
  const schema = useMemo(
    ...
    [p.thetaDeg, p.theta, p.angulo, p.m, p.masa, p.g, p.gravedad, p.showComp, p.showAxes]
  );
  ...
  return (
    <SvgFrame
      title="Plano inclinado sin rozamiento (DCL)"
      subtitle="Solo fuerzas reales: peso y normal"
      badgeLabel="MODO"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div className="v2-panel m-2 mt-0" style={{ background: "rgba(0,0,0,0.40)" }}>
            ...
            <div style={{ marginTop: 8, fontSize: 13, opacity: 0.86, lineHeight: 1.45 }}>
              N = <b>{fmt(N, 3)}</b> N
              <br />
              mg sin(theta) = <b>{fmt(mgPar, 3)}</b> N
              <br />
              a = <b>{fmt(a, 3)}</b> m/s²
            </div>
          </div>
