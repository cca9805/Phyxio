...
export default function CampoYPotencialGravitatorioGraphsCoord({ title, params }) {
  const schema = useMemo(
    () => [
      { key: "M", default: toNum(params?.M, 5.97e24) },
      { key: "rMin", default: toNum(params?.rMin, 6.4e6) },
      { key: "rMax", default: toNum(params?.rMax, 3.6e7) },
      { key: "showG", default: params?.showG ?? true },
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || "Campo y potencial gravitatorio (Coord)"}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          <MathInline latex={"g=\\frac{GM}{r^2},\\quad V=-\\frac{GM}{r}"} />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div className="v2-panel" style={{ padding: 12, fontSize: 13, opacity: 0.9 }}>
            g(r*)=<b>{fmt(gRef, 4)}</b> m/s2 | V(r*)=<b>{fmt(vRef, 2)}</b> x10^6 J/kg
          </div>
        </div>
      </div>

      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">Caida de campo y potencial con la distancia</div>
          ...
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 12, bottom: 10 }}>
              ...
              <Tooltip
                ...
                labelFormatter={(v) => `r=${fmt(v / 1e6, 2)} x10^6 m`}
              />
              {showZero ? <ReferenceLine y={0} stroke="rgba(255,255,255,0.30)" /> : null}
              {showG ? <Line type="monotone" dataKey="g" name="g" stroke="#34d399" strokeWidth={2.8} dot={false} /> : null}
              {showV ? <Line type="monotone" dataKey="V" name="V" stroke="#f59e0b" strokeWidth={2.8} dot={false} /> : null}
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
