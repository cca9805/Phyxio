...
const graph_identity = {
  pregunta_principal: {
    es: "Cuando una fuerza gravitatoria externa deja de ser solo atraccion global y produce deformacion, marea o deriva orbital acumulada",
    en: "When an external gravitational force stops being only global attraction and produces deformation, tides, or accumulated orbital drift",
  },
};
...
function MareasYPerturbacionesGraphsCoordView({ title, params, sharedParams, lang = "es" }) {
  const uiLang = lang === "en" ? "en" : "es";
  const tt = (es, en) => (uiLang === "en" ? en : es);
  const paramBridge = params ?? sharedParams;
  const schema = useMemo(
    () => [
      { key: "M", default: toNum(paramBridge?.M, 7.35e22) },
      { key: "R", default: toNum(paramBridge?.R, 6.37e6) },
      { key: "dMin", default: toNum(paramBridge?.dMin, 2.5e8) },
      { key: "dMax", default: toNum(paramBridge?.dMax, 5e8) },
      { key: "showATide", default: paramBridge?.showATide ?? true },
      { key: "showInvCube", default: paramBridge?.showInvCube ?? true },
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || tt("Mareas y perturbaciones (Coord)", "Tides and perturbations (Coord)")}</div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Dependencia cubica de las mareas", "Cubic tidal dependence")}</div>
...
const MareasYPerturbacionesGraphsCoord = createCoordGraph({
  Component: MareasYPerturbacionesGraphsCoordView,
  displayName: "MareasYPerturbacionesGraphsCoord",
});

MareasYPerturbacionesGraphsCoord.graphContract = MareasYPerturbacionesGraphsCoordView.graphContract;
MareasYPerturbacionesGraphsCoord.graph_identity = MareasYPerturbacionesGraphsCoordView.graph_identity;
MareasYPerturbacionesGraphsCoord.axes = MareasYPerturbacionesGraphsCoordView.axes;
MareasYPerturbacionesGraphsCoord.formula = MareasYPerturbacionesGraphsCoordView.formula;
MareasYPerturbacionesGraphsCoord.profile = MareasYPerturbacionesGraphsCoordView.profile;

export default MareasYPerturbacionesGraphsCoord;
