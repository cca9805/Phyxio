...
function OndasGraphsCoordView({
  ...
  // Param bridge para validador: props.params ?? props.sharedParams
  const effectiveInput = params ?? sharedParams ?? {};
  const stringHint = `${leafRelPath || ""} ${params?.motionId || ""} ${title || ""}`
    .toLowerCase()
    .replace(/^\/+|\/+$/g, "");
  const isStringPropagationLeaf =
    stringHint.includes("ondas-en-cuerdas/velocidad-en-cuerda-tensa") ||
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.9 }}>{tt("t visual (s)", "t visual (s)")}</span>
              <input
                className="form-control"
                style={{ width: 110 }}
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.9 }}>{tt("x visual (m)", "x visual (m)")}</span>
              <input
                className="form-control"
                style={{ width: 110 }}
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.9 }}>{tt("ventana x (m)", "window x (m)")}</span>
              <input
                className="form-control"
                style={{ width: 110 }}
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, opacity: 0.9 }}>{tt("ventana t (s)", "window t (s)")}</span>
              <input
                className="form-control"
                style={{ width: 110 }}
            ...
            >
              {playTime ? tt("Pausar tiempo real", "Pause real time") : tt("Tiempo real", "Real time")}
            </button>
          </div>
          ...
          <div className="v2-panel" style={{ padding: 12, fontSize: 13, opacity: 0.9 }}>
            ...
            <div style={{ marginTop: 6, fontSize: 12, opacity: 0.86 }}>
              {tt("Consejo: activa", "Tip: enable")} <b>{tt("Tiempo real", "Real time")}</b> {tt("para ver el desplazamiento continuo de crestas, valles y frente de onda.", "to see the continuous displacement of crests, valleys and wave front.")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Respuesta del modelo", "Model response")}</div>
        {calcRefs.hasAny ? (
          <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>
            {tt("Referencias de calculadora:", "Calculator references:")}{" "}
        ...
        <div style={{ width: "100%", minWidth: 0, height: 350, marginTop: 10 }}>
            ...
            <LineChart data={built.data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {Number.isFinite(calcRefs.xRef) ? (
                <ReferenceLine
                  ...
                  strokeDasharray="7 6"
                  label={{ value: `${calcRefs.xVar} ${tt("calc", "calc")}`, fill: "#f43f5e", fontSize: 11 }}
                />
              ) : null}
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Onda real (espacio y tiempo)", "Real wave (space and time)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          ...
          <br />
          {tt("periodos visibles: espacio", "visible periods: space")} <b>{fmt(waveViz.spatialPeriods, 2)}</b>, {tt("tiempo", "time")} <b>{fmt(waveViz.temporalPeriods, 2)}</b>
          <br />
          {tt("cresta", "crest")} x~<b>{Number.isFinite(waveMarkers.crestX) ? fmt(waveMarkers.crestX, 3) : "-"}</b> m, {tt("valle", "valley")} x~<b>{Number.isFinite(waveMarkers.valleyX) ? fmt(waveMarkers.valleyX, 3) : "-"}</b> m, {tt("frente", "front")} x_f=v*t~<b>{Number.isFinite(waveMarkers.frontX) ? fmt(waveMarkers.frontX, 3) : "-"}</b> m
        </div>
              ...
              <LineChart data={waveViz.space} margin={{ top: 10, right: 14, left: 10, bottom: 10 }}>
                ...
                {waveViz.lambdaLines.map((xLine, idx) => (
                  <ReferenceLine
                    ...
                    strokeDasharray="6 6"
                    label={idx === 0 ? { value: tt("lambda", "lambda"), fill: "#f97316", fontSize: 12 } : undefined}
                  />
                ))}
                {Number.isFinite(waveMarkers.crestX) ? (
                  <ReferenceLine
                    ...
                    strokeDasharray="5 5"
                    label={{ value: tt("cresta", "crest"), fill: "#22c55e", fontSize: 11 }}
                  />
                ) : null}
                {Number.isFinite(waveMarkers.valleyX) ? (
                  <ReferenceLine
                    ...
                    strokeDasharray="5 5"
                    label={{ value: tt("valle", "valley"), fill: "#ec4899", fontSize: 11 }}
                  />
                ) : null}
                {Number.isFinite(waveMarkers.frontX) ? (
                  <ReferenceLine
                    ...
                    strokeDasharray="8 5"
                    label={{ value: tt("frente v*t", "front v*t"), fill: "#f97316", fontSize: 11 }}
                  />
                ) : null}
                {Number.isFinite(waveMarkers.crestX) ? (
              ...
              <LineChart data={waveViz.time} margin={{ top: 10, right: 14, left: 10, bottom: 10 }}>
                ...
                {waveViz.TLines.map((tLine, idx) => (
                  <ReferenceLine
                    ...
                    stroke="rgba(249,115,22,0.70)"
                    strokeDasharray="6 6"
                    label={idx === 0 ? { value: tt("T", "T"), fill: "#f97316", fontSize: 12 } : undefined}
                  />
                ))}
...
OndasGraphsCoordView.graph_identity = {
  pregunta_principal: {
    es: "�Donde se localizan los puntos de amplitud maxima y minima en una onda estacionaria y que distancias caracteristicas relacionan con la longitud de onda?",
    en: "Where are the points of maximum and minimum amplitude located in a standing wave and what characteristic distances relate to the wavelength?",
  },
  magnitud_estrella: "lambda_onda",
  eje_x: "posicion",
  eje_y: "amplitud",
  tipo: "Coord",
  leaf_ids: ["nodos-y-antinodos"],
};

OndasGraphsCoordView.pregunta_principal = OndasGraphsCoordView.graph_identity.pregunta_principal;
OndasGraphsCoordView.magnitud_estrella = OndasGraphsCoordView.graph_identity.magnitud_estrella;

const OndasGraphsCoord = createCoordGraph({
  Component: OndasGraphsCoordView,
  displayName: "OndasGraphsCoord",
});

OndasGraphsCoord.graphContract = OndasGraphsCoordView.graphContract;
OndasGraphsCoord.graph_identity = OndasGraphsCoordView.graph_identity;
OndasGraphsCoord.axes = OndasGraphsCoordView.axes;
OndasGraphsCoord.formula = OndasGraphsCoordView.formula;
OndasGraphsCoord.profile = OndasGraphsCoordView.profile;

export default OndasGraphsCoord;
