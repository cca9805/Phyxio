...
const graphProfile = {
  pregunta_principal: {
    es: "Que se conserva y que cambia al comparar F_net con tau y a con alpha en la analogia entre traslacion y rotacion?",
    en: "What stays the same and what changes when comparing F_net with tau and a with alpha in the analogy between translation and rotation?",
  },
};
...
function AnalogiaTraslacionalGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Analogia traslacional (Coord)", "Translational analogy (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={showAlphaCurve} onClick={() => setValue("showAlphaCurve", !showAlphaCurve)}>{tt("Curva alpha(tau)", "Curve alpha(tau)")}</Pill>
            <Pill active={showACurve} onClick={() => setValue("showACurve", !showACurve)}>{tt("Curva a(F)", "Curve a(F)")}</Pill>
            <Pill active={showPoint} onClick={() => setValue("showPoint", !showPoint)}>{tt("Puntos actuales", "Current points")}</Pill>
          </div>

          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              alpha = <b>{fmt(alphaNow, 4)}</b> {tt("rad/s² (pendiente 1/I =", "rad/s² (pendiente 1/I =")} <b>{fmt(slopeAlpha, 4)}</b>)
              <br />
              a = <b>{fmt(aNow, 4)}</b> {tt("m/s² (pendiente 1/m =", "m/s² (pendiente 1/m =")} <b>{fmt(slopeA, 4)}</b>)
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Comparacion de linealidad", "Linearity comparison")}</div>
...
const AnalogiaTraslacionalGraphsCoord = createCoordGraph({
  Component: AnalogiaTraslacionalGraphsCoordView,
  displayName: "AnalogiaTraslacionalGraphsCoord",
});

AnalogiaTraslacionalGraphsCoord.graphContract = AnalogiaTraslacionalGraphsCoordView.graphContract;
AnalogiaTraslacionalGraphsCoord.graph_identity = AnalogiaTraslacionalGraphsCoordView.graph_identity;
AnalogiaTraslacionalGraphsCoord.axes = AnalogiaTraslacionalGraphsCoordView.axes;
AnalogiaTraslacionalGraphsCoord.formula = AnalogiaTraslacionalGraphsCoordView.formula;
AnalogiaTraslacionalGraphsCoord.profile = AnalogiaTraslacionalGraphsCoordView.profile;

export default AnalogiaTraslacionalGraphsCoord;
