...
const _graph_meta = {
  pregunta_principal: {
    es: "¿Qué magnitud axial relaciona la posición y el momento lineal respecto a un punto?",
    en: "What axial quantity relates position and linear momentum about a point?",
  },
};
...
function DefinicionMomentoAngularGraphsCoordView({ title, lang = "es", params, sharedParams }) {
  const uiLang = lang === "en" ? "en" : "es";
  const t = (es, en) => (uiLang === "en" ? en : es);

  const schema = useMemo(
    () => [
      { key: "m", default: toNum((params ?? sharedParams)?.m, 2.0) },
      { key: "v", default: toNum((params ?? sharedParams)?.v, 3.0) },
      { key: "r", default: toNum((params ?? sharedParams)?.r, 0.40) },
      { key: "theta", default: toNum((params ?? sharedParams)?.theta, 90) },
      { key: "I", default: toNum((params ?? sharedParams)?.I, 0.25) },
      { key: "omega", default: toNum((params ?? sharedParams)?.omega, 12) },
      { key: "showParticula", default: (params ?? sharedParams)?.showParticula ?? true },
      { key: "showEjeFijo", default: (params ?? sharedParams)?.showEjeFijo ?? true },
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || t("Definición de momento angular", "Angular momentum definition")}</div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{t("Comparación de cálculo de L", "L calculation comparison")}</div>
...
const DefinicionMomentoAngularGraphsCoord = createCoordGraph({
  Component: DefinicionMomentoAngularGraphsCoordView,
  displayName: "DefinicionMomentoAngularGraphsCoord",
});

DefinicionMomentoAngularGraphsCoord.graphContract = DefinicionMomentoAngularGraphsCoordView.graphContract;
DefinicionMomentoAngularGraphsCoord.graph_identity = DefinicionMomentoAngularGraphsCoordView.graph_identity;
DefinicionMomentoAngularGraphsCoord.axes = DefinicionMomentoAngularGraphsCoordView.axes;
DefinicionMomentoAngularGraphsCoord.formula = DefinicionMomentoAngularGraphsCoordView.formula;
DefinicionMomentoAngularGraphsCoord.profile = DefinicionMomentoAngularGraphsCoordView.profile;

export default DefinicionMomentoAngularGraphsCoord;
