...
function DefinicionTorqueGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Definicion de torque (Coord)", "Definition of torque (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={showCurve} onClick={() => setValue("showCurve", !showCurve)}>{tt("Curva tau(theta)", "Curve tau(theta)")}</Pill>
            <Pill active={showPoint} onClick={() => setValue("showPoint", !showPoint)}>{tt("Punto actual", "Current point")}</Pill>
            <Pill active={showMax} onClick={() => setValue("showMax", !showMax)}>{tt("Punto maximo", "Maximum point")}</Pill>
          </div>

          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              d = <b>{fmt(dNow, 4)}</b> {tt("m | tau =", "m | tau =")} <b>{fmt(tauNow, 4)}</b> N*m | tau_max = <b>{fmt(tauMax, 4)}</b> N*m.
              <br />
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Curva tau(theta) con F y r fijos", "tau(theta) curve with fixed F and r")}</div>
...
// graph_identity: torque tau calculation profile
const _coordProfile = {
  pregunta_principal: {
    es: "Calculo del torque tau: como varian tau y el brazo b al cambiar r, theta y la fuerza F?",
    en: "Torque tau calculation: how do tau and lever arm b change when r, theta, and force F vary?",
  },
  magnitud_estrella: "tau",
};
void _coordProfile;

const DefinicionTorqueGraphsCoord = createCoordGraph({
  Component: DefinicionTorqueGraphsCoordView,
  displayName: "DefinicionTorqueGraphsCoord",
});

DefinicionTorqueGraphsCoord.graphContract = DefinicionTorqueGraphsCoordView.graphContract;
DefinicionTorqueGraphsCoord.graph_identity = DefinicionTorqueGraphsCoordView.graph_identity;
DefinicionTorqueGraphsCoord.axes = DefinicionTorqueGraphsCoordView.axes;
DefinicionTorqueGraphsCoord.formula = DefinicionTorqueGraphsCoordView.formula;
DefinicionTorqueGraphsCoord.profile = DefinicionTorqueGraphsCoordView.profile;

export default DefinicionTorqueGraphsCoord;
