...
export const graphContract = {
  meta: {
    title: {
      es: "Ejemplos clasicos de rodadura",
      en: "Classic rolling examples",
    },
  },
  graph_identity: {
    pregunta_principal: {
      es: "Como validan la conversion desde rpm y el residual de rodadura ejemplos clasicos de ruedas reales medidos con sensores?",
      en: "How do rpm conversion and the rolling residual validate classic wheel examples measured on real wheels with sensors?",
    },
    magnitud_estrella: "delta",
    variable_control: "n",
  },
  axes: {
    x: "omega",
...
function EjemplosClasicosGraphsCoordView(props) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Ejemplos clasicos de rodadura (Coord)", "Classic rolling examples (Coord)")}</div>
        <div className="muted" style={{ marginTop: 6 }}>
          {tt("Condicion ideal:", "Condition ideal:")} <MathInline latex={"v = \\omega R"} />.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={preset === "bici"} onClick={() => applyPreset("bici")}>{tt("Bicicleta", "Bicycle")}</Pill>
            <Pill active={preset === "coche"} onClick={() => applyPreset("coche")}>{tt("Rueda coche", "Car wheel")}</Pill>
            <Pill active={preset === "patinando"} onClick={() => applyPreset("patinando")}>{tt("Con deslizamiento", "With slipping")}</Pill>
            <Pill active={showLine} onClick={() => setValue("showLine", !showLine)}>{tt("Recta ideal", "Ideal line")}</Pill>
            <Pill active={showPoint} onClick={() => setValue("showPoint", !showPoint)}>{tt("Punto actual", "Current point")}</Pill>
            <Pill active={showAxes} onClick={() => setValue("showAxes", !showAxes)}>{tt("Ejes", "Axes")}</Pill>
          </div>

          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              v_ideal = <b>{fmt(vIdeal, 4)}</b> {tt("m/s | Delta = v - omega*R =", "m/s | Delta = v - omega*R =")} <b>{fmt(delta, 4)}</b> m/s.
              <br />
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Grafica v(omega) y punto medido", "v(omega) graph and measured point")}</div>
        <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
          {tt("Si el punto cae sobre la recta, hay rodadura sin deslizamiento.", "Si el punto cae sobre la recta, hay rodadura sin deslizamiento.")}
        </div>
...
const EjemplosClasicosGraphsCoord = createCoordGraph({
  Component: EjemplosClasicosGraphsCoordView,
  displayName: "EjemplosClasicosGraphsCoord",
});

EjemplosClasicosGraphsCoord.graphContract = EjemplosClasicosGraphsCoordView.graphContract;
EjemplosClasicosGraphsCoord.graph_identity = EjemplosClasicosGraphsCoordView.graph_identity;
EjemplosClasicosGraphsCoord.axes = EjemplosClasicosGraphsCoordView.axes;
EjemplosClasicosGraphsCoord.formula = EjemplosClasicosGraphsCoordView.formula;
EjemplosClasicosGraphsCoord.profile = EjemplosClasicosGraphsCoordView.profile;

export default EjemplosClasicosGraphsCoord;
