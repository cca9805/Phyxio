...
function RelacionLinealAngularGraphsCoordView({ title, params }) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || tt("Relacion lineal-angular (Coord)", "Linear-angular relationship (Coord)")}</div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Dependencia lineal con el radio", "Linear dependence on radius")}</div>
        <div style={{ width: "100%", height: 360, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="4 4" />
              <XAxis dataKey="r" tickFormatter={(n) => fmt(n, 2)} label={{ value: tt("R (m)", "R (m)"), position: "insideBottomRight", offset: -5 }} />
...
const RelacionLinealAngularGraphsCoord = createCoordGraph({
  Component: RelacionLinealAngularGraphsCoordView,
  displayName: "RelacionLinealAngularGraphsCoord",
});

RelacionLinealAngularGraphsCoord.graphContract = RelacionLinealAngularGraphsCoordView.graphContract;
RelacionLinealAngularGraphsCoord.graph_identity = graph_identity;
RelacionLinealAngularGraphsCoord.axes = RelacionLinealAngularGraphsCoordView.axes;
RelacionLinealAngularGraphsCoord.formula = RelacionLinealAngularGraphsCoordView.formula;
RelacionLinealAngularGraphsCoord.profile = RelacionLinealAngularGraphsCoordView.profile;

export default RelacionLinealAngularGraphsCoord;
