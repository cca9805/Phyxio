...
function RodaduraCinematicaSinDeslizamientoGraphsCoordView({ title, params }) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || tt("Rodadura sin deslizamiento (Coord)", "Rolling without slipping (Coord)")}</div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Relaciones de rodadura", "Rolling relations")}</div>
        <div style={{ width: "100%", height: 340, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={relData} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="4 4" />
              <XAxis dataKey="theta" tickFormatter={(n) => fmt(n, 2)} label={{ value: tt("theta (rad)", "theta (rad)"), position: "insideBottomRight", offset: -5 }} />
              <YAxis tickFormatter={(n) => fmt(n, 2)} label={{ value: tt("Magnitud", "Magnitude"), angle: -90, position: "insideLeft" }} />
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Velocidades caracteristicas", "Characteristic speeds")}</div>
        <div style={{ width: "100%", height: 320, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={speedData} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="4 4" />
              <XAxis dataKey="omega" tickFormatter={(n) => fmt(n, 2)} label={{ value: tt("omega (rad/s)", "omega (rad/s)"), position: "insideBottomRight", offset: -5 }} />
              <YAxis tickFormatter={(n) => fmt(n, 2)} label={{ value: tt("v (m/s)", "v (m/s)"), angle: -90, position: "insideLeft" }} />
...
const RodaduraCinematicaSinDeslizamientoGraphsCoord = createCoordGraph({
  Component: RodaduraCinematicaSinDeslizamientoGraphsCoordView,
  displayName: "RodaduraCinematicaSinDeslizamientoGraphsCoord",
});

RodaduraCinematicaSinDeslizamientoGraphsCoord.graphContract = RodaduraCinematicaSinDeslizamientoGraphsCoordView.graphContract;
RodaduraCinematicaSinDeslizamientoGraphsCoord.graph_identity = graph_identity;
RodaduraCinematicaSinDeslizamientoGraphsCoord.axes = RodaduraCinematicaSinDeslizamientoGraphsCoordView.axes;
RodaduraCinematicaSinDeslizamientoGraphsCoord.formula = RodaduraCinematicaSinDeslizamientoGraphsCoordView.formula;
RodaduraCinematicaSinDeslizamientoGraphsCoord.profile = RodaduraCinematicaSinDeslizamientoGraphsCoordView.profile;

export default RodaduraCinematicaSinDeslizamientoGraphsCoord;
