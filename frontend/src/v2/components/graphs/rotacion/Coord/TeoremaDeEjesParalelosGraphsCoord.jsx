...
const graph_identity = {
  pregunta_principal: {
    es: "Como cambia el momento de inercia I cuando el eje de rotacion se aleja del centro de masas una distancia d?",
    en: "How does the moment of inertia I change when the rotation axis moves away from the center of mass by a distance d?",
  },
  magnitud_estrella: "I",
};
void graph_identity;
...
function TeoremaDeEjesParalelosGraphsCoordView(props) {
  const uiLang = props.lang === "en" ? "en" : "es";
  const decSep = uiLang === "en" ? "." : ",";
  const fmt = (n, dec = 3) => (Number.isFinite(n) ? n.toFixed(dec).replace(".", decSep) : "-");
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Teorema de ejes paralelos (Coord)", "Teorema de axes paralelos (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          >
            <Pill active={showCurve} onClick={() => setValue("showCurve", !showCurve)}>{tt("Curva I(d)", "Curve I(d)")}</Pill>
            <Pill active={showPoint} onClick={() => setValue("showPoint", !showPoint)}>{tt("Punto actual", "Current point")}</Pill>
            <Pill active={showZero} onClick={() => setValue("showZero", !showZero)}>{tt("Linea y=0", "Line y=0")}</Pill>
          </div>

          <div className="v2-panel" style={{ padding: 12 }}>
            <div style={{ fontSize: 13, opacity: 0.9, lineHeight: 1.5 }}>
              <MathInline latex={"I = I_{cm}+m d^2"} /> = <b>{fmt(INow, 4)}</b> {tt("kg*m².", "kg*m².")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Dependencia cuadratica de I con d", "Quadratic dependence of I on d")}</div>
...
const TeoremaDeEjesParalelosGraphsCoord = createCoordGraph({
  Component: TeoremaDeEjesParalelosGraphsCoordView,
  displayName: "TeoremaDeEjesParalelosGraphsCoord",
});

TeoremaDeEjesParalelosGraphsCoord.graphContract = TeoremaDeEjesParalelosGraphsCoordView.graphContract;
TeoremaDeEjesParalelosGraphsCoord.graph_identity = TeoremaDeEjesParalelosGraphsCoordView.graph_identity;
TeoremaDeEjesParalelosGraphsCoord.axes = TeoremaDeEjesParalelosGraphsCoordView.axes;
TeoremaDeEjesParalelosGraphsCoord.formula = TeoremaDeEjesParalelosGraphsCoordView.formula;
TeoremaDeEjesParalelosGraphsCoord.profile = TeoremaDeEjesParalelosGraphsCoordView.profile;

export default TeoremaDeEjesParalelosGraphsCoord;
