...
const graphIdentity = {
  pregunta_principal: {
    es: "¿Cómo varía la potencia térmica transferida por un fluido en convección al cambiar la diferencia de temperatura?",
    en: "How much heat does a fluid in convection transfer as the temperature difference increases?",
  },
...
function ConveccionGraphsCoordView(props) {
  void graphIdentity;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12, background: "rgba(15,23,42,0.74)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.12)" }}>
        <div className="v2-card-title">{tt("Convección · Potencia térmica frente a diferencia de temperatura", "Convection · Thermal power versus temperature difference")}</div>
...
const ConveccionGraphsCoord = createCoordGraph({
  Component: ConveccionGraphsCoordView,
  displayName: "ConveccionGraphsCoord",
});

ConveccionGraphsCoord.graphContract = ConveccionGraphsCoordView.graphContract;
ConveccionGraphsCoord.graph_identity = ConveccionGraphsCoordView.graph_identity;
ConveccionGraphsCoord.axes = ConveccionGraphsCoordView.axes;
ConveccionGraphsCoord.formula = ConveccionGraphsCoordView.formula;
ConveccionGraphsCoord.profile = ConveccionGraphsCoordView.profile;

export default ConveccionGraphsCoord;