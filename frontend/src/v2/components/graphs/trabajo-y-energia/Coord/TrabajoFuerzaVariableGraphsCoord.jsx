...
const graph_identity = {
  pregunta_principal: {
    es: "¿Como cambia la transferencia energetica cuando la fuerza varia a lo largo del desplazamiento?",
    en: "How does energy transfer change when force varies along displacement?",
  },
  variable_control: "x",
  variables_observables: ["F", "W", "Fmed"],
  dominio: "Mecanica clasica",
  fenomeno: {
    es: "Trabajo por fuerza variable como area orientada bajo F(x)",
    en: "Work by a variable force as oriented area under F(x)",
  },
};

function Component({ title, params, sharedParams, lang }) {
  ...
  const schema = useMemo(
    () => [
      { key: "a", default: toNum(p?.a, 8) },
      { key: "b", default: toNum(p?.b, 2) },
      { key: "c", default: toNum(p?.c, 0) },
      { key: "x1", default: toNum(p?.x1, 0) },
      { key: "x2", default: toNum(p?.x2, 4) },
      { key: "showF", default: p?.showF ?? true },
      { key: "showArea", default: p?.showArea ?? true },
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{title || tt("Trabajo con fuerza variable (Coord)", "Work with variable force (Coord)")}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(2,132,199,0.22)", background: "rgba(2,132,199,0.10)" }}>
            ...
            <Pill active={showArea} onClick={() => setValue("showArea", !showArea)}>{tt("Area de integracion", "Integration area")}</Pill>
      ...
      <div className="v2-card" style={{ padding: 12 }}>
        <div className="v2-card-title">{tt("Fuerza variable y zona de trabajo", "Variable force and work zone")}</div>
        <div style={{ width: "100%", height: 360, marginTop: 10 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 10, right: 18, left: 10, bottom: 10 }}>
              ...
              {showArea ? <Line type="monotone" dataKey="inside" name={tt("F(x) en [x1,x2]", "F(x) in [x1,x2]")} stroke="#34d399" strokeWidth={2.5} dot={false} /> : null}
...
const TrabajoFuerzaVariableGraphsCoord = createCoordGraph({
  Component,
  displayName: "TrabajoFuerzaVariableGraphsCoord",
});

TrabajoFuerzaVariableGraphsCoord.graph_identity = graph_identity;

export default TrabajoFuerzaVariableGraphsCoord;
