 ...
 *
 * Estructura esperada:
 *   /src/data_v2_i18n/<ruta_relativa_del_leaf>/graphs/svg.config.(js|jsx|ts|tsx)
 *
 * donde ruta_relativa coincide con meta.yaml -> ruta_relativa
 */
const svgModules = import.meta.glob(
  "/src/data_v2_i18n/**/graphs/svg.config.{js,jsx,ts,tsx}"
);

function keyFromLeafRelPath(leafRelPath) {
  // Probamos extensiones en orden preferido
  const candidates = [
    `/src/data_v2_i18n/${leafRelPath}/graphs/svg.config.jsx`,
    `/src/data_v2_i18n/${leafRelPath}/graphs/svg.config.js`,
    `/src/data_v2_i18n/${leafRelPath}/graphs/svg.config.tsx`,
    `/src/data_v2_i18n/${leafRelPath}/graphs/svg.config.ts`,
  ];
