...
ALLOWED_DIFFS = {
    ...
    "km", "Hz", "kN", "kJ", "mm", "ms", "rpm", "MPa", "kPa", "ft", "MW",
...
def extract_math_tokens(text: str) -> Set[str]:
    ...
    LATEX_BLACKLIST = {
        "boldsymbol", "mathrm", "mathbf", "textrm", "text", "frac", "sqrt", "int", "sum",
    ...
    # Pattern para variables: letras solas, griego escapado, o palabras con subíndices (ignorando comandos)
    pattern = r"(\\Delta\s*[a-zA-Z]+|\\omega|\\theta_0|\\theta|\\tau|\\alpha|\\rho|\\sigma|a_c|v_\{\\mathrm\{med\}\}|[A-Za-z]+(?:_[0-9a-zA-Z]+)*)"
...
def validate_formulas(path: Path) -> Tuple[List[CheckResult], Set[str], Set[str]]:
    ...
    for idx, item in enumerate(items or []):
        ...
        latex_text = " ".join([str(item.get("equation","")), str(item.get("latex","")), json.dumps(item.get("rearrangements",[]), ensure_ascii=False)])
...
def validate_markdown_structure(path: Path, required_sections: List[str], min_words: int | None = None, max_words: int | None = None) -> List[CheckResult]:
    text = read_text(path)
...
def validate_history(path: Path, sections: List[str], min_words: int | None = None, max_words: int | None = None) -> List[CheckResult]:
    return validate_markdown_structure(path, sections, min_words=min_words, max_words=max_words)
...
def validate_symbol_consistency(magnitude_symbols: Set[str], formula_symbols: Set[str], theory_text: str, ejemplos_text: str) -> List[CheckResult]:
    ...
    for token in sorted({u for u in used if u}):
        if token in ALLOWED_DIFFS:
            continue
        if token not in mag_norm and token not in {"pi", "sqrt", "frac"}:
    ...
    for fname, group, sections, minw, maxw in markdown_files:
        ...
        if fpath.exists():
            if "historia" in group:
                ...
                all_results[group] = validate_markdown_structure(fpath, sections, min_words=minw, max_words=maxw)
        else:
