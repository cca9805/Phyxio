...
First decide whether the question asks for per-particle [[energia_media]] or whole-system [[energia_total]]. Then make sure [[temperatura]] is in kelvin. Use {{f:energia_media_particula}} for one typical particle, {{f:energia_total_gas_ideal}} when [[energia_media]] is already known and must be summed, or {{f:energia_total_temperatura}} when [[numero_particulas]] and [[temperatura]] are given. Finally check that [[k_boltzmann]] times [[temperatura]] gives joules.
