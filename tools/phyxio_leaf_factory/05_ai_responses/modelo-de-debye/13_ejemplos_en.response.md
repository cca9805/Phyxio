...
A monoatomic crystalline solid has [[temperatura_debye]] of 300 K. Estimate its [[calor_especifico_molar]] at [[temperatura]] of 30 K using the low-temperature Debye law, and compare it with the classical Dulong-Petit limit. Interpret what the result indicates about active phonon modes and explain why the solid has not yet reached the classical value.
...
Symbolically, the low-temperature value depends on the third power of the T/ΘD ratio, while the high-temperature value depends only on [[constante_gases]]. This shows that [[temperatura_debye]] does not change the classical ceiling; it controls how quickly the solid approaches it.
...
The reduced temperature is dimensionless because it compares two temperatures measured in kelvin. The cubic factor is also dimensionless. The dimensional scale is supplied by [[constante_gases]], whose unit is J mol⁻¹ K⁻¹. Therefore the result for [[calor_especifico_molar]] has the correct unit J mol⁻¹ K⁻¹. The Dulong-Petit expression 3R has the same unit. This confirms that the calculation gives molar heat capacity, not energy, not temperature, and not the total heat capacity of an arbitrary sample.
