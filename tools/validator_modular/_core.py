...
def validate_interpretacion(path: Path, mag_ids: Set[str], formula_ids: Set[str]) -> List[CheckResult]:
    ...
    def _valid_condition_block(value: Any) -> bool:
        """Declarative condition for interpretation rules."""
...
def validate_exact_files(leaf_dir: Path) -> List[CheckResult]:
    ...
    graph_types: List[Any] = []
    meta_path = leaf_dir / "meta.yaml"
    if meta_path.exists():
        try:
            meta_data = load_yaml(meta_path)
            if isinstance(meta_data, dict):
                graph_types = meta_data.get("graficos") or []
        except Exception:
            graph_types = []
    if graph_types:
        required.append("graficos.yaml")
    allowed.add("graficos.yaml")

    missing = [f for f in required if f not in current]
    extra = [f for f in current if f not in allowed]
    return [
...
def extract_js_brace_block(text: str, open_brace_pos: int) -> Tuple[str | None, int]:
    """Extract a JS brace block while respecting nested braces and strings."""
    ...
    for pos in range(open_brace_pos, len(text)):
        ...
        if ch == "{":
            ...
            depth -= 1
            if depth == 0:
                return text[open_brace_pos + 1:pos], pos
    return None, -1

def parse_v5_raw_graphs_registry(active_idx: str) -> Dict[str, Dict[str, str]]:
    """Parse v5 rawGraphs entries: leaf id -> graph type -> imported component name."""
    raw_match = re.search(r'\bconst\s+rawGraphs\s*=', active_idx)
    if not raw_match:
        return {}

    raw_open = active_idx.find("{", raw_match.end())
    raw_body, _ = extract_js_brace_block(active_idx, raw_open)
    if not raw_body:
        return {}

    registered: Dict[str, Dict[str, str]] = {}
    leaf_re = re.compile(r'["\']([^"\']+)["\']\s*:\s*\{')
    graph_re = re.compile(r'\b(Coord|Svg|Dcl|DCL)\s*:\s*\{')
    pos = 0

    while True:
        leaf_match = leaf_re.search(raw_body, pos)
        if not leaf_match:
            break

        leaf_id = leaf_match.group(1)
        leaf_open = raw_body.find("{", leaf_match.end() - 1)
        leaf_body, leaf_end = extract_js_brace_block(raw_body, leaf_open)
        if not leaf_body:
            pos = leaf_match.end()
            continue
        ...
        graph_pos = 0
        while True:
            graph_match = graph_re.search(leaf_body, graph_pos)
            if not graph_match:
                break

            graph_type = "Dcl" if graph_match.group(1) == "DCL" else graph_match.group(1)
            graph_open = leaf_body.find("{", graph_match.end() - 1)
            graph_body, graph_end = extract_js_brace_block(leaf_body, graph_open)
            if not graph_body:
                graph_pos = graph_match.end()
                continue

            component_match = re.search(r'\bcomponent\s*:\s*([A-Za-z_$][\w$]*)', graph_body)
            if component_match:
                comp_map[graph_type] = component_match.group(1)
            graph_pos = graph_end + 1 if graph_end >= 0 else graph_match.end()

        if comp_map:
            registered[leaf_id] = comp_map
        pos = leaf_end + 1 if leaf_end >= 0 else leaf_match.end()
...
def resolve_graph_registry(leaf_dir: Path, meta_data: dict) -> Tuple[Dict[str, Any] | None, List[CheckResult]]:
    ...
    registry_mode = "v5_rawGraphs"
    registered: Dict[str, Dict[str, str]] = parse_v5_raw_graphs_registry(active_idx)
    if not registered:
        export_match = re.search(r'export const graphs\s*=\s*\{(.*?)\n\};', active_idx, re.DOTALL)
        if not export_match:
            return None, [CheckResult(False, "index.js no contiene rawGraphs v5 ni 'export const graphs = {...}' legacy", str(index_path))]

        registry_mode = "legacy_export_const_graphs"
        registered = {}
